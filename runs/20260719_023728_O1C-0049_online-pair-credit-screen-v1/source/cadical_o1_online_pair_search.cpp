...
constexpr const char *kSchema =
    "o1-256-cadical-online-pair-credit-search-result-v1";
constexpr const char *kPotentialSchema = "O1CRIT-POT-V1";
constexpr const char *kRequiredVersion = "3.0.0";
constexpr const char *kDecisionRule = "online_saturating_pair_group_credit";
...
constexpr int kStateBytesPerGroup = 10;
...
Arguments parse_arguments(int argc, char **argv) {
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (argument == "--help") {
      std::cout
          << "usage: cadical_o1_online_pair_search --cnf PATH "
...
static_assert(sizeof(GroupState) == kStateBytesPerGroup,
              "online pair group state must occupy exactly ten bytes");
...
class OnlinePairCreditPropagator final : public CaDiCaL::ExternalPropagator {
public:
  OnlinePairCreditPropagator(CaDiCaL::Solver *solver, PotentialField field,
                             int variable_count,
    ...
    if (!solver_ ||
      ...
      throw std::runtime_error("online pair-credit constructor differs");
    ...
    for (const int variable : decision_variables)
      if (!touched_[variable])
        throw std::runtime_error(
            "decision variable is absent from online pair potential");
  ...
  void notify_assignment(const std::vector<int> &literals) override {
    for (const int literal : literals) {
      ...
      if (variable < 1 || variable >= static_cast<int>(assigned_.size()) ||
          !touched_[variable])
        throw std::runtime_error("unexpected online pair assignment");
      ...
      if (!assigned_[variable]) {
        ...
        throw std::runtime_error(
            "online pair assignment changed without backtrack");
      }
  ...
  void notify_backtrack(size_t new_level) override {
    if (new_level >= levels_.size())
      throw std::runtime_error("invalid online pair backtrack level");
    ...
    for (size_t level = new_level + 1U; level < levels_.size(); ++level)
      for (const int variable : levels_[level]) {
        if (!assigned_[variable])
          throw std::runtime_error(
              "online pair backtrack found unassigned variable");
  ...
  int cb_decide() override {
    ...
    if (!std::isfinite(base_score))
      throw std::runtime_error("global online pair-envelope score is not finite");
    ...
    for (size_t group_index = 0; group_index < groups_.size(); ++group_index) {
      ...
      for (unsigned mask = 0U; mask < 4U; ++mask) {
        ...
        if (!std::isfinite(score))
          throw std::runtime_error("online pair-pattern envelope is not finite");
        patterns.push_back({mask, score});
      }
      if (patterns.size() < 2U)
        throw std::runtime_error("online pair has fewer than two patterns");
      ...
      if (!std::isfinite(gap) || gap < 0.0L)
        throw std::runtime_error("online pair score gap is invalid");
      if (gap == 0.0L)
        continue;
    ...
    if (queue_.empty())
      throw std::runtime_error("selected online pair has no unassigned literal");
  ...
  void finalize() { close_ticket(CloseReason::SolveEnd); }
  ...
  std::string state_bytes() const {
    std::string result;
    result.reserve(states_.size() * static_cast<size_t>(kStateBytesPerGroup));
  ...
  void write_online_json(std::ostream &out) const {
    ...
    const std::string encoded = state_bytes();
...
private:
  ...
  SolverSnapshot snapshot() const {
    ...
    if (result.conflicts < 0 || result.decisions < 0 ||
      ...
      throw std::runtime_error("online pair solver counter differs");
    return result;
  }

  static int64_t checked_delta(int64_t current, int64_t baseline) {
    if (current < baseline)
      throw std::runtime_error("online pair solver counter moved backwards");
    return current - baseline;
  }

  static void checked_accumulate(int64_t &target, int64_t value) {
    if (value < 0 || target > std::numeric_limits<int64_t>::max() - value)
      throw std::runtime_error("online pair counter total overflow");
    target += value;
  }

  uint16_t saturating_counter_add(uint16_t current, int64_t delta) {
    if (delta < 0)
      throw std::runtime_error("online pair negative counter delta");
  ...
  void open_ticket(size_t group_index) {
    if (ticket_.open || group_index >= groups_.size())
      throw std::runtime_error("online pair action-ticket invariant differs");
    ticket_.open = true;
  ...
  void close_ticket(CloseReason reason) {
    ...
    const bool backtracked = reason == CloseReason::Backtrack;
    GroupState &state = states_.at(ticket_.group_index);
    const int64_t credit_delta =
        4 * assignments + std::min<int64_t>(delta_propagations / 64, 31) -
        8 * std::min<int64_t>(delta_conflicts, 31) -
        12 * static_cast<int>(backtracked);
    state.credit = saturating_credit_add(state.credit, credit_delta);
    state.visits = saturating_counter_add(state.visits, 1);
    state.conflict_hits =
        saturating_counter_add(state.conflict_hits, delta_conflicts);
    state.propagation_units =
        saturating_counter_add(state.propagation_units, delta_propagations);
    state.backtrack_hits =
        saturating_counter_add(state.backtrack_hits, backtracked ? 1 : 0);
    ++tickets_closed_;
  ...
  double factor_envelope(size_t factor_index, const PairGroup *group,
    ...
    if (!std::isfinite(best))
      throw std::runtime_error("online pair factor envelope is empty");
    return best;
  }
...
int main(int argc, char **argv) {
  try {
    ...
    auto propagator = std::make_unique<OnlinePairCreditPropagator>(
    ...
    std::cout << ",\"cnf_sha256\":\"" << sha256(cnf_payload)
              ...
              << statistic(solver, "propagations") << "},\"online\":{"
              ...
              << "\",";
    propagator->write_online_json(std::cout);
    ...
    std::cerr << "cadical_o1_online_pair_search: " << error.what() << '\n';
    return 1;
  }
}
