"""Deepest exact learned-clause owner credit for the frozen O1C-0048 pairs."""
...
CLAUSE_ROLE_CREDIT_RESULT_SCHEMA = (
    "o1-256-cadical-clause-role-credit-search-result-v1"
)
CLAUSE_ROLE_CREDIT_DECISION_RULE = "deepest_learned_clause_owner_pattern_credit"
CLAUSE_ROLE_CREDIT_GROUPS = 63
CLAUSE_ROLE_CREDIT_VARIABLES = 126
CLAUSE_ROLE_CREDIT_ACTION_STATE_ENCODING = (
    "i16le-credit,u16le-visits,u16le-conflict-hits,u16le-backtrack-hits"
)
CLAUSE_ROLE_CREDIT_OWNER_STATE_ENCODING = (
    "u32le-first-owner-level,u8-first-owner-mask,"
    "u32le-second-owner-level,u8-second-owner-mask"
)
CLAUSE_ROLE_CREDIT_STATE_ENCODING = (
...
CLAUSE_ROLE_CREDIT_CALLBACK_STATE_ENCODING = "u64le-live-learned-member-bitmap[2]"
CLAUSE_ROLE_CREDIT_COUNTER_SEMANTICS = (
...
CLAUSE_ROLE_CREDIT_SELECTION_FORMULA = (
...
CLAUSE_ROLE_CREDIT_ACTION_BYTES_PER_CELL = 8
CLAUSE_ROLE_CREDIT_ACTION_CELLS_PER_GROUP = 4
CLAUSE_ROLE_CREDIT_ACTION_BYTES_PER_GROUP = 32
CLAUSE_ROLE_CREDIT_OWNER_BYTES_PER_MEMBER = 5
CLAUSE_ROLE_CREDIT_OWNER_MEMBERS_PER_GROUP = 2
CLAUSE_ROLE_CREDIT_OWNER_BYTES_PER_GROUP = 10
CLAUSE_ROLE_CREDIT_BYTES_PER_GROUP = 42
CLAUSE_ROLE_CREDIT_ACTION_STATE_BYTES = 2016
CLAUSE_ROLE_CREDIT_OWNER_STATE_BYTES = 630
CLAUSE_ROLE_CREDIT_CALLBACK_STATE_BYTES = 16
CLAUSE_ROLE_CREDIT_STATE_BYTES = 2662
CLAUSE_ROLE_CREDIT_MIN = -(1 << 15)
CLAUSE_ROLE_CREDIT_MAX = (1 << 15) - 1
CLAUSE_ROLE_CREDIT_COUNTER_MAX = (1 << 16) - 1
...
_CLAUSE_ROLE_CREDIT_FIELDS = _CREDIT_INTEGER_FIELDS | {"trace_sha256"}
...
class ClauseRoleCreditSearchResult:
    status: int
    conflict_limit: int
...
def build_native_clause_role_credit_search(
    *, source: str | Path, output: str | Path
    ...
    if build_error is not None:
        raise build_error
    if result is None:
        raise O1RelationalSearchError("clause-role-credit native build failed")
    return result


def write_clause_role_credit_potential(
    path: str | Path, field: CriticalityPotentialField
) -> str:
    return write_pair_envelope_potential(path, field)


def write_clause_role_credit_decision_variables(
...
def _mapping(value: object, field: str) -> Mapping[str, object]:
    if not isinstance(value, Mapping):
        raise O1RelationalSearchError(f"clause-role-credit {field} differs")
    return value
...
def _nonnegative_integer_group(
    ...
    if set(payload) != names:
        raise O1RelationalSearchError(f"clause-role-credit {field} fields differ")
    result: dict[str, int] = {}
    for name, raw in payload.items():
        if isinstance(raw, bool) or not isinstance(raw, int) or raw < 0:
            raise O1RelationalSearchError(
                f"clause-role-credit {field}.{name} differs"
            )
        result[str(name)] = raw
    return result
...
def _read_input(path: str | Path, field: str) -> tuple[Path, bytes, str]:
    try:
        ...
        raise O1RelationalSearchError(
            f"clause-role-credit {field} input differs"
        ) from exc
    if not payload:
        raise O1RelationalSearchError(f"clause-role-credit {field} input differs")
...
def _decision_values(payload: bytes) -> tuple[int, ...]:
    try:
        ...
        raise O1RelationalSearchError(
            "clause-role-credit decision-variable input differs"
        ) from exc
    if (
        len(values) != CLAUSE_ROLE_CREDIT_VARIABLES
        or len(set(values)) != len(values)
        or any(not 1 <= value <= 256 for value in values)
    ):
        raise O1RelationalSearchError(
            "clause-role-credit requires exactly 63 unique ordered key pairs"
        )
    return values
...
def _potential_field(payload: bytes) -> CriticalityPotentialField:
    try:
        ...
        raise O1RelationalSearchError(
            "clause-role-credit potential input differs"
        ) from exc
...
def _verify_stable_input(
    ...
    try:
        ...
        raise O1RelationalSearchError(
            f"clause-role-credit {field} changed during execution"
        ) from exc
...
def _sat_signed(value: int) -> int:
    return min(CLAUSE_ROLE_CREDIT_MAX, max(CLAUSE_ROLE_CREDIT_MIN, value))


def clause_role_credit_update(
    credits: tuple[int, ...],
    ...
    *,
    ...
    """Penalize one deepest exact member, then optionally clear undone owners."""
    ...
    if (
        ...
        or any(
            ...
            or not CLAUSE_ROLE_CREDIT_MIN <= credit <= CLAUSE_ROLE_CREDIT_MAX
        ...
        raise O1RelationalSearchError("clause-role-credit bounded update differs")

    unique_members = set(matched_members)
    if any(owners[member][0] == 0 for member in unique_members):
        raise O1RelationalSearchError(
            "clause-role-credit matched inactive owner differs"
        )
    selected_member = min(
        unique_members,
        key=lambda member: (-owners[member][0], member // 2, member % 2),
        default=None,
    )
    penalized_cells = (
        ()
        if selected_member is None
        else ((selected_member // 2, owners[selected_member][1]),)
    )
...
def _state_blocks(
    ...
    if len(groups) != CLAUSE_ROLE_CREDIT_GROUPS:
        raise O1RelationalSearchError("clause-role-credit group-state count differs")
    action_output = bytearray()
    owner_output = bytearray()
    for index, group in enumerate(groups):
        if set(group) != _GROUP_FIELDS:
            raise O1RelationalSearchError(
                f"clause-role-credit state group {index} fields differ"
            )
        ...
        if any(
            ...
            raise O1RelationalSearchError(
                f"clause-role-credit state group {index} value differs"
            )
        ...
        if (
            ...
            raise O1RelationalSearchError(
                f"clause-role-credit state group {index} contract differs"
            )
        actions_raw = group["actions"]
        if not isinstance(actions_raw, list) or len(actions_raw) != 4:
            raise O1RelationalSearchError(
                f"clause-role-credit state group {index} actions differ"
            )
        for mask, raw_action in enumerate(actions_raw):
            ...
            if set(action) != _ACTION_FIELDS:
                raise O1RelationalSearchError(
                    f"clause-role-credit state group {index} action fields differ"
                )
            action_values = tuple(
                action[name]
                for name in (
            ...
            if any(
                ...
                raise O1RelationalSearchError(
                    f"clause-role-credit state group {index} action value differs"
                )
            ...
            if (
                ...
                or not CLAUSE_ROLE_CREDIT_MIN <= credit <= CLAUSE_ROLE_CREDIT_MAX
                or any(
                    not 0 <= counter <= CLAUSE_ROLE_CREDIT_COUNTER_MAX
                    for counter in (visits, conflicts, backtracks)
                )
            ):
                raise O1RelationalSearchError(
                    f"clause-role-credit state group {index} action contract differs"
                )
...
def run_clause_role_credit_search(
    *,
    executable: str | Path,
    cnf_path: str | Path,
    ...
    if (
        ...
        raise O1RelationalSearchError(
            "clause-role-credit search limit, seed, or timeout differs"
        )
    ...
    if not set(decisions).issubset(field.observed_variables):
        raise O1RelationalSearchError(
            "clause-role-credit decision variables are absent from potential"
        )
    command = [
    ...
    if execution_error is not None:
        raise O1RelationalSearchError(
            "clause-role-credit search execution failed"
        ) from execution_error
    if completed is None:
        raise O1RelationalSearchError("clause-role-credit search execution failed")
    if completed.returncode:
        detail = completed.stderr.strip() or completed.stdout.strip()
        raise O1RelationalSearchError(
            f"clause-role-credit search execution failed: {detail}"
        )
    try:
        payload = json.loads(completed.stdout)
    except json.JSONDecodeError as exc:
        raise O1RelationalSearchError(
            "clause-role-credit search JSON is invalid"
        ) from exc
    if not isinstance(payload, Mapping) or set(payload) != _TOP_LEVEL_FIELDS:
        raise O1RelationalSearchError("clause-role-credit result fields differ")
    variables = payload["variables"]
    reported_limit = payload["conflict_limit"]
    reported_seed = payload["seed"]
    status = payload["status"]
    if (
        payload["schema"] != CLAUSE_ROLE_CREDIT_RESULT_SCHEMA
        ...
        raise O1RelationalSearchError("clause-role-credit result contract differs")
    model_hex = payload["key_model_hex"]
    if status == 10:
        if not isinstance(model_hex, str) or len(model_hex) != 64:
            raise O1RelationalSearchError("SAT clause-role-credit result lacks key")
        try:
            key_model = bytes.fromhex(model_hex)
        except ValueError as exc:
            raise O1RelationalSearchError(
                "clause-role-credit key encoding differs"
            ) from exc
    elif model_hex is not None:
        raise O1RelationalSearchError(
            "non-SAT clause-role-credit result contains key"
        )
    else:
        key_model = None
    ...
    if set(pattern) != _LEARNED_CLAUSE_FIELDS:
        raise O1RelationalSearchError("clause-role-credit pattern fields differ")
    ...
    identity_integers = (
        ...
        (pattern["pair_count"], CLAUSE_ROLE_CREDIT_GROUPS),
    ...
    if any(
        ...
        raise O1RelationalSearchError("clause-role-credit identity differs")
    reported_offset = pattern["offset"]
    if (
        pattern["decision_rule"] != CLAUSE_ROLE_CREDIT_DECISION_RULE
        ...
        or pattern["update_formula"] != CLAUSE_ROLE_CREDIT_UPDATE_FORMULA
        or pattern["selection_formula"] != CLAUSE_ROLE_CREDIT_SELECTION_FORMULA
    ):
        raise O1RelationalSearchError("clause-role-credit identity differs")
    ...
    credit_payload = _mapping(
        pattern["clause_role_credit"], "learned_clause.clause_role_credit"
    )
    if set(credit_payload) != _CLAUSE_ROLE_CREDIT_FIELDS:
        raise O1RelationalSearchError("clause-role-credit telemetry fields differ")
    credit: dict[str, int] = {}
    for name in _CREDIT_INTEGER_FIELDS:
        value = credit_payload[name]
        if isinstance(value, bool) or not isinstance(value, int) or value < 0:
            raise O1RelationalSearchError(
                f"clause-role-credit telemetry.{name} differs"
            )
        credit[name] = value
    ...
    if set(selection) != _SELECTION_FIELDS:
        raise O1RelationalSearchError("clause-role-credit selection fields differ")
    for name in (
        "cold_group_selections",
        "credit_modulated_group_selections",
        "zero_gap_fallbacks",
        ...
        if isinstance(value, bool) or not isinstance(value, int) or value < 0:
            raise O1RelationalSearchError(
                f"clause-role-credit selection.{name} differs"
            )
    ...
    if (
        ...
        or distinct_selected > 4 * CLAUSE_ROLE_CREDIT_GROUPS
        or differentiated > CLAUSE_ROLE_CREDIT_GROUPS
        or penalized > 4 * CLAUSE_ROLE_CREDIT_GROUPS
    ):
        raise O1RelationalSearchError(
            "clause-role-credit selection metric ledger differs"
        )
    if not _is_sha256(selection["trace_sha256"]):
        raise O1RelationalSearchError("clause-role-credit selection trace differs")
    ...
    if tickets["opened"]:
        if (
            ...
            or not 0 <= first_group < CLAUSE_ROLE_CREDIT_GROUPS
            or isinstance(first_mask, bool)
            or not isinstance(first_mask, int)
            or not 0 <= first_mask < 4
        ):
            raise O1RelationalSearchError(
                "clause-role-credit first cold selection differs"
            )
    elif first_group is not None or first_mask is not None:
        raise O1RelationalSearchError(
            "clause-role-credit empty selection contains first action"
        )
    if (
        ...
        raise O1RelationalSearchError("clause-role-credit telemetry ledger differs")
    if (
        ...
        or credit["selected_deepest_members"] != credit["clauses_with_membership"]
        or credit["selected_deepest_members"] != credit["penalty_updates"]
        or credit["distinct_action_cells"] != credit["selected_deepest_members"]
        or credit["discarded_matched_members"]
        + credit["selected_deepest_members"]
        != credit["matched_owner_members"]
        or credit["multi_member_clauses"] > credit["clauses_with_membership"]
        or credit["multi_member_clauses"] > credit["discarded_matched_members"]
        or credit["deepest_level_ties"] > credit["multi_member_clauses"]
        or credit["selected_at_current_level"]
        + credit["selected_below_current_level"]
        != credit["selected_deepest_members"]
        ...
        raise O1RelationalSearchError("clause-role-credit update ledger differs")

    state = _mapping(pattern["state"], "learned_clause.state")
    if set(state) != _STATE_FIELDS:
        raise O1RelationalSearchError("clause-role-credit state fields differ")
    groups_raw = state["groups"]
    if not isinstance(groups_raw, list) or any(
        not isinstance(group, Mapping) for group in groups_raw
    ):
        raise O1RelationalSearchError("clause-role-credit group states differ")
    ...
    callback_encoded = bytes(CLAUSE_ROLE_CREDIT_CALLBACK_STATE_BYTES)
    encoded = action_encoded + owner_encoded + callback_encoded
    state_integers = (
        (state["bytes_per_group"], CLAUSE_ROLE_CREDIT_BYTES_PER_GROUP),
        (
            state["action_bytes_per_cell"],
            CLAUSE_ROLE_CREDIT_ACTION_BYTES_PER_CELL,
        ),
        (
            state["action_cells_per_group"],
            CLAUSE_ROLE_CREDIT_ACTION_CELLS_PER_GROUP,
        ),
        (
            state["action_bytes_per_group"],
            CLAUSE_ROLE_CREDIT_ACTION_BYTES_PER_GROUP,
        ),
        (
            state["owner_bytes_per_member"],
            CLAUSE_ROLE_CREDIT_OWNER_BYTES_PER_MEMBER,
        ),
        (
            state["owner_members_per_group"],
            CLAUSE_ROLE_CREDIT_OWNER_MEMBERS_PER_GROUP,
        ),
        (
            state["owner_bytes_per_group"],
            CLAUSE_ROLE_CREDIT_OWNER_BYTES_PER_GROUP,
        ),
        (
        ...
        (state["bounded_state_bytes"], CLAUSE_ROLE_CREDIT_STATE_BYTES),
        (state["credit_min"], CLAUSE_ROLE_CREDIT_MIN),
        (state["credit_max"], CLAUSE_ROLE_CREDIT_MAX),
        (state["counter_max"], CLAUSE_ROLE_CREDIT_COUNTER_MAX),
        (state["owner_level_max"], (1 << 32) - 1),
        (state["owner_mask_max"], 3),
    )
    if (
        state["encoding"] != CLAUSE_ROLE_CREDIT_STATE_ENCODING
        or state["action_encoding"] != CLAUSE_ROLE_CREDIT_ACTION_STATE_ENCODING
        or state["owner_encoding"] != CLAUSE_ROLE_CREDIT_OWNER_STATE_ENCODING
        or state["callback_encoding"] != CLAUSE_ROLE_CREDIT_CALLBACK_STATE_ENCODING
        or state["counter_semantics"] != CLAUSE_ROLE_CREDIT_COUNTER_SEMANTICS
        or any(
            isinstance(value, bool) or not isinstance(value, int) or value != expected
        ...
        raise O1RelationalSearchError("clause-role-credit bounded state differs")
    for name in ("saturated_credit_updates", "saturated_counter_updates"):
        value = state[name]
        if isinstance(value, bool) or not isinstance(value, int) or value < 0:
            raise O1RelationalSearchError(f"clause-role-credit state.{name} differs")
    ...
    if (
        ...
        raise O1RelationalSearchError("clause-role-credit owner state differs")
    ...
    if (
        ...
        raise O1RelationalSearchError("clause-role-credit owner ledger differs")
    actions = [
    ...
    if sum(cast(int, action["visits"]) for action in actions) > tickets["closed"]:
        raise O1RelationalSearchError(
            "clause-role-credit state visit ledger differs"
        )
    if any(cast(int, action["credit"]) > 0 for action in actions):
        raise O1RelationalSearchError("clause-role-credit action state differs")
    ...
    if (
        ...
        raise O1RelationalSearchError(
            "clause-role-credit differentiation ledger differs"
        )
    ...
    if (
        ...
        raise O1RelationalSearchError("clause-role-credit state ledger differs")
    if any(delta > stats[name] for name, delta in deltas.items()):
        raise O1RelationalSearchError(
            "clause-role-credit solver counter delta ledger differs"
        )
    ...
    normalized_pattern["clause_role_credit"] = normalized_credit
    ...
    return ClauseRoleCreditSearchResult(
        status=status,
...
write_criticality_potential = write_clause_role_credit_potential
write_decision_variables = write_clause_role_credit_decision_variables
...
__all__ = [
    ...
    "CLAUSE_ROLE_CREDIT_ACTION_STATE_BYTES",
    "CLAUSE_ROLE_CREDIT_ACTION_STATE_ENCODING",
    "CLAUSE_ROLE_CREDIT_BYTES_PER_GROUP",
    "CLAUSE_ROLE_CREDIT_CALLBACK_STATE_BYTES",
    "CLAUSE_ROLE_CREDIT_CALLBACK_STATE_ENCODING",
    "CLAUSE_ROLE_CREDIT_COUNTER_MAX",
    "CLAUSE_ROLE_CREDIT_COUNTER_SEMANTICS",
    "CLAUSE_ROLE_CREDIT_DECISION_RULE",
    "CLAUSE_ROLE_CREDIT_GROUPS",
    "CLAUSE_ROLE_CREDIT_MAX",
    "CLAUSE_ROLE_CREDIT_MIN",
    "CLAUSE_ROLE_CREDIT_OWNER_BYTES_PER_MEMBER",
    "CLAUSE_ROLE_CREDIT_OWNER_BYTES_PER_GROUP",
    "CLAUSE_ROLE_CREDIT_OWNER_MEMBERS_PER_GROUP",
    "CLAUSE_ROLE_CREDIT_OWNER_STATE_BYTES",
    ...
    "write_clause_role_credit_potential",
]
