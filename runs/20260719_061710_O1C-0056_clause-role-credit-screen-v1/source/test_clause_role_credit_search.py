...
import o1_crypto_lab.clause_role_credit_search as learned_clause_module
...
NATIVE_SOURCE = ROOT / "native/cadical_o1_clause_role_credit_search.cpp"
...
def _inputs(
    ...
    write_clause_role_credit_potential(potential, _field(two_groups=two_groups))
    decision_sha = write_clause_role_credit_decision_variables(
...
def _groups() -> list[dict[str, object]]:
    ...
    return [
        ...
        for index in range(CLAUSE_ROLE_CREDIT_GROUPS)
    ]


def _fake_payload(cnf: Path, decision_sha: str) -> dict[str, object]:
    action_bytes = bytes(CLAUSE_ROLE_CREDIT_ACTION_STATE_BYTES)
    owner_bytes = bytes(CLAUSE_ROLE_CREDIT_OWNER_STATE_BYTES)
    callback_bytes = bytes(CLAUSE_ROLE_CREDIT_CALLBACK_STATE_BYTES)
    combined = action_bytes + owner_bytes + callback_bytes
    field = _field()
    return {
        "schema": CLAUSE_ROLE_CREDIT_RESULT_SCHEMA,
        ...
        "learned_clause": {
            ...
            "pair_count": CLAUSE_ROLE_CREDIT_GROUPS,
            "group_width": 2,
            ...
            "selection_formula": CLAUSE_ROLE_CREDIT_SELECTION_FORMULA,
            "queue": {
            ...
            "clause_role_credit": {
                ...
                "multi_member_clauses": 0,
                "selected_deepest_members": 0,
                "selected_at_current_level": 0,
                "selected_below_current_level": 0,
                "discarded_matched_members": 0,
                "deepest_level_ties": 0,
            ...
            "state": {
                "encoding": CLAUSE_ROLE_CREDIT_STATE_ENCODING,
                "action_encoding": CLAUSE_ROLE_CREDIT_ACTION_STATE_ENCODING,
                "owner_encoding": CLAUSE_ROLE_CREDIT_OWNER_STATE_ENCODING,
                "callback_encoding": CLAUSE_ROLE_CREDIT_CALLBACK_STATE_ENCODING,
                "bytes_per_group": CLAUSE_ROLE_CREDIT_BYTES_PER_GROUP,
                "action_bytes_per_cell": CLAUSE_ROLE_CREDIT_ACTION_BYTES_PER_CELL,
                "action_cells_per_group": CLAUSE_ROLE_CREDIT_ACTION_CELLS_PER_GROUP,
                "action_bytes_per_group": CLAUSE_ROLE_CREDIT_ACTION_BYTES_PER_GROUP,
                "owner_bytes_per_member": CLAUSE_ROLE_CREDIT_OWNER_BYTES_PER_MEMBER,
                "owner_members_per_group": CLAUSE_ROLE_CREDIT_OWNER_MEMBERS_PER_GROUP,
                "owner_bytes_per_group": CLAUSE_ROLE_CREDIT_OWNER_BYTES_PER_GROUP,
                ...
                "credit_min": CLAUSE_ROLE_CREDIT_MIN,
                "credit_max": CLAUSE_ROLE_CREDIT_MAX,
                "counter_max": CLAUSE_ROLE_CREDIT_COUNTER_MAX,
                "owner_level_max": (1 << 32) - 1,
                "owner_mask_max": 3,
                "counter_semantics": CLAUSE_ROLE_CREDIT_COUNTER_SEMANTICS,
                "live_owners": 0,
...
def test_reference_deduplicates_members_and_saturates_signed_credit() -> None:
    owners = ((3, 2), (4, 1))
    updated, remaining, undone, cells = clause_role_credit_update(
        (0,) * 4,
        owners,
        matched_members=(0, 0),
    )
    ...
    saturated, _, _, _ = clause_role_credit_update(
        (0, CLAUSE_ROLE_CREDIT_MIN + 1, 0, 0),
        ((2, 1), (0, 0)),
        matched_members=(0,),
    )
    assert saturated[1] == CLAUSE_ROLE_CREDIT_MIN
    assert CLAUSE_ROLE_CREDIT_MAX == 32767


def test_reference_selects_one_deepest_member_and_breaks_ties_by_address() -> None:
    owners = ((3, 3), (4, 3), (5, 1), (6, 2))
    updated, _, _, cells = clause_role_credit_update(
        (0,) * 8,
        owners,
        matched_members=(0, 1, 1, 2, 3),
    )
    assert cells == ((1, 2),)
    assert updated == (0, 0, 0, 0, 0, 0, -32, 0)

    tied, _, _, tied_cells = clause_role_credit_update(
        (0,) * 8,
        ((7, 3), (7, 1), (7, 2), (6, 0)),
        matched_members=(2, 1, 0),
    )
    assert tied_cells == ((0, 3),)
    assert tied == (0, 0, 0, -32, 0, 0, 0, 0)


def test_reference_empty_unit_and_backtrack_have_exact_scope() -> None:
    credits = (0,) * 8
    owners = ((3, 2), (5, 1), (7, 3), (5, 0))
    unchanged, remaining, undone, cells = clause_role_credit_update(
        credits,
        owners,
        new_level=5,
    )
    ...
    unit, _, _, unit_cells = clause_role_credit_update(
        credits,
        owners,
        matched_members=(3,),
    )
...
def test_strict_wrapper_rejects_state_and_credit_mutation(
    ...
    result = run_clause_role_credit_search(
    ...
    cases = (
        ...
        (
            ("learned_clause", "clause_role_credit", "penalty_units"),
            32,
            "update ledger",
        ),
        (
            ("learned_clause", "clause_role_credit", "callback_open"),
            1,
            "update ledger",
        ),
        (
            ("learned_clause", "clause_role_credit", "selected_deepest_members"),
            1,
            "update ledger",
        ),
    )
...
def test_native_role_fixture_selects_one_cell_and_leaves_251_zero(
    tmp_path: Path,
) -> None:
    executable = tmp_path / "role"
    build_native_clause_role_credit_search(source=NATIVE_SOURCE, output=executable)
    completed = subprocess.run(
        [str(executable), "--self-test-role-selection"],
        check=True,
        capture_output=True,
        text=True,
    )
    ...
    assert json.loads(completed.stdout) == {
        ...
        "empty_updates": 0,
        "no_member_updates": 0,
        "unit_nonzero_cells": 1,
        "unit_selected_group": 4,
        "unit_selected_member": 1,
        "multi_matched_members": 3,
        "multi_selected_group": 1,
        "multi_selected_member": 1,
        "multi_selected_level": 7,
        "multi_nonzero_cells": 1,
        "multi_other_zero_cells": 251,
        "tie_selected_group": 0,
        "tie_selected_member": 0,
        "tie_members_at_deepest_level": 3,
    }
...
def test_native_cold_equivalence_zero_credit_and_replay(tmp_path: Path) -> None:
    ...
    first = run_clause_role_credit_search(
    ...
    replay = run_clause_role_credit_search(
        executable=learned_executable,
        cnf_path=cnf,
        potential_path=potential,
        decision_variables_path=decisions,
    ...
    telemetry = first.learned_clause["clause_role_credit"]
    ...
    assert telemetry["clause_callbacks"] == 0
    assert telemetry["selected_deepest_members"] == 0
    assert telemetry["discarded_matched_members"] == 0
    assert telemetry["multi_member_clauses"] == 0
...
@pytest.mark.skipif(not _native_available(), reason="CaDiCaL development files absent")
def test_native_forced_no_learn_path_has_zero_role_credit(tmp_path: Path) -> None:
    _, potential, decisions, _ = _inputs(tmp_path)
    cnf = tmp_path / "forced.cnf"
    units = "".join(
        f"{variable if index % 2 else -variable} 0\n"
        for index, variable in enumerate(_decisions())
    )
    cnf.write_text(f"p cnf 256 {len(_decisions())}\n{units}", encoding="ascii")
    executable = tmp_path / "role"
    build_native_clause_role_credit_search(source=NATIVE_SOURCE, output=executable)
    result = run_clause_role_credit_search(
        executable=executable,
    ...
    telemetry = result.learned_clause["clause_role_credit"]
    assert result.status_name == "SAT"
    assert telemetry["clause_callbacks"] == 0
    assert telemetry["matched_owner_members"] == 0
    assert telemetry["selected_deepest_members"] == 0
    assert telemetry["discarded_matched_members"] == 0
    assert telemetry["penalty_updates"] == 0
    assert telemetry["callback_open"] == 0
    ...
    assert all(
        action["credit"] == 0
        for group in result.learned_clause["state"]["groups"]
        for action in group["actions"]
    )
...
def test_native_learned_clause_penalizes_only_matching_owner_cells(
    ...
    assert telemetry["selected_deepest_members"] == 1
    assert telemetry["selected_at_current_level"] + telemetry[
        "selected_below_current_level"
    ] == 1
    assert telemetry["discarded_matched_members"] == 0
    assert telemetry["multi_member_clauses"] == 0
    assert telemetry["deepest_level_ties"] == 0
