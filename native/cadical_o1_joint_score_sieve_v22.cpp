// O1C-0086 live-bank continuation parent-centered failure-first proof-mining operator.
//
...
constexpr const char *kV22ResultSchema =
    "o1-256-cadical-joint-score-sieve-result-v22";
constexpr const char *kV22ImplementationParentSchema =
...
constexpr const char *kPriorityActionSchema =
    "o1-256-o1c86-failure-first-proof-mining-actions-v1";
...
constexpr const char *kProductionPage11ActiveVaultSha256 =
    "9853f06bc882bfbb6312207bc8c20e0e9ca1500e49aad14594f6d7c66b62a04d";
constexpr size_t kProductionPage11ActiveVaultBytes = 2876731U;
...
const char *semantic_name(PriorityActionSemantic semantic) {
  ...
  throw std::runtime_error("O1C86 action semantic differs");
}
...
const char *machine_action_name(PriorityActionSemantic semantic) {
  ...
  throw std::runtime_error("O1C86 machine action differs");
}
...
public:
  ParentCenteredGroupedJointScoreSieve(
    ...
    if (candidates_.empty())
      throw std::runtime_error("O1C86 potential observes no key coordinate");
    if (production_seal_ &&
        (candidates_.size() != kExpectedProductionCandidateCount ||
         std::binary_search(candidates_.begin(), candidates_.end(), 241)))
      throw std::runtime_error("sealed O1C86 key-coordinate population differs");
    ...
    if (seed_payload.size() != o1c82::kCoordinateBankBytes)
      throw std::runtime_error("O1C86 priority seed byte count differs");
    if (seed_sha256_ != sha256(seed_payload))
      throw std::runtime_error("O1C86 priority seed payload digest differs");
    if (production_seal_ && seed_sha256_ != kLiveContinuationBankSha256)
      throw std::runtime_error("sealed O1C86 priority seed differs");
    ...
    if (priority_.export_seed().records != image.records ||
      ...
      throw std::runtime_error("O1C86 priority seed import round trip differs");
    ...
    if (production_seal_ && initial_eligible_coordinate_count_ !=
      ...
      throw std::runtime_error(
          "sealed O1C86 live-bank eligibility population differs");
  }
  ...
  void notify_assignment(const std::vector<int> &literals) override {
    ...
    for (const int literal : literals) {
      ...
      if (!slot) {
        ...
        throw std::runtime_error(
            "O1C86 shadow assignment changed without backtrack");
      }
  ...
  void notify_new_decision_level() override {
    if (current_level_ == std::numeric_limits<uint32_t>::max())
      throw std::runtime_error("O1C86 decision level exceeds bound");
    ...
    if (pending_action_index_ != no_action()) {
      ...
      if (action.bound_level)
        throw std::runtime_error("O1C86 action bound twice");
  ...
  void notify_backtrack(size_t new_level) override {
    if (new_level > current_level_ ||
      ...
      throw std::runtime_error("O1C86 backtrack level differs");
    ...
    while (!trail_.empty() && trail_.back().level > new_level) {
      ...
      if (!assignment_.at(index))
        throw std::runtime_error("O1C86 shadow trail state differs");
      assignment_[index] = 0;
      trail_.pop_back();
    }
  ...
  int cb_decide() override {
    if (solve_finalized_)
      throw std::runtime_error("O1C86 callback after solve end");
    if (ownership_.has_pending())
      throw std::runtime_error("O1C86 callback overlaps pending proposal");
    if (base_.cb_decide() != 0)
      throw std::runtime_error("O1C86 unchanged v6 callback differs");
  ...
  void finalize_after_solve() {
    if (solve_finalized_)
      throw std::runtime_error("O1C86 solve end finalized twice");
  ...
  const PriorityActionEvent &action(size_t index) const {
    if (index >= action_count_)
      throw std::runtime_error("O1C86 action index differs");
...
private:
  ...
  size_t local(int variable) const {
    ...
    if (iterator == observed.end() || *iterator != variable)
      throw std::runtime_error("O1C86 variable is unobserved");
  ...
  void require_shadow_identity() const {
    ...
    if (shadow != base_.assignment_state())
      throw std::runtime_error("O1C86 shadow assignment differs from v6");
  }
  ...
  void append_probe_trace(uint64_t call, uint64_t probe,
    ...
    if (record.size() != kProbeTraceRecordBytes)
      throw std::runtime_error("O1C86 probe trace width differs");
  ...
  int scan_and_select(uint64_t call) {
    ...
    if (!pending_clause_literals(base_.pending_state()).empty())
      throw std::runtime_error("O1C86 selector overlaps pending v6 clause");
    ...
    if (assignment_before != base_.assignment_state() ||
      ...
      throw std::runtime_error(
          "O1C86 probes mutated authoritative v6 state");
    ...
    if (!current_present_[coordinate])
      throw std::runtime_error("O1C86 priority winner lacks current bounds");
    ...
    if (literal != last_priority_selection_.action_literal)
      throw std::runtime_error("O1C86 current lower-bound mapping differs");
    return propose_action(
  ...
  int propose_action(uint64_t call, uint64_t first_probe,
    ...
    if (action_count_ >= actions_.size() || coordinate >= consumed_.size() ||
      ...
      throw std::runtime_error("O1C86 one-shot action capacity differs");
    const int expected =
        o1c82::ParentCenteredPriority::lower_upper_bound_action_literal(
            variable, bounds.zero, bounds.one);
    if (literal != expected)
      throw std::runtime_error("O1C86 action is not current lower-UB child");
    ...
    if ((semantic == PriorityActionSemantic::FAILURE_FIRST_PROOF_MINING &&
        ...
        (semantic ==
             PriorityActionSemantic::CERTIFIED_STRICT_BOUND_CROSSING &&
         !(lower < threshold_)))
      throw std::runtime_error("O1C86 action certification differs");
    ...
    if (parent_assignment_sha256.size() != 64U)
      throw std::runtime_error("O1C86 parent assignment digest differs");
  ...
  void apply_release(const o1c80::DecisionToken &token) {
    ...
    if (found == actions_.begin() + static_cast<ptrdiff_t>(action_count_) ||
      ...
      throw std::runtime_error("O1C86 action release lifecycle differs");
    found->released = true;
  ...
  void validate_telemetry() const {
    ...
    if (!solve_finalized_ || callback_calls_ != parent_scans_ ||
      ...
      throw std::runtime_error("O1C86 priority telemetry differs");
    ...
    for (size_t index = 0; index < action_count_; ++index) {
      ...
      if (event.token != index + 1U || !event.call || !event.first_probe ||
        ...
        throw std::runtime_error("O1C86 priority action telemetry differs");
    ...
    if (counted_releases != releases_ ||
      ...
      throw std::runtime_error("O1C86 priority release telemetry differs");
    require_shadow_identity();
  }
...
[[maybe_unused]] void print_v22_usage() {
  std::cout << "usage: cadical_o1_joint_score_sieve_v22 --cnf PATH "
...
#ifndef O1_CRYPTO_LAB_O1C86_NO_MAIN
int main(int argc, char **argv) {
  try {
    ...
    if (arguments.seed != 0)
      throw std::runtime_error("O1C86 priority operator requires seed zero");
    ...
    if (production_seal &&
        (vault_payload.size() != kProductionPage11ActiveVaultBytes ||
         vault_sha256 != kProductionPage11ActiveVaultSha256))
      throw std::runtime_error("sealed O1C86 Page-11 active vault differs");
    ...
    std::string result_json;
    {
      ...
      if (!solver.configure("plain") || !solver.set("seed", arguments.seed) ||
        ...
        throw std::runtime_error(
            "CaDiCaL rejected deterministic O1C86 options");
      int variables = 0;
      if (const char *error =
      ...
      out << std::setprecision(std::numeric_limits<double>::max_digits10)
          << "{\"schema\":\"" << kV22ResultSchema
          << "\",\"implementation_parent_schema\":\""
          << kV22ImplementationParentSchema
    ...
    std::cerr << "cadical_o1_joint_score_sieve_v22: " << error.what() << '\n';
    return 1;
  }
}
#endif
