// O1C-0109 live-bank continuation parent-centered failure-first proof-mining operator.
//
...
// one-shot coordinate mask, fixed action telemetry, digest/counter probe
// telemetry, and a fixed 256-row local-prunable breadcrumb sidecar.
// BOUND_LOSING_CHILD remains the legacy ownership transport name; it is never
// used as the semantic name of a failure-first proof-mining action.

#ifdef O1_CRYPTO_LAB_O1C109_PUBLIC_FIXTURE
#define O1_CRYPTO_LAB_O1C80_PUBLIC_FIXTURE
#define O1_CRYPTO_LAB_O1C109_UNDEF_O1C80_FIXTURE
#endif
#define O1_CRYPTO_LAB_O1C80_NO_MAIN
#include "cadical_o1_joint_score_sieve_v18.cpp"
#undef O1_CRYPTO_LAB_O1C80_NO_MAIN
#ifdef O1_CRYPTO_LAB_O1C109_UNDEF_O1C80_FIXTURE
#undef O1_CRYPTO_LAB_O1C80_PUBLIC_FIXTURE
#undef O1_CRYPTO_LAB_O1C109_UNDEF_O1C80_FIXTURE
#endif
...
constexpr const char *kV33ResultSchema =
    "o1-256-cadical-joint-score-sieve-result-v33";
constexpr const char *kV33ImplementationParentSchema =
...
constexpr const char *kPriorityActionSchema =
    "o1-256-o1c109-failure-first-proof-mining-actions-v1";
constexpr const char *kLocalPrunableBreadcrumbSchema =
...
constexpr const char *kO1C108PreparationManifestSha256 =
    "f16d505fc8f5007d6a1ace11c991323d74802eaa6d005c1eaaa3fe71daa72b04";
constexpr size_t kO1C108PreparationManifestBytes = 12468U;
constexpr const char *kO1C107PriorityStateReceiptSha256 =
    "a3578ea3fb591b9227ca11034ac34aba8c170f47a65e05e092d108106f33129e";
constexpr size_t kO1C107PriorityStateReceiptBytes = 51961U;
constexpr const char *kO1C108DerivedResolutionReceiptSha256 =
    "98c353c24b097dd3b6bec974d91a55d8bf789248d571e37554fe8ce2216d9fe8";
constexpr size_t kO1C108DerivedResolutionReceiptBytes = 361008U;
constexpr size_t kProductionLineageOrdinal = 35U;
constexpr size_t kProductionActiveLimit = 246U;
constexpr size_t kProductionActiveClauseCount = 246U;
constexpr size_t kProductionActiveLiteralCount = 688833U;
constexpr size_t kGlobalNoveltyBaselineClauseCount = 3111U;
constexpr size_t kProductionClauseHeadroom = 266U;
constexpr const char *kProductionPage22ActiveVaultSha256 =
    "183878040210ffb542b199148c7151bd2656b6019755a978142f3fbf87ac162f";
constexpr size_t kProductionPage22ActiveVaultBytes = 2756507U;
constexpr const char *kBurnedPage21ActiveVaultSha256 =
    "36091952f38fbe5b73e20311083c7e1bfc30271cfcd6dba2f46f73f051f65fa8";
constexpr size_t kBurnedPage21ActiveVaultBytes = 2762499U;
constexpr const char *kBurnedPage20ActiveVaultSha256 =
    "537f63c5284e15e451739f7369fbe6ee8dddbc5dfdb15b26988269a1e40e5519";
constexpr size_t kBurnedPage20ActiveVaultBytes = 2762455U;
constexpr const char *kBurnedPage19ActiveVaultSha256 =
    "3857519d4a384333d576ec1fe11939ef2a46d82d9ce7c585bc989792c0ceb3e6";
constexpr size_t kBurnedPage19ActiveVaultBytes = 2810555U;
...
constexpr size_t kLocalPrunableBreadcrumbCapacity =
    o1c82::kCoordinateCount;
constexpr size_t kLocalPrunableBreadcrumbRecordBytes = 143U;
...
static_assert(std::string_view(kO1C108PreparationManifestSha256).size() == 64U);
static_assert(kO1C108PreparationManifestBytes == 12468U);
static_assert(std::string_view(kO1C107PriorityStateReceiptSha256).size() == 64U);
static_assert(kO1C107PriorityStateReceiptBytes == 51961U);
static_assert(
    std::string_view(kO1C108DerivedResolutionReceiptSha256).size() == 64U);
static_assert(kO1C108DerivedResolutionReceiptBytes == 361008U);
static_assert(kProductionLineageOrdinal == 35U);
...
static_assert(kProductionActiveLimit == 246U);
static_assert(kProductionActiveLiteralCount == 688833U);
static_assert(kProductionClauseHeadroom == 266U);
static_assert(kGlobalNoveltyBaselineClauseCount == 3111U);
static_assert(kLocalPrunableBreadcrumbCapacity == 256U);
...
const char *semantic_name(PriorityActionSemantic semantic) {
  ...
  throw std::runtime_error("O1C109 action semantic differs");
}
...
const char *machine_action_name(PriorityActionSemantic semantic) {
  ...
  throw std::runtime_error("O1C109 machine action differs");
}
...
struct LocalPrunableBreadcrumb {
  uint64_t sequence = 0;
  uint64_t call = 0;
  uint64_t probe = 0;
  ...
  double parent_upper_bound = 0.0;
  double upper_zero = 0.0;
  double upper_one = 0.0;
  double threshold = 0.0;
  o1c80::ChildSelectionClass classification =
      o1c80::ChildSelectionClass::NEITHER_PRUNABLE;
  bool consumed_before = false;
  bool crossing_eligible = false;
  std::array<char, 65> parent_assignment_sha256{};
};
...
public:
  ParentCenteredGroupedJointScoreSieve(
    ...
    if (candidates_.empty())
      throw std::runtime_error("O1C109 potential observes no key coordinate");
    if (production_seal_ &&
        (candidates_.size() != kExpectedProductionCandidateCount ||
         std::binary_search(candidates_.begin(), candidates_.end(), 241)))
      throw std::runtime_error("sealed O1C109 key-coordinate population differs");
    ...
    if (seed_payload.size() != o1c82::kCoordinateBankBytes)
      throw std::runtime_error("O1C109 priority seed byte count differs");
    if (seed_sha256_ != sha256(seed_payload))
      throw std::runtime_error("O1C109 priority seed payload digest differs");
    if (production_seal_ && seed_sha256_ != kLiveContinuationBankSha256)
      throw std::runtime_error("sealed O1C109 priority seed differs");
    ...
    if (priority_.export_seed().records != image.records ||
      ...
      throw std::runtime_error("O1C109 priority seed import round trip differs");
    ...
    if (production_seal_ && initial_eligible_coordinate_count_ !=
      ...
      throw std::runtime_error(
          "sealed O1C109 live-bank eligibility population differs");
  }
  ...
  void notify_assignment(const std::vector<int> &literals) override {
    ...
    for (const int literal : literals) {
      ...
      if (!slot) {
        ...
        throw std::runtime_error(
            "O1C109 shadow assignment changed without backtrack");
      }
  ...
  void notify_new_decision_level() override {
    if (current_level_ == std::numeric_limits<uint32_t>::max())
      throw std::runtime_error("O1C109 decision level exceeds bound");
    ...
    if (pending_action_index_ != no_action()) {
      ...
      if (action.bound_level)
        throw std::runtime_error("O1C109 action bound twice");
  ...
  void notify_backtrack(size_t new_level) override {
    if (new_level > current_level_ ||
      ...
      throw std::runtime_error("O1C109 backtrack level differs");
    ...
    while (!trail_.empty() && trail_.back().level > new_level) {
      ...
      if (!assignment_.at(index))
        throw std::runtime_error("O1C109 shadow trail state differs");
      assignment_[index] = 0;
      trail_.pop_back();
    }
  ...
  int cb_decide() override {
    if (solve_finalized_)
      throw std::runtime_error("O1C109 callback after solve end");
    if (ownership_.has_pending())
      throw std::runtime_error("O1C109 callback overlaps pending proposal");
    if (base_.cb_decide() != 0)
      throw std::runtime_error("O1C109 unchanged v6 callback differs");
  ...
  void finalize_after_solve() {
    if (solve_finalized_)
      throw std::runtime_error("O1C109 solve end finalized twice");
  ...
  const PriorityActionEvent &action(size_t index) const {
    if (index >= action_count_)
      throw std::runtime_error("O1C109 action index differs");
  ...
  void write_priority_seed_json(std::ostream &out) const {
    out << "\"magic\":\"" << o1c82::kSeedMagic
        ...
        << kO1C107PriorityStateReceiptSha256
        << "\",\"source_priority_state_receipt_bytes\":"
        << kO1C107PriorityStateReceiptBytes
        << ",\"source_preparation_manifest_sha256\":\""
        << kO1C108PreparationManifestSha256
        << "\",\"source_preparation_manifest_bytes\":"
        << kO1C108PreparationManifestBytes
        << ",\"source_derived_resolution_receipt_sha256\":\""
        << kO1C108DerivedResolutionReceiptSha256
        << "\",\"source_derived_resolution_receipt_bytes\":"
        << kO1C108DerivedResolutionReceiptBytes
        << ",\"seed_source\":\""
  ...
  void write_priority_state_json(std::ostream &out) const {
    ...
    out << ",\"candidate_population\":" << candidates_.size()
        ...
        << local_prunable_breadcrumbs_.size()
        << ",\"local_prunable_breadcrumb_record_bytes\":"
        << sizeof(LocalPrunableBreadcrumb)
        << ",\"local_prunable_breadcrumb_state_bytes\":"
        << sizeof(local_prunable_breadcrumbs_)
        << ",\"growing_local_prunable_history_bytes\":0"
  ...
  void write_local_prunable_breadcrumbs_json(std::ostream &out) const {
    validate_telemetry();
    const uint64_t overflow_count =
        local_prunable_match_count_ - local_prunable_retained_count_;
    out << "\"schema\":\"" << kLocalPrunableBreadcrumbSchema
        << "\",\"selection_filter\":[\"ONE_PRUNABLE\",\"BOTH_PRUNABLE\"]"
        << ",\"observation_point\":\"immediately-after-probe-trace-before-"
           "crossing-branch\""
        << ",\"retention_rule\":\"first-256-matches-in-probe-order\""
        << ",\"capacity\":" << local_prunable_breadcrumbs_.size()
        << ",\"total_match_count\":" << local_prunable_match_count_
        << ",\"retained_count\":" << local_prunable_retained_count_
        << ",\"overflow_count\":" << overflow_count
        << ",\"complete\":" << (overflow_count ? "false" : "true")
        << ",\"class_counts\":{\"ONE_PRUNABLE\":"
        << local_prunable_class_counts_[0] << ",\"BOTH_PRUNABLE\":"
        << local_prunable_class_counts_[1]
        << "},\"canonical_digest\":{\"algorithm\":\"SHA-256\""
        << ",\"encoding\":\"o1c109-local-prunable-canonical-le-v1\""
        << ",\"record_bytes\":" << kLocalPrunableBreadcrumbRecordBytes
        << ",\"field_layout\":\"sequence:u64le,call:u64le,probe:u64le,"
           "candidate-index:u32le,coordinate-index:u32le,parent-level:u32le,"
           "parent-assignment-sha256:lowercase-hex64,variable:i32le,losing-"
           "literal:i32le,parent-upper-bound:f64le,U0:f64le,U1:f64le,tau:"
           "f64le,class:u8,consumed-before:u8,crossing-eligible:u8\""
        << ",\"all_matches\":{\"record_count\":"
        << local_prunable_match_count_ << ",\"bytes\":"
        << local_prunable_match_bytes_ << ",\"sha256\":\""
        << local_prunable_all_digest_.hex_digest()
        << "\"},\"overflow\":{\"record_count\":" << overflow_count
        << ",\"bytes\":" << local_prunable_overflow_bytes_
        << ",\"sha256\":\""
        << local_prunable_overflow_digest_.hex_digest()
        << "\"}},\"breadcrumbs\":[";
    for (size_t index = 0; index < local_prunable_retained_count_; ++index) {
      if (index)
        out << ',';
      const LocalPrunableBreadcrumb &row = local_prunable_breadcrumbs_[index];
      out << "{\"sequence\":" << row.sequence << ",\"call\":"
          << row.call << ",\"probe\":" << row.probe
          << ",\"candidate_index\":" << row.candidate
          << ",\"coordinate_index\":" << row.coordinate
          << ",\"parent_level\":" << row.parent_level
          << ",\"parent_assignment_sha256\":\""
          << row.parent_assignment_sha256.data()
          << "\",\"variable\":" << row.variable
          << ",\"losing_literal\":" << row.losing_literal
          << ",\"parent_upper_bound\":" << row.parent_upper_bound
          << ",\"upper_zero\":" << row.upper_zero
          << ",\"upper_one\":" << row.upper_one
          << ",\"threshold\":" << row.threshold
          << ",\"classification\":\""
          << o1c80::selection_class_name(row.classification)
          << "\",\"consumed_before\":"
          << (row.consumed_before ? "true" : "false")
          << ",\"crossing_eligible\":"
          << (row.crossing_eligible ? "true" : "false") << '}';
    }
    out << ']';
  }
...
private:
  ...
  size_t local(int variable) const {
    ...
    if (iterator == observed.end() || *iterator != variable)
      throw std::runtime_error("O1C109 variable is unobserved");
  ...
  void require_shadow_identity() const {
    ...
    if (shadow != base_.assignment_state())
      throw std::runtime_error("O1C109 shadow assignment differs from v6");
  }
  ...
  void append_probe_trace(uint64_t call, uint64_t probe,
    ...
    if (record.size() != kProbeTraceRecordBytes)
      throw std::runtime_error("O1C109 probe trace width differs");
    probe_trace_.update(record);
    probe_trace_bytes_ += record.size();
  }

  void append_local_prunable_breadcrumb(
      uint64_t call, uint64_t probe, size_t candidate_index,
      size_t coordinate, int variable, int losing_literal,
      double parent_upper_bound, const o1c80::ChildUpperBounds &bounds,
      o1c80::ChildSelectionClass classification, bool consumed_before,
      bool crossing_eligible,
      const std::string &parent_assignment_sha256) {
    if (classification != o1c80::ChildSelectionClass::ONE_PRUNABLE &&
        classification != o1c80::ChildSelectionClass::BOTH_PRUNABLE)
      return;
    if (local_prunable_match_count_ ==
            std::numeric_limits<uint64_t>::max() ||
        candidate_index > std::numeric_limits<uint32_t>::max() ||
        coordinate > std::numeric_limits<uint32_t>::max() ||
        parent_assignment_sha256.size() != 64U || !losing_literal ||
        !std::isfinite(parent_upper_bound))
      throw std::runtime_error("O1C109 local-prunable breadcrumb differs");

    LocalPrunableBreadcrumb row;
    row.sequence = ++local_prunable_match_count_;
    row.call = call;
    row.probe = probe;
    row.candidate = static_cast<uint32_t>(candidate_index);
    row.coordinate = static_cast<uint32_t>(coordinate);
    row.parent_level = current_level_;
    row.variable = variable;
    row.losing_literal = losing_literal;
    row.parent_upper_bound = parent_upper_bound;
    row.upper_zero = bounds.zero;
    row.upper_one = bounds.one;
    row.threshold = threshold_;
    row.classification = classification;
    row.consumed_before = consumed_before;
    row.crossing_eligible = crossing_eligible;
    std::copy(parent_assignment_sha256.begin(), parent_assignment_sha256.end(),
              row.parent_assignment_sha256.begin());
    row.parent_assignment_sha256[64] = '\0';

    std::string canonical;
    canonical.reserve(kLocalPrunableBreadcrumbRecordBytes);
    append_u64_le(canonical, row.sequence);
    append_u64_le(canonical, row.call);
    append_u64_le(canonical, row.probe);
    append_u32_le(canonical, row.candidate);
    append_u32_le(canonical, row.coordinate);
    append_u32_le(canonical, row.parent_level);
    canonical.append(row.parent_assignment_sha256.data(), 64U);
    central_append_i32(canonical, row.variable);
    central_append_i32(canonical, row.losing_literal);
    append_u64_le(canonical, f64_bits(row.parent_upper_bound));
    append_u64_le(canonical, f64_bits(row.upper_zero));
    append_u64_le(canonical, f64_bits(row.upper_one));
    append_u64_le(canonical, f64_bits(row.threshold));
    canonical.push_back(static_cast<char>(row.classification));
    canonical.push_back(static_cast<char>(row.consumed_before));
    canonical.push_back(static_cast<char>(row.crossing_eligible));
    if (canonical.size() != kLocalPrunableBreadcrumbRecordBytes)
      throw std::runtime_error(
          "O1C109 local-prunable canonical width differs");
    local_prunable_all_digest_.update(canonical);
    local_prunable_match_bytes_ += canonical.size();
    const size_t class_index =
        classification == o1c80::ChildSelectionClass::ONE_PRUNABLE ? 0U : 1U;
    ++local_prunable_class_counts_[class_index];
    if (local_prunable_retained_count_ <
        local_prunable_breadcrumbs_.size()) {
      local_prunable_breadcrumbs_[local_prunable_retained_count_++] = row;
    } else {
      local_prunable_overflow_digest_.update(canonical);
      local_prunable_overflow_bytes_ += canonical.size();
    }
  }
  ...
  int scan_and_select(uint64_t call) {
    ...
    if (!pending_clause_literals(base_.pending_state()).empty())
      throw std::runtime_error("O1C109 selector overlaps pending v6 clause");
    ...
    const double parent_upper_bound = upward(
        parent_cache.exact_sum, "O1C109 exact parent certified upper bound");
    ...
    for (size_t candidate_index = 0; candidate_index < candidates_.size();
      ...
      const bool consumed_before = consumed_[coordinate];
      const bool crossing_eligible =
          !crossing.available && selection.losing_literal && !consumed_before;
      append_local_prunable_breadcrumb(
          call, probe, candidate_index, coordinate, variable,
          selection.losing_literal, parent_upper_bound, bounds,
          selection.selection, consumed_before, crossing_eligible,
          parent_assignment_sha256);
    ...
    if (assignment_before != base_.assignment_state() ||
      ...
      throw std::runtime_error(
          "O1C109 probes mutated authoritative v6 state");
    ...
    if (!current_present_[coordinate])
      throw std::runtime_error("O1C109 priority winner lacks current bounds");
    ...
    if (literal != last_priority_selection_.action_literal)
      throw std::runtime_error("O1C109 current lower-bound mapping differs");
    return propose_action(
  ...
  int propose_action(uint64_t call, uint64_t first_probe,
    ...
    if (action_count_ >= actions_.size() || coordinate >= consumed_.size() ||
      ...
      throw std::runtime_error("O1C109 one-shot action capacity differs");
    const int expected =
        o1c82::ParentCenteredPriority::lower_upper_bound_action_literal(
            variable, bounds.zero, bounds.one);
    if (literal != expected)
      throw std::runtime_error("O1C109 action is not current lower-UB child");
    ...
    if ((semantic == PriorityActionSemantic::FAILURE_FIRST_PROOF_MINING &&
        ...
        (semantic ==
             PriorityActionSemantic::CERTIFIED_STRICT_BOUND_CROSSING &&
         !(lower < threshold_)))
      throw std::runtime_error("O1C109 action certification differs");
    ...
    if (parent_assignment_sha256.size() != 64U)
      throw std::runtime_error("O1C109 parent assignment digest differs");
    std::copy(parent_assignment_sha256.begin(), parent_assignment_sha256.end(),
  ...
  void apply_release(const o1c101::DecisionToken &token) {
    ...
    if (found == actions_.begin() + static_cast<ptrdiff_t>(action_count_) ||
      ...
      throw std::runtime_error("O1C109 action release lifecycle differs");
    found->released = true;
  ...
  void validate_telemetry() const {
    ...
    const uint64_t local_prunable_overflow_count =
        local_prunable_match_count_ - local_prunable_retained_count_;
    const uint64_t expected_local_prunable_retained =
        std::min<uint64_t>(local_prunable_match_count_,
                           local_prunable_breadcrumbs_.size());
    if (!solve_finalized_ || callback_calls_ != parent_scans_ ||
        ...
        class_total != probe_count_ ||
        local_prunable_match_count_ != class_counts_[2] + class_counts_[3] ||
        local_prunable_class_counts_[0] != class_counts_[2] ||
        local_prunable_class_counts_[1] != class_counts_[3] ||
        local_prunable_retained_count_ != expected_local_prunable_retained ||
        local_prunable_match_bytes_ !=
            local_prunable_match_count_ * kLocalPrunableBreadcrumbRecordBytes ||
        local_prunable_overflow_bytes_ != local_prunable_overflow_count *
                                                kLocalPrunableBreadcrumbRecordBytes ||
        action_count_ > actions_.size() ||
      ...
      throw std::runtime_error("O1C109 priority telemetry differs");
    for (size_t index = 0; index < local_prunable_retained_count_; ++index) {
      const LocalPrunableBreadcrumb &row = local_prunable_breadcrumbs_[index];
      const o1c80::ChildBoundSelection selected =
          o1c80::ExactOneBitBoundReader::select(
              row.variable, {row.upper_zero, row.upper_one}, row.threshold);
      if (row.sequence != index + 1U || !row.call || !row.probe ||
          row.candidate >= candidates_.size() ||
          row.coordinate >= o1c82::kCoordinateCount || row.variable <= 0 ||
          row.coordinate != static_cast<uint32_t>(row.variable - 1) ||
          row.parent_assignment_sha256[64] != '\0' ||
          !std::isfinite(row.parent_upper_bound) ||
          row.classification != selected.selection ||
          row.losing_literal != selected.losing_literal ||
          (row.classification !=
               o1c80::ChildSelectionClass::ONE_PRUNABLE &&
           row.classification !=
               o1c80::ChildSelectionClass::BOTH_PRUNABLE) ||
          (row.crossing_eligible && row.consumed_before))
        throw std::runtime_error(
            "O1C109 local-prunable breadcrumb telemetry differs");
    }
    ...
    for (size_t index = 0; index < action_count_; ++index) {
      ...
      if (event.token != index + 1U || !event.call || !event.first_probe ||
        ...
        throw std::runtime_error("O1C109 priority action telemetry differs");
    ...
    if (counted_releases != releases_ ||
      ...
      throw std::runtime_error("O1C109 priority release telemetry differs");
    require_shadow_identity();
  }
  ...
  uint64_t local_prunable_match_count_ = 0;
  uint64_t local_prunable_retained_count_ = 0;
  uint64_t local_prunable_match_bytes_ = 0;
  uint64_t local_prunable_overflow_bytes_ = 0;
  std::array<uint64_t, 2> local_prunable_class_counts_{};
  Sha256 probe_trace_;
  Sha256 action_trace_;
  Sha256 local_prunable_all_digest_;
  Sha256 local_prunable_overflow_digest_;
  bool solve_finalized_ = false;
};

[[maybe_unused]] void print_v33_usage() {
  std::cout << "usage: cadical_o1_joint_score_sieve_v33 --cnf PATH "
...
#ifndef O1_CRYPTO_LAB_O1C109_NO_MAIN
int main(int argc, char **argv) {
  try {
    ...
    if (arguments.seed != 0)
      throw std::runtime_error("O1C109 priority operator requires seed zero");
    ...
    if (production_seal &&
      ...
      throw std::runtime_error("sealed O1C109 priority seed differs");
    if (production_seal &&
        vault_payload.size() == kBurnedPage21ActiveVaultBytes &&
        vault_sha256 == kBurnedPage21ActiveVaultSha256)
      throw std::runtime_error("burned O1C107 Page-21 active vault rejected");
    if (production_seal &&
        vault_payload.size() == kBurnedPage20ActiveVaultBytes &&
        vault_sha256 == kBurnedPage20ActiveVaultSha256)
      throw std::runtime_error("burned O1C105 Page-20 active vault rejected");
    if (production_seal &&
        vault_payload.size() == kBurnedPage19ActiveVaultBytes &&
        vault_sha256 == kBurnedPage19ActiveVaultSha256)
      throw std::runtime_error("burned O1C103 Page-19 active vault rejected");
    if (production_seal &&
    ...
    if (production_seal &&
        (vault_payload.size() != kProductionPage22ActiveVaultBytes ||
         vault_sha256 != kProductionPage22ActiveVaultSha256))
      throw std::runtime_error("sealed O1C109 Page-22 active vault differs");
    ...
    std::string result_json;
    {
      ...
      if (!solver.configure("plain") || !solver.set("seed", arguments.seed) ||
        ...
        throw std::runtime_error(
            "CaDiCaL rejected deterministic O1C109 options");
      int variables = 0;
      if (const char *error =
      ...
      out << std::setprecision(std::numeric_limits<double>::max_digits10)
          << "{\"schema\":\"" << kV33ResultSchema
          << "\",\"implementation_parent_schema\":\""
          << kV33ImplementationParentSchema
      ...
      propagator->write_local_prunable_breadcrumbs_json(out);
    ...
    std::cerr << "cadical_o1_joint_score_sieve_v33: " << error.what() << '\n';
    return 1;
  }
}
#endif
