...
constexpr const char *kDecisionRule =
    "signed_learned_clause_owner_pattern_credit";
...
Arguments parse_arguments(int argc, char **argv) {
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (argument == "--help") {
      std::cout
          << "usage: cadical_o1_learned_clause_credit_search --cnf PATH "
...
class LearnedClauseCreditPropagator final : public CaDiCaL::ExternalPropagator {
public:
  LearnedClauseCreditPropagator(CaDiCaL::Solver *solver, PotentialField field,
                             int variable_count,
    ...
    if (!solver_ ||
      ...
      throw std::runtime_error("learned-clause-credit constructor differs");
  ...
  void write_learned_clause_json(std::ostream &out) const {
    ...
    out << "\"learned_clause_credit\":{\"clause_callbacks\":"
        << learned_clause_callbacks_
...
private:
  ...
  void validate_ledger() const {
    if (groups_.size() != static_cast<size_t>(kRequiredGroups) ||
        ...
        distinct_action_cells_ != penalty_updates_ ||
        distinct_action_cells_ > matched_owner_members_ ||
  ...
  void penalize_learned_action_cell(size_t group_index, unsigned mask,
                                    unsigned member_bits) {
    if (group_index >= states_.size() || mask > 3U || !member_bits ||
        member_bits > 3U)
  ...
  void close_learned_clause() {
    ...
    int64_t clause_action_cells = 0;
    for (size_t group_index = 0; group_index < owners_.size(); ++group_index) {
      const bool first = learned_member_matched(group_index, 0U);
      const bool second = learned_member_matched(group_index, 1U);
      clause_matched_members += static_cast<int64_t>(first) +
                                static_cast<int64_t>(second);
      if (!first && !second)
        continue;
      const OwnerState &owner = owners_.at(group_index);
      if (first) {
        penalize_learned_action_cell(group_index, owner.masks[0],
                                     second && owner.masks[1] == owner.masks[0]
                                         ? 3U
                                         : 1U);
        ++clause_action_cells;
      }
      if (second && (!first || owner.masks[1] != owner.masks[0])) {
        penalize_learned_action_cell(group_index, owner.masks[1], 2U);
        ++clause_action_cells;
      }
    }
    checked_accumulate(matched_owner_members_, clause_matched_members);
    checked_accumulate(distinct_action_cells_, clause_action_cells);
...
class LearnedClauseCreditLearner final : public CaDiCaL::Learner {
public:
  explicit LearnedClauseCreditLearner(LearnedClauseCreditPropagator *propagator)
...
private:
  LearnedClauseCreditPropagator *propagator_;
};
...
int main(int argc, char **argv) {
  try {
    ...
    auto propagator = std::make_unique<LearnedClauseCreditPropagator>(
        &solver, std::move(field), variables, decisions.variables);
    LearnedClauseCreditLearner learner(propagator.get());
    ...
    std::cerr << "cadical_o1_learned_clause_credit_search: " << error.what() << '\n';
    return 1;
  }
}
