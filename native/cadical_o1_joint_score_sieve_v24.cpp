// O1C-0090 live-bank continuation parent-centered failure-first proof-mining operator.
//
...
constexpr const char *kV24ResultSchema =
    "o1-256-cadical-joint-score-sieve-result-v24";
constexpr const char *kV24ImplementationParentSchema =
...
constexpr const char *kPriorityActionSchema =
    "o1-256-o1c90-failure-first-proof-mining-actions-v1";
...
constexpr const char *kO1C89PreparationManifestSha256 =
...
constexpr const char *kProductionPage13ActiveVaultSha256 =
    "4c1b7d5a6d40fad9439d95433bcc7a60ff3e7ddc0e4542b0cf003cdf4581e546";
constexpr size_t kProductionPage13ActiveVaultBytes = 2846623U;
...
static_assert(std::string_view(kO1C89PreparationManifestSha256).size() == 64U);
...
const char *semantic_name(PriorityActionSemantic semantic) {
  ...
  throw std::runtime_error("O1C90 action semantic differs");
}
...
const char *machine_action_name(PriorityActionSemantic semantic) {
  ...
  throw std::runtime_error("O1C90 machine action differs");
}
...
public:
  ParentCenteredGroupedJointScoreSieve(
    ...
    if (candidates_.empty())
      throw std::runtime_error("O1C90 potential observes no key coordinate");
    if (production_seal_ &&
        (candidates_.size() != kExpectedProductionCandidateCount ||
         std::binary_search(candidates_.begin(), candidates_.end(), 241)))
      throw std::runtime_error("sealed O1C90 key-coordinate population differs");
    ...
    if (seed_payload.size() != o1c82::kCoordinateBankBytes)
      throw std::runtime_error("O1C90 priority seed byte count differs");
    if (seed_sha256_ != sha256(seed_payload))
      throw std::runtime_error("O1C90 priority seed payload digest differs");
    if (production_seal_ && seed_sha256_ != kLiveContinuationBankSha256)
      throw std::runtime_error("sealed O1C90 priority seed differs");
    ...
    if (priority_.export_seed().records != image.records ||
      ...
      throw std::runtime_error("O1C90 priority seed import round trip differs");
    ...
    if (production_seal_ && initial_eligible_coordinate_count_ !=
      ...
      throw std::runtime_error(
          "sealed O1C90 live-bank eligibility population differs");
  }
  ...
  void notify_assignment(const std::vector<int> &literals) override {
    ...
    for (const int literal : literals) {
      ...
      if (!slot) {
        ...
        throw std::runtime_error(
            "O1C90 shadow assignment changed without backtrack");
      }
  ...
  void notify_new_decision_level() override {
    if (current_level_ == std::numeric_limits<uint32_t>::max())
      throw std::runtime_error("O1C90 decision level exceeds bound");
    ...
    if (pending_action_index_ != no_action()) {
      ...
      if (action.bound_level)
        throw std::runtime_error("O1C90 action bound twice");
  ...
  void notify_backtrack(size_t new_level) override {
    if (new_level > current_level_ ||
      ...
      throw std::runtime_error("O1C90 backtrack level differs");
    ...
    while (!trail_.empty() && trail_.back().level > new_level) {
      ...
      if (!assignment_.at(index))
        throw std::runtime_error("O1C90 shadow trail state differs");
      assignment_[index] = 0;
      trail_.pop_back();
    }
  ...
  int cb_decide() override {
    if (solve_finalized_)
      throw std::runtime_error("O1C90 callback after solve end");
    if (ownership_.has_pending())
      throw std::runtime_error("O1C90 callback overlaps pending proposal");
    if (base_.cb_decide() != 0)
      throw std::runtime_error("O1C90 unchanged v6 callback differs");
  ...
  void finalize_after_solve() {
    if (solve_finalized_)
      throw std::runtime_error("O1C90 solve end finalized twice");
  ...
  const PriorityActionEvent &action(size_t index) const {
    if (index >= action_count_)
      throw std::runtime_error("O1C90 action index differs");
...
private:
  ...
  size_t local(int variable) const {
    ...
    if (iterator == observed.end() || *iterator != variable)
      throw std::runtime_error("O1C90 variable is unobserved");
  ...
  void require_shadow_identity() const {
    ...
    if (shadow != base_.assignment_state())
      throw std::runtime_error("O1C90 shadow assignment differs from v6");
  }
  ...
  void append_probe_trace(uint64_t call, uint64_t probe,
    ...
    if (record.size() != kProbeTraceRecordBytes)
      throw std::runtime_error("O1C90 probe trace width differs");
  ...
  int scan_and_select(uint64_t call) {
    ...
    if (!pending_clause_literals(base_.pending_state()).empty())
      throw std::runtime_error("O1C90 selector overlaps pending v6 clause");
    ...
    if (assignment_before != base_.assignment_state() ||
      ...
      throw std::runtime_error(
          "O1C90 probes mutated authoritative v6 state");
    ...
    if (!current_present_[coordinate])
      throw std::runtime_error("O1C90 priority winner lacks current bounds");
    ...
    if (literal != last_priority_selection_.action_literal)
      throw std::runtime_error("O1C90 current lower-bound mapping differs");
    return propose_action(
  ...
  int propose_action(uint64_t call, uint64_t first_probe,
    ...
    if (action_count_ >= actions_.size() || coordinate >= consumed_.size() ||
      ...
      throw std::runtime_error("O1C90 one-shot action capacity differs");
    const int expected =
        o1c82::ParentCenteredPriority::lower_upper_bound_action_literal(
            variable, bounds.zero, bounds.one);
    if (literal != expected)
      throw std::runtime_error("O1C90 action is not current lower-UB child");
    ...
    if ((semantic == PriorityActionSemantic::FAILURE_FIRST_PROOF_MINING &&
        ...
        (semantic ==
             PriorityActionSemantic::CERTIFIED_STRICT_BOUND_CROSSING &&
         !(lower < threshold_)))
      throw std::runtime_error("O1C90 action certification differs");
    ...
    if (parent_assignment_sha256.size() != 64U)
      throw std::runtime_error("O1C90 parent assignment digest differs");
  ...
  void apply_release(const o1c80::DecisionToken &token) {
    ...
    if (found == actions_.begin() + static_cast<ptrdiff_t>(action_count_) ||
      ...
      throw std::runtime_error("O1C90 action release lifecycle differs");
    found->released = true;
  ...
  void validate_telemetry() const {
    ...
    if (!solve_finalized_ || callback_calls_ != parent_scans_ ||
      ...
      throw std::runtime_error("O1C90 priority telemetry differs");
    ...
    for (size_t index = 0; index < action_count_; ++index) {
      ...
      if (event.token != index + 1U || !event.call || !event.first_probe ||
        ...
        throw std::runtime_error("O1C90 priority action telemetry differs");
    ...
    if (counted_releases != releases_ ||
      ...
      throw std::runtime_error("O1C90 priority release telemetry differs");
    require_shadow_identity();
  }
...
[[maybe_unused]] void print_v24_usage() {
  std::cout << "usage: cadical_o1_joint_score_sieve_v24 --cnf PATH "
...
#ifndef O1_CRYPTO_LAB_O1C90_NO_MAIN
int main(int argc, char **argv) {
  try {
    ...
    if (arguments.seed != 0)
      throw std::runtime_error("O1C90 priority operator requires seed zero");
    ...
    if (production_seal &&
        (vault_payload.size() != kProductionPage13ActiveVaultBytes ||
         vault_sha256 != kProductionPage13ActiveVaultSha256))
      throw std::runtime_error("sealed O1C90 Page-13 active vault differs");
    ...
    std::string result_json;
    {
      ...
      if (!solver.configure("plain") || !solver.set("seed", arguments.seed) ||
        ...
        throw std::runtime_error(
            "CaDiCaL rejected deterministic O1C90 options");
      int variables = 0;
      if (const char *error =
      ...
      out << std::setprecision(std::numeric_limits<double>::max_digits10)
          << "{\"schema\":\"" << kV24ResultSchema
          << "\",\"implementation_parent_schema\":\""
          << kV24ImplementationParentSchema
    ...
    std::cerr << "cadical_o1_joint_score_sieve_v24: " << error.what() << '\n';
    return 1;
  }
}
#endif
