// Forced-initial-phase reader successor to the frozen native-v6 vault solver.
//
...
// the deterministic initial phase, and records that reader intervention in the
// top-level payload.  There is intentionally no phase-selection CLI surface.
...
constexpr const char *kV7ResultSchema =
    "o1-256-cadical-joint-score-sieve-result-v7";
constexpr const char *kV7ImplementationParentSchema =
...
constexpr int kForcedInitialPhase = 0;
...
void print_v7_usage() {
  std::cout << "usage: cadical_o1_joint_score_sieve_v7 --cnf PATH "
  ...
  try {
    ...
    std::string result_json;
    {
      ...
      if (!solver.set("phase", kForcedInitialPhase) ||
          ...
          solver.get("factor") != 0 ||
          solver.get("rephase") != 0 || solver.get("lucky") != 0 ||
          solver.get("walk") != 0)
        throw std::runtime_error(
    ...
    std::cerr << "cadical_o1_joint_score_sieve_v7: " << error.what() << '\n';
    return 1;
  }
}
