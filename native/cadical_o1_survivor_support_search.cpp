...
constexpr const char *kSchema =
    "o1-256-cadical-survivor-support-search-result-v1";
...
constexpr int kCreditMinimum = 0;
...
Arguments parse_arguments(int argc, char **argv) {
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (argument == "--help") {
      std::cout
          << "usage: cadical_o1_survivor_support_search --cnf PATH "
...
class SurvivorSupportPropagator final : public CaDiCaL::ExternalPropagator {
public:
  SurvivorSupportPropagator(CaDiCaL::Solver *solver, PotentialField field,
                             int variable_count,
    ...
    if (!solver_ ||
      ...
      throw std::runtime_error("survivor-support constructor differs");
  ...
  void notify_backtrack(size_t new_level) override {
    ...
    apply_survivor_support(new_level);
  ...
  int64_t supported_action_cells() const {
    int64_t result = 0;
    for (const ActionGroupState &state : states_)
      for (const ActionCell &action : state.actions)
  ...
  void write_survivor_json(std::ostream &out) const {
    ...
    out << ",\"maximum_raw_gap\":" << maximum_raw_gap_
        ...
        << ",\"supported_action_cells\":" << supported_action_cells()
    ...
    out << "\"survivor_support\":{\"callbacks\":"
        << support_backtrack_callbacks_
        ...
        << conflict_undone_owner_clears_
        << ",\"nonconflict_undone_owner_clears\":"
        << nonconflict_undone_owner_clears_
        << ",\"conflict_callbacks_with_survivor\":"
        << conflict_callbacks_with_survivor_
        << ",\"conflict_callbacks_without_survivor\":"
        << conflict_callbacks_without_survivor_
        << ",\"survivor_candidates_examined\":"
        << survivor_candidates_examined_
        << ",\"deepest_level_tie_callbacks\":"
        << deepest_level_tie_callbacks_
        << ",\"support_updates\":" << support_updates_
        << ",\"support_units\":" << support_units_
        << ",\"assignment_support_units\":0,\"propagation_support_units\":0"
        << ",\"undone_owner_support_units\":0,\"all_survivor_support_units\":0"
        << ",\"maximum_supported_owner_level\":"
        << maximum_supported_owner_level_ << ",\"last_survivor_group\":";
    if (last_survivor_group_ < 0)
      out << "null";
    else
      out << last_survivor_group_;
    out << ",\"last_survivor_member\":";
    if (last_survivor_member_ < 0)
      out << "null";
    else
      out << last_survivor_member_;
    out << ",\"last_survivor_level\":";
    if (last_survivor_group_ < 0)
      out << "null";
    else
      out << last_survivor_level_;
    out << ",\"last_survivor_mask\":";
    if (last_survivor_mask_ < 0)
      out << "null";
    else
      out << last_survivor_mask_;
    out << ",\"trace_sha256\":\"" << sha256(survivor_trace_) << "\"},";
...
private:
  ...
  void validate_ledger() const {
    if (groups_.size() != static_cast<size_t>(kRequiredGroups) ||
        ...
        pending_.open || support_backtrack_callbacks_ != backtracks_ ||
        support_backtrack_callbacks_ !=
            conflict_backtrack_callbacks_ + nonconflict_backtrack_callbacks_ ||
        undone_owner_clears_ != conflict_undone_owner_clears_ +
                                    nonconflict_undone_owner_clears_ ||
        pending_bound_ != undone_owner_clears_ + live_owners_ ||
        conflict_backtrack_callbacks_ != conflict_callbacks_with_survivor_ +
                                             conflict_callbacks_without_survivor_ ||
        support_updates_ != conflict_callbacks_with_survivor_ ||
        support_units_ != 32 * support_updates_ ||
        deepest_level_tie_callbacks_ > support_updates_ ||
        maximum_supported_owner_level_ > maximum_level_ ||
        (support_updates_ == 0 &&
         (last_survivor_group_ >= 0 || last_survivor_member_ >= 0 ||
          last_survivor_level_ || last_survivor_mask_ >= 0)) ||
        (support_updates_ > 0 &&
         (last_survivor_group_ < 0 || last_survivor_member_ < 0 ||
          !last_survivor_level_ || last_survivor_mask_ < 0 ||
          last_survivor_group_ >= kRequiredGroups ||
          last_survivor_member_ >= 2 || last_survivor_mask_ >= 4)) ||
      ...
      throw std::runtime_error("survivor final telemetry ledger differs");
    ...
    for (size_t group_index = 0; group_index < owners_.size(); ++group_index) {
      for (const ActionCell &action : states_[group_index].actions)
        if (action.credit < 0)
          throw std::runtime_error("survivor action-state invariant differs");
      const PairGroup &group = groups_[group_index];
      for (unsigned member = 0; member < 2U; ++member) {
  ...
  void apply_survivor_support(size_t new_level) {
    ...
    for (size_t group_index = 0; group_index < owners_.size(); ++group_index) {
      ...
      for (unsigned member = 0; member < 2U; ++member) {
        ...
        if (owner_mask > 3U)
          throw std::runtime_error("survivor undone owner mask differs");
    ...
    if (callback_undo_members > live_owners_)
      throw std::runtime_error("survivor live-owner ledger underflow");
    live_owners_ -= callback_undo_members;
    checked_accumulate(undone_owner_clears_, callback_undo_members);
    if (conflict)
      checked_accumulate(conflict_undone_owner_clears_, callback_undo_members);
    else
      checked_accumulate(nonconflict_undone_owner_clears_,
                         callback_undo_members);
    ...
    unsigned selected_member = 0;
    uint32_t selected_level = 0;
    uint8_t selected_mask = 0;
    int64_t selected_level_multiplicity = 0;
    for (size_t group_index = 0; group_index < owners_.size(); ++group_index) {
      const OwnerState &owner = owners_[group_index];
      for (unsigned member = 0; member < 2U; ++member) {
        const uint32_t owner_level = owner.levels[member];
        const uint8_t owner_mask = owner.masks[member];
        if (!owner_level) {
          if (owner_mask)
            throw std::runtime_error(
                "survivor empty owner mask is not canonical");
          continue;
        }
        if (owner_level > new_level || owner_mask > 3U)
          throw std::runtime_error("survivor candidate differs");
        ++survivor_candidates_examined_;
        if (!found || owner_level > selected_level) {
          found = true;
          selected_group = group_index;
          selected_member = member;
          selected_level = owner_level;
          selected_mask = owner_mask;
          selected_level_multiplicity = 1;
        } else if (owner_level == selected_level) {
          ++selected_level_multiplicity;
        }
      }
    }
    if (!found) {
    ...
    if (selected_level_multiplicity > 1)
      ++deepest_level_tie_callbacks_;
    ++conflict_callbacks_with_survivor_;
    ActionCell &supported =
        states_.at(selected_group).actions.at(selected_mask);
    supported.credit = saturating_credit_add(supported.credit, 32);
    supported.conflict_hits = saturating_counter_add(supported.conflict_hits, 1);
    ++support_updates_;
    checked_accumulate(support_units_, 32);
    last_survivor_group_ = static_cast<int>(selected_group);
    last_survivor_member_ = static_cast<int>(selected_member);
    last_survivor_level_ = selected_level;
    last_survivor_mask_ = static_cast<int>(selected_mask);
    maximum_supported_owner_level_ =
        std::max(maximum_supported_owner_level_, selected_level);
    survivor_trace_ += std::to_string(selected_group) + " " +
  ...
  int64_t support_backtrack_callbacks_ = 0;
  int64_t conflict_backtrack_callbacks_ = 0;
  int64_t nonconflict_backtrack_callbacks_ = 0;
  int64_t undone_owner_clears_ = 0;
  int64_t conflict_undone_owner_clears_ = 0;
  int64_t nonconflict_undone_owner_clears_ = 0;
  int64_t conflict_callbacks_with_survivor_ = 0;
  int64_t conflict_callbacks_without_survivor_ = 0;
  int64_t survivor_candidates_examined_ = 0;
  int64_t deepest_level_tie_callbacks_ = 0;
  int64_t support_updates_ = 0;
  int64_t support_units_ = 0;
  int last_survivor_group_ = -1;
  int last_survivor_member_ = -1;
  uint32_t last_survivor_level_ = 0;
  int last_survivor_mask_ = -1;
  uint32_t maximum_supported_owner_level_ = 0;
  std::string survivor_trace_;
...
int main(int argc, char **argv) {
  try {
    ...
    auto propagator = std::make_unique<SurvivorSupportPropagator>(
    ...
    std::cout << ",\"cnf_sha256\":\"" << sha256(cnf_payload)
              ...
              << statistic(solver, "propagations") << "},\"survivor\":{"
              ...
              << "\",";
    propagator->write_survivor_json(std::cout);
    ...
    std::cerr << "cadical_o1_survivor_support_search: " << error.what() << '\n';
    return 1;
  }
}
