...
new identity, resolve the authoritative O1C-0019/22/23 artifact graph and use
nested cross-fitting so no held-out label reaches its hot-reader fit indirectly.
...
6. Preserve O1C-0028 `17c02df` and its finalized capsule as the K256
   packet/state/readout ABI. After O1C-0022 and O1C-0023 finalize, open a new
   attempt, verify their complete authoritative graph and nested-cross-fit each
   held-out fold's immutable hot reader from states generated by that fold's
   upstream reader. Query the held-out state once without replay. Treat packet
   evidence, quantizer, addressing, encoder, recurrence and phase changes as cold.
