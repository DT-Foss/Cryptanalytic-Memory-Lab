...
O1C-0100 completes the deterministic zero-call successor at active limit 249
and publishes the exact ten-file / 3,658,593-byte bundle. The same 19-chunk /
2,074-clause attic is reprojected onto fresh Page 18 / lineage 31 as 249 clauses /
669,910 literals / 2,680,827 B, SHA `5d89bbe0…`, with 9 roots + 43 pinned + 65
new debt + 132 recycled. It admits all 65 prior nonresident undominated clauses,
...
The parent call is censored at the first clause overflow, `252+261=513`, after only ten
...
therefore selected one unchanged Page-15 continuation. Native v26 executed it
to completion, but adapter v29 rejected the returned `priority_seed` because
its exact field set lacked the two source-receipt provenance fields. Actual and
billed conflicts are consequently `null`; this is transport failure, not the
uncensored zero-novelty result required for a residency pivot.
...
is `756,414/47,514 = 15.9198131077x`; O1C-0094 now serializes and round-trips
this quotient exactly in 18,034 retained bytes and at most 29,766 live decoder
bytes. It is a representation result, not a key consequence:
