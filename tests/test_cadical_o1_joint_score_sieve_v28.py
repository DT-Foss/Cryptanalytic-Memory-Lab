...
import o1_crypto_lab.joint_score_sieve_v31 as adapter_v31

ROOT = Path(__file__).resolve().parents[1]
NATIVE = ROOT / "native"
V26 = NATIVE / "cadical_o1_joint_score_sieve_v26.cpp"
V28 = NATIVE / "cadical_o1_joint_score_sieve_v28.cpp"
...
MANIFEST_BYTES = 18_785
MANIFEST_SHA256 = "ba7ad5d9417542d62725ab588dea4a85bc7eff8847f5276bf79a847f44c5470d"
...
HARNESS = r"""
#define O1_CRYPTO_LAB_O1C99_NO_MAIN
#include "cadical_o1_joint_score_sieve_v28.cpp"
...
def native_builds(tmp_path_factory: pytest.TempPathFactory) -> tuple[Path, Path]:
    ...
    for flags, source, executable in (
        (
            [*common, "-DO1_CRYPTO_LAB_O1C99_PUBLIC_FIXTURE"],
            harness_source,
            harness,
        ),
        (common, V28, main),
...
def test_v28_normal_production_binary_compiles_help_and_uuid(
    ...
    assert completed.stdout == (
        "usage: cadical_o1_joint_score_sieve_v28 --cnf PATH --potential PATH "
...
def test_real_native_v28_priority_seed_crosses_exact_v31_adapter_seam(
    native_builds: tuple[Path, Path],
) -> None:
    harness, _ = native_builds
    completed = _run(harness, "import", LIVE_BANK)
    ...
    validated = adapter_v31._validate_seed_report(
        actual,
        seed_sha256=LIVE_BANK_SHA256,
        production_seal=False,
    )
    assert validated == actual
    assert set(actual) == adapter_v31._SEED_FIELDS
...
def test_native_rejects_prior_bank_before_solver(
    ...
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v28: sealed O1C99 priority seed differs\n"
    )
...
def test_native_requires_seed_zero_before_inputs(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v28: O1C99 priority operator requires seed zero\n"
    )
...
def test_native_rejects_every_burned_page_before_solver(
    ...
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == f"cadical_o1_joint_score_sieve_v28: {message}\n"


def test_native_rejects_nonmatching_page17_before_solver(
    native_builds: tuple[Path, Path], tmp_path: Path
) -> None:
    _, main = native_builds
    forged = tmp_path / "forged-page-17-active.bin"
    forged.write_bytes(bytes(2_773_919))
    completed = _sealed_command(main, tmp_path, forged, LIVE_BANK)
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v28: sealed O1C99 Page-17 active vault differs\n"
    )


def test_source_contract_and_o1c98_binding() -> None:
    source = V28.read_text(encoding="utf-8")
    ...
    assert "O1_CRYPTO_LAB_O1C99_PUBLIC_FIXTURE" in source
    assert "O1_CRYPTO_LAB_O1C99_NO_MAIN" in source
    assert "o1-256-cadical-joint-score-sieve-result-v28" in source
    assert "o1-256-o1c99-live-parent-centered-continuation-priority-state-v1" in source
    assert "o1-256-o1c99-failure-first-proof-mining-actions-v1" in source
    ...
    assert manifest["schema"] == ("o1-256-o1c98-page17-causal-rollover-preparation-v1")
    assert manifest["attic"]["union_clause_count"] == 2_074
    assert manifest["page17"]["lineage_ordinal"] == 30
    assert manifest["page17"]["clause_count"] == 249
    assert manifest["page17"]["literal_count"] == 693_183
    assert manifest["page17"]["serialized_bytes"] == 2_773_919
    ...
    assert "kProductionPage17ActiveVaultBytes = 2773919U" in source
    assert "kBurnedPage16ActiveVaultBytes = 2831459U" in source
    assert "kBurnedPage15ActiveVaultBytes = 2843047U" in source
    assert "kProductionLineageOrdinal = 30U" in source
    assert "kProductionActiveLiteralCount = 693183U" in source
    ...
    page17_gate = source.index("vault_sha256 != kProductionPage17ActiveVaultSha256")
...
def _historical_v27_equivalence_recipe() -> None:
    ...
    constexpr size_t kProductionPage17ActiveVaultBytes = 2831459U;
    ...
    """
        ...
        for current, prior in (
            ...
            ("V28", "V26"),
            ("v28", "v26"),
            ("kProductionPage17", "kProductionPage15"),
...
def test_v28_preserves_the_v27_operator_boundary_and_changes_only_seals() -> None:
    current = V28.read_text(encoding="utf-8")
    ...
    assert "o1-256-cadical-joint-score-sieve-result-v28" in current
