...
import o1_crypto_lab.joint_score_sieve_v25 as sieve
...
RECEIPT = ROOT / sieve.O1C85_PRIORITY_RECEIPT_RELATIVE
LIVE_BANK = ROOT / sieve.O1C86_LIVE_BANK_RELATIVE
MANIFEST = ROOT / sieve.O1C86_MANIFEST_RELATIVE
PAGE11 = ROOT / sieve.O1C86_PAGE11_RELATIVE
PAGE10 = (
...
OLD_LIVE_BANK = (
    ROOT / "research/o1c85_page10_transport_recovery_seed_20260720/"
    "final-parent-centered-priority-bank.bin"
)
V24 = ROOT / "src/o1_crypto_lab/joint_score_sieve_v24.py"
FROZEN_V24_SHA256 = "9331233a58bb357fb37155f6ba01af1801a0abfd4f52857eb282ee5976a193a3"
NATIVE_V22_SHA256 = "d4fb10ad0a43e65acf7629ebcb6c87a81277494a8eec569a1723f383608fb38f"
...
def test_o1c86_known_production_bindings_are_exact() -> None:
    assert sieve.JOINT_SCORE_SIEVE_ADAPTER_SCHEMA.endswith("v25-adapter-v1")
    assert sieve.JOINT_SCORE_SIEVE_RESULT_SCHEMA.endswith("result-v22")
    assert "o1c86" in sieve.PRIORITY_STATE_SCHEMA
    assert "o1c86" in sieve.PRIORITY_ACTION_SCHEMA
    assert sieve.PRODUCTION_PAGE11_LINEAGE_ORDINAL == 24
    assert sieve.PRODUCTION_PAGE11_BYTES == 2_876_731
    assert (
        sieve.PRODUCTION_PAGE11_SHA256
...
def test_sealed_live_bank_exact_aggregate_and_old_bank_rejected() -> None:
    ...
    assert sum(record.count for record in records) == 82_330
    assert max(record.count for record in records) == 829
...
def test_output_counts_may_exceed_829_but_must_replay_probe_delta() -> None:
    ...
    assert max(record.count for record in after) == 830
...
def test_o1c85_receipt_replays_and_links_exact_final_bank() -> None:
    receipt = RECEIPT.read_bytes()
    bank = LIVE_BANK.read_bytes()
    assert len(receipt) == sieve.O1C85_PRIORITY_RECEIPT_BYTES == 51_273
    assert _sha(receipt) == sieve.O1C85_PRIORITY_RECEIPT_SHA256
...
def test_published_o1c86_manifest_contract_is_exact() -> None:
    assert (
        sieve.O1C86_MANIFEST_SCHEMA
        == "o1-256-o1c86-page11-causal-rollover-preparation-v1"
    )
    assert sieve.O1C86_MANIFEST_BYTES == 6_893
    assert (
        sieve.O1C86_MANIFEST_SHA256
        == "2e9f30aa193f1f640523e333bbb7990db6cdf32e3f54ebee19600ab307366116"
    )
    assert sieve.O1C86_MANIFEST_ARTIFACTS == frozenset(
        {
    ...
    assert sieve.O1C86_MANIFEST_RELATIVE == Path(
        "research/o1c86_page11_causal_rollover_seed_20260720/"
        "causal-rollover-preparation-manifest.json"
    )
    assert sieve.O1C86_PAGE11_RELATIVE == Path(
        "research/o1c86_page11_causal_rollover_seed_20260720/page-11-active.bin"
    )
    assert sieve.PRODUCTION_PAGE11_ACTIVE_LIMIT == 254
    assert sieve.PRODUCTION_PAGE11_CLAUSE_COUNT == 254
    assert sieve.PRODUCTION_PAGE11_LITERAL_COUNT == 718_881
    assert sieve.PRODUCTION_PAGE11_CATEGORY_COUNTS == {
        "structural_root": 5,
        "pinned_core": 43,
        "inherited_debt": 0,
        "new_debt": 21,
    ...
    assert sieve.PRODUCTION_PAGE11_HEADROOM == {
        "clauses": 258,
        "literals": 881_119,
        "serialized_bytes": 5_511_877,
    }
    payload = MANIFEST.read_bytes()
    assert len(payload) == sieve.O1C86_MANIFEST_BYTES
    assert _sha(payload) == sieve.O1C86_MANIFEST_SHA256
    assert sieve._manifest_contract() == (
        sieve.O1C86_MANIFEST_SCHEMA,
        sieve.O1C86_MANIFEST_BYTES,
        sieve.O1C86_MANIFEST_SHA256,
        sieve.O1C86_MANIFEST_ARTIFACTS,
    )
    sieve._validate_manifest(payload)

    forged = json.loads(payload)
    forged["page11"]["native_capacity_proof"][
...
def test_page11_reader_accepts_only_the_bound_identity(tmp_path: Path) -> None:
    payload = PAGE11.read_bytes()
    assert len(payload) == sieve.PRODUCTION_PAGE11_BYTES
    assert _sha(payload) == sieve.PRODUCTION_PAGE11_SHA256
    resolved, accepted = sieve._read_page11(PAGE11)
    assert resolved == PAGE11.resolve()
    assert accepted == payload

    forged = bytearray(payload)
    forged[len(forged) // 2] ^= 1
    path = tmp_path / "forged-page-11-active.bin"
    path.write_bytes(forged)
    with pytest.raises(O1RelationalSearchError, match="Page11 active projection"):
        sieve._read_page11(path)


def test_burned_page10_and_page9_are_rejected() -> None:
...
def test_historical_v24_adapter_is_untouched() -> None:
    assert _sha(V24.read_bytes()) == FROZEN_V24_SHA256


def test_native_v22_source_contract() -> None:
    source_bytes = SOURCE.read_bytes()
    source = source_bytes.decode("utf-8")
    assert _sha(source_bytes) == NATIVE_V22_SHA256
    ...
    assert sieve.PRODUCTION_PAGE11_SHA256 in source
