...
import o1_crypto_lab.joint_score_sieve_v34 as adapter_v34

ROOT = Path(__file__).resolve().parents[1]
NATIVE = ROOT / "native"
V26 = NATIVE / "cadical_o1_joint_score_sieve_v26.cpp"
V31 = NATIVE / "cadical_o1_joint_score_sieve_v31.cpp"
...
MANIFEST_BYTES = 9_830
MANIFEST_SHA256 = "2e784eea7ef8fb85913e45246935fa26206626bebf52f0dcb5fc8b9672ba59c5"
...
HARNESS = r"""
#define O1_CRYPTO_LAB_O1C105_NO_MAIN
#include "cadical_o1_joint_score_sieve_v31.cpp"
...
def native_builds(tmp_path_factory: pytest.TempPathFactory) -> tuple[Path, Path]:
    ...
    for flags, source, executable in (
        (
            [*common, "-DO1_CRYPTO_LAB_O1C105_PUBLIC_FIXTURE"],
            harness_source,
            harness,
        ),
        (common, V31, main),
...
def test_v31_normal_production_binary_compiles_help_and_uuid(
    ...
    assert completed.stdout == (
        "usage: cadical_o1_joint_score_sieve_v31 --cnf PATH --potential PATH "
...
def test_real_native_v31_priority_seed_crosses_exact_v34_adapter_seam(
    native_builds: tuple[Path, Path],
) -> None:
    harness, _ = native_builds
    completed = _run(harness, "import", LIVE_BANK)
    ...
    validated = adapter_v34._validate_seed_report(
        actual,
        seed_sha256=LIVE_BANK_SHA256,
        production_seal=False,
    )
    assert validated == actual
    assert set(actual) == adapter_v34._SEED_FIELDS
...
def test_native_rejects_prior_bank_before_solver(
    ...
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v31: sealed O1C105 priority seed differs\n"
    )
...
def test_native_requires_seed_zero_before_inputs(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v31: O1C105 priority operator requires seed zero\n"
    )
...
def test_native_rejects_every_burned_page_before_solver(
    ...
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == f"cadical_o1_joint_score_sieve_v31: {message}\n"


def test_native_rejects_nonmatching_page20_before_solver(
    native_builds: tuple[Path, Path], tmp_path: Path
) -> None:
    _, main = native_builds
    forged = tmp_path / "forged-page-20-active.bin"
    forged.write_bytes(bytes(2_762_455))
    completed = _sealed_command(main, tmp_path, forged, LIVE_BANK)
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v31: sealed O1C105 Page-20 active vault differs\n"
    )


def test_source_contract_and_o1c104_binding() -> None:
    source = V31.read_text(encoding="utf-8")
    ...
    assert "O1_CRYPTO_LAB_O1C105_PUBLIC_FIXTURE" in source
    assert "O1_CRYPTO_LAB_O1C105_NO_MAIN" in source
    assert "o1-256-cadical-joint-score-sieve-result-v31" in source
    assert "o1-256-o1c105-live-parent-centered-continuation-priority-state-v1" in source
    assert "o1-256-o1c105-failure-first-proof-mining-actions-v1" in source
    ...
    assert manifest["schema"] == (
        "o1-256-o1c104-page20-causal-rollover-preparation-v1"
    )
    assert manifest["emitted_causal_attic"]["union_clause_count"] == 2_603
    ...
    assert manifest["page20"]["lineage_ordinal"] == 33
    assert manifest["page20"]["clause_count"] == 247
    assert manifest["page20"]["literal_count"] == 690_319
    assert manifest["page20"]["serialized_bytes"] == 2_762_455
    assert (
        manifest["science_boundary"][
            "inherited_certified_derived_resolution_clause_count"
        ]
        == 5
    )
    assert (
        manifest["science_boundary"]["new_certified_derived_resolution_clause_count"]
        == 84
    )
    assert manifest["science_boundary"]["derived_occurrence_count"] == 0
    ...
    assert "kProductionPage20ActiveVaultBytes = 2762455U" in source
    ...
    assert "kProductionLineageOrdinal = 33U" in source
    assert "kProductionActiveLiteralCount = 690319U" in source
    ...
    page20_gate = source.index("vault_sha256 != kProductionPage20ActiveVaultSha256")
...
def test_v31_preserves_the_v30_operator_boundary_and_changes_only_seals() -> None:
    current = V31.read_text(encoding="utf-8")
    previous = (NATIVE / "cadical_o1_joint_score_sieve_v30.cpp").read_text(
        encoding="utf-8"
    )
    for token in (
        "scan_and_select",
    ...
    assert "o1-256-cadical-joint-score-sieve-result-v31" in current
