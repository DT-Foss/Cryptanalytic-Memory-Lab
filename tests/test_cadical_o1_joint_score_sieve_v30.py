...
import o1_crypto_lab.joint_score_sieve_v33 as adapter_v33

ROOT = Path(__file__).resolve().parents[1]
NATIVE = ROOT / "native"
V26 = NATIVE / "cadical_o1_joint_score_sieve_v26.cpp"
V30 = NATIVE / "cadical_o1_joint_score_sieve_v30.cpp"
...
STATE_RECEIPT = (
    ...
    / "research"
    / "o1c102_page19_causal_rollover_seed_20260721"
    / "o1c101-priority-state-receipt.json"
)
PAGE15 = (
    ROOT
    / "research"
...
DERIVED_RECEIPT = (
    ...
    / "research"
    / "o1c102_page19_causal_rollover_seed_20260721"
    / "o1c102-derived-resolution-closure-receipt.json"
)
PAGE18 = (
    ROOT
    / "research"
...
MANIFEST_BYTES = 8_012
MANIFEST_SHA256 = "9e3e2dd88c5688b88ff2f7673f161577f3b5cafc36bf2c060cc4388d5dfdaad0"
...
HARNESS = r"""
#define O1_CRYPTO_LAB_O1C103_NO_MAIN
#include "cadical_o1_joint_score_sieve_v30.cpp"
...
def frozen_inputs() -> tuple[Path, ...]:
    exact = (
        ...
        (STATE_RECEIPT, 52_013, STATE_RECEIPT_SHA256),
...
def native_builds(tmp_path_factory: pytest.TempPathFactory) -> tuple[Path, Path]:
    ...
    for flags, source, executable in (
        (
            [*common, "-DO1_CRYPTO_LAB_O1C103_PUBLIC_FIXTURE"],
            harness_source,
            harness,
        ),
        (common, V30, main),
...
def test_v30_normal_production_binary_compiles_help_and_uuid(
    ...
    assert completed.stdout == (
        "usage: cadical_o1_joint_score_sieve_v30 --cnf PATH --potential PATH "
...
def test_live_evolved_bank_import_and_machine_readable_provenance(
    ...
    assert seed["source_priority_state_receipt_bytes"] == 52_013
...
def test_real_native_v30_priority_seed_crosses_exact_v33_adapter_seam(
    native_builds: tuple[Path, Path],
) -> None:
    harness, _ = native_builds
    completed = _run(harness, "import", LIVE_BANK)
    ...
    validated = adapter_v33._validate_seed_report(
        actual,
        seed_sha256=LIVE_BANK_SHA256,
        production_seal=False,
    )
    assert validated == actual
    assert set(actual) == adapter_v33._SEED_FIELDS
...
def test_native_rejects_prior_bank_before_solver(
    ...
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v30: sealed O1C103 priority seed differs\n"
    )
...
def test_native_requires_seed_zero_before_inputs(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v30: O1C103 priority operator requires seed zero\n"
    )
...
def test_native_rejects_every_burned_page_before_solver(
    ...
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == f"cadical_o1_joint_score_sieve_v30: {message}\n"


def test_native_rejects_nonmatching_page19_before_solver(
    native_builds: tuple[Path, Path], tmp_path: Path
) -> None:
    _, main = native_builds
    forged = tmp_path / "forged-page-19-active.bin"
    forged.write_bytes(bytes(2_810_555))
    completed = _sealed_command(main, tmp_path, forged, LIVE_BANK)
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v30: sealed O1C103 Page-19 active vault differs\n"
    )


def test_source_contract_and_o1c102_binding() -> None:
    source = V30.read_text(encoding="utf-8")
    ...
    assert "O1_CRYPTO_LAB_O1C103_PUBLIC_FIXTURE" in source
    assert "O1_CRYPTO_LAB_O1C103_NO_MAIN" in source
    assert "o1-256-cadical-joint-score-sieve-result-v30" in source
    assert "o1-256-o1c103-live-parent-centered-continuation-priority-state-v1" in source
    assert "o1-256-o1c103-failure-first-proof-mining-actions-v1" in source
    ...
    assert manifest["schema"] == (
        "o1-256-o1c102-page19-causal-rollover-preparation-v1"
    )
    assert manifest["emitted_causal_attic"]["union_clause_count"] == 2_338
    assert manifest["logical_known_registry"]["combined_clause_count"] == 2_343
    assert manifest["logical_known_registry"]["derived_clause_count"] == 5
    assert manifest["page19"]["lineage_ordinal"] == 32
    assert manifest["page19"]["clause_count"] == 248
    assert manifest["page19"]["literal_count"] == 702_343
    assert manifest["page19"]["serialized_bytes"] == 2_810_555
    assert manifest["science_boundary"]["certified_derived_resolution_clause_count"] == 5
    ...
    assert "kProductionPage19ActiveVaultBytes = 2810555U" in source
    ...
    assert "kProductionLineageOrdinal = 32U" in source
    assert "kProductionActiveLiteralCount = 702343U" in source
    assert "kProductionClauseHeadroom = 264U" in source
    assert "kGlobalNoveltyBaselineClauseCount = 2343U" in source
    ...
    page19_gate = source.index("vault_sha256 != kProductionPage19ActiveVaultSha256")
...
def test_v30_preserves_the_v29_operator_boundary_and_changes_only_seals() -> None:
    current = V30.read_text(encoding="utf-8")
    previous = (NATIVE / "cadical_o1_joint_score_sieve_v29.cpp").read_text(
        encoding="utf-8"
    )
    for token in (
        "scan_and_select",
    ...
    assert "kProductionActiveLimit = 248U" in current
    assert "kProductionClauseHeadroom = 264U" in current
    assert "kBurnedPage18ActiveVaultSha256" in current
    assert "o1-256-cadical-joint-score-sieve-result-v30" in current
