...
import o1_crypto_lab.joint_score_sieve_v31 as sieve
...
BUNDLE = ROOT / "research/o1c98_page17_causal_rollover_seed_20260720"
RECEIPT = ROOT / sieve.O1C97_PRIORITY_RECEIPT_RELATIVE
LIVE_BANK = ROOT / sieve.O1C98_LIVE_BANK_RELATIVE
MANIFEST = ROOT / sieve.O1C98_MANIFEST_RELATIVE
PAGE17 = ROOT / sieve.O1C98_PAGE17_RELATIVE
PAGE16 = (
    ROOT / "research/o1c96_page16_transport_recovery_seed_20260720/page-16-active.bin"
)
...
def test_o1c98_known_production_bindings_are_exact() -> None:
    assert sieve.JOINT_SCORE_SIEVE_ADAPTER_SCHEMA.endswith("v31-adapter-v1")
    assert sieve.JOINT_SCORE_SIEVE_RESULT_SCHEMA.endswith("result-v28")
    assert "o1c99" in sieve.PRIORITY_STATE_SCHEMA
    assert "o1c99" in sieve.PRIORITY_ACTION_SCHEMA
    assert sieve.NATIVE_SOURCE_BYTES == SOURCE.stat().st_size
    assert sieve.NATIVE_SOURCE_SHA256 == _sha(SOURCE.read_bytes())
    assert sieve.PRODUCTION_PAGE16_LINEAGE_ORDINAL == 30
    assert sieve.PRODUCTION_PAGE16_BYTES == 2_773_919
    assert sieve.PRODUCTION_PAGE16_SHA256 == (
        "0c25ce470df0945fb05914bab107ecea05531166575ec88ebf7d15bb9a22fbfd"
    )
    assert sieve.O1C98_MANIFEST_BYTES == MANIFEST.stat().st_size
    assert sieve.O1C98_MANIFEST_SHA256 == _sha(MANIFEST.read_bytes())
...
def test_sealed_live_bank_exact_aggregate_and_old_bank_rejected() -> None:
    ...
    assert sum(record.count for record in records) == 316_312
    assert max(record.count for record in records) == 2_921
    assert min(record.count for record in records if record.count) == 230
...
def test_output_counts_may_grow_but_must_replay_exact_probe_delta() -> None:
    ...
    assert max(record.count for record in after) == 2_922
...
def test_o1c97_receipt_replays_and_links_exact_carried_bank() -> None:
    receipt = RECEIPT.read_bytes()
    bank = LIVE_BANK.read_bytes()
    assert len(receipt) == sieve.O1C97_PRIORITY_RECEIPT_BYTES == 52_011
    assert _sha(receipt) == sieve.O1C97_PRIORITY_RECEIPT_SHA256
...
def test_published_o1c98_manifest_contract_is_exact() -> None:
    assert sieve.O1C98_MANIFEST_ARTIFACTS == frozenset(
        {
    ...
    assert len(sieve.O1C98_PUBLISHED_ARTIFACTS) == 10
    payload = MANIFEST.read_bytes()
    assert len(payload) == sieve.O1C98_MANIFEST_BYTES
    assert _sha(payload) == sieve.O1C98_MANIFEST_SHA256
    assert sieve._manifest_contract() == (
        sieve.O1C98_MANIFEST_SCHEMA,
        sieve.O1C98_MANIFEST_BYTES,
        sieve.O1C98_MANIFEST_SHA256,
        sieve.O1C98_MANIFEST_ARTIFACTS,
    )
    sieve._validate_manifest(payload)

    manifest = json.loads(payload)
    assert manifest["science_boundary"]["imported_fully_emitted_clause_count"] == 263
    assert manifest["science_boundary"]["imported_globally_novel_clause_count"] == 262
    assert manifest["rollover"]["clause_count"] == 262
    assert manifest["parent"]["classification"] == (
        "PARENT_CENTERED_CONTINUATION_NOVEL_CLAUSE_GAIN"
    )
    assert manifest["page17"]["clause_count"] == 249
    assert (
        manifest["carried_context"]["historical_failure_receipt_carried_forward"]
        is False
    )
...
@pytest.mark.parametrize(
    ("section", "field", "value"),
    (
        ("science_boundary", "imported_globally_novel_clause_count", 261),
        ("rollover", "clause_count", 261),
        ("page17", "literal_count", 693_182),
        ("parent", "page16_burned", False),
    ),
)
...
def test_page17_reader_accepts_only_fresh_identity(tmp_path: Path) -> None:
    payload = PAGE17.read_bytes()
    assert len(payload) == sieve.PRODUCTION_PAGE16_BYTES
    assert _sha(payload) == sieve.PRODUCTION_PAGE16_SHA256
    resolved, accepted = sieve._read_page17(PAGE17)
    assert resolved == PAGE17.resolve()
    assert accepted == payload

    forged = bytearray(payload)
    forged[len(forged) // 2] ^= 1
    path = tmp_path / "forged-page-17-active.bin"
    path.write_bytes(forged)
    with pytest.raises(O1RelationalSearchError, match="Page17 active projection"):
        sieve._read_page17(path)


@pytest.mark.parametrize(
    ("path", "pattern"),
    (
        (PAGE16, "burned Page16"),
...
def test_burned_page16_through_page9_are_rejected(path: Path, pattern: str) -> None:
    with pytest.raises(O1RelationalSearchError, match=pattern):
        sieve._read_page17(path)
...
def test_dynamic_executable_identity_is_checked_immediately_before_one_launch() -> None:
...
def test_native_v28_source_contract() -> None:
    ...
    assert sieve.O1C98_MANIFEST_SHA256 in source
    assert sieve.PRODUCTION_PAGE16_SHA256 in source
    assert sieve.BURNED_PAGE16_SHA256 in source
