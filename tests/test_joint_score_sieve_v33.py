...
import o1_crypto_lab.joint_score_sieve_v33 as sieve
from o1_crypto_lab.o1_relational_search import O1RelationalSearchError


ROOT = Path(__file__).resolve().parents[1]
SOURCE = ROOT / sieve.NATIVE_SOURCE_RELATIVE
BUNDLE = ROOT / sieve.O1C102_BUNDLE_RELATIVE
MANIFEST = ROOT / sieve.O1C102_MANIFEST_RELATIVE
PAGE19 = ROOT / sieve.O1C102_PAGE19_RELATIVE
RECEIPT = ROOT / sieve.O1C101_PRIORITY_RECEIPT_RELATIVE
DERIVED_RECEIPT = ROOT / sieve.O1C102_DERIVED_RECEIPT_RELATIVE
LIVE_BANK = ROOT / sieve.O1C102_LIVE_BANK_RELATIVE
...
def _seed_report() -> dict[str, object]:
    return {
        ...
        "source_priority_state_receipt_sha256": (
            sieve.O1C101_PRIORITY_RECEIPT_SHA256
        ),
        "source_priority_state_receipt_bytes": sieve.O1C101_PRIORITY_RECEIPT_BYTES,
        "source_preparation_manifest_sha256": sieve.O1C102_MANIFEST_SHA256,
        "source_preparation_manifest_bytes": sieve.O1C102_MANIFEST_BYTES,
        "source_derived_resolution_receipt_sha256": (
            sieve.O1C102_DERIVED_RECEIPT_SHA256
        ),
        "source_derived_resolution_receipt_bytes": (
            sieve.O1C102_DERIVED_RECEIPT_BYTES
        ),
...
def test_o1c103_production_bindings_and_native_source_are_exact() -> None:
    assert sieve.JOINT_SCORE_SIEVE_ADAPTER_SCHEMA.endswith("v33-adapter-v1")
    assert sieve.JOINT_SCORE_SIEVE_RESULT_SCHEMA.endswith("result-v30")
    assert "o1c103" in sieve.PRIORITY_STATE_SCHEMA
    assert "o1c103" in sieve.PRIORITY_ACTION_SCHEMA
    assert sieve.NATIVE_SOURCE_BYTES == SOURCE.stat().st_size
    assert sieve.NATIVE_SOURCE_SHA256 == _sha(SOURCE.read_bytes())
    assert sieve.PRODUCTION_PAGE19_LINEAGE_ORDINAL == 32
    assert sieve.PRODUCTION_PAGE19_BYTES == 2_810_555
    assert sieve.PRODUCTION_PAGE19_LITERAL_COUNT == 702_343
    assert sieve.PRODUCTION_PAGE19_HEADROOM == {
        "clauses": 264,
        "literals": 897_657,
        "serialized_bytes": 5_578_053,
    }
    assert sieve.GLOBAL_NOVELTY_BASELINE_CLAUSE_COUNT == 2_343
    assert sieve.O1C102_MANIFEST_BYTES == 8_012
    assert sieve.O1C102_DERIVED_RECEIPT_BYTES == 326_232
...
def test_page19_reader_rejects_every_burned_page_before_acceptance(
    ...
    with pytest.raises(O1RelationalSearchError, match=pattern):
        sieve._read_page19("ignored")


def test_page19_reader_accepts_only_exact_fresh_projection(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = b"\0" * sieve.PRODUCTION_PAGE19_BYTES
    monkeypatch.setattr(
        sieve._v9._v8._v7._v1,
        "_read_input",
        lambda *_args, **_kwargs: (
            Path("/sealed/page-19-active.bin"),
            payload,
            sieve.PRODUCTION_PAGE19_SHA256,
        ),
    )
    path, accepted = sieve._read_page19("ignored")
    assert path == Path("/sealed/page-19-active.bin")
    assert accepted is payload


def test_published_o1c102_bundle_and_both_receipts_are_exact() -> None:
    assert BUNDLE.is_dir()
    assert {path.name for path in BUNDLE.iterdir()} == sieve.O1C102_PUBLISHED_ARTIFACTS
    manifest = MANIFEST.read_bytes()
    receipt = RECEIPT.read_bytes()
    derived = DERIVED_RECEIPT.read_bytes()
    bank = LIVE_BANK.read_bytes()
    page19 = PAGE19.read_bytes()
    assert len(manifest) == sieve.O1C102_MANIFEST_BYTES
    assert _sha(manifest) == sieve.O1C102_MANIFEST_SHA256
    assert len(derived) == sieve.O1C102_DERIVED_RECEIPT_BYTES
    assert _sha(derived) == sieve.O1C102_DERIVED_RECEIPT_SHA256
    assert len(page19) == sieve.PRODUCTION_PAGE19_BYTES
    assert _sha(page19) == sieve.PRODUCTION_PAGE19_SHA256
    sieve._validate_manifest(manifest)
    sieve._validate_receipt(receipt, bank)
    sieve._validate_derived_receipt(derived)


def test_prelaunch_and_recheck_contract_cover_manifest_bank_both_receipts_and_page19() -> (
    None
):
    ...
    assert "derived_resolution_receipt_path" in run_signature.parameters
    assert "sealed_page19_path" in run_signature.parameters
...
def test_v33_uses_dedicated_v3_ownership_validator_and_unchanged_state_actions() -> (
    None
):
    source = Path(sieve.__file__).read_text(encoding="utf-8")
