...
import o1_crypto_lab.o1c109_apple8_parent_centered_continuation_run as runner
from o1_crypto_lab import joint_score_sieve_v36 as v36
...
FAKE_NATIVE = b"fake-native-v33-executable\n"
...
class Fixture:
    def __init__(
        ...
        self.certification_exception: BaseException | None = None
        ...
        self.native_override: v36.JointScoreSieveV36Result | None = None
        self.last_native_result: v36.JointScoreSieveV36Result | None = None
        ...
        self.inherited_receipt = (
            root / runner.INHERITED_DERIVED_RECEIPT_RELATIVE
        ).read_bytes()
        self.inherited_closure = (
            root / runner.INHERITED_DERIVED_CLOSURE_RELATIVE
        ).read_bytes()
        self.inherited_overlay = (
            root / runner.INHERITED_DERIVED_OVERLAY_RELATIVE
        ).read_bytes()
        self.o1c104_receipt = (
            root / runner.O1C104_DERIVED_RECEIPT_RELATIVE
        ).read_bytes()
        self.o1c104_closure = (
            root / runner.O1C104_DERIVED_CLOSURE_RELATIVE
        ).read_bytes()
        self.o1c104_overlay = (
            root / runner.O1C104_DERIVED_OVERLAY_RELATIVE
        ).read_bytes()
        self.new_receipt = (root / runner.DERIVED_RECEIPT_RELATIVE).read_bytes()
        self.new_closure = (root / runner.DERIVED_CLOSURE_RELATIVE).read_bytes()
        self.new_overlay = (root / runner.DERIVED_OVERLAY_RELATIVE).read_bytes()
        self.page22 = (self.published / runner.ACTIVE_PROJECTION_NAME).read_bytes()
        ...
        monkeypatch.setattr(
            runner._native_v36._v9._v8._v7._v1,
            "_potential",
            lambda payload: self.semantic_field,
        )
        monkeypatch.setattr(
            runner._native_v36._v9,
    ...
    @staticmethod
    def empty_breadcrumbs() -> dict[str, object]:
        empty = hashlib.sha256(b"").hexdigest()
        return {
            "schema": v36.LOCAL_PRUNABLE_BREADCRUMB_SCHEMA,
            ...
            "canonical_digest": {
                "algorithm": "SHA-256",
                "encoding": v36.LOCAL_PRUNABLE_BREADCRUMB_ENCODING,
                "record_bytes": v36.LOCAL_PRUNABLE_BREADCRUMB_RECORD_BYTES,
                "field_layout": v36.LOCAL_PRUNABLE_BREADCRUMB_LAYOUT,
                "all_matches": {
                    "record_count": 0,
                    "bytes": 0,
                    "sha256": empty,
    ...
    @staticmethod
    def one_breadcrumb() -> dict[str, object]:
        row: dict[str, object] = {
            "sequence": 1,
            "call": 1,
            "probe": 1,
            ...
            "variable": 1,
            "losing_literal": -1,
            "parent_upper_bound": 0.0,
            "upper_zero": 0.0,
            "upper_one": 1.0,
            "threshold": runner.THRESHOLD,
            "classification": "ONE_PRUNABLE",
            "consumed_before": False,
            "crossing_eligible": True,
        }
        encoded = b"".join(
            (
                struct.pack("<QQQIII", 1, 1, 1, 0, 0, 0),
                b"0" * 64,
                struct.pack("<ii", 1, -1),
                struct.pack(
                    "<ddddBBB", 0.0, 0.0, 1.0, runner.THRESHOLD, 2, False, True
                ),
            )
        )
        assert len(encoded) == v36.LOCAL_PRUNABLE_BREADCRUMB_RECORD_BYTES
        empty = _sha(b"")
        return {
            "schema": v36.LOCAL_PRUNABLE_BREADCRUMB_SCHEMA,
            "selection_filter": ["ONE_PRUNABLE", "BOTH_PRUNABLE"],
            ...
            "overflow_count": 0,
            "complete": True,
            "class_counts": {"ONE_PRUNABLE": 1, "BOTH_PRUNABLE": 0},
            "canonical_digest": {
                "algorithm": "SHA-256",
                "encoding": v36.LOCAL_PRUNABLE_BREADCRUMB_ENCODING,
                "record_bytes": v36.LOCAL_PRUNABLE_BREADCRUMB_RECORD_BYTES,
    ...
    def command(self, argv: list[str], **kwargs: object) -> SimpleNamespace:
        ...
        if "-o" in argv:
            ...
            assert staged_source == closure_root / runner.SOURCE_PATHS["native_v33"]
        ...
        return SimpleNamespace(
            returncode=0,
            stdout=b"bad usage\n" if self.bad_smoke else runner.NATIVE_V33_USAGE,
            stderr=b"",
        )
    ...
    def native_result(
        ...
        *,
        ...
        breadcrumbs: dict[str, object] | None = None,
    ) -> v36.JointScoreSieveV36Result:
        ...
        adapter_memory: dict[str, object] = {
            "memory_series_schema": (v36._v9.JOINT_SCORE_SIEVE_MEMORY_SERIES_SCHEMA),
            "memory_sample_limit": v36._v9.JOINT_SCORE_SIEVE_MAXIMUM_MEMORY_SAMPLES,
        ...
        raw: dict[str, object] = {
            "schema": v36.JOINT_SCORE_SIEVE_RESULT_SCHEMA,
            "seed": 0,
            "threshold": runner.THRESHOLD,
            "conflict_limit": 128,
            "status": status,
            ...
            "potential_sha256": _sha(self.inputs["potential"].read_bytes()),
            "active_vault_sha256": _sha(self.page22),
            "priority_seed": priority_seed,
            "priority_state": state,
            "priority_actions": actions,
            "decision_ownership": {"schema": "fixture-ownership"},
            "local_prunable_breadcrumbs": breadcrumbs or self.empty_breadcrumbs(),
            "stats": raw_stats,
            "base_sieve": base,
            "vault": vault,
            "resources": resources,
        ...
        return v36.JointScoreSieveV36Result(
            status=status,
            conflict_limit=128,
            threshold=runner.THRESHOLD,
            ...
            local_prunable_breadcrumbs=cast(
                dict[str, object], raw["local_prunable_breadcrumbs"]
            ),
    ...
    def adapter(self, **kwargs: object) -> object:
        ...
        assert intent["page22_burned"] is True
        assert intent["lineage35_burned"] is True
        assert intent["page21_retry_or_replay_authorized"] is False
        assert intent["page20_retry_or_replay_authorized"] is False
        assert intent["page19_retry_or_replay_authorized"] is False
        assert intent["page18_retry_or_replay_authorized"] is False
        assert intent["page17_retry_or_replay_authorized"] is False
        ...
        assert kwargs["expected_source_sha256"] == self.source_sha["native_v33"]
        assert kwargs["source_path"] == (
            executable.parents[1]
            / runner.NATIVE_CLOSURE_DIRECTORY
            / runner.SOURCE_PATHS["native_v33"]
        )
        ...
        assert Path(cast(Path, kwargs["vault_path"])).read_bytes() == self.page22
        assert Path(cast(Path, kwargs["priority_seed_path"])).read_bytes() == self.bank
        staged_bundle = Path(cast(Path, kwargs["rollover_bundle_dir"]))
        assert staged_bundle == executable.parents[1] / "initial"
        assert (
            staged_bundle.joinpath(runner.INHERITED_DERIVED_RECEIPT_NAME).read_bytes()
            == self.inherited_receipt
        )
        assert (
            staged_bundle.joinpath(runner.INHERITED_DERIVED_CLOSURE_NAME).read_bytes()
            == self.inherited_closure
        )
        assert (
            staged_bundle.joinpath(runner.INHERITED_DERIVED_OVERLAY_NAME).read_bytes()
            == self.inherited_overlay
        )
        assert (
            staged_bundle.joinpath(runner.O1C104_DERIVED_RECEIPT_NAME).read_bytes()
            == self.o1c104_receipt
        )
        assert (
            staged_bundle.joinpath(runner.O1C104_DERIVED_CLOSURE_NAME).read_bytes()
            == self.o1c104_closure
        )
        assert (
            staged_bundle.joinpath(runner.O1C104_DERIVED_OVERLAY_NAME).read_bytes()
            == self.o1c104_overlay
        )
        assert (
            staged_bundle.joinpath(runner.DERIVED_RECEIPT_NAME).read_bytes()
            == self.new_receipt
        )
        assert (
            staged_bundle.joinpath(runner.DERIVED_CLOSURE_NAME).read_bytes()
            == self.new_closure
        )
        assert (
            staged_bundle.joinpath(runner.DERIVED_OVERLAY_NAME).read_bytes()
            == self.new_overlay
        )
    ...
    def certify_page22_inputs(self, **kwargs: object) -> object:
        self.certification_calls += 1
        assert kwargs == {
            "bundle_dir": self.published,
            "cnf_path": self.inputs["cnf"],
            "potential_path": self.inputs["potential"],
            "grouping_path": self.inputs["grouping"],
            "vault_path": self.published / runner.ACTIVE_PROJECTION_NAME,
            "vault_caps": runner.O1C66_VAULT_CAPS,
            "threshold": runner.THRESHOLD,
        }
        if self.certification_exception is not None:
            raise self.certification_exception
        return SimpleNamespace(page22_sha256=runner.PAGE22_SHA256)
    ...
    def semantic_replay(
        ...
        assert input_vault.sha256 == _sha(self.page22)
        return replayed
        ...
        config = {
            ...
            "preparation": {
                ...
                "inherited_derived_resolution_receipt": (
                    runner.INHERITED_DERIVED_RECEIPT_RELATIVE.as_posix()
                ),
                "inherited_derived_resolution_receipt_sha256": (
                    runner.INHERITED_DERIVED_RECEIPT_SHA256
                ),
                "inherited_derived_resolution_receipt_bytes": (
                    runner.INHERITED_DERIVED_RECEIPT_BYTES
                ),
                "inherited_derived_resolution_closure": (
                    runner.INHERITED_DERIVED_CLOSURE_RELATIVE.as_posix()
                ),
                "inherited_derived_resolution_closure_sha256": (
                    runner.INHERITED_DERIVED_CLOSURE_SHA256
                ),
                "inherited_derived_resolution_closure_bytes": (
                    runner.INHERITED_DERIVED_CLOSURE_BYTES
                ),
                "inherited_derived_resolution_overlay": (
                    runner.INHERITED_DERIVED_OVERLAY_RELATIVE.as_posix()
                ),
                "inherited_derived_resolution_overlay_sha256": (
                    runner.INHERITED_DERIVED_OVERLAY_SHA256
                ),
                "inherited_derived_resolution_overlay_bytes": (
                    runner.INHERITED_DERIVED_OVERLAY_BYTES
                ),
                "o1c104_derived_resolution_receipt": (
                    runner.O1C104_DERIVED_RECEIPT_RELATIVE.as_posix()
                ),
                "o1c104_derived_resolution_receipt_sha256": (
                    runner.O1C104_DERIVED_RECEIPT_SHA256
                ),
                "o1c104_derived_resolution_receipt_bytes": (
                    runner.O1C104_DERIVED_RECEIPT_BYTES
                ),
                "o1c104_derived_resolution_closure": (
                    runner.O1C104_DERIVED_CLOSURE_RELATIVE.as_posix()
                ),
                "o1c104_derived_resolution_closure_sha256": (
                    runner.O1C104_DERIVED_CLOSURE_SHA256
                ),
                "o1c104_derived_resolution_closure_bytes": (
                    runner.O1C104_DERIVED_CLOSURE_BYTES
                ),
                "o1c104_derived_resolution_overlay": (
                    runner.O1C104_DERIVED_OVERLAY_RELATIVE.as_posix()
                ),
                "o1c104_derived_resolution_overlay_sha256": (
                    runner.O1C104_DERIVED_OVERLAY_SHA256
                ),
                "o1c104_derived_resolution_overlay_bytes": (
                    runner.O1C104_DERIVED_OVERLAY_BYTES
                ),
                "new_derived_resolution_receipt": (
                    runner.DERIVED_RECEIPT_RELATIVE.as_posix()
                ),
                "new_derived_resolution_receipt_sha256": (
                    runner.DERIVED_RECEIPT_SHA256
                ),
                "new_derived_resolution_receipt_bytes": runner.DERIVED_RECEIPT_BYTES,
                "new_derived_resolution_closure": (
                    runner.DERIVED_CLOSURE_RELATIVE.as_posix()
                ),
                "new_derived_resolution_closure_sha256": (
                    runner.DERIVED_CLOSURE_SHA256
                ),
                "new_derived_resolution_closure_bytes": runner.DERIVED_CLOSURE_BYTES,
                "new_derived_resolution_overlay": (
                    runner.DERIVED_OVERLAY_RELATIVE.as_posix()
                ),
                "new_derived_resolution_overlay_sha256": (
                    runner.DERIVED_OVERLAY_SHA256
                ),
                "new_derived_resolution_overlay_bytes": runner.DERIVED_OVERLAY_BYTES,
            ...
            "native": {
                "source": runner.SOURCE_PATHS["native_v33"],
                "source_sha256": self.source_sha["native_v33"],
                ...
                "adapter_schema": v36.JOINT_SCORE_SIEVE_ADAPTER_SCHEMA,
                "result_schema": v36.JOINT_SCORE_SIEVE_RESULT_SCHEMA,
                "page22_sha256": runner.PAGE22_SHA256,
                "page22_bytes": runner.PAGE22_SERIALIZED_BYTES,
            ...
            "next_action": (
                "Burn Page 22 / lineage 35 exactly once; never retry or replay "
                "Pages 21 through 9."
            ),
        }
...
def test_preflight_validates_published_bundle_without_regeneration(
    ...
    assert published["real_v8_input_certification_verified_before_intent"] is True
    ...
    assert len(cast(dict[str, object], row["prepared_artifacts"])) == 20
    prepared = runner._load_published_preparation(
        fixture.published,
        fixture.published / runner.PREPARATION_MANIFEST_NAME,
    )
    residency = json.loads(prepared.artifacts[runner.RESIDENCY_NAME])
    assert residency["current_projection"]["encoding_only"]["sha256"] == (
        runner.PAGE22_SHA256
    )
    assert (
        residency["emitted_selector_candidate"]["encoding_only"]["encoding_only"][
            "sha256"
        ]
        == runner.PURE_EMITTED_CANDIDATE_SHA256
    )
    assert residency["emitted_selector_candidate"]["activated"] is False
    receipt = json.loads(fixture.new_receipt)
    registry = receipt["logical_known_registry"]
    combined = registry["combined"]["clause_sha256"]
    assert len(prepared.globally_known_clause_sha256) == 3_111
    assert len(combined[:2692]) == 2_692
    assert len(combined[2692:2958]) == 266
    assert len(combined[2958:]) == 153
    assert set(combined) == prepared.globally_known_clause_sha256
    assert fixture.certification_calls == 1
    assert fixture.adapter_calls == fixture.build_calls == fixture.smoke_calls == 0


def test_real_v8_input_certification_failure_precedes_every_side_effect(
    fixture: Fixture,
) -> None:
    fixture.certification_exception = RuntimeError("forced Page22 theorem failure")
    with pytest.raises(
        runner.O1C109RunError,
        match="real Page-22 v8 input certification failed before intent",
    ):
        fixture.execute()
    assert fixture.certification_calls == 1
    assert fixture.build_calls == 0
    assert fixture.smoke_calls == 0
...
def test_preflight_rejects_capacity_proof_mutation_before_intent(
    ...
    target = cast(dict[str, Any], manifest["page22"])
    for field in path[:-1]:
        target = cast(dict[str, Any], target)[field]
    cast(dict[str, Any], target)[path[-1]] = replacement
    with pytest.raises(
        runner.O1C109RunError, match="Page-22 native capacity proof differs"
    ):
        runner._validate_native_capacity_proof(manifest["page22"])
    assert fixture.adapter_calls == fixture.build_calls == fixture.smoke_calls == 0
    assert not list((fixture.root / "runs").glob(f"*_{runner.CAPSULE_SUFFIX}"))
...
def test_preflight_rejects_published_mutation_without_build_or_call(
    ...
    with pytest.raises(runner.O1C109RunError, match="artifact seal differs"):
        runner.preflight(
            fixture.config_path,
            root=fixture.root,
            system_probe=fixture.system,
...
@pytest.mark.parametrize(
    "name",
    (
        runner.INHERITED_DERIVED_RECEIPT_NAME,
        runner.INHERITED_DERIVED_CLOSURE_NAME,
        runner.INHERITED_DERIVED_OVERLAY_NAME,
        runner.O1C104_DERIVED_RECEIPT_NAME,
        runner.O1C104_DERIVED_CLOSURE_NAME,
        runner.O1C104_DERIVED_OVERLAY_NAME,
        runner.DERIVED_RECEIPT_NAME,
        runner.DERIVED_CLOSURE_NAME,
        runner.DERIVED_OVERLAY_NAME,
    ),
)
def test_preflight_rejects_each_derived_namespace_tamper_before_build(
    fixture: Fixture,
    name: str,
) -> None:
    artifact = fixture.published / name
    payload = artifact.read_bytes()
    artifact.write_bytes(bytes((payload[0] ^ 1,)) + payload[1:])
    with pytest.raises(
        runner.O1C109RunError,
        match="published preparation/receipt preflight differs",
    ):
        runner.preflight(
            fixture.config_path,
            root=fixture.root,
...
def test_one_build_smoke_and_observed_identity_precede_intent(
    ...
    assert build["smoke"]["stdout"] == {
        "serialized_bytes": len(runner.NATIVE_V33_USAGE),
        "sha256": _sha(runner.NATIVE_V33_USAGE),
    }
...
def test_staged_closure_tamper_is_rejected_before_intent(
    ...
    with pytest.raises(runner.O1C109RunError, match="staged native closure"):
        fixture.execute()
...
def test_bad_help_smoke_is_preintent_and_leaves_no_capsule(fixture: Fixture) -> None:
    ...
    with pytest.raises(runner.O1C109RunError, match="--help smoke seal differs"):
        fixture.execute()
...
def test_source_mutation_after_smoke_never_burns_or_rebuilds(fixture: Fixture) -> None:
    ...
    with pytest.raises(runner.O1C109RunError, match="changed before native call"):
        fixture.execute()
    capsules = list((fixture.root / "runs").glob(f"*_{runner.CAPSULE_SUFFIX}"))
    assert len(capsules) == 1
    assert not (capsules[0] / "episodes/00/intent.json").exists()
    with pytest.raises(runner.O1C109RunError, match="pre-intent capsule"):
        fixture.execute()
    assert fixture.build_calls == fixture.smoke_calls == 1
    assert fixture.adapter_calls == 0
...
def test_binary_rehash_immediately_before_adapter_is_terminal(
    ...
    assert episode["page22_burned"] is True
    assert episode["lineage35_burned"] is True
...
def test_missing_binary_after_intent_is_recoverable_terminal(
    ...
    assert failure["exception_type"] == "O1C109RunError"
...
def test_self_consistent_forged_completed_capsule_is_never_republished(
    ...
    with pytest.raises(runner.O1C109RunError):
        fixture.execute()
...
def test_resealed_forged_sat_key_fails_independent_public_verification(
    ...
    with pytest.raises(runner.O1C109RunError, match="not publicly verified"):
        fixture.execute()
    assert not authoritative.exists()
    assert fixture.adapter_calls == fixture.build_calls == fixture.smoke_calls == 1


def test_resealed_self_consistent_forged_novelty_fails_v36_semantic_replay(
    fixture: Fixture,
) -> None:
    result = fixture.execute()
    capsule = fixture.root / cast(str, result["capsule"])
    ...
    episode["science"]["active_page22_new_clauses"] = 1
    ...
    with pytest.raises(runner.O1C109RunError, match="semantic replay"):
        fixture.execute()
...
def test_novelty_uses_complete_published_attic_digest_set(fixture: Fixture) -> None:
    ...
    assert science["active_page22_new_clauses"] == 1
...
def test_local_prunable_breadcrumbs_are_archived_operational_non_science(
    fixture: Fixture,
) -> None:
    empty_result = fixture.native_result()
    breadcrumb_result = fixture.native_result(
        breadcrumbs=fixture.one_breadcrumb(), outcome_counts=(254, 0, 1, 0)
    )
    empty_evidence = runner._validate_native_result(
        empty_result, cast(str, empty_result.native_stdout).encode()
    )
    breadcrumb_evidence = runner._validate_native_result(
        breadcrumb_result, cast(str, breadcrumb_result.native_stdout).encode()
    )
    empty_operation, empty_science, empty_classification, empty_stop = (
        runner._science_and_operation(empty_evidence)
    )
    operation, science, classification, stop = runner._science_and_operation(
        breadcrumb_evidence
    )

    assert science == empty_science
    assert classification == empty_classification
    assert stop == empty_stop
    assert {key: value for key, value in operation.items() if key != "local_prunable_breadcrumbs"} == {
        key: value
        for key, value in empty_operation.items()
        if key != "local_prunable_breadcrumbs"
    }
    assert operation["local_prunable_breadcrumbs"] == {
        "total_match_count": 1,
        "retained_count": 1,
        "overflow_count": 0,
        "complete": True,
        "class_counts": {"ONE_PRUNABLE": 1, "BOTH_PRUNABLE": 0},
        "retained_consumed_before_count": 0,
        "retained_crossing_eligible_count": 1,
        "affects_action_selection": False,
        "is_science_gain": False,
        "is_key_bit_belief": False,
    }

    fixture.native_override = breadcrumb_result
    result = fixture.execute()
    episode = cast(list[dict[str, Any]], result["episodes"])[0]
    capsule = fixture.root / cast(str, result["capsule"])
    archived = cast(dict[str, Any], episode["archived_native_components"])
    sidecar_path = capsule / "episodes/00/local-prunable-breadcrumbs.json"
    assert archived["local-prunable-breadcrumbs.json"] == runner._artifact_row(
        sidecar_path, relative_to=sidecar_path.parent
    )
    assert json.loads(sidecar_path.read_bytes()) == fixture.one_breadcrumb()
    assert fixture.execute() == result
    assert fixture.adapter_calls == fixture.build_calls == fixture.smoke_calls == 1
...
def test_production_config_has_no_predicted_binary_identity() -> None:
    ...
    assert config["native"]["page22_sha256"] == runner.PAGE22_SHA256
    assert config["native"]["page22_bytes"] == runner.PAGE22_SERIALIZED_BYTES
    assert config["preparation"]["manifest_sha256"] == runner.PUBLISHED_MANIFEST_SHA256
    assert config["preparation"]["priority_state_receipt_sha256"] == (
        runner.PRIORITY_RECEIPT_SHA256
    )
    assert config["preparation"]["new_derived_resolution_receipt_sha256"] == (
        runner.DERIVED_RECEIPT_SHA256
    )
    assert "joint_score_sieve_v36" in source
    assert "prepare_o1c100_page17_causal_rollover" not in source
    assert runner.SOURCE_PATHS["bounded_ownership_header"].endswith(
        "o1c101_bounded_decision_ownership.hpp"
    )
    for name, relative in runner.SOURCE_PATHS.items():
        assert config["source"]["expected_sha256"][name] == _sha(
            (Path(__file__).parents[1] / relative).read_bytes()
        )
    assert config["budgets"]["lineage_call_ordinals"] == [35]
    ...
    assert "Page 22" in config["next_action"]
