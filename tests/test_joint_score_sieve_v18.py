...
import o1_crypto_lab.joint_score_sieve_v18 as sieve_v18
...
NATIVE_SOURCE = ROOT / "native/cadical_o1_joint_score_sieve_v15.cpp"
...
def _source_result(count: int) -> tuple[dict[str, object], str]:
    ...
    source: dict[str, object] = {
        "schema": "public-staging-source-v1",
        "sieve": {
            "state": {
...
def _artifacts(tmp_path: Path) -> StagingArtifacts:
    field = _field()
    cnf = tmp_path / "staging.cnf"
    cnf.write_text(
        "p cnf 256 5\n6 0\n2 4 5 0\n2 4 -5 0\n2 -4 5 0\n2 -4 -5 0\n",
        encoding="ascii",
    )
    potential = tmp_path / "staging.potential"
    grouping_path = tmp_path / "staging.grouping"
    sieve_v18.write_joint_score_sieve_potential(potential, field)
    sieve_v18.write_joint_score_sieve_grouping(grouping_path, field)
    ...
    identity = vault_identity_from_sources(
        ...
        bound_rule=sieve_v18.JOINT_SCORE_SIEVE_BOUND_RULE,
        threshold=12.0,
    )
    rank_vault = ThresholdNoGoodVault(
        identity,
    ...
    rank_table_path = tmp_path / "staging.rank-table"
    ...
    assert frontier_plan.unassigned_literal_count == 5
...
def native_fixture(tmp_path_factory: pytest.TempPathFactory) -> Path:
    ...
    executable = tmp_path_factory.mktemp("o1c77-v18-native") / "native-v15"
    completed = subprocess.run(
        [
            ...
            "-Wextra",
            "-Werror",
            "-DO1_CRYPTO_LAB_O1C77_PUBLIC_FIXTURE",
            f"-I{CADICAL_INCLUDE}",
            str(NATIVE_SOURCE),
            str(CADICAL_LIBRARY),
            "-o",
...
def _command(executable: Path, artifacts: StagingArtifacts) -> list[str]:
    return [
        str(executable),
        "--cnf",
        str(artifacts.cnf),
...
def _native_payload(executable: Path, artifacts: StagingArtifacts) -> dict[str, Any]:
    completed = subprocess.run(
...
def _parse(
    payload: object, artifacts: StagingArtifacts
) -> sieve_v18.JointScoreSieveV18Result:
    return sieve_v18._parse_native_payload(
        payload,
        ...
        require_staging_activation=True,
    )


def test_native_returns_effective_overlay_without_outer_override(
    ...
    assert payload["reader_rank_role"] == "effective-derived-order"
    assert reader["ranked_literals"] == list(
        artifacts.staging_plan.effective_rank_literals
    )
    assert staging["mechanism_activated"] is True
    assert staging["overlay_effective_returns"] >= 1
    assert staging["cb_decide_calls"] == reader["cb_decide_calls"]
    assert staging["cb_decide_calls"] == payload["frontier"]["cb_decide_calls"]
    assert result.staging_plan == artifacts.staging_plan
    assert result.staging_reader == staging
    assert result.reader == reader


def test_adapter_rejects_overlay_origin_and_callback_trace_tamper(
    native_fixture: Path, tmp_path: Path
) -> None:
    artifacts = _artifacts(tmp_path)
    payload = copy.deepcopy(_native_payload(native_fixture, artifacts))
    payload["staging"]["overlay_return_events"][0]["kind"] = "source-contrast"
    with pytest.raises(O1RelationalSearchError, match="joint-score-sieve-v18"):
        _parse(payload, artifacts)

    payload = copy.deepcopy(_native_payload(native_fixture, artifacts))
    payload["staging"]["callback_trace_sha256"] = "00" * 32
    with pytest.raises(O1RelationalSearchError, match="joint-score-sieve-v18"):
        _parse(payload, artifacts)
...
def test_adapter_rejects_staging_state_size_tamper(
    native_fixture: Path, tmp_path: Path, field: str
) -> None:
    artifacts = _artifacts(tmp_path)
    payload = copy.deepcopy(_native_payload(native_fixture, artifacts))
    payload["staging"][field] = 1

    with pytest.raises(O1RelationalSearchError, match="bounded staging state"):
        _parse(payload, artifacts)


def test_native_cli_requires_both_plans(native_fixture: Path) -> None:
    help_result = subprocess.run(
        [str(native_fixture), "--help"], capture_output=True, text=True, check=False
    )
    assert help_result.returncode == 0
    assert "--frontier-plan PATH" in help_result.stdout
    assert "--staging-plan PATH" in help_result.stdout
