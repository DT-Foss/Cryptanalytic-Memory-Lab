...
NATIVE = ROOT / "native"
V24 = NATIVE / "cadical_o1_joint_score_sieve_v24.cpp"
...
PRIOR_BANK_SHA256 = "0203de9f1732b095bf30062cb8a07b018ded829ee99f18ffbca715c653c0cc6a"
MANIFEST_SHA256 = "e46ca7373bc3a94efc30dcd309728005e3bee8b93983dc2c396f45bd487dd458"
...
FROZEN_V24_SHA256 = "036dc34182d9eef1aba014fead0cb32094464cd2032743c989e073f92ade402c"
CADICAL_INCLUDE = Path("/opt/homebrew/opt/cadical/include")
CADICAL_LIBRARY = Path("/opt/homebrew/opt/cadical/lib/libcadical.a")


HARNESS = r"""
#define O1_CRYPTO_LAB_O1C92_NO_MAIN
#include "cadical_o1_joint_score_sieve_v25.cpp"
...
def frozen_inputs() -> tuple[Path, Path, Path, Path, Path, Path]:
    ...
    assert MANIFEST.stat().st_size == 20_129
    ...
    return PRIOR_BANK, PAGE13, PAGE12, PAGE11, PAGE10, PAGE9
...
def native_builds(tmp_path_factory: pytest.TempPathFactory) -> tuple[Path, Path]:
    ...
    builds = (
        (
            [*common, "-DO1_CRYPTO_LAB_O1C92_PUBLIC_FIXTURE"],
            harness_source,
            harness,
        ),
        (common, V25, main),
...
def test_v25_normal_production_binary_compiles_and_help_surface(
    ...
    assert completed.stdout == (
        "usage: cadical_o1_joint_score_sieve_v25 --cnf PATH --potential PATH "
...
def test_live_bank_retains_failure_first_one_shot_and_seed_zero_semantics(
    ...
    assert actions["schema"] == "o1-256-o1c92-failure-first-proof-mining-actions-v1"
...
def test_native_production_gate_rejects_prior_live_bank_before_solver(
    native_builds: tuple[Path, Path],
    frozen_inputs: tuple[Path, Path, Path, Path, Path, Path],
    tmp_path: Path,
) -> None:
    _, main = native_builds
    old_bank, _, _, _, _, page9 = frozen_inputs
    completed = _sealed_command(main, tmp_path, page9, old_bank)
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v25: sealed O1C92 priority seed differs\n"
    )
...
def test_native_requires_seed_zero_before_reading_solver_inputs(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v25: O1C92 priority operator requires seed zero\n"
    )
...
def test_native_production_gate_rejects_exact_page9_before_solver(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v25: burned O1C84 Page-9 active vault rejected\n"
    )
...
def test_native_production_gate_rejects_exact_page10_before_solver(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v25: burned O1C85 Page-10 active vault rejected\n"
    )
...
def test_native_production_gate_rejects_exact_page11_before_solver(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v25: burned O1C86 Page-11 active vault rejected\n"
    )
...
def test_native_production_gate_rejects_exact_page12_before_solver(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v25: burned O1C88 Page-12 active vault rejected\n"
    )
...
def test_native_production_gate_rejects_exact_page13_before_solver(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v25: burned O1C90 Page-13 active vault rejected\n"
    )


def test_native_production_gate_rejects_nonmatching_page14_payload(
    native_builds: tuple[Path, Path], tmp_path: Path
) -> None:
    _, main = native_builds
    forged_page14 = tmp_path / "forged-page-14-active.bin"
    forged_page14.write_bytes(bytes(2_817_779))
    completed = _sealed_command(main, tmp_path, forged_page14, LIVE_BANK)
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v25: sealed O1C92 Page-14 active vault differs\n"
    )


def test_source_contract_and_historical_native_freeze() -> None:
    source = V25.read_text(encoding="utf-8")
    manifest: dict[str, Any] = json.loads(MANIFEST.read_bytes())
    assert hashlib.sha256(V24.read_bytes()).hexdigest() == FROZEN_V24_SHA256
    assert '#include "cadical_o1_joint_score_sieve_v18.cpp"' in source
    assert '#include "o1c82_parent_centered_priority.hpp"' in source
    assert "O1_CRYPTO_LAB_O1C92_PUBLIC_FIXTURE" in source
    assert "O1_CRYPTO_LAB_O1C92_NO_MAIN" in source
    assert "o1-256-cadical-joint-score-sieve-result-v25" in source
    assert "o1-256-o1c92-live-parent-centered-continuation-priority-state-v1" in source
    assert "o1-256-o1c92-failure-first-proof-mining-actions-v1" in source
    ...
    assert manifest["page14"]["lineage_ordinal"] == 27
    assert manifest["page14"]["active_limit"] == 252
    assert manifest["page14"]["clause_count"] == 252
    assert manifest["page14"]["literal_count"] == 704_145
    assert manifest["page14"]["new_clause_residency"][
        "new_union_index_stop_exclusive"
    ] == 1_551
    assert manifest["science_boundary"]["imported_globally_novel_clause_count"] == 260
    assert manifest["page14"]["native_capacity_proof"]["caps"][
        "maximum_clauses"
    ] == 512
    ...
    assert "kProductionPage14ActiveVaultBytes = 2817779U" in source
    ...
    assert "kProductionLineageOrdinal = 27U" in source
    assert "kProductionActiveLimit = 252U" in source
    assert "kProductionActiveClauseCount = 252U" in source
    assert "kProductionActiveLiteralCount = 704145U" in source
    assert "kGlobalNoveltyBaselineClauseCount = 1551U" in source
    assert "kO1C91ImportedNovelClauseCount = 260U" in source
    assert (
    ...
    page14_gate = source.index("vault_sha256 != kProductionPage14ActiveVaultSha256")
...
def test_v25_operator_body_is_byte_equivalent_to_frozen_v24() -> None:
    source = V25.read_text(encoding="utf-8")
    provenance_constants = """constexpr const char *kO1C91PreparationManifestSha256 =
...
"""
    frozen_provenance_constants = """constexpr const char *kO1C89PreparationManifestSha256 =
    "467e519df281db4fc10de9223195dfedba9fd51edc93b40883f59fd3821e29ec";
constexpr size_t kProductionLineageOrdinal = 26U;
constexpr size_t kProductionActiveLimit = 253U;
constexpr size_t kGlobalNoveltyBaselineClauseCount = 1291U;
constexpr size_t kO1C89ImportedNovelClauseCount = 259U;
"""
    provenance_assertions = """static_assert(std::string_view(kO1C91PreparationManifestSha256).size() == 64U);
...
"""
    frozen_provenance_assertions = """static_assert(std::string_view(kO1C89PreparationManifestSha256).size() == 64U);
static_assert(kProductionLineageOrdinal == 26U);
static_assert(kProductionActiveLimit + kO1C89ImportedNovelClauseCount ==
              kMaximumVaultClauses);
static_assert(kGlobalNoveltyBaselineClauseCount == 1291U);

"""
    burned_page13_constants = """constexpr const char *kBurnedPage13ActiveVaultSha256 =
    "4c1b7d5a6d40fad9439d95433bcc7a60ff3e7ddc0e4542b0cf003cdf4581e546";
constexpr size_t kBurnedPage13ActiveVaultBytes = 2846623U;
"""
    burned_page13_gate = """    if (production_seal &&
...
"""
    for delta in (burned_page13_constants, burned_page13_gate):
    ...
    replacements = (
        ...
        ("kProductionPage14", "kProductionPage13"),
        (PAGE14_SHA256, PAGE13_SHA256),
        ("2817779U", "2846623U"),
        ("Page-14", "Page-13"),
    )
    for current, prior in replacements:
        normalized = normalized.replace(current, prior)
    assert normalized == V24.read_text(encoding="utf-8")
