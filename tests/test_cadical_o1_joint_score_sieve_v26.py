...
NATIVE = ROOT / "native"
V25 = NATIVE / "cadical_o1_joint_score_sieve_v25.cpp"
...
MANIFEST_SHA256 = "187f09309b2d866549441d713f29bfed696c140f5c5a99536001c889f5836a24"
...
FROZEN_V25_SHA256 = "4119466d0412886d67edd8b11bb04d3b59abc85894f0b33c95b156c4f542e141"
CADICAL_INCLUDE = Path("/opt/homebrew/opt/cadical/include")
CADICAL_LIBRARY = Path("/opt/homebrew/opt/cadical/lib/libcadical.a")


HARNESS = r"""
#define O1_CRYPTO_LAB_O1C95_NO_MAIN
#include "cadical_o1_joint_score_sieve_v26.cpp"
...
def frozen_inputs() -> tuple[Path, Path, Path, Path, Path, Path, Path]:
    ...
    assert STATE_RECEIPT.stat().st_size == 52_014
    assert _sha(STATE_RECEIPT) == STATE_RECEIPT_SHA256
    assert PAGE15.stat().st_size == 2_843_047
    assert _sha(PAGE15) == PAGE15_SHA256
    assert PAGE14.stat().st_size == 2_817_779
    assert _sha(PAGE14) == PAGE14_SHA256
    assert PAGE13.stat().st_size == 2_846_623
    assert _sha(PAGE13) == PAGE13_SHA256
    assert MANIFEST.stat().st_size == 24_136
    assert _sha(MANIFEST) == MANIFEST_SHA256
    assert PAGE12.stat().st_size == 2_725_423
    assert _sha(PAGE12) == PAGE12_SHA256
    ...
    return PRIOR_BANK, PAGE14, PAGE13, PAGE12, PAGE11, PAGE10, PAGE9
...
def native_builds(tmp_path_factory: pytest.TempPathFactory) -> tuple[Path, Path]:
    ...
    builds = (
        (
            [*common, "-DO1_CRYPTO_LAB_O1C95_PUBLIC_FIXTURE"],
            harness_source,
            harness,
        ),
        (common, V26, main),
...
def test_v26_normal_production_binary_compiles_and_help_surface(
    ...
    assert completed.stdout == (
        "usage: cadical_o1_joint_score_sieve_v26 --cnf PATH --potential PATH "
...
def test_live_evolved_bank_import_and_machine_readable_provenance(
    ...
    assert seed_report["source_priority_state_receipt_sha256"] == (
        STATE_RECEIPT_SHA256
    )
    assert seed_report["source_priority_state_receipt_bytes"] == 52_014
...
def test_live_bank_retains_failure_first_one_shot_and_seed_zero_semantics(
    ...
    assert actions["schema"] == "o1-256-o1c95-failure-first-proof-mining-actions-v1"
...
def test_certified_strict_crossing_still_precedes_live_bank_priority(
    ...
    state = _mapping(payload["priority_state"])
    assert state["consumed_coordinate_count"] == 1


def test_certified_crossing_release_never_rearms_consumed_coordinate(
    native_builds: tuple[Path, Path],
) -> None:
    harness, _ = native_builds
    completed = _run(harness, "crossing-rearm", LIVE_BANK)
    assert completed.returncode == 0, completed.stderr
    payload: dict[str, Any] = json.loads(completed.stdout)
    actions = _mapping(payload["priority_actions"])
    assert actions["one_shot_rule"] == (
...
def test_native_production_gate_rejects_prior_live_bank_before_solver(
    native_builds: tuple[Path, Path],
    frozen_inputs: tuple[Path, Path, Path, Path, Path, Path, Path],
    tmp_path: Path,
) -> None:
    _, main = native_builds
    old_bank, _, _, _, _, _, page9 = frozen_inputs
    completed = _sealed_command(main, tmp_path, page9, old_bank)
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v26: sealed O1C95 priority seed differs\n"
    )
...
def test_native_requires_seed_zero_before_reading_solver_inputs(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v26: O1C95 priority operator requires seed zero\n"
    )
...
def test_native_production_gate_rejects_exact_page9_before_solver(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v26: burned O1C84 Page-9 active vault rejected\n"
    )
...
def test_native_production_gate_rejects_exact_page10_before_solver(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v26: burned O1C85 Page-10 active vault rejected\n"
    )
...
def test_native_production_gate_rejects_exact_page11_before_solver(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v26: burned O1C86 Page-11 active vault rejected\n"
    )


def test_native_production_gate_rejects_exact_page12_before_solver(
    native_builds: tuple[Path, Path], tmp_path: Path
) -> None:
    _, main = native_builds
    completed = _sealed_command(main, tmp_path, PAGE12, LIVE_BANK)
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v26: burned O1C88 Page-12 active vault rejected\n"
    )


def test_native_production_gate_rejects_exact_page13_before_solver(
    native_builds: tuple[Path, Path], tmp_path: Path
) -> None:
    _, main = native_builds
    completed = _sealed_command(main, tmp_path, PAGE13, LIVE_BANK)
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v26: burned O1C90 Page-13 active vault rejected\n"
    )


def test_native_production_gate_rejects_exact_page14_before_solver(
    native_builds: tuple[Path, Path], tmp_path: Path
) -> None:
    _, main = native_builds
    completed = _sealed_command(main, tmp_path, PAGE14, LIVE_BANK)
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v26: burned O1C92 Page-14 active vault rejected\n"
    )


def test_native_production_gate_rejects_nonmatching_page15_payload(
    native_builds: tuple[Path, Path], tmp_path: Path
) -> None:
    _, main = native_builds
    forged_page15 = tmp_path / "forged-page-15-active.bin"
    forged_page15.write_bytes(bytes(2_843_047))
    completed = _sealed_command(main, tmp_path, forged_page15, LIVE_BANK)
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v26: sealed O1C95 Page-15 active vault differs\n"
    )


def test_source_contract_and_historical_native_freeze() -> None:
    source = V26.read_text(encoding="utf-8")
    manifest: dict[str, Any] = json.loads(MANIFEST.read_bytes())
    assert hashlib.sha256(V25.read_bytes()).hexdigest() == FROZEN_V25_SHA256
    assert '#include "cadical_o1_joint_score_sieve_v18.cpp"' in source
    assert '#include "o1c82_parent_centered_priority.hpp"' in source
    assert "O1_CRYPTO_LAB_O1C95_PUBLIC_FIXTURE" in source
    assert "O1_CRYPTO_LAB_O1C95_NO_MAIN" in source
    assert "o1-256-cadical-joint-score-sieve-result-v26" in source
    assert "o1-256-o1c95-live-parent-centered-continuation-priority-state-v1" in source
    assert "o1-256-o1c95-failure-first-proof-mining-actions-v1" in source
    ...
    assert manifest["page15"]["lineage_ordinal"] == 28
    assert manifest["page15"]["active_limit"] == 251
    assert manifest["page15"]["clause_count"] == 251
    assert manifest["page15"]["literal_count"] == 710_463
    assert manifest["page15"]["serialized_bytes"] == 2_843_047
    assert manifest["page15"]["new_clause_residency"][
        "new_union_index_stop_exclusive"
    ] == 1_812
    assert manifest["science_boundary"]["imported_globally_novel_clause_count"] == 261
    assert manifest["page15"]["native_capacity_proof"]["caps"][
        "maximum_clauses"
    ] == 512
    assert manifest["final_priority_bank"]["sha256"] == LIVE_BANK_SHA256
    assert manifest["final_priority_bank"]["receipt_sha256"] == (
        STATE_RECEIPT_SHA256
    )
    assert manifest["final_priority_bank"]["receipt_serialized_bytes"] == 52_014
    assert PAGE15_SHA256 in source
    assert PAGE14_SHA256 in source
    assert PAGE13_SHA256 in source
    assert PAGE12_SHA256 in source
    assert PAGE11_SHA256 in source
    assert PAGE10_SHA256 in source
    assert PAGE9_SHA256 in source
    assert "kProductionPage15ActiveVaultBytes = 2843047U" in source
    assert "kBurnedPage14ActiveVaultBytes = 2817779U" in source
    assert "kBurnedPage13ActiveVaultBytes = 2846623U" in source
    assert "kBurnedPage12ActiveVaultBytes = 2725423U" in source
    ...
    assert "kO1C92PriorityStateReceiptBytes = 52014U" in source
    assert "kProductionLineageOrdinal = 28U" in source
    assert "kProductionActiveLimit = 251U" in source
    assert "kProductionActiveClauseCount = 251U" in source
    assert "kProductionActiveLiteralCount = 710463U" in source
    assert "kGlobalNoveltyBaselineClauseCount = 1812U" in source
    assert "kO1C93ImportedNovelClauseCount = 261U" in source
    assert (
    ...
    page13_gate = source.index("vault_sha256 == kBurnedPage13ActiveVaultSha256")
    page12_gate = source.index("vault_sha256 == kBurnedPage12ActiveVaultSha256")
    ...
    page15_gate = source.index("vault_sha256 != kProductionPage15ActiveVaultSha256")
...
def test_v26_operator_body_is_byte_equivalent_to_frozen_v25() -> None:
    source = V26.read_text(encoding="utf-8")
    provenance_constants = """constexpr const char *kO1C93PreparationManifestSha256 =
    "187f09309b2d866549441d713f29bfed696c140f5c5a99536001c889f5836a24";
constexpr const char *kO1C92PriorityStateReceiptSha256 =
    "1c69bb329819ff873758e72ccfd69649310e5dd089c68665c34d0a287821c1e6";
constexpr size_t kO1C92PriorityStateReceiptBytes = 52014U;
constexpr size_t kProductionLineageOrdinal = 28U;
...
"""
    frozen_provenance_constants = """constexpr const char *kO1C91PreparationManifestSha256 =
    "e46ca7373bc3a94efc30dcd309728005e3bee8b93983dc2c396f45bd487dd458";
constexpr size_t kProductionLineageOrdinal = 27U;
constexpr size_t kProductionActiveLimit = 252U;
constexpr size_t kProductionActiveClauseCount = 252U;
constexpr size_t kProductionActiveLiteralCount = 704145U;
constexpr size_t kGlobalNoveltyBaselineClauseCount = 1551U;
constexpr size_t kO1C91ImportedNovelClauseCount = 260U;
"""
    provenance_assertions = """static_assert(std::string_view(kO1C93PreparationManifestSha256).size() == 64U);
...
"""
    frozen_provenance_assertions = """static_assert(std::string_view(kO1C91PreparationManifestSha256).size() == 64U);
static_assert(kProductionLineageOrdinal == 27U);
static_assert(kProductionActiveLimit == kProductionActiveClauseCount);
static_assert(kProductionActiveLimit + kO1C91ImportedNovelClauseCount ==
              kMaximumVaultClauses);
static_assert(kProductionActiveLimit + kO1C91ImportedNovelClauseCount == 512U);
static_assert(kProductionActiveLimit == 252U);
static_assert(kProductionActiveLiteralCount == 704145U);
static_assert(kO1C91ImportedNovelClauseCount == 260U);
static_assert(kGlobalNoveltyBaselineClauseCount == 1551U);

"""
    burned_page14_constants = """constexpr const char *kBurnedPage14ActiveVaultSha256 =
    "00a5a4a7b33f1c09c8df24162709b17994bad5825d92476a5f5283a3bf025c7e";
constexpr size_t kBurnedPage14ActiveVaultBytes = 2817779U;
"""
    burned_page14_gate = """    if (production_seal &&
...
"""
    receipt_seed_json = """        << "\\",\\"source_priority_state_receipt_sha256\\":\\""
        << kO1C92PriorityStateReceiptSha256
        << "\\",\\"source_priority_state_receipt_bytes\\":"
        << kO1C92PriorityStateReceiptBytes
        << ",\\"seed_source\\":\\""
"""
    frozen_seed_json = """        << "\\",\\"seed_source\\":\\""
"""
    for delta in (burned_page14_constants, burned_page14_gate):
        assert delta in source
        source = source.replace(delta, "")
    assert receipt_seed_json in source
    source = source.replace(receipt_seed_json, frozen_seed_json)
    assert provenance_constants in source
    source = source.replace(provenance_constants, frozen_provenance_constants)
    assert provenance_assertions in source
    normalized = source.replace(
        provenance_assertions, frozen_provenance_assertions
    )
    replacements = (
        ("O1C-0095", "O1C-0092"),
        ("O1C95", "O1C92"),
        ...
        ("kProductionPage15", "kProductionPage14"),
        (PAGE15_SHA256, PAGE14_SHA256),
        ("2843047U", "2817779U"),
        ("Page-15", "Page-14"),
    )
    for current, prior in replacements:
        normalized = normalized.replace(current, prior)
    assert normalized == V25.read_text(encoding="utf-8")
