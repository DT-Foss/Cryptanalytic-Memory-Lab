...
PRIOR_BANK = (
    ROOT
    / "research"
    / "o1c85_page10_transport_recovery_seed_20260720"
    / "final-parent-centered-priority-bank.bin"
)
PAGE10 = (
    ROOT
...
PRIOR_BANK_SHA256 = "05b8acf3ecd5423016e5d7ef7d649f790e758e3477a943fe7306280064a4c630"
...
FROZEN_V21_SHA256 = "b3541a83c3aeba5ed392174731e450816ce86923688296c5f5fecd3241a19bd3"
CADICAL_INCLUDE = Path("/opt/homebrew/opt/cadical/include")
CADICAL_LIBRARY = Path("/opt/homebrew/opt/cadical/lib/libcadical.a")


HARNESS = r"""
#define O1_CRYPTO_LAB_O1C86_NO_MAIN
#include "cadical_o1_joint_score_sieve_v22.cpp"
...
def frozen_inputs() -> tuple[Path, Path, Path]:
    ...
    return PRIOR_BANK, PAGE10, PAGE9
...
def native_builds(tmp_path_factory: pytest.TempPathFactory) -> tuple[Path, Path]:
    ...
    builds = (
        (
            [*common, "-DO1_CRYPTO_LAB_O1C86_PUBLIC_FIXTURE"],
            harness_source,
            harness,
        ),
        (common, V22, main),
...
def test_v22_normal_production_binary_compiles_and_help_surface(
    ...
    assert completed.stdout == (
        "usage: cadical_o1_joint_score_sieve_v22 --cnf PATH --potential PATH "
...
def test_live_bank_retains_failure_first_one_shot_and_seed_zero_semantics(
    ...
    assert actions["schema"] == "o1-256-o1c86-failure-first-proof-mining-actions-v1"
...
def test_native_production_gate_rejects_prior_live_bank_before_solver(
    native_builds: tuple[Path, Path],
    frozen_inputs: tuple[Path, Path, Path],
    tmp_path: Path,
) -> None:
    _, main = native_builds
    old_bank, _, page9 = frozen_inputs
    completed = _sealed_command(main, tmp_path, page9, old_bank)
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v22: sealed O1C86 priority seed differs\n"
    )
...
def test_native_requires_seed_zero_before_reading_solver_inputs(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v22: O1C86 priority operator requires seed zero\n"
    )
...
def test_native_production_gate_rejects_exact_page9_before_solver(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v22: burned O1C84 Page-9 active vault rejected\n"
    )
...
def test_native_production_gate_rejects_exact_page10_before_solver(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v22: burned O1C85 Page-10 active vault rejected\n"
    )


def test_source_contract_and_historical_native_freeze() -> None:
    source = V22.read_text(encoding="utf-8")
    assert hashlib.sha256(V21.read_bytes()).hexdigest() == FROZEN_V21_SHA256
    assert '#include "cadical_o1_joint_score_sieve_v18.cpp"' in source
    assert '#include "o1c82_parent_centered_priority.hpp"' in source
    assert "O1_CRYPTO_LAB_O1C86_PUBLIC_FIXTURE" in source
    assert "O1_CRYPTO_LAB_O1C86_NO_MAIN" in source
    assert "o1-256-cadical-joint-score-sieve-result-v22" in source
    assert "o1-256-o1c86-live-parent-centered-continuation-priority-state-v1" in source
    assert "o1-256-o1c86-failure-first-proof-mining-actions-v1" in source
    ...
    assert "kProductionPage11ActiveVaultBytes = 2876731U" in source
    ...
    page11_gate = source.index("vault_sha256 != kProductionPage11ActiveVaultSha256")
    solver = source.index("CaDiCaL::Solver solver;")
    assert seed_gate < page10_gate < page9_gate < page11_gate < solver
