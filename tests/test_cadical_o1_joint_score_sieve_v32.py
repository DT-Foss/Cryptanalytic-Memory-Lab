...
import o1_crypto_lab.joint_score_sieve_v35 as adapter_v35
...
DERIVED_RECEIPT = (
    ...
    / "research"
    / "o1c106_page21_type_safe_rollover_seed_20260721"
    / "o1c104-derived-resolution-closure-receipt.json"
)
PAGE18 = (
    ROOT
    / "research"
...
MANIFEST_BYTES = 9_376
MANIFEST_SHA256 = "91044c235473c1a24fdeeb283454babc5ebc800ea19236840dd7193d6f3c96c2"
...
def frozen_inputs() -> tuple[Path, ...]:
    exact = (
        ...
        (STATE_RECEIPT, 52_015, STATE_RECEIPT_SHA256),
...
def native_builds(tmp_path_factory: pytest.TempPathFactory) -> tuple[Path, Path]:
    ...
    for flags, source, executable in (
        (
            [*common, "-DO1_CRYPTO_LAB_O1C107_PUBLIC_FIXTURE"],
            harness_source,
            harness,
        ),
        (common, V32, main),
...
def test_v32_normal_production_binary_compiles_help_and_uuid(
    ...
    assert completed.stdout == (
        "usage: cadical_o1_joint_score_sieve_v32 --cnf PATH --potential PATH "
...
def test_live_evolved_bank_import_and_machine_readable_provenance(
    ...
    assert seed["source_priority_state_receipt_bytes"] == 52_015
...
def test_real_native_v32_priority_seed_crosses_exact_v35_adapter_seam(
    native_builds: tuple[Path, Path],
) -> None:
    harness, _ = native_builds
    completed = _run(harness, "import", LIVE_BANK)
    ...
    validated = adapter_v35._validate_seed_report(
        actual,
        seed_sha256=LIVE_BANK_SHA256,
        production_seal=False,
    )
    assert validated == actual
    assert set(actual) == adapter_v35._SEED_FIELDS
...
def test_native_rejects_prior_bank_before_solver(
    ...
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v32: sealed O1C107 priority seed differs\n"
    )
...
def test_native_requires_seed_zero_before_inputs(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v32: O1C107 priority operator requires seed zero\n"
    )
...
def test_native_rejects_every_burned_page_before_solver(
    ...
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == f"cadical_o1_joint_score_sieve_v32: {message}\n"


def test_native_rejects_nonmatching_page21_before_solver(
    native_builds: tuple[Path, Path], tmp_path: Path
) -> None:
    _, main = native_builds
    forged = tmp_path / "forged-page-21-active.bin"
    forged.write_bytes(bytes(2_762_499))
    completed = _sealed_command(main, tmp_path, forged, LIVE_BANK)
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v32: sealed O1C107 Page-21 active vault differs\n"
    )


def test_source_contract_and_o1c106_binding() -> None:
    source = V32.read_text(encoding="utf-8")
    ...
    assert "O1_CRYPTO_LAB_O1C107_PUBLIC_FIXTURE" in source
    assert "O1_CRYPTO_LAB_O1C107_NO_MAIN" in source
    assert "o1-256-cadical-joint-score-sieve-result-v32" in source
    assert "o1-256-o1c107-live-parent-centered-continuation-priority-state-v1" in source
    assert "o1-256-o1c107-failure-first-proof-mining-actions-v1" in source
    ...
    assert manifest["schema"] == (
        "o1-256-o1c106-page21-type-safe-rollover-preparation-v1"
    )
    assert manifest["parent_terminal"]["page20_burned"] is True
    assert manifest["parent_terminal"]["lineage33_burned"] is True
    assert manifest["logical_known_registry"]["combined_clause_count"] == 2_692
    assert manifest["logical_known_registry"]["emitted_clause_count"] == 2_603
    assert manifest["logical_known_registry"]["inherited_derived_clause_count"] == 5
    assert manifest["logical_known_registry"]["new_derived_clause_count"] == 84
    assert manifest["derived_resolution_namespaces"]["inherited"][
        "resident_clause_count"
    ] == 3
    assert manifest["derived_resolution_namespaces"]["new"][
        "resident_clause_count"
    ] == 41
    assert manifest["derived_resolution_namespaces"]["new"]["receipt_sha256"] == (
        DERIVED_RECEIPT_SHA256
    )
    assert manifest["derived_resolution_namespaces"][
        "causal_attic_occurrence_rows_added"
    ] == 0
    assert manifest["page21"]["lineage_ordinal"] == 34
    assert manifest["page21"]["clause_count"] == 247
    assert manifest["page21"]["literal_count"] == 690_330
    assert manifest["page21"]["serialized_bytes"] == 2_762_499
    assert manifest["page21"]["active_sha256"] == PAGE21_SHA256
    assert manifest["page21"]["headroom"]["clauses"] == 265
    assert manifest["certification"]["active_pass_count"] == 247
    ...
    assert "kProductionPage21ActiveVaultBytes = 2762499U" in source
    ...
    assert "kProductionLineageOrdinal = 34U" in source
    assert "kProductionActiveLiteralCount = 690330U" in source
    assert "kProductionClauseHeadroom = 265U" in source
    assert "kGlobalNoveltyBaselineClauseCount = 2692U" in source
    ...
    page21_gate = source.index("vault_sha256 != kProductionPage21ActiveVaultSha256")
...
def test_v32_preserves_the_v31_operator_boundary_and_changes_only_seals() -> None:
    current = V32.read_text(encoding="utf-8")
    previous = (NATIVE / "cadical_o1_joint_score_sieve_v31.cpp").read_text(
        encoding="utf-8"
    )
    for token in (
        "scan_and_select",
    ...
    assert "kProductionActiveLimit = 247U" in current
    assert "kProductionClauseHeadroom = 265U" in current
    assert "kBurnedPage20ActiveVaultSha256" in current
    assert "kBurnedPage19ActiveVaultSha256" in current
    assert "o1-256-cadical-joint-score-sieve-result-v32" in current

    burned_page20_constants = '''constexpr const char *kBurnedPage20ActiveVaultSha256 =
    "537f63c5284e15e451739f7369fbe6ee8dddbc5dfdb15b26988269a1e40e5519";
constexpr size_t kBurnedPage20ActiveVaultBytes = 2762455U;
'''
    burned_page20_gate = '''    if (production_seal &&
...
'''
    assert burned_page20_constants in current
    assert burned_page20_gate in current
    normalized = current.replace(burned_page20_constants, "")
    normalized = normalized.replace(burned_page20_gate, "")
    for fresh, parent in (
        ("O1C-0107", "O1C-0105"),
        ("O1C107", "O1C105"),
        ("o1c107", "o1c105"),
        ("V32", "V31"),
        ("v32", "v31"),
        ("kO1C106PreparationManifest", "kO1C104PreparationManifest"),
        (
        ...
        ("9376U", "9830U"),
        ("kProductionLineageOrdinal = 34U", "kProductionLineageOrdinal = 33U"),
        ("kProductionLineageOrdinal == 34U", "kProductionLineageOrdinal == 33U"),
        ("kProductionActiveLiteralCount = 690330U", "kProductionActiveLiteralCount = 690319U"),
        ("kProductionActiveLiteralCount == 690330U", "kProductionActiveLiteralCount == 690319U"),
        ("kProductionPage21", "kProductionPage20"),
        (
        ...
        normalized = normalized.replace(fresh, parent)
