...
import o1_crypto_lab.o1c107_apple8_parent_centered_continuation_run as runner
from o1_crypto_lab import joint_score_sieve_v35 as v35
...
FAKE_NATIVE = b"fake-native-v32-executable\n"
...
class Fixture:
    def __init__(
        ...
        self.native_override: v35.JointScoreSieveV35Result | None = None
        self.last_native_result: v35.JointScoreSieveV35Result | None = None
        ...
        self.page21 = (self.published / runner.ACTIVE_PROJECTION_NAME).read_bytes()
        ...
        monkeypatch.setattr(
            runner._native_v35._v9._v8._v7._v1,
            "_potential",
            lambda payload: self.semantic_field,
        )
        monkeypatch.setattr(
            runner._native_v35._v9,
    ...
    def command(self, argv: list[str], **kwargs: object) -> SimpleNamespace:
        ...
        if "-o" in argv:
            ...
            assert staged_source == closure_root / runner.SOURCE_PATHS["native_v32"]
        ...
        return SimpleNamespace(
            returncode=0,
            stdout=b"bad usage\n" if self.bad_smoke else runner.NATIVE_V32_USAGE,
            stderr=b"",
        )
    ...
    def native_result(
        ...
        *,
        ...
        adapter_memory: dict[str, object] = {
            "memory_series_schema": (v35._v9.JOINT_SCORE_SIEVE_MEMORY_SERIES_SCHEMA),
            "memory_sample_limit": v35._v9.JOINT_SCORE_SIEVE_MAXIMUM_MEMORY_SAMPLES,
        ...
        raw: dict[str, object] = {
            "schema": v35.JOINT_SCORE_SIEVE_RESULT_SCHEMA,
            "seed": 0,
            ...
            "potential_sha256": _sha(self.inputs["potential"].read_bytes()),
            "active_vault_sha256": _sha(self.page21),
        ...
        return v35.JointScoreSieveV35Result(
            status=status,
            conflict_limit=128,
            threshold=runner.THRESHOLD,
    ...
    def adapter(self, **kwargs: object) -> object:
        ...
        assert intent["page21_burned"] is True
        assert intent["lineage34_burned"] is True
        ...
        assert kwargs["expected_source_sha256"] == self.source_sha["native_v32"]
        assert kwargs["source_path"] == (
            executable.parents[1]
            / runner.NATIVE_CLOSURE_DIRECTORY
            / runner.SOURCE_PATHS["native_v32"]
        )
        ...
        assert Path(cast(Path, kwargs["vault_path"])).read_bytes() == self.page21
    ...
    def certify_page21_inputs(self, **kwargs: object) -> object:
        ...
        return SimpleNamespace(page21_sha256=runner.PAGE21_SHA256)
    ...
    def semantic_replay(
        ...
        assert input_vault.sha256 == _sha(self.page21)
        return replayed
    ...
    def write_config(self) -> None:
        ...
        config = {
            ...
            "native": {
                "source": runner.SOURCE_PATHS["native_v32"],
                "source_sha256": self.source_sha["native_v32"],
                ...
                "adapter_schema": v35.JOINT_SCORE_SIEVE_ADAPTER_SCHEMA,
                "result_schema": v35.JOINT_SCORE_SIEVE_RESULT_SCHEMA,
                "page21_sha256": runner.PAGE21_SHA256,
                "page21_bytes": runner.PAGE21_SERIALIZED_BYTES,
            ...
            "next_action": (
                "Burn Page 21 / lineage 34 exactly once; never retry or replay "
                "Pages 20 through 9."
            ),
        }
...
def test_preflight_validates_published_bundle_without_regeneration(
    ...
    assert len(cast(dict[str, object], row["prepared_artifacts"])) == 17
    ...
    assert residency["current_projection"]["encoding_only"]["sha256"] == (
        runner.PAGE21_SHA256
    )
    assert (
        residency["emitted_causal_attic"]["active_projection"]["encoding_only"][
            "sha256"
        ]
        == runner.EMITTED_ONLY_ACTIVE_PROJECTION_SHA256
    )
    assert runner.EMITTED_ONLY_ACTIVE_PROJECTION_SHA256 != runner.PAGE21_SHA256
    assert (
    ...
    inherited = registry["inherited_derived"]["clause_sha256"]
    new = registry["new_derived"]["clause_sha256"]
    assert len(prepared.globally_known_clause_sha256) == 2_692
    assert len(inherited) == 5
    assert len(new) == 84
    assert set(inherited).issubset(prepared.globally_known_clause_sha256)
    assert set(new).issubset(prepared.globally_known_clause_sha256)
...
def test_real_v8_input_certification_failure_precedes_every_side_effect(
    ...
    fixture.certification_exception = RuntimeError("forced Page21 theorem failure")
    with pytest.raises(
        runner.O1C107RunError,
        match="real Page-21 v8 input certification failed before intent",
    ):
        fixture.execute()
    assert fixture.certification_calls == 1
...
def test_preflight_rejects_capacity_proof_mutation_before_intent(
    ...
    target = cast(dict[str, Any], manifest["page21"])
    for field in path[:-1]:
        target = cast(dict[str, Any], target)[field]
    cast(dict[str, Any], target)[path[-1]] = replacement
    with pytest.raises(
        runner.O1C107RunError, match="Page-21 native capacity proof differs"
    ):
        runner._validate_native_capacity_proof(manifest["page21"])
    assert fixture.adapter_calls == fixture.build_calls == fixture.smoke_calls == 0
    assert not list((fixture.root / "runs").glob(f"*_{runner.CAPSULE_SUFFIX}"))
...
def test_preflight_rejects_published_mutation_without_build_or_call(
    ...
    with pytest.raises(runner.O1C107RunError, match="artifact seal differs"):
        runner.preflight(
            fixture.config_path,
            root=fixture.root,
            system_probe=fixture.system,
...
def test_one_build_smoke_and_observed_identity_precede_intent(
    ...
    assert build["smoke"]["stdout"] == {
        "serialized_bytes": len(runner.NATIVE_V32_USAGE),
        "sha256": _sha(runner.NATIVE_V32_USAGE),
    }
...
def test_staged_closure_tamper_is_rejected_before_intent(
    ...
    with pytest.raises(runner.O1C107RunError, match="staged native closure"):
        fixture.execute()
...
def test_bad_help_smoke_is_preintent_and_leaves_no_capsule(fixture: Fixture) -> None:
    ...
    with pytest.raises(runner.O1C107RunError, match="--help smoke seal differs"):
        fixture.execute()
...
def test_source_mutation_after_smoke_never_burns_or_rebuilds(fixture: Fixture) -> None:
    ...
    with pytest.raises(runner.O1C107RunError, match="changed before native call"):
        fixture.execute()
    capsules = list((fixture.root / "runs").glob(f"*_{runner.CAPSULE_SUFFIX}"))
    assert len(capsules) == 1
    assert not (capsules[0] / "episodes/00/intent.json").exists()
    with pytest.raises(runner.O1C107RunError, match="pre-intent capsule"):
        fixture.execute()
    assert fixture.build_calls == fixture.smoke_calls == 1
    assert fixture.adapter_calls == 0
...
def test_binary_rehash_immediately_before_adapter_is_terminal(
    ...
    assert episode["page21_burned"] is True
    assert episode["lineage34_burned"] is True
...
def test_missing_binary_after_intent_is_recoverable_terminal(
    ...
    assert failure["exception_type"] == "O1C107RunError"
...
def test_self_consistent_forged_completed_capsule_is_never_republished(
    ...
    with pytest.raises(runner.O1C107RunError):
        fixture.execute()
...
def test_resealed_forged_sat_key_fails_independent_public_verification(
    ...
    with pytest.raises(runner.O1C107RunError, match="not publicly verified"):
        fixture.execute()
    assert not authoritative.exists()
    assert fixture.adapter_calls == fixture.build_calls == fixture.smoke_calls == 1


def test_resealed_self_consistent_forged_novelty_fails_v35_semantic_replay(
    fixture: Fixture,
) -> None:
    result = fixture.execute()
    capsule = fixture.root / cast(str, result["capsule"])
    ...
    episode["science"]["active_page21_new_clauses"] = 1
    ...
    with pytest.raises(runner.O1C107RunError, match="semantic replay"):
        fixture.execute()
...
def test_novelty_uses_complete_published_attic_digest_set(fixture: Fixture) -> None:
    ...
    assert science["active_page21_new_clauses"] == 1
...
def test_production_config_has_no_predicted_binary_identity() -> None:
    ...
    assert config["native"]["page21_sha256"] == runner.PAGE21_SHA256
    assert config["native"]["page21_bytes"] == runner.PAGE21_SERIALIZED_BYTES
    ...
    assert "joint_score_sieve_v35" in source
    ...
    assert config["budgets"]["lineage_call_ordinals"] == [34]
    ...
    assert "Page 21" in config["next_action"]
