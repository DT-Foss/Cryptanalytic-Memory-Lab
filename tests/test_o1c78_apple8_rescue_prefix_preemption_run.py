...
import o1_crypto_lab.o1c78_apple8_rescue_prefix_preemption_run as frontier_run
...
OBSERVED = tuple(
    sorted(
        set(range(1, 11))
        | {abs(literal) for literal in frontier_run.PREFIX_LITERALS}
    )
)
IDENTITY = vault_identity_from_sources(
    ...
    bound_rule="o1c78-staging-runner-test-bound-v1",
...
def prepared() -> frontier_run.PreparedStream:
    ...
    state = initialize_causal_residency(
        ...
        parent_lineage_ordinal=17,
        first_lineage_ordinal=18,
        active_limit=256,
    )
    ...
    prefix_plan = RescuePrefixPreemptionPlan(frontier_run.PREFIX_LITERALS)
    return frontier_run.prepared_stream_from_state(
        ...
        prefix_plan_document={"plan": prefix_plan.describe()},
    )
...
def _install_fake_validator(monkeypatch: pytest.MonkeyPatch) -> None:
    def validate(
        ...
        **_kwargs: object,
        ...
        staging_plan = cast(
            ResidualPolarityStagingPlan, _kwargs["staging_plan"]
        )
        occurrences = tuple(
        ...
        once_literals = (
            (frontier_run.PREFIX_LITERALS[0],) if activated else ()
        )
        once_payload = struct.pack(
            f"<{len(once_literals)}i", *once_literals
        )
        skipped_rescue = 1 if cast(bool, fake.rescue_skip) else 0
        skipped_falsifying = (
            len(frontier_run.PREFIX_LITERALS)
            - len(once_literals)
            - skipped_rescue
        )
        prefix = {
            "plan_sha256": prefix_plan.sha256,
            "active_vault_sha256": active.sha256,
            "parent_staging_plan_sha256": staging_plan.sha256,
            "baseline_trace_sha256": frontier_run.FROZEN_BASELINE_TRACE_SHA256,
            "prefix_order_encoding": "signed-i32le",
            "prefix_order_bytes": prefix_plan.serialized_bytes,
            "prefix_order_sha256": prefix_plan.prefix_order_sha256,
            "prefix_literals": list(prefix_plan.prefix_literals),
            "cursor": len(prefix_plan.prefix_literals),
            "rows_consumed": len(prefix_plan.prefix_literals),
            "once_returns": len(once_literals),
            "skipped_preassigned_falsifying": skipped_falsifying,
            "skipped_preassigned_rescue": skipped_rescue,
            "cb_decide_calls": len(once_literals) + 3,
            "parent_cb_decide_calls": 3,
            "first_once_return_call": (1 if once_literals else None),
            "first_parent_call": len(once_literals) + 1,
            "all_rows_consumed_before_first_parent_call": True,
            "mechanism_activated": activated,
            "once_return_sequence_count": len(once_literals),
            "once_return_sequence_bytes": len(once_payload),
            "once_return_sequence_hex": once_payload.hex(),
            "once_return_sequence_sha256": hashlib.sha256(
                once_payload
            ).hexdigest(),
            "once_return_events": [
                {
                ...
                for index, literal in enumerate(once_literals)
            ],
        }
        prefix_activation = frontier_run._validate_prefix_activation(
            prefix_preemption=prefix,
            prefix_plan=prefix_plan,
            trace_sha256=trace,
        )
        return {
...
def test_single_call_schedule_persists_intent_and_binds_all_three_plans(
    tmp_path: Path,
    prepared: frontier_run.PreparedStream,
    ...
    def invoke(
        ...
        prefix = parse_rescue_prefix_preemption_plan(prefix_path.read_bytes())
        assert intent["prefix_plan"] == prefix.describe()
...
@pytest.mark.parametrize(
    ("activated", "rescue_skip", "trace", "safe_prunes", "classification"),
    (
        (
            True,
            False,
        ...
        (False, False, "cd" * 32, 0, frontier_run.NO_ACTIVATION),
        (True, True, "de" * 32, 0, frontier_run.NO_ACTIVATION),
        (True, False, "ef" * 32, 0, frontier_run.MECHANISM_ONLY),
        (True, False, "12" * 32, 2, frontier_run.SAFE_PRUNE_GAIN),
    ),
)
def test_fixed_baseline_and_safe_prune_classification_are_not_conflated(
    ...
    assert outcome.mechanism_activated is (
        activated
        and not rescue_skip
        and trace != frontier_run.FROZEN_BASELINE_TRACE_SHA256
    )
...
def test_invalid_return_consumes_lineage_without_retry(
    ...
    def invalid(*_args: object, **_kwargs: object) -> dict[str, object]:
        raise frontier_run.O1C78RunError("synthetic invalid native return")
...
def test_pre_invocation_guard_failure_consumes_lineage_but_not_call_budget(
    tmp_path: Path,
    prepared: frontier_run.PreparedStream,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    def reject_before_call(
        _binding: Mapping[str, object] | None, *, when: str
    ) -> None:
        assert when == "before"
        raise frontier_run.O1C78RunError("synthetic pre-call guard failure")

    monkeypatch.setattr(
        frontier_run, "_validate_call_window_executable", reject_before_call
    )
    ...
    capsule = tmp_path / "pre-call-failure"
    outcome = _execute(capsule, prepared, invoke)
    ...
    assert episode["native_call_issued"] is False
    assert episode["native_calls_consumed"] == 0
    assert episode["requested_conflicts"] == 0
    assert failure["native_call_issued"] is False
    ...
    result = frontier_run.build_result(
        outcome=outcome,
        capsule_relative="runs/pre-call-failure",
        source_commit="ad" * 20,
        started_at="2026-07-20T00:20:00+02:00",
    )
    frontier_run.write_recovery_source(capsule, result)
    recovered = frontier_run.recover_publication(capsule)
    recovered_resources = cast(Mapping[str, object], recovered["resources"])
    assert recovered_resources["native_solver_calls"] == 0
    assert recovered_resources["requested_conflicts"] == 0


@pytest.mark.parametrize(
    ("status", "key_model", "classification"),
    (
        (10, b"k" * 32, frontier_run.PUBLIC_EXACT_RECOVERY),
        (20, None, frontier_run.THRESHOLD_REGION_EXHAUSTED),
    ),
)
def test_terminal_status_precedes_prefix_and_clause_readouts(
    tmp_path: Path,
    prepared: frontier_run.PreparedStream,
    monkeypatch: pytest.MonkeyPatch,
    status: int,
    ...
    outcome = _execute(
        ...
        prepared,
        cast(frontier_run.EpisodeInvoker, invoke),
        verifier=lambda model: model == b"k" * 32,
    )
    assert outcome.classification == classification
    assert outcome.native_calls == 1
...
def test_failed_call_publication_recovery_uses_zero_callbacks(
    ...
    assert outcome.native_calls == 1
    assert outcome.requested_conflicts == 128
    failed_episode = outcome.episodes[0]
    failed = cast(Mapping[str, object], failed_episode["terminal_failure"])
    assert failed_episode["native_call_issued"] is True
    assert failed["native_call_issued"] is True
    assert failed["native_result_returned"] is False
    assert failed["native_calls_consumed"] == 1
    assert failed["requested_conflicts_consumed"] == 128
    result = frontier_run.build_result(
        outcome=outcome,
    ...
    assert tuple(calls) == calls_before == ((0, 18),)
...
def test_completed_publication_recovery_revalidates_prefix_sidecars(
    tmp_path: Path,
    prepared: frontier_run.PreparedStream,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    def provenance(**kwargs: object) -> None:
        ...
        prefix = cast(
            RescuePrefixPreemptionPlan, kwargs["prefix_plan"]
        )
        assert tuple(prefix.prefix_literals) == frontier_run.PREFIX_LITERALS
        provenance_checks.append((frontier.sha256, staging.sha256))

    monkeypatch.setattr(
        frontier_run, "_validate_archived_v19_provenance", provenance
    )
...
def test_sealed_capsule_republishes_result_without_preflight_or_execution(
    ...
    assert "one fresh `K=256` Page 5" in run_document
    assert "sealed frontier, staging, and 11-row prefix plans" in run_document
...
def test_frozen_config_targets_o1c77_page5_lineage18_and_v19() -> None:
    config = frontier_run.load_config(
        frontier_run.lab_root() / frontier_run.CONFIG_RELATIVE
    )
    parent = cast(Mapping[str, object], config["parent"])
    native = cast(Mapping[str, object], config["native"])
    budgets = cast(Mapping[str, object], config["budgets"])
    assert parent["result_sha256"] == frontier_run.PARENT_RESULT_SHA256
    assert parent["last_lineage_ordinal"] == 17
    assert budgets["lineage_call_ordinals"] == [18]
    assert native["adapter_schema"] == (
        "o1-256-joint-score-sieve-v19-adapter-v1"
    )
    assert native["result_schema"] == (
        "o1-256-cadical-joint-score-sieve-result-v16"
    )
    assert native["prefix_plan_sha256"] == frontier_run.PREFIX_ORDER_SHA256


def test_prefix_gate_rejects_out_of_order_once_returns() -> None:
    plan = RescuePrefixPreemptionPlan(frontier_run.PREFIX_LITERALS)
    returned = (plan.prefix_literals[1], plan.prefix_literals[0])
    payload = struct.pack("<2i", *returned)
    evidence: dict[str, object] = {
        "plan_sha256": plan.sha256,
        "prefix_order_encoding": "signed-i32le",
        "prefix_order_bytes": plan.serialized_bytes,
        "prefix_order_sha256": plan.prefix_order_sha256,
        "prefix_literals": list(plan.prefix_literals),
        "cursor": len(plan.prefix_literals),
        "rows_consumed": len(plan.prefix_literals),
        "once_returns": 2,
        "skipped_preassigned_falsifying": len(plan.prefix_literals) - 2,
        "skipped_preassigned_rescue": 0,
        "cb_decide_calls": 5,
        "parent_cb_decide_calls": 3,
        "first_once_return_call": 1,
        "first_parent_call": 3,
        "all_rows_consumed_before_first_parent_call": True,
        "mechanism_activated": True,
        "once_return_sequence_count": 2,
        "once_return_sequence_bytes": len(payload),
        "once_return_sequence_hex": payload.hex(),
        "once_return_sequence_sha256": hashlib.sha256(payload).hexdigest(),
        "once_return_events": [
            {"call": 1, "prefix_index": 1, "literal": returned[0]},
            {"call": 2, "prefix_index": 0, "literal": returned[1]},
        ],
    }
    with pytest.raises(
        frontier_run.O1C78RunError, match="ordered subsequence"
    ):
        frontier_run._validate_prefix_activation(
            prefix_preemption=evidence,
            prefix_plan=plan,
            trace_sha256="f0" * 32,
        )
