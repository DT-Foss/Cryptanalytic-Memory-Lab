...
import o1_crypto_lab.o1c76_apple8_causal_frontier_prepare as prepare
...
class LoadedPreparation:
    directory: Path
    prepared: prepare.PreparedFrontier
...
def loaded_preparation(
    ...
    directory = tmp_path_factory.mktemp("o1c76-real-parent") / "seed"
    manifest = prepare.prepare_o1c76_causal_frontier(
    ...
    prepared = prepare.load_prepared_frontier(
        directory,
...
def test_real_parent_replay_is_exact_and_zero_call(
    ...
    assert prepared.active_projection.sha256 == prepare.PAGE3_SHA256
    assert state.current_projection.lineage_ordinal == 16
...
def test_generic_target_free_plan_selects_exact_public_frontier(
    ...
    assert plan.active_vault_sha256 == prepare.PAGE3_SHA256
...
def test_manifest_seals_exact_regular_file_inventory(
    ...
    assert artifact_set["artifact_count"] == len(rows) == 16
...
def test_loader_rejects_inventory_and_byte_mutations(
    ...
    if mutation == "extra":
        ...
        match = "artifact"
    with pytest.raises(prepare.O1C76PreparationError, match=match):
        prepare.load_prepared_frontier(
            polluted,
            expected_manifest_sha256=prepare.EXPECTED_PREPARED_MANIFEST_SHA256,
        )
...
def test_loader_rejects_manifest_digest_before_artifact_replay(
    ...
    with pytest.raises(prepare.O1C76PreparationError, match="manifest"):
        prepare.load_prepared_frontier(
...
def test_atomic_publisher_refuses_existing_destination(tmp_path: Path) -> None:
    ...
    with pytest.raises(prepare.O1C76PreparationError, match="already exists"):
...
def test_cli_reports_bounded_preparation_failure(
    ...
    def fail(**_kwargs: object) -> dict[str, object]:
        raise prepare.O1C76PreparationError("sealed parent differs")

    monkeypatch.setattr(prepare, "prepare_o1c76_causal_frontier", fail)
    assert prepare.main(["--output-dir", (tmp_path / "bad").as_posix()]) == 1
    captured = capsys.readouterr()
    assert captured.out == ""
    assert captured.err == "O1C-0076: sealed parent differs\n"
