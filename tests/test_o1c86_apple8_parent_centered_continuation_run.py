...
import o1_crypto_lab.o1c86_apple8_parent_centered_continuation_run as runner
from o1_crypto_lab import joint_score_sieve_v25 as v25
...
FAKE_NATIVE = b"fake-native-v22-executable\n"
...
class Fixture:
    def __init__(
        ...
        self.page11 = (self.published / runner.ACTIVE_PROJECTION_NAME).read_bytes()
    ...
    def command(self, argv: list[str], **kwargs: object) -> SimpleNamespace:
        ...
        return SimpleNamespace(
            returncode=0,
            stdout=b"bad usage\n" if self.bad_smoke else runner.NATIVE_V22_USAGE,
            stderr=b"",
        )
    ...
    def native_result(
        ...
        *,
        ...
        raw: dict[str, object] = {
            "schema": v25.JOINT_SCORE_SIEVE_RESULT_SCHEMA,
            "seed": 0,
            "active_vault_sha256": _sha(self.page11),
        ...
        return v25.JointScoreSieveV25Result(
            status=status,
            conflict_limit=128,
    ...
    def adapter(self, **kwargs: object) -> object:
        ...
        assert intent["page11_burned"] is True
        assert intent["lineage24_burned"] is True
        ...
        assert kwargs["expected_source_sha256"] == self.source_sha["native_v22"]
        ...
        assert Path(cast(Path, kwargs["vault_path"])).read_bytes() == self.page11
        assert kwargs["sealed_page11_path"] == kwargs["vault_path"]
    ...
    def write_config(self) -> None:
        ...
        config = {
            ...
            "native": {
                "source": runner.SOURCE_PATHS["native_v22"],
                "source_sha256": self.source_sha["native_v22"],
                ...
                "adapter_schema": v25.JOINT_SCORE_SIEVE_ADAPTER_SCHEMA,
                "result_schema": v25.JOINT_SCORE_SIEVE_RESULT_SCHEMA,
                "page11_sha256": runner.PAGE11_SHA256,
                "page11_bytes": runner.PAGE11_SERIALIZED_BYTES,
            ...
            "next_action": "Burn Page 11 / lineage 24 once; never retry or replay.",
        }
...
def test_preflight_rejects_published_mutation_without_build_or_call(
    ...
    with pytest.raises(runner.O1C86RunError, match="artifact seal differs"):
        runner.preflight(
            fixture.config_path,
            root=fixture.root,
            system_probe=fixture.system,
...
def test_one_build_smoke_and_observed_identity_precede_intent(
    ...
    assert build["smoke"]["stdout"] == {
        "serialized_bytes": len(runner.NATIVE_V22_USAGE),
        "sha256": _sha(runner.NATIVE_V22_USAGE),
    }
...
def test_bad_help_smoke_is_preintent_and_leaves_no_capsule(fixture: Fixture) -> None:
    ...
    with pytest.raises(runner.O1C86RunError, match="--help smoke seal differs"):
        fixture.execute()
...
def test_source_mutation_after_smoke_never_burns_or_rebuilds(fixture: Fixture) -> None:
    ...
    with pytest.raises(runner.O1C86RunError, match="changed before native call"):
        fixture.execute()
    capsules = list((fixture.root / "runs").glob(f"*_{runner.CAPSULE_SUFFIX}"))
    assert len(capsules) == 1
    assert not (capsules[0] / "episodes/00/intent.json").exists()
    with pytest.raises(runner.O1C86RunError, match="pre-intent capsule"):
        fixture.execute()
    assert fixture.build_calls == fixture.smoke_calls == 1
    assert fixture.adapter_calls == 0
...
def test_binary_rehash_immediately_before_adapter_is_terminal(
    ...
    assert episode["page11_burned"] is True
    assert episode["lineage24_burned"] is True
...
def test_novelty_uses_complete_published_attic_digest_set(fixture: Fixture) -> None:
    ...
    assert science["active_page11_new_clauses"] == 1
...
def test_production_config_has_no_predicted_binary_identity() -> None:
    ...
    assert "joint_score_sieve_v25" in source
    assert "prepare_o1c86_page11_causal_rollover" not in source
    for name, relative in runner.SOURCE_PATHS.items():
        assert config["source"]["expected_sha256"][name] == _sha(
            (Path(__file__).parents[1] / relative).read_bytes()
        )
    assert config["budgets"]["lineage_call_ordinals"] == [24]
