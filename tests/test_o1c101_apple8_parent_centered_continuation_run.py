...
import o1_crypto_lab.o1c101_apple8_parent_centered_continuation_run as runner
from o1_crypto_lab import joint_score_sieve_v32 as v32
...
FAKE_NATIVE = b"fake-native-v29-executable\n"
...
class Fixture:
    def __init__(
        ...
        self.native_override: v32.JointScoreSieveV32Result | None = None
        self.last_native_result: v32.JointScoreSieveV32Result | None = None
        ...
        self.failure_receipt = (root / runner.FAILURE_RECEIPT_RELATIVE).read_bytes()
        self.page18 = (self.published / runner.ACTIVE_PROJECTION_NAME).read_bytes()
        ...
        monkeypatch.setattr(
            runner._native_v32._v9._v8._v7._v1,
            "_potential",
            lambda payload: self.semantic_field,
        )
        monkeypatch.setattr(
            runner._native_v32._v9,
    ...
    def command(self, argv: list[str], **kwargs: object) -> SimpleNamespace:
        ...
        if "-o" in argv:
            ...
            assert staged_source == closure_root / runner.SOURCE_PATHS["native_v29"]
        ...
        return SimpleNamespace(
            returncode=0,
            stdout=b"bad usage\n" if self.bad_smoke else runner.NATIVE_V29_USAGE,
            stderr=b"",
        )
    ...
    def native_result(
        ...
        *,
        ...
        adapter_memory: dict[str, object] = {
            "memory_series_schema": (v32._v9.JOINT_SCORE_SIEVE_MEMORY_SERIES_SCHEMA),
            "memory_sample_limit": v32._v9.JOINT_SCORE_SIEVE_MAXIMUM_MEMORY_SAMPLES,
        ...
        raw: dict[str, object] = {
            "schema": v32.JOINT_SCORE_SIEVE_RESULT_SCHEMA,
            "seed": 0,
            ...
            "potential_sha256": _sha(self.inputs["potential"].read_bytes()),
            "active_vault_sha256": _sha(self.page18),
        ...
        return v32.JointScoreSieveV32Result(
            status=status,
            conflict_limit=128,
            threshold=runner.THRESHOLD,
    ...
    def adapter(self, **kwargs: object) -> object:
        ...
        assert intent["page18_burned"] is True
        assert intent["lineage31_burned"] is True
        ...
        assert kwargs["expected_source_sha256"] == self.source_sha["native_v29"]
        assert kwargs["source_path"] == (
            executable.parents[1]
            / runner.NATIVE_CLOSURE_DIRECTORY
            / runner.SOURCE_PATHS["native_v29"]
        )
        ...
        assert Path(cast(Path, kwargs["vault_path"])).read_bytes() == self.page18
        assert kwargs["sealed_page18_path"] == kwargs["vault_path"]
        assert Path(cast(Path, kwargs["priority_seed_path"])).read_bytes() == self.bank
        assert (
            Path(cast(Path, kwargs["terminal_failure_receipt_path"])).read_bytes()
    ...
    def semantic_replay(
        ...
        assert input_vault.sha256 == _sha(self.page18)
        return replayed
    ...
    def write_config(self) -> None:
        ...
        config = {
            ...
            "preparation": {
                ...
                "terminal_failure_receipt": runner.FAILURE_RECEIPT_RELATIVE.as_posix(),
                "terminal_failure_receipt_sha256": runner.PARENT_FAILURE_SHA256,
                "terminal_failure_receipt_bytes": runner.PARENT_FAILURE_BYTES,
            ...
            "native": {
                "source": runner.SOURCE_PATHS["native_v29"],
                "source_sha256": self.source_sha["native_v29"],
                ...
                "adapter_schema": v32.JOINT_SCORE_SIEVE_ADAPTER_SCHEMA,
                "result_schema": v32.JOINT_SCORE_SIEVE_RESULT_SCHEMA,
                "page18_sha256": runner.PAGE18_SHA256,
                "page18_bytes": runner.PAGE18_SERIALIZED_BYTES,
            ...
            "next_action": "Burn Page 18 / lineage 31 once; never retry or replay.",
        }
...
def test_preflight_rejects_capacity_proof_mutation_before_intent(
    ...
    target = cast(dict[str, Any], manifest["page18"])
    for field in path[:-1]:
        target = cast(dict[str, Any], target)[field]
    cast(dict[str, Any], target)[path[-1]] = replacement
    with pytest.raises(
        runner.O1C101RunError, match="Page-18 native capacity proof differs"
    ):
        runner._validate_native_capacity_proof(manifest["page18"])
    assert fixture.adapter_calls == fixture.build_calls == fixture.smoke_calls == 0
...
def test_preflight_rejects_published_mutation_without_build_or_call(
    ...
    with pytest.raises(runner.O1C101RunError, match="artifact seal differs"):
        runner.preflight(
            fixture.config_path,
            root=fixture.root,
            system_probe=fixture.system,
...
def test_one_build_smoke_and_observed_identity_precede_intent(
    ...
    assert build["smoke"]["stdout"] == {
        "serialized_bytes": len(runner.NATIVE_V29_USAGE),
        "sha256": _sha(runner.NATIVE_V29_USAGE),
    }
...
def test_staged_closure_tamper_is_rejected_before_intent(
    ...
    with pytest.raises(runner.O1C101RunError, match="staged native closure"):
        fixture.execute()
...
def test_bad_help_smoke_is_preintent_and_leaves_no_capsule(fixture: Fixture) -> None:
    ...
    with pytest.raises(runner.O1C101RunError, match="--help smoke seal differs"):
        fixture.execute()
...
def test_source_mutation_after_smoke_never_burns_or_rebuilds(fixture: Fixture) -> None:
    ...
    with pytest.raises(runner.O1C101RunError, match="changed before native call"):
        fixture.execute()
    capsules = list((fixture.root / "runs").glob(f"*_{runner.CAPSULE_SUFFIX}"))
    assert len(capsules) == 1
    assert not (capsules[0] / "episodes/00/intent.json").exists()
    with pytest.raises(runner.O1C101RunError, match="pre-intent capsule"):
        fixture.execute()
    assert fixture.build_calls == fixture.smoke_calls == 1
    assert fixture.adapter_calls == 0
...
def test_missing_binary_after_intent_is_recoverable_terminal(
    ...
    assert failure["exception_type"] == "O1C101RunError"
...
def test_self_consistent_forged_completed_capsule_is_never_republished(
    ...
    with pytest.raises(runner.O1C101RunError):
        fixture.execute()
...
def test_resealed_forged_sat_key_fails_independent_public_verification(
    ...
    with pytest.raises(runner.O1C101RunError, match="not publicly verified"):
        fixture.execute()
    assert not authoritative.exists()
    assert fixture.adapter_calls == fixture.build_calls == fixture.smoke_calls == 1


def test_resealed_self_consistent_forged_novelty_fails_v32_semantic_replay(
    fixture: Fixture,
) -> None:
    result = fixture.execute()
    capsule = fixture.root / cast(str, result["capsule"])
    ...
    episode["science"]["active_page18_new_clauses"] = 1
    ...
    with pytest.raises(runner.O1C101RunError, match="semantic replay"):
        fixture.execute()
...
def test_novelty_uses_complete_published_attic_digest_set(fixture: Fixture) -> None:
    ...
    assert science["active_page18_new_clauses"] == 1
...
def test_production_config_has_no_predicted_binary_identity() -> None:
    ...
    assert config["native"]["page18_sha256"] == runner.PAGE18_SHA256
    assert config["native"]["page18_bytes"] == runner.PAGE18_SERIALIZED_BYTES
    ...
    assert config["preparation"]["terminal_failure_receipt_sha256"] == (
        runner.PARENT_FAILURE_SHA256
    )
    assert "joint_score_sieve_v32" in source
    ...
    assert config["budgets"]["lineage_call_ordinals"] == [31]
    ...
    assert "Page 18" in config["next_action"]
