...
import o1_crypto_lab.joint_score_sieve_v35 as sieve
from o1_crypto_lab.o1_relational_search import O1RelationalSearchError


ROOT = Path(__file__).resolve().parents[1]
SOURCE = ROOT / sieve.NATIVE_SOURCE_RELATIVE
BUNDLE = ROOT / sieve.O1C106_BUNDLE_RELATIVE
MANIFEST = ROOT / sieve.O1C106_MANIFEST_RELATIVE
PAGE21 = ROOT / sieve.O1C106_PAGE21_RELATIVE
AUDIT = ROOT / sieve.O1C106_CERTIFICATION_AUDIT_RELATIVE
...
LIVE_BANK = ROOT / sieve.O1C106_LIVE_BANK_RELATIVE
...
def _seed_report() -> dict[str, object]:
    return {
        ...
        "source_preparation_manifest_sha256": sieve.O1C106_MANIFEST_SHA256,
        "source_preparation_manifest_bytes": sieve.O1C106_MANIFEST_BYTES,
...
def test_o1c107_production_bindings_and_native_source_are_exact() -> None:
    assert sieve.JOINT_SCORE_SIEVE_ADAPTER_SCHEMA.endswith("v35-adapter-v1")
    assert sieve.JOINT_SCORE_SIEVE_RESULT_SCHEMA.endswith("result-v32")
    assert "o1c107" in sieve.PRIORITY_STATE_SCHEMA
    assert "o1c107" in sieve.PRIORITY_ACTION_SCHEMA
    assert sieve.NATIVE_SOURCE_BYTES == SOURCE.stat().st_size
    assert sieve.NATIVE_SOURCE_SHA256 == _sha(SOURCE.read_bytes())
    assert sieve.PRODUCTION_PAGE21_LINEAGE_ORDINAL == 34
    assert sieve.PRODUCTION_PAGE21_BYTES == 2_762_499
    assert sieve.PRODUCTION_PAGE21_LITERAL_COUNT == 690_330
    assert sieve.PRODUCTION_PAGE21_HEADROOM == {
        "clauses": 265,
        "literals": 909_670,
    ...
    assert sieve.EMITTED_ONLY_ACTIVE_PROJECTION_SHA256 != (
        sieve.PRODUCTION_PAGE21_SHA256
    )
    assert sieve.GLOBAL_NOVELTY_BASELINE_CLAUSE_COUNT == 2_692
    assert sieve.O1C106_MANIFEST_BYTES == 9_376
    assert sieve.O1C106_CERTIFICATION_AUDIT_BYTES == 144_771
...
def test_page21_reader_rejects_burned_pages_before_acceptance(
    ...
    with pytest.raises(O1RelationalSearchError, match=pattern):
        sieve._read_page21("ignored")


def test_published_o1c106_bundle_manifest_and_audit_are_exact() -> None:
    assert BUNDLE.is_dir()
    assert {path.name for path in BUNDLE.iterdir()} == (
        sieve.O1C106_PUBLISHED_ARTIFACTS
    )
    assert MANIFEST.stat().st_size == sieve.O1C106_MANIFEST_BYTES
    assert _sha(MANIFEST.read_bytes()) == sieve.O1C106_MANIFEST_SHA256
    assert AUDIT.stat().st_size == sieve.O1C106_CERTIFICATION_AUDIT_BYTES
    assert _sha(AUDIT.read_bytes()) == sieve.O1C106_CERTIFICATION_AUDIT_SHA256
    assert PAGE21.stat().st_size == sieve.PRODUCTION_PAGE21_BYTES
    assert _sha(PAGE21.read_bytes()) == sieve.PRODUCTION_PAGE21_SHA256
    manifest = sieve._validate_o1c106_manifest(MANIFEST.read_bytes())
    assert manifest["attempt_id"] == "O1C-0106"


def test_real_production_page21_gate_certifies_247_without_monkeypatch() -> None:
    validated = sieve.validate_o1c107_page21_inputs(
        bundle_dir=BUNDLE,
        cnf_path=CNF,
        ...
        threshold=sieve.O1C106_THRESHOLD,
    )
    assert validated.input_vault.clause_count == 247
    assert validated.input_vault.literal_count == 690_330
    assert validated.input_vault.sha256 == sieve.PRODUCTION_PAGE21_SHA256
    assert validated.input_vault.clause_aggregate_sha256 == (
        sieve.PRODUCTION_PAGE21_CLAUSE_AGGREGATE_SHA256
    )
    page = cast(Mapping[str, object], validated.certification_audit["page21"])
...
@pytest.mark.parametrize(
    ...
    (
        (sieve._o1c106.PREPARATION_MANIFEST_NAME, "manifest seal"),
        (sieve._o1c106.CERTIFICATION_AUDIT_NAME, "artifact.*seal"),
    ),
)
def test_zero_launch_gate_rejects_manifest_and_audit_tamper(
    ...
    with pytest.raises(O1RelationalSearchError, match=pattern):
        sieve.validate_o1c107_page21_inputs(
...
def test_v35_run_uses_the_same_gate_before_its_only_native_launch() -> None:
    signature = inspect.signature(sieve.validate_o1c107_page21_inputs)
    ...
    helper_source = inspect.getsource(sieve.validate_o1c107_page21_inputs)
    ...
    assert run_source.count("validate_o1c107_page21_inputs(") == 1
    assert run_source.count("_execute_native(") == 1
    assert run_source.index("validate_o1c107_page21_inputs(") < run_source.index(
        "_execute_native("
    )
