...
PRIOR_BANK_SHA256 = "658fd2856b83d1a0ff8d28e92a604c99b3843a49a589811bf9b61845959ec31f"
MANIFEST_SHA256 = "467e519df281db4fc10de9223195dfedba9fd51edc93b40883f59fd3821e29ec"
...
FROZEN_V23_SHA256 = "bfe77177d19a3ab0a0e133c3ebc4965fbba34b23417daeddf6c92468aac5d5c9"
CADICAL_INCLUDE = Path("/opt/homebrew/opt/cadical/include")
CADICAL_LIBRARY = Path("/opt/homebrew/opt/cadical/lib/libcadical.a")


HARNESS = r"""
#define O1_CRYPTO_LAB_O1C90_NO_MAIN
#include "cadical_o1_joint_score_sieve_v24.cpp"
...
def frozen_inputs() -> tuple[Path, Path, Path, Path, Path]:
    ...
    assert MANIFEST.stat().st_size == 15_590
    ...
    return PRIOR_BANK, PAGE12, PAGE11, PAGE10, PAGE9
...
def native_builds(tmp_path_factory: pytest.TempPathFactory) -> tuple[Path, Path]:
    ...
    builds = (
        (
            [*common, "-DO1_CRYPTO_LAB_O1C90_PUBLIC_FIXTURE"],
            harness_source,
            harness,
        ),
        (common, V24, main),
...
def test_v24_normal_production_binary_compiles_and_help_surface(
    ...
    assert completed.stdout == (
        "usage: cadical_o1_joint_score_sieve_v24 --cnf PATH --potential PATH "
...
def test_live_bank_retains_failure_first_one_shot_and_seed_zero_semantics(
    ...
    assert actions["schema"] == "o1-256-o1c90-failure-first-proof-mining-actions-v1"
...
def test_native_production_gate_rejects_prior_live_bank_before_solver(
    native_builds: tuple[Path, Path],
    frozen_inputs: tuple[Path, Path, Path, Path, Path],
    tmp_path: Path,
) -> None:
    _, main = native_builds
    old_bank, _, _, _, page9 = frozen_inputs
    completed = _sealed_command(main, tmp_path, page9, old_bank)
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v24: sealed O1C90 priority seed differs\n"
    )
...
def test_native_requires_seed_zero_before_reading_solver_inputs(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v24: O1C90 priority operator requires seed zero\n"
    )
...
def test_native_production_gate_rejects_exact_page9_before_solver(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v24: burned O1C84 Page-9 active vault rejected\n"
    )
...
def test_native_production_gate_rejects_exact_page10_before_solver(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v24: burned O1C85 Page-10 active vault rejected\n"
    )
...
def test_native_production_gate_rejects_exact_page11_before_solver(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v24: burned O1C86 Page-11 active vault rejected\n"
    )
...
def test_native_production_gate_rejects_exact_page12_before_solver(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v24: burned O1C88 Page-12 active vault rejected\n"
    )


def test_native_production_gate_rejects_nonmatching_page13_payload(
    native_builds: tuple[Path, Path], tmp_path: Path
) -> None:
    _, main = native_builds
    forged_page13 = tmp_path / "forged-page-13-active.bin"
    forged_page13.write_bytes(bytes(2_846_623))
    completed = _sealed_command(main, tmp_path, forged_page13, LIVE_BANK)
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v24: sealed O1C90 Page-13 active vault differs\n"
    )


def test_source_contract_and_historical_native_freeze() -> None:
    source = V24.read_text(encoding="utf-8")
    manifest: dict[str, Any] = json.loads(MANIFEST.read_bytes())
    assert hashlib.sha256(V23.read_bytes()).hexdigest() == FROZEN_V23_SHA256
    assert '#include "cadical_o1_joint_score_sieve_v18.cpp"' in source
    assert '#include "o1c82_parent_centered_priority.hpp"' in source
    assert "O1_CRYPTO_LAB_O1C90_PUBLIC_FIXTURE" in source
    assert "O1_CRYPTO_LAB_O1C90_NO_MAIN" in source
    assert "o1-256-cadical-joint-score-sieve-result-v24" in source
    assert "o1-256-o1c90-live-parent-centered-continuation-priority-state-v1" in source
    assert "o1-256-o1c90-failure-first-proof-mining-actions-v1" in source
    ...
    assert manifest["page13"]["lineage_ordinal"] == 26
    assert manifest["page13"]["active_limit"] == 253
    assert manifest["page13"]["clause_count"] == 253
    assert manifest["page13"]["new_clause_residency"][
    ...
    assert "kProductionPage13ActiveVaultBytes = 2846623U" in source
    ...
    assert "kProductionLineageOrdinal = 26U" in source
    assert "kProductionActiveLimit = 253U" in source
    assert "kGlobalNoveltyBaselineClauseCount = 1291U" in source
    assert "kO1C89ImportedNovelClauseCount = 259U" in source
    ...
    page13_gate = source.index("vault_sha256 != kProductionPage13ActiveVaultSha256")
...
def test_v24_operator_body_is_byte_equivalent_to_frozen_v23() -> None:
    source = V24.read_text(encoding="utf-8")
    provenance_constants = """constexpr const char *kO1C89PreparationManifestSha256 =
...
"""
    provenance_assertions = """static_assert(std::string_view(kO1C89PreparationManifestSha256).size() == 64U);
...
"""
    ...
    replacements = (
        ...
        ("kProductionPage13", "kProductionPage12"),
        (PAGE13_SHA256, PAGE12_SHA256),
        ("2846623U", "2725423U"),
        ("Page-13", "Page-12"),
    )
    for current, prior in replacements:
        normalized = normalized.replace(current, prior)
    assert normalized == V23.read_text(encoding="utf-8")
