...
import o1_crypto_lab.o1c92_apple8_parent_centered_continuation_run as runner
from o1_crypto_lab import joint_score_sieve_v28 as v28
...
FAKE_NATIVE = b"fake-native-v25-executable\n"
...
class Fixture:
    def __init__(
        ...
        self.native_override: v28.JointScoreSieveV28Result | None = None
        ...
        self.page14 = (self.published / runner.ACTIVE_PROJECTION_NAME).read_bytes()
    ...
    def command(self, argv: list[str], **kwargs: object) -> SimpleNamespace:
        ...
        return SimpleNamespace(
            returncode=0,
            stdout=b"bad usage\n" if self.bad_smoke else runner.NATIVE_V25_USAGE,
            stderr=b"",
        )
    ...
    def native_result(
        ...
        *,
        ...
        raw: dict[str, object] = {
            "schema": v28.JOINT_SCORE_SIEVE_RESULT_SCHEMA,
            "seed": 0,
            "active_vault_sha256": _sha(self.page14),
        ...
        return v28.JointScoreSieveV28Result(
            status=status,
            conflict_limit=128,
    ...
    def adapter(self, **kwargs: object) -> object:
        ...
        assert intent["page14_burned"] is True
        assert intent["lineage27_burned"] is True
        assert "page13_burned" not in intent
        assert "lineage26_burned" not in intent
        ...
        assert kwargs["expected_source_sha256"] == self.source_sha["native_v25"]
        ...
        assert Path(cast(Path, kwargs["vault_path"])).read_bytes() == self.page14
        assert kwargs["sealed_page14_path"] == kwargs["vault_path"]
    ...
    def write_config(self) -> None:
        ...
        config = {
            ...
            "native": {
                "source": runner.SOURCE_PATHS["native_v25"],
                "source_sha256": self.source_sha["native_v25"],
                ...
                "adapter_schema": v28.JOINT_SCORE_SIEVE_ADAPTER_SCHEMA,
                "result_schema": v28.JOINT_SCORE_SIEVE_RESULT_SCHEMA,
                "page14_sha256": runner.PAGE14_SHA256,
                "page14_bytes": runner.PAGE14_SERIALIZED_BYTES,
            ...
            "next_action": "Burn Page 14 / lineage 27 once; never retry or replay.",
        }
...
@pytest.mark.parametrize(
    ("path", "replacement"),
    [
        (("clause_headroom_guarantee", "page14_input_clauses"), 251),
        (
            (
        ...
        (("clause_headroom_guarantee", "parent_centered_action_capacity"), 257),
        (
            ("clause_headroom_guarantee", "spare_clause_slots_beyond_action_capacity"),
            3,
        ),
        (("literal_future_emission_safety_claimed",), True),
        (("serialized_byte_future_emission_safety_claimed",), True),
    ],
)
def test_preflight_rejects_capacity_proof_mutation_before_intent(
    ...
    manifest_path = fixture.published / runner.PREPARATION_MANIFEST_NAME
    manifest = cast(dict[str, Any], json.loads(manifest_path.read_bytes()))
    target = cast(dict[str, Any], manifest["page14"])["native_capacity_proof"]
    for field in path[:-1]:
        target = cast(dict[str, Any], target)[field]
    cast(dict[str, Any], target)[path[-1]] = replacement
    payload = canonical_json_bytes(manifest)
    manifest_path.write_bytes(payload)
    monkeypatch.setattr(runner, "PUBLISHED_MANIFEST_BYTES", len(payload))
    monkeypatch.setattr(runner, "PUBLISHED_MANIFEST_SHA256", _sha(payload))
    fixture.write_config()

    with pytest.raises(
        runner.O1C92RunError, match="Page-14 native capacity proof differs"
    ):
        runner.preflight(
            fixture.config_path,
            root=fixture.root,
...
def test_preflight_rejects_published_mutation_without_build_or_call(
    ...
    with pytest.raises(runner.O1C92RunError, match="artifact seal differs"):
        runner.preflight(
            fixture.config_path,
            root=fixture.root,
            system_probe=fixture.system,
...
def test_one_build_smoke_and_observed_identity_precede_intent(
    ...
    assert build["smoke"]["stdout"] == {
        "serialized_bytes": len(runner.NATIVE_V25_USAGE),
        "sha256": _sha(runner.NATIVE_V25_USAGE),
    }
...
def test_bad_help_smoke_is_preintent_and_leaves_no_capsule(fixture: Fixture) -> None:
    ...
    with pytest.raises(runner.O1C92RunError, match="--help smoke seal differs"):
        fixture.execute()
...
def test_source_mutation_after_smoke_never_burns_or_rebuilds(fixture: Fixture) -> None:
    ...
    with pytest.raises(runner.O1C92RunError, match="changed before native call"):
        fixture.execute()
    capsules = list((fixture.root / "runs").glob(f"*_{runner.CAPSULE_SUFFIX}"))
    assert len(capsules) == 1
    assert not (capsules[0] / "episodes/00/intent.json").exists()
    with pytest.raises(runner.O1C92RunError, match="pre-intent capsule"):
        fixture.execute()
    assert fixture.build_calls == fixture.smoke_calls == 1
    assert fixture.adapter_calls == 0
...
def test_binary_rehash_immediately_before_adapter_is_terminal(
    ...
    assert episode["page14_burned"] is True
    assert episode["lineage27_burned"] is True
...
def test_novelty_uses_complete_published_attic_digest_set(fixture: Fixture) -> None:
    ...
    assert science["active_page14_new_clauses"] == 1
...
def test_production_config_has_no_predicted_binary_identity() -> None:
    ...
    assert "joint_score_sieve_v28" in source
    assert "prepare_o1c91_page14_causal_rollover" not in source
    for name, relative in runner.SOURCE_PATHS.items():
        assert config["source"]["expected_sha256"][name] == _sha(
            (Path(__file__).parents[1] / relative).read_bytes()
        )
    assert config["budgets"]["lineage_call_ordinals"] == [27]
