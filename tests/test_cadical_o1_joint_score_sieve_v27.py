...
import o1_crypto_lab.joint_score_sieve_v30 as adapter_v30

ROOT = Path(__file__).resolve().parents[1]
NATIVE = ROOT / "native"
V26 = NATIVE / "cadical_o1_joint_score_sieve_v26.cpp"
V27 = NATIVE / "cadical_o1_joint_score_sieve_v27.cpp"
...
MANIFEST_BYTES = 6_414
MANIFEST_SHA256 = "68d42b0f4cfaaf8a5b03f4b61515a8032860623dd5517fc87dac87b087a1c7b7"
...
HARNESS = r"""
#define O1_CRYPTO_LAB_O1C97_NO_MAIN
#include "cadical_o1_joint_score_sieve_v27.cpp"
...
def frozen_inputs() -> tuple[Path, ...]:
    exact = (
        ...
        (STATE_RECEIPT, 52_014, STATE_RECEIPT_SHA256),
...
def native_builds(tmp_path_factory: pytest.TempPathFactory) -> tuple[Path, Path]:
    ...
    for flags, source, executable in (
        (
            [*common, "-DO1_CRYPTO_LAB_O1C97_PUBLIC_FIXTURE"],
            harness_source,
            harness,
        ),
        (common, V27, main),
...
def test_v27_normal_production_binary_compiles_help_and_uuid(
    ...
    assert completed.stdout == (
        "usage: cadical_o1_joint_score_sieve_v27 --cnf PATH --potential PATH "
...
def test_live_evolved_bank_import_and_machine_readable_provenance(
    ...
    assert seed["source_priority_state_receipt_bytes"] == 52_014
...
def test_real_native_v27_priority_seed_crosses_exact_v30_adapter_seam(
    native_builds: tuple[Path, Path],
) -> None:
    harness, _ = native_builds
    completed = _run(harness, "import", LIVE_BANK)
    ...
    validated = adapter_v30._validate_seed_report(
        actual,
        seed_sha256=LIVE_BANK_SHA256,
        production_seal=False,
    )
    assert validated == actual
    assert set(actual) == adapter_v30._SEED_FIELDS
...
def test_native_rejects_prior_bank_before_solver(
    ...
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v27: sealed O1C97 priority seed differs\n"
    )
...
def test_native_requires_seed_zero_before_inputs(
    ...
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v27: O1C97 priority operator requires seed zero\n"
    )
...
def test_native_rejects_every_burned_page_before_solver(
    ...
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == f"cadical_o1_joint_score_sieve_v27: {message}\n"


def test_native_rejects_nonmatching_page16_before_solver(
    native_builds: tuple[Path, Path], tmp_path: Path
) -> None:
    _, main = native_builds
    forged = tmp_path / "forged-page-16-active.bin"
    forged.write_bytes(bytes(2_831_459))
    completed = _sealed_command(main, tmp_path, forged, LIVE_BANK)
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v27: sealed O1C97 Page-16 active vault differs\n"
    )


def test_source_contract_and_o1c96_binding() -> None:
    source = V27.read_text(encoding="utf-8")
    ...
    assert "O1_CRYPTO_LAB_O1C97_PUBLIC_FIXTURE" in source
    assert "O1_CRYPTO_LAB_O1C97_NO_MAIN" in source
    assert "o1-256-cadical-joint-score-sieve-result-v27" in source
    assert "o1-256-o1c97-live-parent-centered-continuation-priority-state-v1" in source
    assert "o1-256-o1c97-failure-first-proof-mining-actions-v1" in source
    ...
    assert manifest["schema"] == (
        "o1-256-o1c96-page16-transport-recovery-preparation-v1"
    )
    assert manifest["attic"]["union_clause_count"] == 1_812
    assert manifest["page16"]["lineage_ordinal"] == 29
    assert manifest["page16"]["clause_count"] == 251
    assert manifest["page16"]["literal_count"] == 707_566
    assert manifest["page16"]["serialized_bytes"] == 2_831_459
    ...
    assert "kProductionPage16ActiveVaultBytes = 2831459U" in source
    assert "kBurnedPage15ActiveVaultBytes = 2843047U" in source
    assert "kProductionLineageOrdinal = 29U" in source
    assert "kProductionActiveLiteralCount = 707566U" in source
    assert "kProductionClauseHeadroom = 261U" in source
    ...
    page16_gate = source.index("vault_sha256 != kProductionPage16ActiveVaultSha256")
    solver = source.index("CaDiCaL::Solver solver;")
    assert seed_gate < page15_gate < page14_gate < page9_gate < page16_gate < solver
...
def test_v27_operator_body_is_byte_equivalent_to_frozen_v26() -> None:
    source = V27.read_text(encoding="utf-8")
...
"""
    ...
    for current, prior in (
        ...
        ("V27", "V26"),
        ("v27", "v26"),
        ("kProductionPage16", "kProductionPage15"),
