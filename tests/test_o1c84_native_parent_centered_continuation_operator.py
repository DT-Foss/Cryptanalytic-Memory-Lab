...
FROZEN_V19_SHA256 = "823832d9f5aa6dfae85c3bdf55bb738187405323bdc418851de8654de0a64a28"
CADICAL_INCLUDE = Path("/opt/homebrew/opt/cadical/include")
CADICAL_LIBRARY = Path("/opt/homebrew/opt/cadical/lib/libcadical.a")


HARNESS = r"""
#define O1_CRYPTO_LAB_O1C84_NO_MAIN
#include "cadical_o1_joint_score_sieve_v20.cpp"
...
def frozen_inputs(tmp_path_factory: pytest.TempPathFactory) -> tuple[Path, Path]:
    ...
    old = tmp_path_factory.mktemp("o1c84-old-bank") / "fresh-priority.seed"
...
def native_builds(tmp_path_factory: pytest.TempPathFactory) -> tuple[Path, Path]:
    ...
    builds = (
        (
            [*common, "-DO1_CRYPTO_LAB_O1C84_PUBLIC_FIXTURE"],
            harness_source,
            harness,
        ),
        (common, V20, main),
...
def test_v20_compiles_complete_main_and_help_surface(
    ...
    assert completed.stdout == (
        "usage: cadical_o1_joint_score_sieve_v20 --cnf PATH --potential PATH "
...
def test_live_bank_retains_failure_first_one_shot_and_seed_zero_semantics(
    ...
    assert actions["schema"] == "o1-256-o1c84-failure-first-proof-mining-actions-v1"
...
def test_native_production_gate_rejects_old_fresh_bank_before_solver(
    ...
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v20: "
        "sealed O1C84 priority seed differs\n"
    )


def test_native_production_gate_rejects_every_non_page9_vault_before_solver(
    native_builds: tuple[Path, Path], tmp_path: Path
) -> None:
    _, main = native_builds
    wrong = tmp_path / "not-page-09-active.bin"
    wrong.write_bytes(PAGE9.read_bytes()[:-1])
    completed = _sealed_command(main, tmp_path, wrong, LIVE_BANK)
    assert completed.returncode == 1
    assert completed.stdout == ""
    assert completed.stderr == (
        "cadical_o1_joint_score_sieve_v20: "
        "sealed O1C84 Page-9 active vault differs\n"
    )


def test_source_contract_and_historical_native_freeze() -> None:
    source = V20.read_text(encoding="utf-8")
    assert hashlib.sha256(V19.read_bytes()).hexdigest() == FROZEN_V19_SHA256
    assert '#include "cadical_o1_joint_score_sieve_v18.cpp"' in source
    assert '#include "o1c82_parent_centered_priority.hpp"' in source
    assert "O1_CRYPTO_LAB_O1C84_PUBLIC_FIXTURE" in source
    assert "O1_CRYPTO_LAB_O1C84_NO_MAIN" in source
    assert "o1-256-cadical-joint-score-sieve-result-v20" in source
    assert LIVE_BANK_SHA256 in source
    assert PAGE9_SHA256 in source
    assert "kProductionPage9ActiveVaultBytes = 2885959U" in source
    ...
    page_gate = source.index("vault_sha256 != kProductionPage9ActiveVaultSha256")
