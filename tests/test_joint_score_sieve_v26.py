...
import o1_crypto_lab.joint_score_sieve_v26 as sieve
...
RECEIPT = ROOT / sieve.O1C86_PRIORITY_RECEIPT_RELATIVE
LIVE_BANK = ROOT / sieve.O1C87_LIVE_BANK_RELATIVE
MANIFEST = ROOT / sieve.O1C87_MANIFEST_RELATIVE
PAGE12 = ROOT / sieve.O1C87_PAGE12_RELATIVE
...
OLD_LIVE_BANK = (
    ROOT / "research/o1c86_page11_causal_rollover_seed_20260720/"
    "final-parent-centered-priority-bank.bin"
)
V25 = ROOT / "src/o1_crypto_lab/joint_score_sieve_v25.py"
FROZEN_V25_SHA256 = "d992c78994ef87e3d8f402255711b3768713c94c3349f9e2d33c5c988a8685e7"
...
def test_o1c87_known_production_bindings_are_exact() -> None:
    assert sieve.JOINT_SCORE_SIEVE_ADAPTER_SCHEMA.endswith("v26-adapter-v1")
    assert sieve.JOINT_SCORE_SIEVE_RESULT_SCHEMA.endswith("result-v23")
    assert "o1c87" in sieve.PRIORITY_STATE_SCHEMA
    assert "o1c87" in sieve.PRIORITY_ACTION_SCHEMA
    assert sieve.PRODUCTION_PAGE12_LINEAGE_ORDINAL == 25
    assert sieve.PRODUCTION_PAGE12_BYTES == 2_725_423
    assert (
        sieve.PRODUCTION_PAGE12_SHA256
...
def test_sealed_live_bank_exact_aggregate_and_old_bank_rejected() -> None:
    ...
    assert sum(record.count for record in records) == 182_368
    assert max(record.count for record in records) == 1_576
...
def test_output_counts_may_exceed_1576_but_must_replay_probe_delta() -> None:
    ...
    assert max(record.count for record in after) == 1_577
...
def test_o1c86_receipt_replays_and_links_exact_final_bank() -> None:
    receipt = RECEIPT.read_bytes()
    bank = LIVE_BANK.read_bytes()
    assert len(receipt) == sieve.O1C86_PRIORITY_RECEIPT_BYTES == 52_009
    assert _sha(receipt) == sieve.O1C86_PRIORITY_RECEIPT_SHA256
...
def test_published_o1c87_manifest_contract_is_exact() -> None:
    assert (
        sieve.O1C87_MANIFEST_SCHEMA
        == "o1-256-o1c87-page12-causal-rollover-preparation-v1"
    )
    assert sieve.O1C87_MANIFEST_BYTES == 6_910
    assert (
        sieve.O1C87_MANIFEST_SHA256
        == "64427e4861507e373cc02b52b9c0f2d25d62f26cf9362af681b9bc90ef4a57b6"
    )
    assert sieve.O1C87_MANIFEST_ARTIFACTS == frozenset(
        {
    ...
    assert sieve.O1C87_PUBLISHED_ARTIFACTS == sieve.O1C87_MANIFEST_ARTIFACTS | {
        "causal-rollover-preparation-manifest.json"
    }
    assert len(sieve.O1C87_PUBLISHED_ARTIFACTS) == 10
    assert sieve.O1C87_MANIFEST_RELATIVE == Path(
        "research/o1c87_page12_causal_rollover_seed_20260720/"
        "causal-rollover-preparation-manifest.json"
    )
    assert sieve.O1C87_PAGE12_RELATIVE == Path(
        "research/o1c87_page12_causal_rollover_seed_20260720/page-12-active.bin"
    )
    assert sieve.PRODUCTION_PAGE12_ACTIVE_LIMIT == 254
    assert sieve.PRODUCTION_PAGE12_CLAUSE_COUNT == 254
    assert sieve.PRODUCTION_PAGE12_LITERAL_COUNT == 681_054
    assert sieve.PRODUCTION_PAGE12_CATEGORY_COUNTS == {
        "structural_root": 5,
        "pinned_core": 43,
        "inherited_debt": 0,
        "new_debt": 202,
    ...
    assert sieve.PRODUCTION_PAGE12_HEADROOM == {
        "clauses": 258,
        "literals": 918_946,
        "serialized_bytes": 5_663_185,
    }
    payload = MANIFEST.read_bytes()
    assert len(payload) == sieve.O1C87_MANIFEST_BYTES
    assert _sha(payload) == sieve.O1C87_MANIFEST_SHA256
    assert sieve._manifest_contract() == (
        sieve.O1C87_MANIFEST_SCHEMA,
        sieve.O1C87_MANIFEST_BYTES,
        sieve.O1C87_MANIFEST_SHA256,
        sieve.O1C87_MANIFEST_ARTIFACTS,
    )
    sieve._validate_manifest(payload)

    manifest = json.loads(payload)
    assert manifest["attic"]["union_clause_count"] == 1_032
    for field in (
        "literal_future_emission_safety_claimed",
        "serialized_byte_future_emission_safety_claimed",
    ):
        forged = json.loads(payload)
        forged["page12"]["native_capacity_proof"][field] = True
        with pytest.raises(O1RelationalSearchError, match="manifest contract"):
            sieve._validate_manifest(canonical_json_bytes(forged))


def test_page12_reader_accepts_only_the_bound_identity(tmp_path: Path) -> None:
    payload = PAGE12.read_bytes()
    assert len(payload) == sieve.PRODUCTION_PAGE12_BYTES
    assert _sha(payload) == sieve.PRODUCTION_PAGE12_SHA256
    resolved, accepted = sieve._read_page12(PAGE12)
    assert resolved == PAGE12.resolve()
    assert accepted == payload

    forged = bytearray(payload)
    forged[len(forged) // 2] ^= 1
    path = tmp_path / "forged-page-12-active.bin"
    path.write_bytes(forged)
    with pytest.raises(O1RelationalSearchError, match="Page12 active projection"):
        sieve._read_page12(path)


def test_burned_page11_page10_and_page9_are_rejected() -> None:
...
def test_historical_v25_adapter_is_untouched() -> None:
    assert _sha(V25.read_bytes()) == FROZEN_V25_SHA256


def test_native_v23_source_contract() -> None:
    ...
    assert sieve.PRODUCTION_PAGE12_SHA256 in source
