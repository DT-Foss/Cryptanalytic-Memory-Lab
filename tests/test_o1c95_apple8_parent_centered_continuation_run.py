...
import o1_crypto_lab.o1c95_apple8_parent_centered_continuation_run as runner
from o1_crypto_lab import joint_score_sieve_v29 as v29
...
FAKE_NATIVE = b"fake-native-v26-executable\n"
...
class Fixture:
    def __init__(
        ...
        self.native_override: v29.JointScoreSieveV29Result | None = None
        ...
        self.page15 = (self.published / runner.ACTIVE_PROJECTION_NAME).read_bytes()
    ...
    def command(self, argv: list[str], **kwargs: object) -> SimpleNamespace:
        ...
        return SimpleNamespace(
            returncode=0,
            stdout=b"bad usage\n" if self.bad_smoke else runner.NATIVE_V26_USAGE,
            stderr=b"",
        )
    ...
    def native_result(
        ...
        *,
        ...
        raw: dict[str, object] = {
            "schema": v29.JOINT_SCORE_SIEVE_RESULT_SCHEMA,
            "seed": 0,
            "active_vault_sha256": _sha(self.page15),
        ...
        return v29.JointScoreSieveV29Result(
            status=status,
            conflict_limit=128,
    ...
    def adapter(self, **kwargs: object) -> object:
        ...
        assert intent["page15_burned"] is True
        assert intent["lineage28_burned"] is True
        ...
        assert kwargs["expected_source_sha256"] == self.source_sha["native_v26"]
        ...
        assert Path(cast(Path, kwargs["vault_path"])).read_bytes() == self.page15
        assert kwargs["sealed_page15_path"] == kwargs["vault_path"]
    ...
    def write_config(self) -> None:
        ...
        config = {
            ...
            "native": {
                "source": runner.SOURCE_PATHS["native_v26"],
                "source_sha256": self.source_sha["native_v26"],
                ...
                "adapter_schema": v29.JOINT_SCORE_SIEVE_ADAPTER_SCHEMA,
                "result_schema": v29.JOINT_SCORE_SIEVE_RESULT_SCHEMA,
                "page15_sha256": runner.PAGE15_SHA256,
                "page15_bytes": runner.PAGE15_SERIALIZED_BYTES,
            ...
            "next_action": "Burn Page 15 / lineage 28 once; never retry or replay.",
        }
...
@pytest.mark.parametrize(
    ("path", "replacement"),
    [
        (("clause_headroom_guarantee", "page15_input_clauses"), 252),
        (
            (
...
def test_preflight_rejects_capacity_proof_mutation_before_intent(
    ...
    target = cast(dict[str, Any], manifest["page15"])["native_capacity_proof"]
    for field in path[:-1]:
    ...
    with pytest.raises(
        runner.O1C95RunError, match="Page-15 native capacity proof differs"
    ):
        runner.preflight(
            fixture.config_path,
            root=fixture.root,
...
def test_preflight_rejects_published_mutation_without_build_or_call(
    ...
    with pytest.raises(runner.O1C95RunError, match="artifact seal differs"):
        runner.preflight(
            fixture.config_path,
            root=fixture.root,
            system_probe=fixture.system,
...
def test_one_build_smoke_and_observed_identity_precede_intent(
    ...
    assert build["smoke"]["stdout"] == {
        "serialized_bytes": len(runner.NATIVE_V26_USAGE),
        "sha256": _sha(runner.NATIVE_V26_USAGE),
    }
...
def test_bad_help_smoke_is_preintent_and_leaves_no_capsule(fixture: Fixture) -> None:
    ...
    with pytest.raises(runner.O1C95RunError, match="--help smoke seal differs"):
        fixture.execute()
...
def test_source_mutation_after_smoke_never_burns_or_rebuilds(fixture: Fixture) -> None:
    ...
    with pytest.raises(runner.O1C95RunError, match="changed before native call"):
        fixture.execute()
    capsules = list((fixture.root / "runs").glob(f"*_{runner.CAPSULE_SUFFIX}"))
    assert len(capsules) == 1
    assert not (capsules[0] / "episodes/00/intent.json").exists()
    with pytest.raises(runner.O1C95RunError, match="pre-intent capsule"):
        fixture.execute()
    assert fixture.build_calls == fixture.smoke_calls == 1
    assert fixture.adapter_calls == 0
...
def test_novelty_uses_complete_published_attic_digest_set(fixture: Fixture) -> None:
    ...
    assert science["active_page15_new_clauses"] == 1
...
def test_production_config_has_no_predicted_binary_identity() -> None:
    ...
    assert "joint_score_sieve_v29" in source
    assert "prepare_o1c93_page15_causal_rollover" not in source
    for name, relative in runner.SOURCE_PATHS.items():
        assert config["source"]["expected_sha256"][name] == _sha(
            (Path(__file__).parents[1] / relative).read_bytes()
        )
    assert config["budgets"]["lineage_call_ordinals"] == [28]
