"""Fail-closed O1C-0086 adapter for Page 11 and the sealed live bank.

Version 25 preserves v24's proof-mining and result-validation semantics while
moving the native boundary to result/source v22 and fresh lineage 24 Page 11.
...
JOINT_SCORE_SIEVE_ADAPTER_SCHEMA = "o1-256-joint-score-sieve-v25-adapter-v1"
JOINT_SCORE_SIEVE_RESULT_SCHEMA = "o1-256-cadical-joint-score-sieve-result-v22"
...
PRIORITY_ACTION_SCHEMA = "o1-256-o1c86-failure-first-proof-mining-actions-v1"
...
LIVE_BANK_SHA256 = "2c0c4ccba476bc642778b68234cc497c1776d144092ea9f1aead367559f59b07"
LIVE_BANK_TOTAL_COUNT = 82_330
LIVE_BANK_MAXIMUM_COUNT = 829
LIVE_BANK_ELIGIBLE_COORDINATES = 255

O1C86_MANIFEST_SCHEMA = "o1-256-o1c86-page11-causal-rollover-preparation-v1"
O1C86_MANIFEST_BYTES = 6_893
O1C86_MANIFEST_SHA256 = (
    "2e9f30aa193f1f640523e333bbb7990db6cdf32e3f54ebee19600ab307366116"
)
O1C86_MANIFEST_ARTIFACTS = frozenset(
    {
        "lineage-24-new-chunk.vault",
        "page-11-active.bin",
        "residency.json",
        ...
        "o1c85-priority-state-receipt.json",
    }
)
O1C86_MANIFEST_RELATIVE = Path(
    "research/o1c86_page11_causal_rollover_seed_20260720/"
    "causal-rollover-preparation-manifest.json"
)
O1C86_PAGE11_RELATIVE = Path(
    "research/o1c86_page11_causal_rollover_seed_20260720/page-11-active.bin"
)

O1C85_PRIORITY_RECEIPT_SCHEMA = (
    "o1-256-o1c85-live-parent-centered-continuation-priority-state-v1"
)
O1C85_PRIORITY_RECEIPT_BYTES = 51_273
O1C85_PRIORITY_RECEIPT_SHA256 = (
    "288d91298200ae69f84e6616c9a445c87b092ff56ac8671033ae3c3b4dd8b0a9"
)
PRODUCTION_PAGE11_BYTES = 2_876_731
PRODUCTION_PAGE11_SHA256 = (
    "9853f06bc882bfbb6312207bc8c20e0e9ca1500e49aad14594f6d7c66b62a04d"
)
PRODUCTION_PAGE11_LINEAGE_ORDINAL = 24
PRODUCTION_PAGE11_ACTIVE_LIMIT = 254
PRODUCTION_PAGE11_CLAUSE_COUNT = 254
PRODUCTION_PAGE11_LITERAL_COUNT = 718_881
PRODUCTION_PAGE11_CATEGORY_COUNTS: Mapping[str, int] = {
    "structural_root": 5,
    "pinned_core": 43,
    "inherited_debt": 0,
    "new_debt": 21,
...
PRODUCTION_PAGE11_HEADROOM: Mapping[str, int] = {
    "clauses": 258,
    "literals": 881_119,
...
NATIVE_SOURCE_RELATIVE = Path("native/cadical_o1_joint_score_sieve_v22.cpp")
O1C86_LIVE_BANK_RELATIVE = Path(
    "research/o1c86_page11_causal_rollover_seed_20260720/"
    "final-parent-centered-priority-bank.bin"
)
O1C85_PRIORITY_RECEIPT_RELATIVE = Path(
...
class JointScoreSieveV25Error(O1RelationalSearchError):
    """Native-v22 execution or continuation validation failed."""
...
@dataclass(frozen=True)
class JointScoreSieveV25Result:
    """Typed native-v22 result and its validated evolved bank."""
...
def _error(field: str) -> JointScoreSieveV25Error:
    return JointScoreSieveV25Error(f"joint-score-sieve-v25 {field} differs")
...
def _read_page11(path: str | Path) -> tuple[Path, bytes]:
    io_v1 = _v9._v8._v7._v1
    resolved, payload, digest = io_v1._read_input(path, "Page11")
    ...
    if len(payload) != PRODUCTION_PAGE11_BYTES or digest != PRODUCTION_PAGE11_SHA256:
        raise _error("Page11 active projection seal")
    return resolved, payload


def _manifest_contract() -> tuple[str, int, str, frozenset[str]]:
    """Return the sealed O1C-0086 preparation contract."""

    return (
        O1C86_MANIFEST_SCHEMA,
        _positive(O1C86_MANIFEST_BYTES, "O1C86 manifest byte count"),
        _sha(O1C86_MANIFEST_SHA256, "O1C86 manifest digest"),
        O1C86_MANIFEST_ARTIFACTS,
    )
...
def _validate_manifest(payload: bytes) -> None:
    ...
    try:
        ...
        raise _error("O1C86 manifest JSON") from exc
    if canonical != payload:
        raise _error("O1C86 canonical manifest encoding")
    ...
    page11 = _mapping(artifacts.get("page-11-active.bin"), "manifest Page11")
    receipt = _mapping(
        artifacts.get("o1c85-priority-state-receipt.json"),
        "manifest priority receipt",
    )
    final_bank = _mapping(root.get("final_priority_bank"), "manifest final bank")
    page11_report = _mapping(root.get("page11"), "manifest Page11 report")
    capacity = _mapping(
        page11_report.get("native_capacity_proof"), "manifest Page11 capacity"
    )
    clause_capacity = _mapping(
        capacity.get("clause_headroom_guarantee"),
        "manifest Page11 clause capacity",
    )
    residual_headroom = _mapping(
        capacity.get("recorded_residual_headroom"),
        "manifest Page11 residual headroom",
    )
    if (
        root.get("schema") != schema
        or root.get("attempt_id") != "O1C-0086"
        ...
        or parent.get("attempt_id") != "O1C-0085"
        ...
        or parent.get("source_lineage_ordinal") != 23
        or parent.get("source_active_sha256") != BURNED_PAGE10_SHA256
        ...
        or parent.get("global_novelty_baseline_clause_count") != 807
        or science.get("imported_science_attempt_id") != "O1C-0085"
        or science.get("imported_science_kind")
        != "fully-emitted-globally-novel-clauses"
        or science.get("imported_fully_emitted_clause_count") != 23
        or science.get("imported_globally_novel_clause_count") != 23
        or science.get("imported_literal_count") != 67_130
        ...
        or rollover.get("source_active_sha256") != BURNED_PAGE10_SHA256
        or rollover.get("clause_count") != 23
        or rollover.get("literal_count") != 67_130
        or rollover.get("all_occurrences_new") is not True
        or rollover.get("all_occurrences_unique") is not True
        or rollover.get("api")
        != "advance_causal_residency(next_lineage_ordinal=24,next_active_limit=254)"
        ...
        or page11.get("serialized_bytes") != PRODUCTION_PAGE11_BYTES
        or page11.get("sha256") != PRODUCTION_PAGE11_SHA256
        or page11.get("role") != "fresh-lineage-24-page11-science-input"
        or receipt.get("serialized_bytes") != O1C85_PRIORITY_RECEIPT_BYTES
        or receipt.get("sha256") != O1C85_PRIORITY_RECEIPT_SHA256
        or receipt.get("role") != "canonical-o1c85-evolved-priority-state-receipt"
        or final_bank.get("serialized_bytes") != BANK_BYTES
        or final_bank.get("sha256") != LIVE_BANK_SHA256
        or final_bank.get("receipt_serialized_bytes") != O1C85_PRIORITY_RECEIPT_BYTES
        or final_bank.get("receipt_sha256") != O1C85_PRIORITY_RECEIPT_SHA256
        ...
        or final_bank.get("minimum_nonzero_evolved_count") != 38
        ...
        or page11_report.get("active_sha256") != PRODUCTION_PAGE11_SHA256
        or page11_report.get("serialized_bytes") != PRODUCTION_PAGE11_BYTES
        or page11_report.get("lineage_ordinal") != PRODUCTION_PAGE11_LINEAGE_ORDINAL
        or page11_report.get("active_limit") != PRODUCTION_PAGE11_ACTIVE_LIMIT
        or page11_report.get("clause_count") != PRODUCTION_PAGE11_CLAUSE_COUNT
        or page11_report.get("literal_count") != PRODUCTION_PAGE11_LITERAL_COUNT
        or page11_report.get("category_counts") != PRODUCTION_PAGE11_CATEGORY_COUNTS
        or page11_report.get("headroom") != PRODUCTION_PAGE11_HEADROOM
        or page11_report.get("fresh_identity") is not True
        ...
        or residual_headroom != {"literals": 881_119, "serialized_bytes": 5_511_877}
        ...
        raise _error("O1C86 Page11 lineage-24 manifest contract")
...
def _validate_receipt(payload: bytes, bank: bytes) -> None:
    try:
        document = _mapping(
            ...
            "O1C85 priority receipt",
        )
        if document.get("schema") != O1C85_PRIORITY_RECEIPT_SCHEMA:
            raise _error("O1C85 priority receipt schema")
        ...
        raise _error("O1C85 priority receipt contract") from exc
    if receipt_bank != bank:
        raise _error("O1C85 priority receipt bank linkage")
...
def _parse_native_payload(
    ...
    *,
...
) -> JointScoreSieveV25Result:
    """Validate one native-v22 document and its unchanged v6 lifecycle."""
    ...
    return JointScoreSieveV25Result(
        status=base.status,
...
def _validate_prelaunch(
    *,
    ...
    manifest, manifest_bytes = _read_exact(
        manifest_path,
        label="O1C86 manifest",
        expected_bytes=manifest_expected_bytes,
        expected_sha256=manifest_expected_sha256,
    )
    receipt, receipt_bytes = _read_exact(
        receipt_path,
        label="O1C85 priority receipt",
        expected_bytes=O1C85_PRIORITY_RECEIPT_BYTES,
        expected_sha256=O1C85_PRIORITY_RECEIPT_SHA256,
    )
    ...
    page11, page11_bytes = _read_page11(page11_path)
...
def run_joint_score_sieve(
    *,
    ...
    sealed_page11_path: str | Path | None = None,
) -> JointScoreSieveV25Result:
    """Validate a prebuilt native-v22 executable, then launch it exactly once."""
    ...
    try:
        ...
        manifest = (
            ...
            else root / O1C86_MANIFEST_RELATIVE
        )
        receipt = (
            Path(priority_state_receipt_path)
            if priority_state_receipt_path is not None
            else root / O1C85_PRIORITY_RECEIPT_RELATIVE
        )
        page11 = (
            Path(sealed_page11_path)
            if sealed_page11_path is not None
            else root / O1C86_PAGE11_RELATIVE
        )
        ...
        if (
            len(vault_bytes) != PRODUCTION_PAGE11_BYTES
            or vault_sha != PRODUCTION_PAGE11_SHA256
            or vault_bytes != sealed.page11_bytes
        ):
            raise _error("Page11 production seal")
        ...
        if (
            input_vault.sha256 != PRODUCTION_PAGE11_SHA256
            or _candidate_order(field) != PRODUCTION_CANDIDATES
        ):
            raise _error("Page11 parsed production seal")
        ...
        for original, resolved, before, name in (
            ...
            (manifest, sealed.manifest_path, sealed.manifest_bytes, "O1C86 manifest"),
            ...
            (page11, sealed.page11_path, sealed.page11_bytes, "sealed Page11"),
        ...
        if completed.returncode:
            ...
            raise JointScoreSieveV25Error(
                "joint-score-sieve-v25 execution failed: "
        ...
        message = str(exc)
        if not message.startswith("joint-score-sieve-v25"):
            message = f"joint-score-sieve-v25 adapter failed: {message}"
