"""Burn-on-intent runner for the single O1C-0097 Page-16 call.

The runner has one scientific side effect: after an immutable intent is
persisted it may invoke native v27 once, with seed zero and 128 requested
...
the published, sealed O1C-0096 transport-recovery bundle; it never regenerates
...
from . import joint_score_sieve_v30 as _native_v30
...
from .o1c96_page16_transport_recovery_prepare import (
...
NATIVE_BUILD_SCHEMA = "o1-256-o1c97-native-v27-build-v1"
...
CONFIG_RELATIVE = Path("configs/o1c97_apple8_parent_centered_continuation_v1.json")
RESULT_RELATIVE = Path(
    "research/O1C0097_APPLE8_PARENT_CENTERED_CONTINUATION_RESULT_20260720.json"
)
CAPSULE_SUFFIX = "O1C-0097_apple8-parent-centered-continuation-v1"

LOCAL_EPISODE_ORDINAL = 0
LINEAGE_CALL_ORDINAL = 29
SEED = 0
...
NATIVE_V27_USAGE = (
    b"usage: cadical_o1_joint_score_sieve_v27 --cnf PATH "
...
PUBLISHED_ARTIFACT_SEALS: Mapping[str, tuple[int, str, str]] = {
    ...
    FAILURE_RECEIPT_NAME: (
        PARENT_FAILURE_BYTES,
        PARENT_FAILURE_SHA256,
        "canonical-o1c95-terminal-failure-receipt",
    ),
    RECEIPT_NAME: (
        PRIORITY_RECEIPT_BYTES,
        PRIORITY_RECEIPT_SHA256,
        "unchanged-canonical-o1c92-priority-state-receipt",
    ),
    OCCURRENCES_NAME: (
        646_472,
    ...
    ACTIVE_PROJECTION_NAME: (
        PAGE16_SERIALIZED_BYTES,
        PAGE16_SHA256,
        "fresh-lineage-29-page16-science-input",
    ),
    RESIDENCY_NAME: (
        PAGE16_RESIDENCY_DOCUMENT_BYTES,
        PAGE16_RESIDENCY_DOCUMENT_SHA256,
...
SOURCE_PATHS = {
    "runner": "src/o1_crypto_lab/o1c97_apple8_parent_centered_continuation_run.py",
    "runner_tests": "tests/test_o1c97_apple8_parent_centered_continuation_run.py",
    "adapter_v30": "src/o1_crypto_lab/joint_score_sieve_v30.py",
    "adapter_v30_tests": "tests/test_joint_score_sieve_v30.py",
    "page16_transport_recovery_preparation": "src/o1_crypto_lab/o1c96_page16_transport_recovery_prepare.py",
    "page16_transport_recovery_preparation_tests": "tests/test_o1c96_page16_transport_recovery_prepare.py",
    ...
    "native_v27": "native/cadical_o1_joint_score_sieve_v27.cpp",
    "native_v27_tests": "tests/test_cadical_o1_joint_score_sieve_v27.py",
...
class O1C97RunError(RuntimeError):
...
def _mapping(value: object, field: str) -> Mapping[str, object]:
    if not isinstance(value, Mapping) or not all(isinstance(k, str) for k in value):
        raise O1C97RunError(f"{field} differs")
...
def _relative_text(value: object, field: str) -> str:
    ...
    if path.is_absolute() or not path.parts or ".." in path.parts:
        raise O1C97RunError(f"{field} escapes the lab")
...
def _lab_path(
    ...
    try:
        ...
        raise O1C97RunError(f"{field} cannot be resolved") from exc
    expected = (
    ...
    if (
        ...
        raise O1C97RunError(f"{field} is not a canonical lab path")
    return resolved


def _external_path(value: object, field: str, *, directory: bool = False) -> Path:
    if not isinstance(value, str) or not Path(value).is_absolute():
        raise O1C97RunError(f"{field} is not absolute")
    ...
    if stat.S_ISLNK(metadata.st_mode) or not expected:
        raise O1C97RunError(f"{field} is not canonical")
    return resolved
...
def _read_canonical_json(path: Path, field: str) -> Mapping[str, object]:
    ...
    try:
        ...
        raise O1C97RunError(f"{field} is not JSON") from exc
    if canonical_json_bytes(value) != payload:
        raise O1C97RunError(f"{field} is not canonical JSON")
    return value


def _atomic_create(path: Path, payload: bytes, *, mode: int = 0o444) -> None:
    if not isinstance(payload, bytes):
        raise O1C97RunError("atomic payload differs")
    ...
    try:
        ...
        if path.read_bytes() != payload:
            raise O1C97RunError(f"atomic write verification failed for {path.name}")
        path.chmod(mode)
        ...
        raise O1C97RunError(f"artifact already exists: {path.name}") from exc
...
def load_config(path: str | Path, *, root: Path | None = None) -> dict[str, object]:
    ...
    if not config_path.is_relative_to(lab) or config_path.is_symlink():
        raise O1C97RunError("config escapes the lab")
    config = dict(_read_canonical_json(config_path, "O1C97 config"))
    if _pending(config):
        raise O1C97RunError("config contains PENDING: " + ", ".join(_pending(config)))
    if (
        set(config)
        != {
            "schema",
        ...
        raise O1C97RunError("config fields differ")
    ...
    if (
        ...
        raise O1C97RunError("frozen O1C92 parent differs")
    ...
    if (
        ...
        raise O1C97RunError("frozen preparation source differs")
    ...
    if set(inputs) != {
        ...
        raise O1C97RunError("input fields differ")
    ...
    if (
        ...
        or native.get("adapter_schema") != _native_v30.JOINT_SCORE_SIEVE_ADAPTER_SCHEMA
        or native.get("result_schema") != _native_v30.JOINT_SCORE_SIEVE_RESULT_SCHEMA
        or native.get("page16_sha256") != PAGE16_SHA256
        or native.get("page16_bytes") != PAGE16_SERIALIZED_BYTES
    ...
    if dict(paths) != SOURCE_PATHS or set(digests) != set(SOURCE_PATHS):
        raise O1C97RunError("source inventory differs")
    ...
    if dict(budgets) != expected_budgets:
        raise O1C97RunError("budgets differ")
    return config
...
def _sibling_solver_pids() -> tuple[int, ...]:
    try:
        ...
        raise O1C97RunError("solver process census failed") from exc
    current = os.getpid()
    found: list[int] = []
...
def _validate_priority_receipt(payload: bytes, bank: bytes) -> Mapping[str, object]:
    try:
        ...
        raise O1C97RunError("priority state receipt is not JSON") from exc
    if canonical_json_bytes(receipt) != payload:
        raise O1C97RunError("priority state receipt is not canonical JSON")
    hexadecimal = receipt.get("bank_hex")
    try:
        receipt_bank = bytes.fromhex(cast(str, hexadecimal))
    except (TypeError, ValueError) as exc:
        raise O1C97RunError("priority state receipt bank differs") from exc
    if (
        receipt.get("schema")
        ...
        raise O1C97RunError("priority state receipt contract differs")
    return receipt
...
def _validate_occurrence_ledger(payload: bytes) -> frozenset[str]:
    ...
    if (
        ...
        raise O1C97RunError("published occurrence ledger contract differs")
    ...
    for ordinal, value in enumerate(records):
        ...
        if (
            ...
            raise O1C97RunError("published occurrence record differs")
    ...
    if set(by_index) != set(range(ATTIC_UNION_CLAUSE_COUNT)) or len(digests) != (
        ...
        raise O1C97RunError("published attic union digest inventory differs")
    return digests
...
def _validate_published_documents(artifacts: Mapping[str, bytes]) -> frozenset[str]:
    ...
    projection = _mapping(
        residency.get("current_projection"), "published Page-16 projection"
    )
    encoding = _mapping(projection.get("encoding_only"), "published Page-16 vault")
    ...
    if (
        ...
        raise O1C97RunError("published Page-16 residency contract differs")
    if (
        ...
        raise O1C97RunError("published supporting artifact contract differs")
...
def _validate_native_capacity_proof(value: object) -> Mapping[str, object]:
    """Derive and verify the exact Page-16 native-capacity proof before intent."""

    page = _mapping(value, "published Page-16 capacity source")
    expected_caps = {
        "maximum_clauses": 512,
        "maximum_literals": 1_600_000,
        "maximum_serialized_bytes": 8_388_608,
    }
    headroom = _mapping(page.get("headroom"), "published Page-16 headroom")
    input_clauses = _nonnegative(page.get("clause_count"), "Page-16 input clauses")
    additional = _nonnegative(headroom.get("clauses"), "Page-16 clause headroom")
    if (
        ...
        raise O1C97RunError("published Page-16 native capacity proof differs")
    proof: dict[str, object] = {
        "caps": expected_caps,
        "clause_headroom_guarantee": {
            "native_vault_maximum_clauses": expected_caps["maximum_clauses"],
            "page16_input_clauses": input_clauses,
...
def _validate_published_manifest(
    ...
    if (
        ...
        or manifest.get("attempt_id") != "O1C-0096"
    ):
        raise O1C97RunError("published preparation manifest identity differs")
    ...
    page16 = _mapping(manifest.get("page16"), "published Page-16")
    _validate_native_capacity_proof(page16)
    ...
    if dict(zero_call) != {
        ...
        raise O1C97RunError("published zero-call authorization differs")
    if (
        ...
        raise O1C97RunError("published parent boundary differs")
    if (
        ...
        raise O1C97RunError("published rollover/attic boundary differs")
    if (
        page16.get("lineage_ordinal") != LINEAGE_CALL_ORDINAL
        or page16.get("active_limit") != PAGE16_ACTIVE_LIMIT
        or page16.get("active_sha256") != PAGE16_SHA256
        or page16.get("clause_count") != PAGE16_CLAUSE_COUNT
        or page16.get("literal_count") != PAGE16_LITERAL_COUNT
        or page16.get("serialized_bytes") != PAGE16_SERIALIZED_BYTES
        or page16.get("category_counts") != PAGE16_CATEGORY_COUNTS
        or page16.get("headroom") != PAGE16_HEADROOM
        or page16.get("fresh_identity") is not True
    ):
        raise O1C97RunError("published Page-16 seal differs")
    if (
        ...
        raise O1C97RunError("published continuation bank seal differs")
    rows_raw = _mapping(manifest.get("artifacts"), "published artifact rows")
    if set(rows_raw) != set(PUBLISHED_ARTIFACT_SEALS):
        raise O1C97RunError("published artifact row inventory differs")
    ...
    for name, (size, digest, role) in PUBLISHED_ARTIFACT_SEALS.items():
        ...
        if (
            ...
            raise O1C97RunError("published artifact seal differs")
        rows[name] = row
...
def _load_published_preparation(
    ...
    try:
        ...
        raise O1C97RunError("published preparation inventory is unreadable") from exc
    if (
        manifest_path != directory / PREPARATION_MANIFEST_NAME
        or {path.name for path in children} != PREPARATION_ARTIFACT_NAMES
    ):
        raise O1C97RunError("published preparation inventory differs")
    ...
    for path in children:
        try:
            ...
            raise O1C97RunError("published preparation artifact is unreadable") from exc
        if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
            raise O1C97RunError("published preparation artifact type differs")
    ...
    if (
        ...
        raise O1C97RunError("published preparation manifest seal differs")
...
def _preflight_bundle(
    ...
    *,
    ...
    if (
        ...
        raise O1C97RunError("parent input SHA preflight differs")
    ...
    if (
        ...
        raise O1C97RunError("published preparation/receipt preflight differs")
    ...
    for name in ("cnf", "potential", "grouping", "o1c73_config"):
        ...
        if observed != inputs[f"{name}_sha256"]:
            raise O1C97RunError(f"input {name} SHA preflight differs")
    ...
    for name in SOURCE_PATHS:
        ...
        if observed != expected[name]:
            raise O1C97RunError(f"source {name} SHA preflight differs")
    ...
    if (
        ...
        or source_sha["native_v27"] != native["source_sha256"]
    ):
        raise O1C97RunError("native toolchain/source SHA preflight differs")

    prepared = _load_published_preparation(published_directory, published_manifest)
    page16_manifest = _mapping(
        prepared.manifest.get("page16"), "published Page-16 preflight"
    )
    capacity_proof = _validate_native_capacity_proof(page16_manifest)
    prepared_rows = {
        name: {
    ...
    if (
        ...
        raise O1C97RunError("Darwin/arm64/RAM/disk/process preflight differs")
    row = {
...
def _executable_identity(path: Path, *, relative_path: str) -> dict[str, object]:
    try:
        ...
        raise O1C97RunError("native executable is unreadable") from exc
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
        raise O1C97RunError("native executable type differs")
    size = path.stat().st_size
    if size <= 0:
        raise O1C97RunError("native executable is empty")
    return {
        "path": relative_path,
...
def _compile_native(
    *,
    ...
    library = _external_path(native["cadical_library"], "CaDiCaL library")
    source = _lab_path(root, native["source"], "native v27 source")
    if (
        _sha_file(source) != native["source_sha256"]
        or _sha_file(library) != native["cadical_library_sha256"]
    ):
        raise O1C97RunError("native build input changed after preflight")
    ...
    try:
        ...
        raise O1C97RunError("native v27 compilation could not run") from exc
    ...
    try:
        ...
        raise O1C97RunError("native v27 --help smoke could not run") from exc
    ...
    if (
        ...
        or smoke_stdout != NATIVE_V27_USAGE
        or smoke_stderr
        or after_smoke != before_smoke
    ):
        raise O1C97RunError("native v27 --help smoke seal differs")
...
def _stage_capsule(
    *,
    ...
    if final.exists() or stage.exists():
        raise O1C97RunError("timestamped capsule already exists")
...
def _rehash_call_inputs(
    *,
    ...
    if _sha_file(config_path) != preflight_row.get("config_sha256"):
        raise O1C97RunError("config changed before native call")
    ...
    for name in ("cnf", "potential", "grouping", "o1c73_config"):
        if (
            ...
            raise O1C97RunError(f"input {name} changed before native call")
    ...
    if prepared_rows.get(RECEIPT_NAME) != receipt_row:
        raise O1C97RunError("staged priority receipt seal differs")
    ...
    for name, value in expected_rows.items():
        ...
        if path.stat().st_size != row.get("serialized_bytes") or _sha_file(
            ...
            raise O1C97RunError("staged call artifact seal differs")
    ...
    if invocation_path.read_bytes() != canonical_json_bytes(invocation) or _sha_file(
        ...
        raise O1C97RunError("staged invocation/build seal differs")
    ...
    if (
        ...
        or smoke.get("stdout")
        != {
            "serialized_bytes": len(NATIVE_V27_USAGE),
            "sha256": sha256_bytes(NATIVE_V27_USAGE),
        }
        ...
        raise O1C97RunError("staged native build/smoke seal differs")
...
def _native_stdout(result: object) -> bytes:
    ...
    if isinstance(value, str):
        ...
        raise O1C97RunError("adapter omitted byte-exact native stdout")
    if _sha(
        getattr(result, "native_stdout_sha256", None), "native stdout digest"
    ) != sha256_bytes(payload):
        raise O1C97RunError("native stdout digest differs")
    return payload
...
def _validate_result_conservation(
    ...
    if set(counters) != {*outcome_names, "child_bound_evaluations"}:
        raise O1C97RunError("native post-result conservation differs")
    outcomes = tuple(
...
def _validate_native_result(result: object, stdout: bytes) -> NativeEvidence:
    if not isinstance(result, _native_v30.JointScoreSieveV30Result):
        raise O1C97RunError("adapter v30 result type differs")
    raw = _mapping(result.raw, "native result")
    try:
        decoded = _mapping(json.loads(stdout), "native stdout JSON")
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise O1C97RunError("native stdout JSON differs") from exc
    if (
        decoded != raw
        or raw.get("schema") != _native_v30.JOINT_SCORE_SIEVE_RESULT_SCHEMA
    ):
        raise O1C97RunError("native stdout/result projection differs")
    ...
    if (
        ...
        raise O1C97RunError("native v27 production binding differs")
    if (
        raw.get("priority_state") != priority_state
        or raw.get("priority_actions") != actions
    ):
        raise O1C97RunError("native promoted priority projection differs")
    if raw.get("decision_ownership") != ownership:
        raise O1C97RunError("native promoted ownership projection differs")
    if result.status == 10 and not isinstance(result.key_model, bytes):
        raise O1C97RunError("SAT result omitted key model")
    if result.status != 10 and result.key_model is not None:
        raise O1C97RunError("non-SAT result returned key model")
...
def _science_and_operation(
    ...
    *,
    ...
    if action_count != len(actions):
        raise O1C97RunError("priority action count differs")
    ...
    for row in emitted_rows:
        ...
        try:
            ...
            raise O1C97RunError("emitted clause serialization differs") from exc
    ...
    if novel > active_new:
        raise O1C97RunError("global novelty exceeds active-vault novelty")
    ...
    if evidence.status == 10 and not public_model_verified:
        raise O1C97RunError("SAT key failed exact public ChaCha verification")
    if evidence.status != 10 and public_model_verified:
        raise O1C97RunError("public model verification without SAT differs")
    model_gain = evidence.status == 10 and public_model_verified
    closure_gain = evidence.status == 20 or terminal_empty
    # Native v27 emits no entropy or domain estimate.  These fields stay
    ...
    science = {
        ...
        "active_page16_new_clauses": active_new,
    ...
    *,
    ...
    try:
        ...
        result = adapter_run(
            ...
            sealed_page16_path=capsule / "initial" / ACTIVE_PROJECTION_NAME,
        )
        ...
        # Adapter v30 attaches the completed process record even when semantic
...
def _manifest_bytes(capsule: Path) -> tuple[bytes, int]:
    ...
    for path in sorted(capsule.rglob("*")):
        ...
        if stat.S_ISLNK(metadata.st_mode):
            raise O1C97RunError("capsule contains a symlink")
        if stat.S_ISDIR(metadata.st_mode):
            continue
        if not stat.S_ISREG(metadata.st_mode):
            raise O1C97RunError("capsule contains a special file")
...
def _run_markdown(result: Mapping[str, object]) -> bytes:
    ...
    lines = [
        "# O1C-0097 Page-16 parent-centered run",
        "",
        ...
        "- Page 16 and lineage 29 burned when `episodes/00/intent.json` was persisted.",
...
def _publish_exact(path: Path, payload: bytes) -> None:
    if path.exists():
        if path.is_symlink() or not path.is_file() or path.read_bytes() != payload:
            raise O1C97RunError("authoritative result already differs")
        return
...
def _finalize_capsule(
    *, capsule: Path, authoritative: Path, result: Mapping[str, object]
    ...
    if persistent_bytes > MAXIMUM_PERSISTENT_ARTIFACT_BYTES:
        raise O1C97RunError("persistent artifact budget exceeded")
...
def _existing_result(root: Path) -> dict[str, object] | None:
    ...
    if not authoritative.exists():
        return None
    value = dict(_read_canonical_json(authoritative, "authoritative O1C97 result"))
    if value.get("schema") != RESULT_SCHEMA or value.get("attempt_id") != ATTEMPT_ID:
        raise O1C97RunError("authoritative O1C97 result differs")
    return value
...
def _validated_capsule_result(
    ...
    if not all(path.is_file() and not path.is_symlink() for path in required):
        if (capsule / "episodes/00/intent.json").is_file():
            raise O1C97RunError("burned incomplete capsule forbids retry or replay")
        raise O1C97RunError("incomplete pre-intent capsule blocks a fresh call")
    manifest, _ = _manifest_bytes(capsule)
    if manifest != (capsule / "artifacts.sha256").read_bytes():
        raise O1C97RunError("existing capsule manifest differs")
    result_path = capsule / "result.json"
    ...
    if (
        ...
        raise O1C97RunError("existing capsule result differs")
...
def _public_verifier(
    *, root: Path, config: Mapping[str, object]
    ...
    if _sha_file(config_path) != inputs["o1c73_config_sha256"]:
        raise O1C97RunError("public verifier config SHA differs")
    try:
        ...
        raise O1C97RunError("public ChaCha verifier cannot be established") from exc
...
def run(
    ...
    *,
    root: Path | None = None,
    adapter_run: AdapterRun = _native_v30.run_joint_score_sieve,
    ...
    if existing is not None:
        if len(capsules) != 1:
            raise O1C97RunError("authoritative O1C97 result lacks one sealed capsule")
        ...
        if capsule_payload != authoritative_payload or capsule_result != existing:
            raise O1C97RunError("authoritative O1C97 result differs from capsule")
        return existing
    if capsules:
        if len(capsules) != 1:
            raise O1C97RunError("multiple O1C97 capsules forbid replay")
    ...
    result = {
        ...
        "next_action": "Never retry or replay Page 16 / lineage 29 or Pages 15 through 9; preserve the final priority bank only as a continuation seed.",
    }
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Preflight or execute O1C97's one Page-16 parent-centered call"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 0
    except O1C97RunError as exc:
        print(f"O1C97: {exc}", file=sys.stderr)
        return 2
