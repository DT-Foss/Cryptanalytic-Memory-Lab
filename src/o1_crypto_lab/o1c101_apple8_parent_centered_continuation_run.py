"""Burn-on-intent runner for the single O1C-0101 Page-18 call.

The runner has one scientific side effect: after an immutable intent is
persisted it may invoke native v29 once, with seed zero and 128 requested
...
bit belief or science gain by themselves.  Its preparation gate reads only
    the published, sealed O1C-0100 telemetry-recovery bundle; it never regenerates
...
from . import joint_score_sieve_v32 as _native_v32
...
from .o1c100_page18_telemetry_recovery_prepare import (
...
NATIVE_BUILD_SCHEMA = "o1-256-o1c101-native-v29-build-v2"
...
CONFIG_RELATIVE = Path("configs/o1c101_apple8_parent_centered_continuation_v1.json")
RESULT_RELATIVE = Path(
    "research/O1C0101_APPLE8_PARENT_CENTERED_CONTINUATION_RESULT_20260721.json"
)
CAPSULE_SUFFIX = "O1C-0101_apple8-parent-centered-continuation-v1"

LOCAL_EPISODE_ORDINAL = 0
LINEAGE_CALL_ORDINAL = 31
SEED = 0
...
PUBLISHED_PREPARATION_RELATIVE = Path(
    "research/o1c100_page18_telemetry_recovery_seed_20260721"
)
...
FAILURE_RECEIPT_RELATIVE = PUBLISHED_PREPARATION_RELATIVE / FAILURE_RECEIPT_NAME
...
INPUT_BANK_TOTAL_COUNT = 316_312
PARENT_CENTERED_ACTION_CAPACITY = 256

NATIVE_V29_USAGE = (
    b"usage: cadical_o1_joint_score_sieve_v29 --cnf PATH "
...
PUBLISHED_ARTIFACT_SEALS: Mapping[str, tuple[int, str, str]] = {
    ...
    RECEIPT_NAME: (
        PRIORITY_RECEIPT_BYTES,
        PRIORITY_RECEIPT_SHA256,
        "unchanged-canonical-o1c97-priority-state-receipt",
    ),
    OCCURRENCES_NAME: (
    ...
    FAILURE_RECEIPT_NAME: (
        PARENT_FAILURE_BYTES,
        PARENT_FAILURE_SHA256,
        "canonical-o1c99-post-call-terminal-failure-receipt",
    ),
    ACTIVE_PROJECTION_NAME: (
        PAGE18_SERIALIZED_BYTES,
        PAGE18_SHA256,
        "fresh-lineage-31-page18-science-input",
    ),
    RESIDENCY_NAME: (
        PAGE18_RESIDENCY_DOCUMENT_BYTES,
        PAGE18_RESIDENCY_DOCUMENT_SHA256,
...
SOURCE_PATHS = {
    "runner": "src/o1_crypto_lab/o1c101_apple8_parent_centered_continuation_run.py",
    "runner_tests": "tests/test_o1c101_apple8_parent_centered_continuation_run.py",
    "adapter_v32": "src/o1_crypto_lab/joint_score_sieve_v32.py",
    "adapter_v32_tests": "tests/test_joint_score_sieve_v32.py",
    "page18_telemetry_recovery_preparation": "src/o1_crypto_lab/o1c100_page18_telemetry_recovery_prepare.py",
    "page18_telemetry_recovery_preparation_tests": "tests/test_o1c100_page18_telemetry_recovery_prepare.py",
    ...
    "native_v29": "native/cadical_o1_joint_score_sieve_v29.cpp",
    "native_v29_tests": "tests/test_cadical_o1_joint_score_sieve_v29.py",
...
class O1C101RunError(RuntimeError):
...
def _mapping(value: object, field: str) -> Mapping[str, object]:
    if not isinstance(value, Mapping) or not all(isinstance(k, str) for k in value):
        raise O1C101RunError(f"{field} differs")
...
def _relative_text(value: object, field: str) -> str:
    ...
    if path.is_absolute() or not path.parts or ".." in path.parts:
        raise O1C101RunError(f"{field} escapes the lab")
...
def _lab_path(
    ...
    try:
        ...
        raise O1C101RunError(f"{field} cannot be resolved") from exc
    expected = (
    ...
    if (
        ...
        raise O1C101RunError(f"{field} is not a canonical lab path")
    return resolved


def _external_path(value: object, field: str, *, directory: bool = False) -> Path:
    if not isinstance(value, str) or not Path(value).is_absolute():
        raise O1C101RunError(f"{field} is not absolute")
    ...
    if stat.S_ISLNK(metadata.st_mode) or not expected:
        raise O1C101RunError(f"{field} is not canonical")
    return resolved
...
def _read_canonical_json(path: Path, field: str) -> Mapping[str, object]:
    ...
    try:
        ...
        raise O1C101RunError(f"{field} is not JSON") from exc
    if canonical_json_bytes(value) != payload:
        raise O1C101RunError(f"{field} is not canonical JSON")
    return value


def _atomic_create(path: Path, payload: bytes, *, mode: int = 0o444) -> None:
    if not isinstance(payload, bytes):
        raise O1C101RunError("atomic payload differs")
    ...
    try:
        ...
        if path.read_bytes() != payload:
            raise O1C101RunError(f"atomic write verification failed for {path.name}")
        path.chmod(mode)
        ...
        raise O1C101RunError(f"artifact already exists: {path.name}") from exc
...
def load_config(path: str | Path, *, root: Path | None = None) -> dict[str, object]:
    ...
    if not config_path.is_relative_to(lab) or config_path.is_symlink():
        raise O1C101RunError("config escapes the lab")
    config = dict(_read_canonical_json(config_path, "O1C101 config"))
    if _pending(config):
        raise O1C101RunError("config contains PENDING: " + ", ".join(_pending(config)))
    if (
        set(config)
        != {
            "schema",
        ...
        raise O1C101RunError("config fields differ")
    ...
    if (
        ...
        raise O1C101RunError("frozen O1C99 parent differs")
    ...
    if (
        ...
        != {
            ...
            "terminal_failure_receipt_bytes",
        ...
        or _relative_text(
            preparation.get("terminal_failure_receipt"),
            "terminal failure receipt",
        )
        != FAILURE_RECEIPT_RELATIVE.as_posix()
        or preparation.get("terminal_failure_receipt_sha256") != PARENT_FAILURE_SHA256
        or preparation.get("terminal_failure_receipt_bytes") != PARENT_FAILURE_BYTES
        or preparation.get("candidate_order_sha256")
        != CONTINUATION_CANDIDATE_ORDER_SHA256
    ):
        raise O1C101RunError("frozen preparation source differs")
    ...
    if set(inputs) != {
        ...
        raise O1C101RunError("input fields differ")
    ...
    if (
        ...
        or native.get("adapter_schema") != _native_v32.JOINT_SCORE_SIEVE_ADAPTER_SCHEMA
        or native.get("result_schema") != _native_v32.JOINT_SCORE_SIEVE_RESULT_SCHEMA
        or native.get("page18_sha256") != PAGE18_SHA256
        or native.get("page18_bytes") != PAGE18_SERIALIZED_BYTES
    ...
    if dict(paths) != SOURCE_PATHS or set(digests) != set(SOURCE_PATHS):
        raise O1C101RunError("source inventory differs")
    ...
    if dict(budgets) != expected_budgets:
        raise O1C101RunError("budgets differ")
    return config
...
def _sibling_solver_pids() -> tuple[int, ...]:
    try:
        ...
        raise O1C101RunError("solver process census failed") from exc
    current = os.getpid()
    found: list[int] = []
...
def _validate_priority_receipt(payload: bytes, bank: bytes) -> Mapping[str, object]:
    try:
        ...
        raise O1C101RunError("priority state receipt is not JSON") from exc
    if canonical_json_bytes(receipt) != payload:
        raise O1C101RunError("priority state receipt is not canonical JSON")
    hexadecimal = receipt.get("bank_hex")
    try:
        receipt_bank = bytes.fromhex(cast(str, hexadecimal))
    except (TypeError, ValueError) as exc:
        raise O1C101RunError("priority state receipt bank differs") from exc
    if (
        receipt.get("schema")
        ...
        raise O1C101RunError("priority state receipt contract differs")
    return receipt
...
def _validate_occurrence_ledger(payload: bytes) -> frozenset[str]:
    ...
    if (
        ...
        raise O1C101RunError("published occurrence ledger contract differs")
    ...
    for ordinal, value in enumerate(records):
        ...
        if (
            ...
            raise O1C101RunError("published occurrence record differs")
    ...
    if set(by_index) != set(range(ATTIC_UNION_CLAUSE_COUNT)) or len(digests) != (
        ...
        raise O1C101RunError("published attic union digest inventory differs")
    return digests
...
def _validate_published_documents(artifacts: Mapping[str, bytes]) -> frozenset[str]:
    ...
    projection = _mapping(
        residency.get("current_projection"), "published Page-18 projection"
    )
    encoding = _mapping(projection.get("encoding_only"), "published Page-18 vault")
    ...
    if (
        ...
        or len(activation_entries) != PAGE18_ACTIVATION_COUNT
        or len(used) != PAGE18_ACTIVATION_COUNT
        or used[-1] != PAGE18_SHA256
        or last_activation.get("lineage_ordinal") != LINEAGE_CALL_ORDINAL
        or last_activation.get("active_sha256") != PAGE18_SHA256
        or last_activation.get("selected_clause_count") != PAGE18_CLAUSE_COUNT
        or residency.get("schema") != "o1-score-threshold-causal-residency-v1"
        or residency.get("active_limit") != PAGE18_ACTIVE_LIMIT
        or projection.get("lineage_ordinal") != LINEAGE_CALL_ORDINAL
        or projection.get("category_counts") != PAGE18_CATEGORY_COUNTS
        or encoding.get("sha256") != PAGE18_SHA256
        or encoding.get("clause_count") != PAGE18_CLAUSE_COUNT
        or encoding.get("literal_count") != PAGE18_LITERAL_COUNT
        or encoding.get("serialized_bytes") != PAGE18_SERIALIZED_BYTES
        or attic.get("chunk_count") != ATTIC_CHUNK_COUNT
        or residency_union.get("clause_count") != ATTIC_UNION_CLAUSE_COUNT
    ):
        raise O1C101RunError("published Page-18 residency contract differs")
    if (
        ...
        raise O1C101RunError("published supporting artifact contract differs")
...
def _validate_native_capacity_proof(value: object) -> Mapping[str, object]:
    """Derive and verify the exact Page-18 native-capacity proof before intent."""

    page = _mapping(value, "published Page-18 capacity source")
    expected_caps = {
        "maximum_clauses": 512,
        "maximum_literals": 1_600_000,
        "maximum_serialized_bytes": 8_388_608,
    }
    headroom = _mapping(page.get("headroom"), "published Page-18 headroom")
    input_clauses = _nonnegative(page.get("clause_count"), "Page-18 input clauses")
    additional = _nonnegative(headroom.get("clauses"), "Page-18 clause headroom")
    if (
        O1C66_VAULT_CAPS.describe() != expected_caps
        or input_clauses != PAGE18_CLAUSE_COUNT
        or additional != PAGE18_HEADROOM["clauses"]
        or dict(headroom) != PAGE18_HEADROOM
        or input_clauses + additional != expected_caps["maximum_clauses"]
        or additional < PARENT_CENTERED_ACTION_CAPACITY
    ):
        raise O1C101RunError("published Page-18 native capacity proof differs")
    proof: dict[str, object] = {
        "caps": expected_caps,
        "clause_headroom_guarantee": {
            "native_vault_maximum_clauses": expected_caps["maximum_clauses"],
            "page18_input_clauses": input_clauses,
        ...
        "recorded_residual_headroom": {
            "literals": PAGE18_HEADROOM["literals"],
            "serialized_bytes": PAGE18_HEADROOM["serialized_bytes"],
        },
...
def _disabled_o1c96_published_manifest_validator(
    ...
    if (
        ...
        raise O1C101RunError("published preparation manifest identity differs")
    ...
    page18 = _mapping(manifest.get("page18"), "published Page-16")
    _validate_native_capacity_proof(page18)
    ...
    if dict(zero_call) != {
        ...
        raise O1C101RunError("published zero-call authorization differs")
    if (
        ...
        or parent.get("terminal_failure_sha256") != PARENT_FAILURE_SHA256
        ...
        raise O1C101RunError("published parent boundary differs")
    if (
        ...
        raise O1C101RunError("published rollover/attic boundary differs")
    if (
        page18.get("lineage_ordinal") != LINEAGE_CALL_ORDINAL
        or page18.get("active_limit") != PAGE18_ACTIVE_LIMIT
        or page18.get("active_sha256") != PAGE18_SHA256
        or page18.get("clause_count") != PAGE18_CLAUSE_COUNT
        or page18.get("literal_count") != PAGE18_LITERAL_COUNT
        or page18.get("serialized_bytes") != PAGE18_SERIALIZED_BYTES
        or page18.get("category_counts") != PAGE18_CATEGORY_COUNTS
        or page18.get("headroom") != PAGE18_HEADROOM
        or page18.get("fresh_identity") is not True
    ):
        raise O1C101RunError("published Page-16 seal differs")
    if (
        ...
        raise O1C101RunError("published continuation bank seal differs")
    rows_raw = _mapping(manifest.get("artifacts"), "published artifact rows")
    if set(rows_raw) != set(PUBLISHED_ARTIFACT_SEALS):
        raise O1C101RunError("published artifact row inventory differs")
    ...
    for name, (size, digest, role) in PUBLISHED_ARTIFACT_SEALS.items():
        ...
        if (
            ...
            raise O1C101RunError("published artifact seal differs")
        rows[name] = row
    ...
    raise O1C101RunError("historical manifest validator is disabled")


def _validate_published_manifest(
    manifest: Mapping[str, object], artifacts: Mapping[str, bytes]
) -> tuple[dict[str, Mapping[str, object]], frozenset[str]]:
    """Validate the exact O1C100 recovery bundle, then stage fresh Page 18."""

    payload = canonical_json_bytes(manifest)
    _native_v32._validate_manifest(payload)
    page18 = _mapping(manifest.get("page18"), "published Page-18")
    capacity = _validate_native_capacity_proof(page18)
    if (
        manifest.get("schema") != PREPARATION_SCHEMA
        or manifest.get("attempt_id") != "O1C-0100"
        or page18.get("lineage_ordinal") != LINEAGE_CALL_ORDINAL
        or page18.get("active_limit") != PAGE18_ACTIVE_LIMIT
        or page18.get("active_sha256") != PAGE18_SHA256
        or page18.get("clause_count") != PAGE18_CLAUSE_COUNT
        or page18.get("literal_count") != PAGE18_LITERAL_COUNT
        ...
        raise O1C101RunError("published O1C100 Page-18 boundary differs")
    ...
    for name, (size, digest, role) in PUBLISHED_ARTIFACT_SEALS.items():
        ...
        if (
            ...
            raise O1C101RunError("published artifact seal differs")
        rows[name] = row
    _native_v32._validate_failure_receipt(artifacts[FAILURE_RECEIPT_NAME])
    known = _validate_published_documents(artifacts)
    return rows, known
...
def _load_published_preparation(
    ...
    try:
        ...
        raise O1C101RunError("published preparation inventory is unreadable") from exc
    if (
        manifest_path != directory / PREPARATION_MANIFEST_NAME
        or {path.name for path in children} != PREPARATION_ARTIFACT_NAMES
    ):
        raise O1C101RunError("published preparation inventory differs")
    ...
    for path in children:
        try:
            ...
            raise O1C101RunError(
                "published preparation artifact is unreadable"
            ) from exc
        if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
            raise O1C101RunError("published preparation artifact type differs")
    ...
    if (
        ...
        raise O1C101RunError("published preparation manifest seal differs")
...
def _preflight_bundle(
    ...
    *,
    ...
    if (
        ...
        raise O1C101RunError("parent input SHA preflight differs")
    ...
    failure_receipt_path = _lab_path(
        root,
        preparation["terminal_failure_receipt"],
        "terminal failure receipt",
    )
    if (
        ...
        or failure_receipt_path.stat().st_size
        != preparation["terminal_failure_receipt_bytes"]
        or _sha_file(failure_receipt_path)
        != preparation["terminal_failure_receipt_sha256"]
    ):
        raise O1C101RunError("published preparation/receipt preflight differs")
    ...
    for name in ("cnf", "potential", "grouping", "o1c73_config"):
        ...
        if observed != inputs[f"{name}_sha256"]:
            raise O1C101RunError(f"input {name} SHA preflight differs")
    ...
    for name in SOURCE_PATHS:
        ...
        if observed != expected[name]:
            raise O1C101RunError(f"source {name} SHA preflight differs")
    ...
    if (
        ...
        or source_sha["native_v29"] != native["source_sha256"]
    ):
        raise O1C101RunError("native toolchain/source SHA preflight differs")

    prepared = _load_published_preparation(published_directory, published_manifest)
    page18_manifest = _mapping(
        prepared.manifest.get("page18"), "published Page-18 preflight"
    )
    capacity_proof = _validate_native_capacity_proof(page18_manifest)
    prepared_rows = {
        name: {
    ...
    failure_receipt_payload = failure_receipt_path.read_bytes()
    _validate_priority_receipt(
        receipt_payload, prepared.artifacts[CONTINUATION_BANK_NAME]
    )
    _native_v32._validate_failure_receipt(failure_receipt_payload)
    if failure_receipt_payload != prepared.artifacts[FAILURE_RECEIPT_NAME]:
        raise O1C101RunError("terminal failure receipt publication linkage differs")
    ...
    if (
        ...
        raise O1C101RunError("Darwin/arm64/RAM/disk/process preflight differs")
    row = {
        ...
        "terminal_failure_receipt": {
            "serialized_bytes": len(failure_receipt_payload),
            "sha256": _sha_file(failure_receipt_path),
        },
    ...
    return PreflightBundle(
        ...
        failure_receipt=failure_receipt_payload,
    )
...
def _executable_identity(path: Path, *, relative_path: str) -> dict[str, object]:
    try:
        ...
        raise O1C101RunError("native executable is unreadable") from exc
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
        raise O1C101RunError("native executable type differs")
    size = path.stat().st_size
    if size <= 0:
        raise O1C101RunError("native executable is empty")
    return {
        "path": relative_path,
...
def _verify_staged_native_closure(
    *,
    ...
    if len(expected_files) != len(NATIVE_CLOSURE_NAMES):
        raise O1C101RunError("staged native closure basenames collide")
    ...
    try:
        ...
        for candidate in closure_root.rglob("*"):
            ...
            if stat.S_ISLNK(metadata.st_mode):
                raise O1C101RunError("staged native closure contains a symlink")
            if stat.S_ISDIR(metadata.st_mode):
                observed_directories.add(relative)
                if stat.S_IMODE(metadata.st_mode) != 0o555:
                    raise O1C101RunError("staged native closure directory is writable")
            elif stat.S_ISREG(metadata.st_mode):
                observed_files.add(relative)
                if stat.S_IMODE(metadata.st_mode) != 0o444:
                    raise O1C101RunError("staged native closure file is writable")
            else:
                raise O1C101RunError("staged native closure contains a special file")
    except OSError as exc:
        raise O1C101RunError("staged native closure is unreadable") from exc
    if (
        ...
        raise O1C101RunError("staged native closure inventory differs")
    ...
    for name in NATIVE_CLOSURE_NAMES:
        ...
        if row["sha256"] != digests.get(name):
            raise O1C101RunError(f"staged native closure {name} digest differs")
        rows.append(row)
    normalized_expected = [
        dict(_mapping(value, "expected staged closure row")) for value in expected_rows
    ]
    if rows != normalized_expected:
        raise O1C101RunError("staged native closure rows differ")
    return rows
    ...
    *,
    ...
    for name in NATIVE_CLOSURE_NAMES:
        ...
        if approved_source_sha256.get(name) != expected_sha:
            raise O1C101RunError("native closure preflight/config seal differs")
        source_path = _lab_path(root, configured, f"native closure {name}")
        payload = source_path.read_bytes()
        if sha256_bytes(payload) != expected_sha:
            raise O1C101RunError(f"native closure {name} changed after preflight")
...
def _compile_native(
    *,
    ...
    staged_source = _relative_text(source_paths["native_v29"], "staged v29 source")
    if (
        native["source"] != staged_source
        or _mapping(verified_closure[0], "staged v29 row").get("sha256")
        != native["source_sha256"]
        or _sha_file(library) != native["cadical_library_sha256"]
    ):
        raise O1C101RunError("native build input changed after preflight")
    ...
    try:
        ...
        raise O1C101RunError("native v29 compilation could not run") from exc
    ...
    try:
        ...
        raise O1C101RunError("native v29 --help smoke could not run") from exc
    ...
    if (
        ...
        or smoke_stdout != NATIVE_V29_USAGE
        or smoke_stderr
        or after_smoke != before_smoke
    ):
        raise O1C101RunError("native v29 --help smoke seal differs")
    verified_closure = _verify_staged_native_closure(
        stage=stage,
        config=config,
        expected_rows=closure_rows,
    ...
    if _sha_file(library) != native["cadical_library_sha256"]:
        raise O1C101RunError("native library changed during build/smoke")
    normalized_command = [
        *command[:-1],
    ...
    build = {
        ...
        "staged_source": dict(_mapping(verified_closure[0], "staged v29 row")),
...
def _validate_staged_native_build(
    *,
    ...
    if (
        ...
        or smoke.get("stdout")
        != {
            "serialized_bytes": len(NATIVE_V29_USAGE),
            "sha256": sha256_bytes(NATIVE_V29_USAGE),
        }
        or smoke.get("stderr") != {"serialized_bytes": 0, "sha256": sha256_bytes(b"")}
        or smoke.get("executable") != executable
    ):
        raise O1C101RunError("staged native build provenance differs")
...
def _stage_capsule(
    *,
    ...
    if final.exists() or stage.exists():
        raise O1C101RunError("timestamped capsule already exists")
    ...
    try:
        ...
        if sha256_bytes(config_payload) != bundle.row.get(
            ...
            raise O1C101RunError("config changed before closure snapshot")
        ...
        invocation = {
            ...
            "terminal_failure_receipt_sha256": PARENT_FAILURE_SHA256,
            "candidate_order_sha256": CONTINUATION_CANDIDATE_ORDER_SHA256,
            "global_novelty_baseline_clause_count": ATTIC_UNION_CLAUSE_COUNT,
            "threshold": THRESHOLD,
            "seed": SEED,
...
def _rehash_call_inputs(
    *,
    ...
    if _sha_file(config_path) != preflight_row.get("config_sha256"):
        raise O1C101RunError("config changed before native call")
    ...
    for name in ("cnf", "potential", "grouping", "o1c73_config"):
        if (
            ...
            raise O1C101RunError(f"input {name} changed before native call")
    ...
    failure_receipt_row = _mapping(
        preflight_row.get("terminal_failure_receipt"),
        "approved terminal failure receipt",
    )
    if (
        prepared_rows.get(RECEIPT_NAME) != receipt_row
        or prepared_rows.get(FAILURE_RECEIPT_NAME) != failure_receipt_row
        or invocation.get("priority_state_receipt_sha256") != receipt_row.get("sha256")
        or invocation.get("terminal_failure_receipt_sha256")
        != failure_receipt_row.get("sha256")
    ):
        raise O1C101RunError("staged receipt seal differs")
    ...
    for name, value in expected_rows.items():
        ...
        if path.stat().st_size != row.get("serialized_bytes") or _sha_file(
            ...
            raise O1C101RunError("staged call artifact seal differs")
    ...
    if (
        ...
        raise O1C101RunError("staged invocation/build seal differs")
    ...
    if (
        ...
        raise O1C101RunError("staged native build/smoke seal differs")
    if require_executable_identity:
        _require_executable_identity(
...
def _native_stdout(result: object) -> bytes:
    ...
    if isinstance(value, str):
        ...
        raise O1C101RunError("adapter omitted byte-exact native stdout")
    if _sha(
        getattr(result, "native_stdout_sha256", None), "native stdout digest"
    ) != sha256_bytes(payload):
        raise O1C101RunError("native stdout digest differs")
    return payload
...
def _validate_result_conservation(
    ...
    if set(counters) != {*outcome_names, "child_bound_evaluations"}:
        raise O1C101RunError("native post-result conservation differs")
    outcomes = tuple(
...
def _adapter_memory_samples(
    ...
    for raw in rows_raw:
        ...
        if (
            ...
            raise O1C101RunError("adapter memory sample differs")
    ...
    if (
        ...
        != _native_v32._v9.JOINT_SCORE_SIEVE_MEMORY_SERIES_SCHEMA
        or memory.get("memory_sample_limit")
        != _native_v32._v9.JOINT_SCORE_SIEVE_MAXIMUM_MEMORY_SAMPLES
        ...
        raise O1C101RunError("adapter memory ledger differs")
    return tuple(rows)


def _validate_native_result(result: object, stdout: bytes) -> NativeEvidence:
    if not isinstance(result, _native_v32.JointScoreSieveV32Result):
        raise O1C101RunError("adapter v32 result type differs")
    ...
    try:
        ...
        raise O1C101RunError("native stdout JSON differs") from exc
    if (
        decoded != raw
        or raw.get("schema") != _native_v32.JOINT_SCORE_SIEVE_RESULT_SCHEMA
    ):
        raise O1C101RunError("native stdout/result projection differs")
    ...
    if (
        ...
        or raw.get("active_vault_sha256") != PAGE18_SHA256
        ...
        raise O1C101RunError("native v29 production binding differs")
    if (
        raw.get("priority_state") != priority_state
        or raw.get("priority_actions") != actions
    ):
        raise O1C101RunError("native promoted priority projection differs")
    if raw.get("decision_ownership") != ownership:
        raise O1C101RunError("native promoted ownership projection differs")
    if result.status == 10 and not isinstance(result.key_model, bytes):
        raise O1C101RunError("SAT result omitted key model")
    if result.status != 10 and result.key_model is not None:
        raise O1C101RunError("non-SAT result returned key model")
...
def _science_and_operation(
    ...
    *,
    ...
    if action_count != len(actions):
        raise O1C101RunError("priority action count differs")
    ...
    for row in emitted_rows:
        ...
        try:
            ...
            raise O1C101RunError("emitted clause serialization differs") from exc
    ...
    if novel > active_new:
        raise O1C101RunError("global novelty exceeds active-vault novelty")
    ...
    if evidence.status == 10 and not public_model_verified:
        raise O1C101RunError("SAT key failed exact public ChaCha verification")
    if evidence.status != 10 and public_model_verified:
        raise O1C101RunError("public model verification without SAT differs")
    model_gain = evidence.status == 10 and public_model_verified
    closure_gain = evidence.status == 20 or terminal_empty
    # Native v29 emits no entropy or domain estimate.  These fields stay
    ...
    science = {
        ...
        "active_page18_new_clauses": active_new,
    ...
    *,
    ...
    try:
        ...
        result = adapter_run(
            ...
            terminal_failure_receipt_path=(capsule / "initial" / FAILURE_RECEIPT_NAME),
            sealed_page18_path=capsule / "initial" / ACTIVE_PROJECTION_NAME,
        )
        ...
        # Adapter v32 attaches the completed process record even when semantic
...
def _manifest_bytes(capsule: Path) -> tuple[bytes, int]:
    ...
    for path in sorted(capsule.rglob("*")):
        metadata = path.lstat()
        if stat.S_ISLNK(metadata.st_mode):
            raise O1C101RunError("capsule contains a symlink")
        if stat.S_ISDIR(metadata.st_mode):
            continue
        if not stat.S_ISREG(metadata.st_mode):
            raise O1C101RunError("capsule contains a special file")
...
def _run_markdown(result: Mapping[str, object]) -> bytes:
    ...
    lines = [
        "# O1C-0101 Page-18 parent-centered run",
        "",
        ...
        "- Page 18 and lineage 31 burned when `episodes/00/intent.json` was persisted.",
...
def _publish_exact(path: Path, payload: bytes) -> None:
    if path.exists():
        if path.is_symlink() or not path.is_file() or path.read_bytes() != payload:
            raise O1C101RunError("authoritative result already differs")
        return
...
def _finalize_capsule(
    *, capsule: Path, authoritative: Path, result: Mapping[str, object]
    ...
    if persistent_bytes > MAXIMUM_PERSISTENT_ARTIFACT_BYTES:
        raise O1C101RunError("persistent artifact budget exceeded")
...
def _existing_result(root: Path) -> dict[str, object] | None:
    ...
    if not authoritative.exists():
        return None
    value = dict(_read_canonical_json(authoritative, "authoritative O1C101 result"))
    if value.get("schema") != RESULT_SCHEMA or value.get("attempt_id") != ATTEMPT_ID:
        raise O1C101RunError("authoritative O1C101 result differs")
    return value
...
def _validate_capsule_artifact_row(
    ...
    *,
    ...
    try:
        ...
        raise O1C101RunError(f"capsule artifact {name} is unreadable") from exc
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
        raise O1C101RunError(f"capsule artifact {name} type differs")
    expected = _artifact_row(path, relative_to=directory)
    if set(row) != {"path", "serialized_bytes", "sha256"} or row != expected:
        raise O1C101RunError(f"capsule artifact {name} seal differs")
    if canonical_json:
...
def _validated_timestamp(value: object, field: str) -> str:
    ...
    try:
        ...
        raise O1C101RunError(f"{field} differs") from exc
...
def _semantic_replay_completed_native(
    *,
    ...
    """Replay archived stdout through v32 using the original sealed inputs."""
    ...
    for name in ("cnf", "potential", "grouping"):
        ...
        if sha256_bytes(payload) != input_sha[name]:
            raise O1C101RunError(f"semantic replay {name} seal differs")
        payloads[name] = payload
    page18 = prepared.artifacts[ACTIVE_PROJECTION_NAME]
    bank = prepared.artifacts[CONTINUATION_BANK_NAME]
    if (
        len(page18) != PAGE18_SERIALIZED_BYTES
        or sha256_bytes(page18) != PAGE18_SHA256
        or len(bank) != CONTINUATION_BANK_BYTES
        or sha256_bytes(bank) != CONTINUATION_BANK_SHA256
    ):
        raise O1C101RunError("semantic replay Page18/bank seal differs")
    io_v1 = _native_v32._v9._v8._v7._v1
    try:
        field = io_v1._potential(payloads["potential"])
        grouping = _native_v32._v9.validate_joint_score_sieve_grouping(
            field, payloads["grouping"]
        )
        if grouping.potential_sha256 != input_sha["potential"]:
            raise O1C101RunError("semantic replay grouping identity differs")
        input_vault = _native_v32.parse_threshold_no_good_vault(
            page18,
            observed_variables=field.observed_variables,
            caps=O1C66_VAULT_CAPS,
        )
        identity = _native_v32.vault_identity_from_sources(
            cnf_sha256=input_sha["cnf"],
            potential_sha256=input_sha["potential"],
            grouping_sha256=input_sha["grouping"],
            observed_variables=field.observed_variables,
            bound_rule=_native_v32._v9.JOINT_SCORE_SIEVE_BOUND_RULE,
            threshold=THRESHOLD,
        )
        _native_v32.validate_threshold_no_good_vault_identity(
            input_vault, expected=identity
        )
        _native_v32._v9._v8._certify_input_vault(
            input_vault,
            field=field,
            grouping=grouping,
            threshold=THRESHOLD,
        )
        priority_records = _native_v32._decode_live_bank(
            bank,
            expected_sha256=CONTINUATION_BANK_SHA256,
            sealed_input=True,
        )
        memory_samples = _adapter_memory_samples(adapter_memory)
        raw = _native_v32._v21.load_native_json(stdout)
        replayed = _native_v32._parse_native_payload(
            raw,
        ...
        raise O1C101RunError("archived native semantic replay differs") from exc
    ...
    if replay_memory != adapter_memory:
        raise O1C101RunError("semantic replay adapter memory differs")
    ...
    *,
    ...
    if not all(path.is_file() and not path.is_symlink() for path in required):
        if (capsule / "episodes/00/intent.json").is_file():
            raise O1C101RunError("burned incomplete capsule forbids retry or replay")
        raise O1C101RunError("incomplete pre-intent capsule blocks a fresh call")
    manifest, _ = _manifest_bytes(capsule)
    if manifest != (capsule / "artifacts.sha256").read_bytes():
        raise O1C101RunError("existing capsule manifest differs")
    episode_dir = capsule / "episodes" / "00"
    ...
    try:
        ...
        raise O1C101RunError("existing capsule inventory is unreadable") from exc
    if (
        ...
        raise O1C101RunError("existing capsule inventory differs")
    ...
    if (
        ...
        raise O1C101RunError("existing capsule config differs")
    ...
    if (
        ...
        != _validate_native_capacity_proof(prepared.manifest["page18"])
        or published.get("native_capacity_proof_verified_before_intent") is not True
        or preflight.get("page18_sha256") != PAGE18_SHA256
        ...
        or preflight.get("terminal_failure_receipt")
        != prepared_rows[FAILURE_RECEIPT_NAME]
        ...
        raise O1C101RunError("existing capsule preflight provenance differs")
    ...
    if (
        ...
        or invocation.get("page18_sha256") != PAGE18_SHA256
        ...
        or invocation.get("terminal_failure_receipt_sha256") != PARENT_FAILURE_SHA256
        ...
        raise O1C101RunError("existing capsule invocation provenance differs")
    _rehash_call_inputs(
        root=root,
        config_path=config_path,
        config=config,
    ...
    if (
        ...
        or intent.get("page18_sha256") != PAGE18_SHA256
        ...
        or intent.get("terminal_failure_receipt_sha256") != PARENT_FAILURE_SHA256
        ...
        raise O1C101RunError("existing capsule burn intent differs")

    episode = dict(_read_canonical_json(episode_path, "capsule episode"))
    if episode != episode_preview:
        raise O1C101RunError("capsule episode changed during validation")
    ...
    if (
        ...
        raise O1C101RunError("existing capsule episode lineage differs")
    ...
    if episode.get("completed") is True:
        ...
        if (
            ...
            raise O1C101RunError("existing completed episode call contract differs")
        ...
        if set(archived) != set(NATIVE_ARCHIVE_NAMES):
            raise O1C101RunError("archived native component inventory differs")
        component_paths = {
        ...
        if episode.get("final_priority_bank") != archived[FINAL_BANK_NAME]:
            raise O1C101RunError("final priority bank row differs")
        ...
        try:
            raw = dict(_native_v32._v21.load_native_json(stdout))
        except Exception as exc:
            raise O1C101RunError("archived native stdout differs") from exc
        archived_raw = dict(
        ...
        if status == 10:
            if (
                ...
                raise O1C101RunError("archived SAT key model differs")
            if (
                len(key_model) != 32
                ...
                raise O1C101RunError("archived SAT key model is not publicly verified")
        elif status in (0, 20):
            if key_hex is not None or episode.get("key_model_sha256") is not None:
                raise O1C101RunError("archived non-SAT key model differs")
        else:
            raise O1C101RunError("archived native status differs")
        try:
            bank_hex = bytes.fromhex(cast(str, priority_state.get("bank_hex")))
        except (TypeError, ValueError) as exc:
            raise O1C101RunError("archived final bank encoding differs") from exc
        if (
            raw != archived_raw
            or evidence.raw != raw
            ...
            raise O1C101RunError("archived native result cross-binding differs")
        operation, science, classification, stop = _science_and_operation(
            evidence,
        ...
        if (
            ...
            raise O1C101RunError("completed episode science projection differs")
        ...
        if (
            ...
            raise O1C101RunError("terminal episode call count differs")
        if (
            ...
            raise O1C101RunError("terminal episode contract differs")
        ...
        if (
            ...
            or failure.get("page18_burned") is not True
            or failure.get("lineage31_burned") is not True
            ...
            raise O1C101RunError("terminal failure cross-binding differs")
        stdout_row = episode.get("native_stdout")
        if failure.get("native_stdout") != stdout_row:
            raise O1C101RunError("terminal native stdout row differs")
        if not native_inventory:
            if not (
                failure.get("phase") == "PRE_CALL"
                and failure.get("exception_type") == "O1C101RunError"
                ...
                raise O1C101RunError("missing terminal executable provenance differs")
        else:
            ...
            if observed_executable != executable and not (
                ...
                raise O1C101RunError("terminal executable provenance differs")
        expected_episode_inventory = {
            "intent.json",
            "episode.json",
        ...
        raise O1C101RunError("existing capsule episode completion differs")

    try:
        observed_episode_inventory = {path.name for path in episode_dir.iterdir()}
    except OSError as exc:
        raise O1C101RunError("episode inventory is unreadable") from exc
    if observed_episode_inventory != expected_episode_inventory:
        raise O1C101RunError("episode artifact inventory differs")
    ...
    if (
        ...
        != "Never retry or replay Page 18 / lineage 31 or Pages 17 through 9; preserve the final priority bank only as a continuation seed."
        or (capsule / "RUN.md").read_bytes() != _run_markdown(result)
    ):
        raise O1C101RunError("existing capsule result differs")
...
def _public_verifier(
    *, root: Path, config: Mapping[str, object]
    ...
    if _sha_file(config_path) != inputs["o1c73_config_sha256"]:
        raise O1C101RunError("public verifier config SHA differs")
    try:
        ...
        raise O1C101RunError("public ChaCha verifier cannot be established") from exc
...
def run(
    ...
    *,
    root: Path | None = None,
    adapter_run: AdapterRun = _native_v32.run_joint_score_sieve,
    ...
    if existing is not None:
        if len(capsules) != 1:
            raise O1C101RunError("authoritative O1C101 result lacks one sealed capsule")
        ...
        if capsule_payload != authoritative_payload or capsule_result != existing:
            raise O1C101RunError("authoritative O1C101 result differs from capsule")
        return existing
    if capsules:
        if len(capsules) != 1:
            raise O1C101RunError("multiple O1C101 capsules forbid replay")
    ...
    result = {
        ...
        "next_action": "Never retry or replay Page 18 / lineage 31 or Pages 17 through 9; preserve the final priority bank only as a continuation seed.",
    }
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Preflight or execute O1C101's one Page-18 parent-centered call"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 0
    except O1C101RunError as exc:
        print(f"O1C101: {exc}", file=sys.stderr)
        return 2
