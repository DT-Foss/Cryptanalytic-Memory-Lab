"""One-call W11 screen for exact learned-clause owner credit."""
...
from .learned_clause_credit_search import (
...
ATTEMPT_ID = "O1C-0055"
RESULT_SCHEMA = "o1-256-learned-clause-credit-screen-result-v1"
...
NATIVE_SOURCE = Path("native/cadical_o1_learned_clause_credit_search.cpp")
ADAPTER_SOURCE = Path("src/o1_crypto_lab/learned_clause_credit_search.py")
RUNNER_SOURCE = Path("src/o1_crypto_lab/o1c55_learned_clause_credit_screen.py")
O1C53_RESULT = Path(
...
EXACT_RECOVERY_CLASSIFICATION = "LEARNED_CLAUSE_CREDIT_EXACT_W11_CLOSE"
MEMBERSHIP_NO_CLOSE_CLASSIFICATION = (
    "LEARNED_CLAUSE_CREDIT_MEMBERSHIP_NO_EXACT_W11_CLOSE"
)
NO_MEMBERSHIP_CLASSIFICATION = "LEARNED_CLAUSE_CREDIT_NO_MEMBERSHIP_NO_EXACT_W11_CLOSE"
...
class O1C55ScreenError(RuntimeError):
    """The frozen learned-clause-credit screen boundary was violated."""


def _mapping(value: object, field: str) -> Mapping[str, object]:
    if not isinstance(value, Mapping):
        raise O1C55ScreenError(f"{field} differs")
    return value
...
def _json_object(payload: bytes, field: str) -> dict[str, object]:
    try:
        ...
        raise O1C55ScreenError(f"{field} JSON differs") from exc
    if not isinstance(value, dict):
        raise O1C55ScreenError(f"{field} JSON root differs")
    return value
...
def _commit_bound_bytes(
    ...
    try:
        ...
        raise O1C55ScreenError(f"{field} is not bound to source commit") from exc
    if completed.stdout != payload:
        raise O1C55ScreenError(f"{field} differs from source commit")
...
def _authoritative_output(root: Path, output_path: str | Path) -> Path:
    ...
    if output != (root / RESULT_RELATIVE).resolve() or output.exists():
        raise O1C55ScreenError("result path differs or authoritative result exists")
    return output
...
def call_ledger() -> dict[str, object]:
    call = {
        "ordinal": 1,
        "stage": "qualification",
        "mechanism": "exact-learned-clause-owner-credit",
        "arm": "primary",
...
def classify_result(*, exact_recovery: bool, penalty_updates: int) -> str:
    if not isinstance(exact_recovery, bool):
        raise O1C55ScreenError("exact recovery classification input differs")
    penalties = _integer(penalty_updates, "penalty updates")
    if penalties < 0:
        raise O1C55ScreenError("penalty updates differ")
    if exact_recovery:
...
def _model_honors_fixed_spins(model: bytes, fixed: Mapping[int, int]) -> bool:
    if len(model) != 32:
        raise O1C55ScreenError("search model width differs")
    bits = key_bits(model)
    return all(
...
def _search_row(
    ...
    *,
    ...
    if result.conflict_limit != CONFLICT_LIMIT:
        raise O1C55ScreenError("learned-clause conflict cap differs")
    ...
    if result.status_name == "SAT" and (
        ...
        raise O1C55ScreenError("SAT model failed W11 public/prefix verification")
    ...
    return {
        "name": "learned_clause_credit_primary_w11",
        "mechanism": "exact-learned-clause-owner-credit",
        "arm": "primary",
...
def _validate_o1c53_boundary(
    ...
    *,
    ...
    ledger = _mapping(result.get("call_ledger"), "O1C53 call ledger")
    w11 = _mapping(result.get("w11_search"), "O1C53 W11")
    primary = _mapping(w11.get("survivor_primary"), "O1C53 survivor primary")
    stats = _mapping(primary.get("stats"), "O1C53 survivor stats")
    if (
        ...
        or stats.get("conflicts") != CONFLICT_LIMIT
        or primary.get("status") != "UNKNOWN"
        or primary.get("model_truth_exact") is not False
    ):
        raise O1C55ScreenError("frozen O1C53 matched-work boundary differs")
    return {
...
def _validate_native_contract(
    ...
    telemetry = _mapping(
        learned.get("learned_clause_credit"), "learned clause telemetry"
    )
    if (
        learned.get("decision_rule") != LEARNED_CLAUSE_CREDIT_DECISION_RULE
        ...
        != LEARNED_CLAUSE_CREDIT_CALLBACK_STATE_BYTES
        or state.get("bounded_state_bytes") != LEARNED_CLAUSE_CREDIT_STATE_BYTES
        ...
        raise O1C55ScreenError("learned-clause native contract differs")
...
def _validate_executed_work(row: Mapping[str, object]) -> int:
    ...
    if not 0 <= conflicts <= CONFLICT_LIMIT:
        raise O1C55ScreenError("executed search conflict boundary differs")
    exact = row.get("model_truth_exact") is True
    status = row.get("status")
    if exact:
        if status != "SAT":
            raise O1C55ScreenError("exact recovery status differs")
    elif status != "UNKNOWN" or conflicts != CONFLICT_LIMIT:
        raise O1C55ScreenError(
...
def run(
    ...
    for name, payload in source_bytes.items():
        if _sha256(payload) != expected.get(name):
            raise O1C55ScreenError(f"frozen O1C48 source differs: {name}")
    if (
        expected.get("reveal") != EXPECTED_REVEAL_SHA256
        or expected.get("primary_potential") != EXPECTED_PRIMARY_POTENTIAL_SHA256
    ):
        raise O1C55ScreenError("frozen reveal/potential hash differs")
    ...
    if (
        ...
        raise O1C55ScreenError("frozen public target differs")
    ...
    try:
        ...
        raise O1C55ScreenError("revealed key encoding differs") from exc
    ...
    if (
        ...
        raise O1C55ScreenError("revealed key differs from frozen public target")
    ...
    if primary_plan.describe() != EXPECTED_PRIMARY_PLAN:
        raise O1C55ScreenError("frozen primary pair plan differs")
    ...
    if (
        ...
        raise O1C55ScreenError("frozen primary pair partition differs")
    plan_validation = {
    ...
    if tuple(sorted(residual)) != EXPECTED_W11_RESIDUAL_VARIABLES:
        raise O1C55ScreenError("frozen W11 residual coordinates differ")
    fixed = {
        variable: spin
    ...
    baseline_path = (root / O1C53_RESULT).resolve(strict=True)
    baseline_bytes = baseline_path.read_bytes()
    if _sha256(baseline_bytes) != O1C53_RESULT_SHA256:
        raise O1C55ScreenError("O1C53 baseline hash differs")
    baseline = _json_object(baseline_bytes, "O1C53 baseline")
    baseline_boundary = _validate_o1c53_boundary(
        baseline,
    ...
    with tempfile.TemporaryDirectory(prefix="o1c55-") as temporary:
        workspace = Path(temporary)
        executable = workspace / "cadical-o1-learned-clause-credit"
        native_build = build_native_learned_clause_credit_search(
        ...
        old_instance = _mapping(
            _mapping(baseline.get("architecture"), "O1C53 architecture").get(
                "public_instance"
            ),
            "O1C53 public instance",
        )
        if (
            ...
            raise O1C55ScreenError("public Full256 instance differs")
        ...
        if prefix != EXPECTED_W11_PREFIX:
            raise O1C55ScreenError("frozen W11 prefix differs")
        decisions_path = workspace / "primary.ordered-pair-variables"
        decision_sha256 = write_learned_clause_credit_decision_variables(
            decisions_path, primary_plan.ordered_variables
        )
        search_result = run_learned_clause_credit_search(
    ...
    if elapsed > MAXIMUM_WALL_SECONDS or peak_rss > MAXIMUM_PEAK_RSS_BYTES:
        raise O1C55ScreenError("one-call resource boundary exceeded")
    learned = _mapping(search["learned_clause"], "learned clause")
    credit = _mapping(learned.get("learned_clause_credit"), "credit telemetry")
    ...
    result: dict[str, object] = {
        ...
        "claim_level": "CONSUMED_POST_REVEAL_EXACT_LEARNED_CLAUSE_SCREEN",
        "hypothesis": (
            "exact minimized learned-clause membership supplies causal negative "
            "credit to the live O1 pair action that helped create the conflict, "
            "improving W11 closure under the frozen 512-conflict budget"
        ),
        "boundary": {
        ...
        "architecture": {
            ...
            "action_state_bytes": LEARNED_CLAUSE_CREDIT_ACTION_STATE_BYTES,
            "owner_state_bytes": LEARNED_CLAUSE_CREDIT_OWNER_STATE_BYTES,
            "callback_state_bytes": LEARNED_CLAUSE_CREDIT_CALLBACK_STATE_BYTES,
        },
        "w11_search": search,
        "gate": {
        ...
        "baseline": {"O1C-0053": baseline_boundary},
        "baseline_sha256": {"O1C-0053": O1C53_RESULT_SHA256},
        "source_sha256": {
        ...
        "next_action": (
            ...
            if exact_recovery
                ...
                "retain exact learned-clause membership as the next causal stream "
                "primitive; close this fixed -32 rule without tuning"
                if penalty_updates
    ...
    if config_file.read_bytes() != config_bytes:
        raise O1C55ScreenError("config changed during one-call screen")
    for name, before in source_bytes.items():
        if paths[name].read_bytes() != before:
            raise O1C55ScreenError(f"source changed during one-call screen: {name}")
    ...
    if baseline_path.read_bytes() != baseline_bytes:
        raise O1C55ScreenError("O1C53 baseline changed during one-call screen")
    if output.exists():
        raise O1C55ScreenError("authoritative result appeared during one-call screen")
    if ledger["native_solver_calls"] != 1 or ledger["full256_calls"] != 0:
        raise O1C55ScreenError("final call ledger differs")
