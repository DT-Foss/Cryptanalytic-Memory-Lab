"""O1C-0078 bounded rescue-prefix-preemption science call.
...
consumes only the frozen O1C-0078 seed; it never replays the mutable parent
...
from . import joint_score_sieve_v19 as _native_v19
...
ATTEMPT_ID = "O1C-0078"
CONFIG_SCHEMA = "o1-256-apple8-rescue-prefix-preemption-config-v1"
PREFLIGHT_SCHEMA = "o1-256-apple8-rescue-prefix-preemption-preflight-v1"
TARGET_FREE_GATE_SCHEMA = (
    "o1-256-o1c78-target-free-rescue-prefix-preemption-preflight-v1"
)
TARGET_FREE_GATE_CLASSIFICATION = (
    "O1C78_TARGET_FREE_RESCUE_PREFIX_PREEMPTION_PREFLIGHT_PASS"
)
INVOCATION_SCHEMA = "o1-256-apple8-rescue-prefix-preemption-invocation-v1"
INTENT_SCHEMA = "o1-256-apple8-rescue-prefix-preemption-intent-v1"
EPISODE_SCHEMA = "o1-256-apple8-rescue-prefix-preemption-episode-v1"
RESULT_SCHEMA = "o1-256-apple8-rescue-prefix-preemption-result-v1"
RECOVERY_SOURCE_SCHEMA = (
    "o1-256-apple8-rescue-prefix-preemption-pre-finalization-source-v1"
)
PUBLICATION_RECOVERY_SCHEMA = (
    "o1-256-apple8-rescue-prefix-preemption-publication-recovery-v1"
)
...
CONFIG_RELATIVE = Path(
    "configs/o1c78_apple8_rescue_prefix_preemption_v1.json"
)
TARGET_FREE_PREFLIGHT_RELATIVE = Path(
    "research/O1C0078_TARGET_FREE_RESCUE_PREFIX_PREEMPTION_PREFLIGHT_20260720.json"
)
RESULT_RELATIVE = Path(
    "research/O1C0078_APPLE8_RESCUE_PREFIX_PREEMPTION_RESULT_20260720.json"
)
CAPSULE_SUFFIX = "O1C-0078_apple8-rescue-prefix-preemption-v1"
...
PARENT_LAST_LINEAGE_ORDINAL = 17
RANK_SOURCE_SHA256 = (
    "cd523334672dd75c068c2dd32fe218fb7ae55644c0d56e6347271bba3a9c1858"
)
PARENT_FINAL_ACTIVE_SHA256 = "07c73013705898e228a05b0578b0f8090a6f094c427dbd8f32d856467b08e208"
...
INITIAL_CHUNK_CLAUSES = (202, 311, 0, 37, 0, 0, 0, 0, 0, 0)
...
SAFE_PRUNE_GAIN = "RESCUE_PREFIX_PREEMPTION_SAFE_PRUNE_GAIN"
NOVEL_CLAUSE_GAIN = "RESCUE_PREFIX_PREEMPTION_NOVEL_CLAUSE_GAIN"
MECHANISM_ONLY = "RESCUE_PREFIX_PREEMPTION_MECHANISM_ONLY"
NO_ACTIVATION = "RESCUE_PREFIX_PREEMPTION_NO_ACTIVATION_NO_GAIN"
OPERATIONAL_TERMINAL = "RESCUE_PREFIX_PREEMPTION_OPERATIONAL_TERMINAL"

SOURCE_PATHS = {
    "runner": "src/o1_crypto_lab/o1c78_apple8_rescue_prefix_preemption_run.py",
    "causal_attic_v1": "src/o1_crypto_lab/causal_attic_v1.py",
    "causal_residency_v1": "src/o1_crypto_lab/causal_residency_v1.py",
    "preparation": (
        "src/o1_crypto_lab/o1c78_apple8_rescue_prefix_preemption_prepare.py"
    ),
    ...
    "rescue_prefix_preemption_v1": (
        "src/o1_crypto_lab/rescue_prefix_preemption_v1.py"
    ),
    "adapter_v19": "src/o1_crypto_lab/joint_score_sieve_v19.py",
    "native_v16": "native/cadical_o1_joint_score_sieve_v16.cpp",
}


class O1C78RunError(RuntimeError):
...
def _mapping(value: object, field: str) -> Mapping[str, object]:
    if not isinstance(value, Mapping) or not all(
        ...
        raise O1C78RunError(f"{field} differs")
...
def _relative_contract(value: object, field: str) -> str:
    ...
    if path.is_absolute() or not path.parts or ".." in path.parts:
        raise O1C78RunError(f"{field} escapes the lab")
...
def _relative(root: Path, value: object, field: str) -> Path:
    ...
    try:
        ...
        raise O1C78RunError(f"{field} cannot be resolved") from exc
...
def _regular(path: Path, field: str) -> Path:
    try:
        ...
        raise O1C78RunError(f"{field} cannot be read") from exc
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
        raise O1C78RunError(f"{field} is not a regular file")
    return path
...
def _sha256_file(path: Path) -> str:
    ...
    try:
        ...
        raise O1C78RunError(f"cannot hash {path}") from exc
...
def _read_json(path: Path, field: str) -> Mapping[str, object]:
    ...
    try:
        ...
        raise O1C78RunError(f"{field} JSON differs") from exc
...
def _atomic_create(path: Path, payload: bytes, *, immutable: bool = False) -> None:
    if not isinstance(payload, bytes) or path.exists() or path.is_symlink():
        raise O1C78RunError(f"owned artifact already exists: {path.name}")
    try:
        ...
        if immutable:
            path.chmod(0o444)
    except O1C78RunError:
        raise
    except OSError as exc:
        raise O1C78RunError(f"cannot publish owned artifact: {path.name}") from exc
    if path.read_bytes() != payload:
        raise O1C78RunError(f"owned artifact reread differs: {path.name}")
...
def _canonical_gzip(payload: bytes) -> bytes:
    if not isinstance(payload, bytes):
        raise O1C78RunError("native evidence payload differs")
    output = io.BytesIO()
    with gzip.GzipFile(
    ...
    try:
        ...
        raise O1C78RunError("native evidence compression differs") from exc
    if restored != payload:
        raise O1C78RunError("native evidence compression differs")
    return compressed
...
def _read_compressed_json(
    ...
    if metadata.get("schema") != NATIVE_EVIDENCE_SCHEMA:
        raise O1C78RunError(f"{field} schema differs")
    name = metadata.get("path")
    if not isinstance(name, str) or Path(name).name != name:
        raise O1C78RunError(f"{field} path differs")
    ...
    if (
        ...
        raise O1C78RunError(f"{field} compressed artifact differs")
    try:
        payload = gzip.decompress(compressed)
        value = json.loads(payload)
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise O1C78RunError(f"{field} payload differs") from exc
    if (
        len(payload)
        ...
        raise O1C78RunError(f"{field} canonical payload differs")
    return _mapping(value, field)
...
def _validate_artifact_row(root: Path, value: object, field: str) -> Path:
    ...
    if path_value.is_absolute() or ".." in path_value.parts:
        raise O1C78RunError(f"{field} path escapes")
    ...
    if (
        ...
        raise O1C78RunError(f"{field} artifact differs")
    return path
...
def _validate_regular_capsule_tree(capsule: Path) -> None:
    try:
        ...
        if stat.S_ISLNK(root_metadata.st_mode) or not stat.S_ISDIR(
            ...
            raise O1C78RunError("capsule tree root differs")
        for path in capsule.rglob("*"):
            mode = path.lstat().st_mode
            if stat.S_ISLNK(mode) or not (
                stat.S_ISREG(mode) or stat.S_ISDIR(mode)
            ):
                raise O1C78RunError("capsule tree contains a non-regular entry")
    except O1C78RunError:
        raise
    except OSError as exc:
        raise O1C78RunError("capsule tree cannot be inspected") from exc
    ...
    *,
    ...
    try:
        ...
        if vault.identity != identity:
            raise O1C78RunError("causal-residency vault identity differs")
        return vault
    except O1C78RunError:
        raise
    except ThresholdNoGoodVaultError as exc:
        raise O1C78RunError("causal-residency vault artifact differs") from exc
...
def _parse_occurrence_document(
    ...
    if document.get("schema") != CAUSAL_ATTIC_OCCURRENCE_SCHEMA:
        raise O1C78RunError("occurrence schema differs")
    ...
    for ordinal, raw in enumerate(records):
        ...
        if row.get("ordinal") != ordinal or union_index >= len(clauses):
            raise O1C78RunError("occurrence ordinal differs")
...
def _coerce_prepared(value: PreparedRescuePrefixPreemption) -> PreparedStream:
    ...
    try:
        ...
        prefix_plan = value.prefix_plan
        prefix_plan_document = value.prefix_plan_document
        prefix_plan_binary = value.prefix_plan_binary
        rank_decision = value.rank_decision
    except AttributeError as exc:
        raise O1C78RunError("prepared causal-residency object differs") from exc
    if (
    ...
    ) or not isinstance(prefix_plan, RescuePrefixPreemptionPlan
    ):
        raise O1C78RunError("prepared prefix-preemption object differs")
    try:
        validate_causal_frontier_plan(
        ...
        if parse_causal_frontier_plan(
            ...
            raise O1C78RunError("prepared causal-frontier binary differs")
        validate_residual_polarity_staging_plan(
            staging_plan,
            active_vault=state.active_projection,
        ...
        if parse_residual_polarity_staging_plan(
            ...
            raise O1C78RunError("prepared staging binary differs")
        if staging_plan.parent_frontier_plan_sha256 != frontier_plan.sha256:
            raise O1C78RunError("prepared plan ancestry differs")
        validate_rescue_prefix_preemption_plan(
            prefix_plan,
            active_vault=state.active_projection,
        ...
        if parse_rescue_prefix_preemption_plan(
            cast(bytes, prefix_plan_binary),
            active_vault=state.active_projection,
        ) != prefix_plan:
            raise O1C78RunError("prepared prefix-preemption binary differs")
        prefix_document = _mapping(
            prefix_plan_document, "prepared prefix plan document"
        )
        if (
            prefix_document.get("plan") != prefix_plan.describe()
            or prefix_document.get("source_result_sha256")
            != O1C78_SOURCE_RESULT_SHA256
            or prefix_document.get("source_assignment_sha256")
            != O1C78_SOURCE_ASSIGNMENT_SHA256
            or prefix_document.get("active_vault_sha256")
            != state.active_projection.sha256
            or prefix_document.get("parent_staging_plan_sha256")
            != staging_plan.sha256
            or prefix_document.get("baseline_trace_sha256")
            != FROZEN_BASELINE_TRACE_SHA256
        ):
            raise O1C78RunError("prepared prefix provenance differs")
    except (
        CausalFrontierError,
        ResidualPolarityStagingError,
        RescuePrefixPreemptionError,
    ) as exc:
        raise O1C78RunError("prepared prefix-preemption object differs") from exc
...
def load_prepared_stream(
    ...
    """Load O1C78 preparation through its attempt-specific validator."""

    try:
        prepared = load_prepared_rescue_prefix_preemption(
            directory, expected_manifest_sha256=expected_manifest_sha256
        )
    except Exception as exc:
        if isinstance(exc, O1C78RunError):
            raise
        raise O1C78RunError("prepared causal-frontier validation differs") from exc
...
def prepared_stream_from_state(
    ...
    *,
    ...
    return PreparedStream(
        ...
        prefix_plan,
        dict(prefix_plan_document or {"plan": prefix_plan.describe()}),
        prefix_plan.serialized,
        rank_decision,
    )
...
def _validate_archived_v19_provenance(
    *,
    ...
    try:
        _native_v19.validate_native_lifecycle(raw)
    except Exception as exc:
        raise O1C78RunError("archived native v19 lifecycle differs") from exc
    ...
    try:
        ...
        raise O1C78RunError("archived native conflict provenance differs") from exc
    expected_model = key_model.hex() if key_model is not None else None
    if (
        raw.get("schema") != _native_v19.JOINT_SCORE_SIEVE_RESULT_SCHEMA
        or raw.get("implementation_parent_schema")
        != _native_v19.JOINT_SCORE_SIEVE_IMPLEMENTATION_PARENT_SCHEMA
        or raw.get("implementation_release_parent_schema")
        != _native_v19.JOINT_SCORE_SIEVE_RELEASE_PARENT_SCHEMA
        ...
        or raw.get("prefix_preemption_plan_sha256") != prefix_plan.sha256
        or raw.get("prefix_preemption_source_result_sha256")
        != O1C78_SOURCE_RESULT_SHA256
        ...
        or raw.get("prefix_preemption") != prefix_preemption
        ...
        or prefix_preemption.get("plan_sha256") != prefix_plan.sha256
        or prefix_preemption.get("source_result_sha256")
        != O1C78_SOURCE_RESULT_SHA256
        or prefix_preemption.get("source_assignment_sha256")
        != O1C78_SOURCE_ASSIGNMENT_SHA256
        or prefix_preemption.get("parent_staging_plan_sha256")
        != O1C78_PARENT_STAGING_PLAN_SHA256
        or staging_plan.sha256 != O1C78_PARENT_STAGING_PLAN_SHA256
        or prefix_preemption.get("active_vault_sha256")
        != O1C78_ACTIVE_VAULT_SHA256
        or active.sha256 != O1C78_ACTIVE_VAULT_SHA256
        or prefix_preemption.get("baseline_trace_sha256")
        != O1C78_BASELINE_TRACE_SHA256
        or FROZEN_BASELINE_TRACE_SHA256 != O1C78_BASELINE_TRACE_SHA256
        ...
        raise O1C78RunError("archived native v19 provenance differs")


def _prefix_return_literals(
    prefix_preemption: Mapping[str, object],
) -> tuple[int, ...]:
    """Decode and independently bind the native once-return sequence."""

    raw_hex = prefix_preemption.get("once_return_sequence_hex")
    if not isinstance(raw_hex, str) or len(raw_hex) % 8:
        raise O1C78RunError("prefix once-return sequence differs")
    try:
        payload = bytes.fromhex(raw_hex)
    except ValueError as exc:
        raise O1C78RunError("prefix once-return sequence differs") from exc
    count = _nonnegative_int(
        prefix_preemption.get("once_return_sequence_count"),
        "prefix once-return count",
    )
    if (
        len(payload) != 4 * count
        or prefix_preemption.get("once_return_sequence_bytes") != len(payload)
        or prefix_preemption.get("once_return_sequence_sha256")
        != sha256_bytes(payload)
    ):
        raise O1C78RunError("prefix once-return sequence binding differs")
    if not payload:
        return ()
    import struct

    return cast(tuple[int, ...], struct.unpack(f"<{count}i", payload))


def _validate_prefix_activation(
    *,
    prefix_preemption: Mapping[str, object],
    prefix_plan: RescuePrefixPreemptionPlan,
    trace_sha256: str,
) -> dict[str, object]:
    """Recompute the predeclared qualification gate from archived telemetry."""

    planned = tuple(prefix_plan.prefix_literals)
    returned = _prefix_return_literals(prefix_preemption)
    rows_consumed = _nonnegative_int(
        prefix_preemption.get("rows_consumed"), "prefix rows consumed"
    )
    cursor = _nonnegative_int(prefix_preemption.get("cursor"), "prefix cursor")
    once_returns = _nonnegative_int(
        prefix_preemption.get("once_returns"), "prefix once returns"
    )
    skipped_falsifying = _nonnegative_int(
        prefix_preemption.get("skipped_preassigned_falsifying"),
        "prefix falsifying skips",
    )
    skipped_rescue = _nonnegative_int(
        prefix_preemption.get("skipped_preassigned_rescue"),
        "prefix rescue skips",
    )
    first_parent_call = prefix_preemption.get("first_parent_call")
    parent_call_valid = (
        not isinstance(first_parent_call, bool)
        and isinstance(first_parent_call, int)
        and first_parent_call >= 1
    )
    native_mechanism = prefix_preemption.get("mechanism_activated")
    if not isinstance(native_mechanism, bool):
        raise O1C78RunError("prefix native activation differs")
    if native_mechanism is not (once_returns > 0):
        raise O1C78RunError("prefix native activation count differs")

    # A linear scan proves exact order preservation and at-most-once use.
    matched_indices: list[int] = []
    cursor_in_plan = 0
    for literal in returned:
        while cursor_in_plan < len(planned) and planned[cursor_in_plan] != literal:
            cursor_in_plan += 1
        if cursor_in_plan == len(planned):
            raise O1C78RunError("prefix once returns are not an ordered subsequence")
        matched_indices.append(cursor_in_plan)
        cursor_in_plan += 1
    ordered_once = (
        len(set(matched_indices)) == len(matched_indices)
        and len(returned) == once_returns
    )
    callback_count = _nonnegative_int(
        prefix_preemption.get("cb_decide_calls"), "prefix callback count"
    )
    parent_callback_count = _nonnegative_int(
        prefix_preemption.get("parent_cb_decide_calls"),
        "prefix parent callback count",
    )
    first_once_return_call = prefix_preemption.get("first_once_return_call")
    first_once_valid = (
        first_once_return_call is None
        or (
            not isinstance(first_once_return_call, bool)
            and isinstance(first_once_return_call, int)
            and first_once_return_call >= 1
        )
    )
    event_rows = _sequence(
        prefix_preemption.get("once_return_events"),
        "prefix once-return events",
    )
    expected_events = [
        {"call": ordinal + 1, "prefix_index": plan_index, "literal": literal}
        for ordinal, (plan_index, literal) in enumerate(
            zip(matched_indices, returned, strict=True)
        )
    ]
    chronology = (
        parent_callback_count > 0
        and callback_count == once_returns + parent_callback_count
        and first_once_valid
        and first_once_return_call == (1 if once_returns else None)
        and first_parent_call == once_returns + 1
        and list(event_rows) == expected_events
    )
    exact_plan = (
        prefix_preemption.get("prefix_literals") == list(planned)
        and prefix_preemption.get("prefix_order_encoding") == "signed-i32le"
        and prefix_preemption.get("prefix_order_bytes") == 4 * len(planned)
        and prefix_preemption.get("prefix_order_sha256")
        == sha256_bytes(prefix_plan.serialized)
        and prefix_preemption.get("plan_sha256") == prefix_plan.sha256
    )
    consumed_before_parent = (
        prefix_preemption.get("all_rows_consumed_before_first_parent_call")
        is True
        and parent_call_valid
        and chronology
    )
    structural = (
        exact_plan
        and rows_consumed == len(planned)
        and cursor == len(planned)
        and once_returns + skipped_falsifying == len(planned)
        and skipped_rescue == 0
        and ordered_once
        and once_returns > 0
        and native_mechanism
        and consumed_before_parent
    )
    trace_changed = trace_sha256 != FROZEN_BASELINE_TRACE_SHA256
    return {
        "plan_recovered_exactly": exact_plan,
        "rows_consumed": rows_consumed,
        "once_returns": once_returns,
        "skipped_preassigned_falsifying": skipped_falsifying,
        "skipped_preassigned_rescue": skipped_rescue,
        "returned_prefix_literals": list(returned),
        "returned_ordered_subsequence_once": ordered_once,
        "callback_chronology_recomputed": chronology,
        "first_parent_call": first_parent_call,
        "all_rows_consumed_before_first_parent_call": consumed_before_parent,
        "native_mechanism_activated": native_mechanism,
        ...
        "qualified_mechanism_activated": structural and trace_changed,
    }
...
def _validated_episode_result(
    ...
    *,
    ...
    """Bind one adapter-certified v19 return to its exact residency page."""

    try:
        if require_concrete_result and not isinstance(
            result, _native_v19.JointScoreSieveV19Result
        ):
            raise O1C78RunError("native result type differs")
        ...
        prefix_preemption = _mapping(
            getattr(result, "prefix_preemption"),
            "native prefix-preemption reader",
        )
        result_frontier_plan = getattr(result, "frontier_plan")
        result_staging_plan = getattr(result, "staging_plan")
        result_prefix_plan = getattr(result, "prefix_preemption_plan")
        telemetry = _mapping(
        ...
        if (
            ...
            or result_prefix_plan != prefix_plan
        ):
            raise O1C78RunError("native split-vault result binding differs")
        ...
        if peak > MEMORY_LIMIT_BYTES or wall > int(TIMEOUT_SECONDS * 1_000_000):
            raise O1C78RunError("native resource boundary differs")
        ...
        if pending != 0:
            raise O1C78RunError("native result retained an incomplete clause")
        ...
        if (
            ...
            raise O1C78RunError("native emitted ledger input binding differs")
        ...
        if not isinstance(eligible, Sequence) or isinstance(
            ...
            raise O1C78RunError("native typed emission ledger differs")
        for item, occurrence in zip(eligible, parsed.occurrences, strict=True):
            if (
                ...
                raise O1C78RunError("native typed emission occurrence differs")
        ...
        if not isinstance(available, bool):
            raise O1C78RunError("native next-vault availability differs")
        if available:
            if terminal_reason is not None or not isinstance(
                next_vault, ThresholdNoGoodVault
            ):
                raise O1C78RunError("available native next vault differs")
        elif terminal_reason not in CAPACITY_REASONS or next_vault is not None:
            raise O1C78RunError("native rollover reason differs")
        ...
        if status == 10:
            ...
            raise O1C78RunError("non-SAT stream result returned a candidate")
        _validate_archived_v19_provenance(
            raw=raw,
            reader=reader,
            frontier=frontier,
            staging=staging,
            prefix_preemption=prefix_preemption,
        ...
        if (
            ...
            raise O1C78RunError("frontier activation ledger differs")
        ...
        if (
            ...
            raise O1C78RunError("staging activation ledger differs")
        prefix_activation = _validate_prefix_activation(
            prefix_preemption=prefix_preemption,
            prefix_plan=prefix_plan,
        ...
        return {
            ...
            "prefix_preemption": dict(prefix_preemption),
            ...
            "qualified_prefix_mechanism": prefix_activation[
                "qualified_mechanism_activated"
            ],
            "trace_sha256": trace_sha256,
            "baseline_trace_sha256": FROZEN_BASELINE_TRACE_SHA256,
            "baseline_trace_changed": prefix_activation[
        ...
        raise O1C78RunError("native O1C-0078 result differs") from exc
...
def validate_native_executable(
    ...
    try:
        ...
        if (
            ...
            raise O1C78RunError("native executable mode differs")
        ...
        raise O1C78RunError("native executable cannot be read") from exc
    ...
    if (
        ...
        raise O1C78RunError("native executable identity differs")
...
def _initial_artifacts(
    ...
    if initial.exists():
        raise O1C78RunError("initial causal-residency directory already exists")
    ...
    prefix_plan_row = _copy_immutable(
        initial / PREFIX_PLAN_BINARY_NAME, prepared.prefix_plan_binary
    )
    prefix_plan_document_row = _copy_immutable(
        initial / "prefix-plan.json",
    ...
    return {
        ...
        "prefix_plan": prefix_plan_row,
        "prefix_plan_document": prefix_plan_document_row,
        "residency": state.describe(),
    }
...
def _failure_episode(
    *,
    ...
    if native_result_returned and not native_call_issued:
        raise O1C78RunError("native result cannot precede native call")
    native_calls_consumed = int(native_call_issued)
    requested_conflicts_consumed = (
        REQUESTED_CONFLICTS_PER_EPISODE if native_call_issued else 0
    )
    failure = {
        ...
        "requested_conflicts_consumed": requested_conflicts_consumed,
        "retry_authorized": False,
        "truth_key_bytes_read": False,
        **_native_failure(exc),
    }
    ...
    episode = {
        ...
        "native_call_issued": native_call_issued,
        "native_calls_consumed": native_calls_consumed,
        "requested_conflicts": requested_conflicts_consumed,
...
def _activation_ledger_document(
    ...
    raise O1C78RunError("residency activation ledger differs")
...
def _validate_call_window_executable(
    ...
    if observed != binding:
        raise O1C78RunError(f"native executable changed {when} call")
...
def execute_stream(
    *,
    ...
    """Consume exactly local 0 / lineage 18, without replay or retry."""
    ...
    if (
        ...
        raise O1C78RunError("stream execution input differs")
    try:
        validate_activation_replay(prepared.state)
    except CausalResidencyError as exc:
        raise O1C78RunError("initial residency replay differs") from exc
    ...
    prefix_plan_path = capsule / "initial" / PREFIX_PLAN_BINARY_NAME
    if rank_path.read_bytes() != prepared.rank_source.serialized:
        raise O1C78RunError("rank-source capsule bytes differ")
    try:
        if parse_causal_frontier_plan(
            frontier_plan_path.read_bytes(),
            active_vault=prepared.state.active_projection,
        ) != prepared.frontier_plan:
            raise O1C78RunError("frontier-plan capsule bytes differ")
    except CausalFrontierError as exc:
        raise O1C78RunError("frontier-plan capsule bytes differ") from exc
    try:
        if parse_residual_polarity_staging_plan(
            staging_plan_path.read_bytes()
        ) != prepared.staging_plan:
            raise O1C78RunError("staging-plan capsule bytes differ")
    except ResidualPolarityStagingError as exc:
        raise O1C78RunError("staging-plan capsule bytes differ") from exc
    try:
        if parse_rescue_prefix_preemption_plan(
            prefix_plan_path.read_bytes(),
            active_vault=prepared.state.active_projection,
        ) != prepared.prefix_plan:
            raise O1C78RunError("prefix-plan capsule bytes differ")
    except RescuePrefixPreemptionError as exc:
        raise O1C78RunError("prefix-plan capsule bytes differ") from exc
    ...
    for local_ordinal, lineage_ordinal in zip(
        ...
        if reread_active.serialized != active_before.serialized:
            raise O1C78RunError("episode active input reread differs")
        intent = {
            ...
            "prefix_plan": prepared.prefix_plan.describe(),
            "prefix_plan_artifact": _artifact_row(
                prefix_plan_path, relative_to=capsule / "initial"
            ),
            "residency_before": state.describe(),
        ...
        try:
            ...
            stream_id = f"o1c78-episode-{local_ordinal:02d}"
            validated = _validated_episode_result(
                ...
                prefix_plan=prepared.prefix_plan,
                stream_id=stream_id,
            ...
            mechanism_activated = cast(
                bool, validated["qualified_prefix_mechanism"]
            )
            ...
            try:
                ...
                raise O1C78RunError("episode residency reprojection differs") from exc
            if (
            ...
            if reread_rollover.serialized != rollover.serialized:
                raise O1C78RunError("rollover chunk reread differs")
            ...
            prefix_evidence = _write_compressed_json(
                episode_dir / "prefix-preemption-reader.json.gz",
                validated["prefix_preemption"],
            )
            ...
            if recovered_active.serialized != next_state.active_projection.serialized:
                raise O1C78RunError("episode active output reread differs")
            occurrence_delta = {
            ...
            episode = {
                ...
                "prefix_preemption_reader_evidence": prefix_evidence,
                "frontier_activation": {
                ...
                "staging_activation": {
                    ...
                    "inherited_diagnostic_only": True,
                    "native_trace_sha256": validated["trace_sha256"],
                    "frozen_baseline_trace_sha256": validated[
                        "baseline_trace_sha256"
                    ],
                ...
                "prefix_preemption_activation": {
                    **dict(
                        cast(
                            Mapping[str, object],
                            validated["prefix_activation"],
                        )
                    ),
                    "trace_change_alone_is_science_gain": False,
                    "page_change_is_science_gain": False,
                    "inherited_reader_activity_is_science_gain": False,
                    "unit_activation_is_science_gain": False,
                    "safe_threshold_prunes": validated[
                        "safe_threshold_prunes"
                    ],
    ...
    if native_calls != MAXIMUM_NATIVE_SOLVER_CALLS or requested_total != (
        ...
        raise O1C78RunError("stream bounded call ledger differs")
    if safe_prune_total:
        ...
        reason = "qualified-prefix-preemption-without-predeclared-science-gain"
    else:
        classification = NO_ACTIVATION
        reason = "no-qualified-prefix-preemption-or-science-gain"
    return StreamOutcome(
        classification,
        reason,
        tuple(episodes),
...
def build_result(
    *,
    ...
    if (
        ...
        raise O1C78RunError("result input differs")
    return {
        ...
        "claim_boundary": {
            ...
            "qualified_prefix_preemption_activated": outcome.mechanism_activated,
        ...
        "next_action": (
            "Do not retry the frozen 11-row prefix or replay lineage 18. "
            "Carry any exact prune, clause, or verified model forward; "
            "otherwise choose a new target-free mechanism from this capsule."
        ),
    }
...
def write_recovery_source(capsule: Path, result: Mapping[str, object]) -> Path:
    ...
    if (
        ...
        raise O1C78RunError("pre-finalization recovery result differs")
...
def _markdown(result: Mapping[str, object]) -> bytes:
    ...
    return (
        ...
        "- Active frontier input: one fresh `K=256` Page 5\n"
        "- Reader inputs: sealed frontier, staging, and 11-row prefix plans\n"
...
def _manifest_payload(
    ...
    if capsule.is_symlink() or not capsule.is_dir():
        raise O1C78RunError("capsule manifest root differs")
    payloads: dict[str, bytes] = {}
    for path in capsule.rglob("*"):
        metadata = path.lstat()
        if stat.S_ISLNK(metadata.st_mode):
            raise O1C78RunError("capsule manifest contains a symlink")
        if stat.S_ISREG(metadata.st_mode):
            relative = path.relative_to(capsule).as_posix()
            if relative != "artifacts.sha256" and relative not in virtual:
                payloads[relative] = path.read_bytes()
        elif not stat.S_ISDIR(metadata.st_mode):
            raise O1C78RunError("capsule manifest contains a special file")
...
def finalize_capsule(
    ...
    if (
        ...
        raise O1C78RunError("terminal O1C78 publication already exists")
    ...
    for _ in range(16):
        ...
        raise O1C78RunError("persistent artifact ledger did not converge")
    result_payload = canonical_json_bytes(result)
    run_payload = _markdown(result)
    manifest, persistent = _manifest_payload(
        capsule, {"RUN.md": run_payload, "result.json": result_payload}
    ...
    if (
        ...
        raise O1C78RunError("persistent artifact byte budget differs")
    ...
    if authoritative.read_bytes() != result_payload:
        raise O1C78RunError("authoritative result reread differs")


def _existing_authoritative(path: Path) -> dict[str, object] | None:
    if not path.exists():
        return None
    value = dict(_read_json(path, "authoritative O1C78 result"))
    if value.get("schema") != RESULT_SCHEMA or value.get("attempt_id") != ATTEMPT_ID:
        raise O1C78RunError("authoritative O1C78 result differs")
    if canonical_json_bytes(value) != path.read_bytes():
        raise O1C78RunError("authoritative O1C78 result is not canonical")
    return value
...
def _republish_sealed_capsule(
    ...
    try:
        ...
        raise O1C78RunError("sealed capsule escapes lab") from exc
    if (
        ...
        raise O1C78RunError("sealed O1C78 capsule differs")
    payload = result_path.read_bytes()
    if canonical_json_bytes(result) != payload:
        raise O1C78RunError("sealed O1C78 result canonical bytes differ")
...
def load_config(
    ...
    """Load the complete frozen O1C78 contract without writing or calling."""

    lab = (root or lab_root()).resolve(strict=True)
    config_path = Path(path).resolve(strict=True)
    if not config_path.is_relative_to(lab):
        raise O1C78RunError("O1C78 config escapes the lab")
    config = dict(_read_json(config_path, "O1C78 config"))
    required_root = {
        "schema",
        "attempt_id",
        "parent",
    ...
    if (
        ...
        raise O1C78RunError("frozen O1C78 config fields differ")
    ...
    if (
        ...
        raise O1C78RunError("frozen O1C77 parent differs")
    preparation = _mapping(config["preparation"], "config preparation")
    if set(preparation) != {"directory", "manifest_sha256"}:
        raise O1C78RunError("prepared causal-residency config differs")
    ...
    if set(inputs) != required_inputs:
        raise O1C78RunError("frozen stream inputs differ")
    ...
    if (
        ...
        != SOURCE_PATHS["native_v16"]
        or _relative_contract(native.get("executable"), "native executable")
        != "build/o1c78/native-joint-score-sieve"
        or native.get("adapter_schema")
        != _native_v19.JOINT_SCORE_SIEVE_ADAPTER_SCHEMA
        or native.get("result_schema")
        != _native_v19.JOINT_SCORE_SIEVE_RESULT_SCHEMA
        ...
        or native.get("prefix_order_sha256") != PREFIX_ORDER_SHA256
        ...
        raise O1C78RunError("native rescue-prefix-preemption config differs")
    ...
    if (
        ...
        raise O1C78RunError("source freeze config differs")
    ...
    if native.get("expected_source_sha256") != expected_sources.get("native_v16"):
        raise O1C78RunError("native/source digest binding differs")
    gate = _mapping(
    ...
    if (
        ...
        raise O1C78RunError("target-free preflight config differs")
    ...
    if dict(budgets) != expected_budgets:
        raise O1C78RunError("frozen O1C78 budgets differ")
    return config
...
def _validate_initial_contract(prepared: PreparedStream) -> None:
    ...
    if facts != expected:
        raise O1C78RunError("frozen prepared rescue-prefix preemption differs")
    if (
        ...
        or prepared.frontier_plan.false_literal_count != 2_409
        or prepared.frontier_plan.true_literal_count != 0
        or prepared.frontier_plan.unassigned_literal_count != 29
        or prepared.frontier_plan.selected_clause_literal_count != 2_438
        ...
        != "6ab071e611809ee898e81d0659ff0736453dd390d26c739383826c94276ad086"
        or tuple(
            row.rank_index for row in prepared.staging_plan.intersections
        )
        != (28, 131, 224, 226, 235)
        or tuple(row.rank_index for row in prepared.staging_plan.overlays)
        != (224, 226)
        or prepared.prefix_plan.sha256 != PREFIX_PLAN_BINARY_SHA256
        or tuple(prepared.prefix_plan.prefix_literals) != PREFIX_LITERALS
        or prepared.prefix_plan.serialized_bytes != 44
        or prepared.prefix_plan.prefix_order_sha256 != PREFIX_ORDER_SHA256
    ):
        raise O1C78RunError("prepared three-plan prefix binding differs")
    validate_activation_replay(state)
    validate_causal_frontier_plan(
    ...
    validate_rescue_prefix_preemption_plan(
        prepared.prefix_plan,
        active_vault=state.active_projection,
        parent_staging_plan_sha256=prepared.staging_plan.sha256,
        baseline_trace_sha256=FROZEN_BASELINE_TRACE_SHA256,
        required_prefix_literals=PREFIX_LITERALS,
    )
    validate_o1c78_production_plan(prepared.prefix_plan)
...
def _validate_target_free_gate(
    ...
    if sha256_bytes(payload) != expected_sha256:
        raise O1C78RunError("target-free preflight digest differs")
    try:
        value = json.loads(payload)
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise O1C78RunError("target-free preflight JSON differs") from exc
    ...
    required_true = (
        ...
        "prefix_binary_round_trip_fixture_passed",
        "prefix_chronology_fixture_passed",
        "rescue_skip_rejection_fixture_passed",
    ...
    if (
        ...
        or row.get("prefix_plan") != prepared.prefix_plan.describe()
        ...
        raise O1C78RunError("target-free preflight contract differs")
    return dict(row)
...
def preflight(
    ...
    *,
    ...
    if (
        ...
        raise O1C78RunError("O1C77 parent artifact identity differs")
    ...
    for name in ("cnf", "potential", "grouping", "o1c73_config"):
        ...
        if observed != expected:
            raise O1C78RunError(f"frozen input {name} differs")
    ...
    for name, relative in paths.items():
        ...
        if observed != expected:
            raise O1C78RunError(f"source {name} differs")
    ...
    try:
        ...
        raise O1C78RunError("source commit binding cannot be established") from exc
    if require_commit_binding:
        try:
            ...
            if ancestor.returncode != 0:
                raise O1C78RunError("source freeze is not an execution ancestor")
            for name, relative in paths.items():
            ...
            raise O1C78RunError("source commit blob binding differs") from exc
        if dirty:
            raise O1C78RunError("execution worktree is not clean")
    try:
        disk_free = shutil.disk_usage(lab).free
    except OSError as exc:
        raise O1C78RunError("disk resource preflight failed") from exc
    if disk_free < MINIMUM_DISK_FREE_BYTES:
        raise O1C78RunError("disk resource preflight differs")
    return {
...
def _rebuild_initial_from_capsule(
    ...
    if not chunk_rows:
        raise O1C78RunError("recovery initial chunks differ")
    ...
    for index, raw in enumerate(chunk_rows):
        ...
        if row.get("chunk_index") != index:
            raise O1C78RunError("recovery initial chunk ordering differs")
        ...
        try:
            ...
            raise O1C78RunError("recovery initial chunk differs") from exc
        chunks.append(chunk)
    ...
    try:
        ...
        raise O1C78RunError("recovery initial attic differs") from exc
    ...
    if _read_json(relation_path, "recovery initial relations") != (
        ...
        raise O1C78RunError("recovery initial relations differ")
    ...
    try:
        ...
        raise O1C78RunError("recovery initial residency differs") from exc
    if (
        ...
        raise O1C78RunError("recovery initial residency artifacts differ")
    return state
...
def _recover_completed_episode(
    *,
    ...
    if journal != expected or journal.get("schema") != EPISODE_SCHEMA:
        raise O1C78RunError("recovery episode journal differs")
    ...
    if (
        ...
        or intent.get("prefix_plan") != prefix_plan.describe()
    ):
        raise O1C78RunError("recovery intent binding differs")
    active_path = _validate_artifact_row(
    ...
    if (
        ...
        raise O1C78RunError("recovery active input differs")
    ...
    try:
        ...
        raise O1C78RunError("recovery frontier plan differs") from exc
    if recovered_plan != frontier_plan:
        raise O1C78RunError("recovery frontier plan differs")
    ...
    try:
        ...
        raise O1C78RunError("recovery staging plan differs") from exc
    if (
        recovered_staging != staging_plan
        or recovered_staging.parent_frontier_plan_sha256 != frontier_plan.sha256
    ):
        raise O1C78RunError("recovery staging plan differs")
    prefix_path = _validate_artifact_row(
        capsule / "initial",
        intent.get("prefix_plan_artifact"),
        "recovery prefix plan",
    )
    try:
        recovered_prefix = parse_rescue_prefix_preemption_plan(
            prefix_path.read_bytes(), active_vault=active
        )
    except RescuePrefixPreemptionError as exc:
        raise O1C78RunError("recovery prefix plan differs") from exc
    if recovered_prefix != prefix_plan:
        raise O1C78RunError("recovery prefix plan differs")
    ...
    try:
        parsed = parse_vault_telemetry(
            telemetry_payload,
            stream_id=f"o1c78-episode-{local:02d}",
            expected_sha256=sha256_bytes(telemetry_payload),
        )
    except CausalAtticError as exc:
        raise O1C78RunError("recovery occurrence ledger differs") from exc
    if (
        parsed.input_identity != active.identity
        or parsed.input_vault_sha256 != active.sha256
        or parsed.input_clause_count != active.clause_count
        ...
        raise O1C78RunError("recovery telemetry input differs")
    ...
    prefix_preemption = _read_compressed_json(
        episode_dir,
        expected.get("prefix_preemption_reader_evidence"),
        "recovery prefix-preemption reader",
    )
    status = _nonnegative_int(expected.get("status"), "recovery native status")
    if status not in (0, 10, 20):
        raise O1C78RunError("recovery native status differs")
    work = _mapping(expected.get("work"), "recovery work")
    try:
        stats = validate_vault_soft_conflict_ledger(work)
    except Exception as exc:
        raise O1C78RunError("recovery conflict ledger differs") from exc
    ...
    if (
        ...
        or expected.get("native_call_issued") is not True
        ...
        raise O1C78RunError("recovery native episode ledger differs")
    raw_model = native_raw.get("key_model_hex")
    if status == 10:
        if not isinstance(raw_model, str) or len(raw_model) != 64:
            raise O1C78RunError("recovery public model encoding differs")
        try:
            key_model: bytes | None = bytes.fromhex(raw_model)
        except ValueError as exc:
            raise O1C78RunError("recovery public model encoding differs") from exc
    else:
        key_model = None
    _validate_archived_v19_provenance(
        raw=native_raw,
    ...
    if expected.get("frontier_activation") != expected_activation:
        raise O1C78RunError("recovery frontier activation differs")
    effective_returns = _nonnegative_int(
        staging.get("overlay_effective_returns"),
    ...
    if (
        ...
        raise O1C78RunError("recovery staging activation differs")
    prefix_activation = _validate_prefix_activation(
        prefix_preemption=prefix_preemption,
        prefix_plan=prefix_plan,
        trace_sha256=trace_sha256,
    )
    expected_prefix_activation = {
        **prefix_activation,
        "trace_change_alone_is_science_gain": False,
        "page_change_is_science_gain": False,
        "inherited_reader_activity_is_science_gain": False,
    ...
    if (
        expected.get("prefix_preemption_activation")
        != expected_prefix_activation
    ):
        raise O1C78RunError("recovery prefix activation differs")
    ...
    if (
        ...
        raise O1C78RunError("recovery native/public provenance differs")
    ...
    for occurrence in parsed.occurrences:
        ...
        if key not in globally_known:
            if occurrence.classification != "new":
                raise O1C78RunError("recovery global novelty differs")
            globally_known.add(key)
            expected_novel.append(occurrence.clause)
    if chunk.clauses != tuple(expected_novel):
        raise O1C78RunError("recovery rollover population differs")
    try:
        next_state = advance_causal_residency(
            state,
            chunk=chunk,
        ...
        raise O1C78RunError("recovery residency reprojection differs") from exc
    consumed_science_inputs = science_inputs | {active.sha256}
    if next_state.active_projection.sha256 in consumed_science_inputs:
        raise O1C78RunError("recovery residency repeats a science input")
    ...
    if (
        ...
        or occurrence_delta.get("stream_id") != f"o1c78-episode-{local:02d}"
        or occurrence_delta.get("occurrence_count") != len(parsed.occurrences)
        or occurrence_delta.get("records") != expected_records
    ):
        raise O1C78RunError("recovery occurrence delta differs")
    ...
    if (
        ...
        raise O1C78RunError("recovery residency boundary differs")
...
def _read_recovery_source(
    ...
    if recovery_source is None:
        ...
        if canonical_json_bytes(source) != source_path.read_bytes():
            raise O1C78RunError("pre-finalization recovery source bytes differ")
    else:
        source = _read_json(Path(recovery_source), "pre-finalization recovery source")
        if canonical_json_bytes(source) != source_path.read_bytes():
            raise O1C78RunError("pre-finalization recovery source path differs")
    ...
    if (
        ...
        raise O1C78RunError("pre-finalization recovery source contract differs")
...
def recover_publication(
    ...
    """Replay sealed O1C78 sidecars without native or verification calls."""
    ...
    if (
        ...
        raise O1C78RunError("recovery invocation differs")
    invocation_sha = _sha256_file(invocation_path)
    ...
    if not synthetic_fixture:
        ...
        if (
            ...
            or native_build.get("frontier_plan_sha256")
            != FRONTIER_PLAN_BINARY_SHA256
            or native_build.get("staging_plan_sha256")
            != STAGING_PLAN_BINARY_SHA256
            or native_build.get("prefix_plan_sha256")
            != PREFIX_PLAN_BINARY_SHA256
            or native_build.get("prefix_order_sha256")
            != PREFIX_ORDER_SHA256
            ...
            or invocation.get("frontier_plan")
            != frozen_preflight.get("frontier_plan")
            or invocation.get("staging_plan")
            != frozen_preflight.get("staging_plan")
            or invocation.get("prefix_plan")
            != frozen_preflight.get("prefix_plan")
            or _mapping(
                _mapping(
            ...
            != frozen_preflight.get("prepared_manifest_sha256")
            or bindings.get("frontier_plan_sha256")
            != native_build.get("frontier_plan_sha256")
            or bindings.get("staging_plan_sha256")
            != native_build.get("staging_plan_sha256")
            or bindings.get("prefix_plan_sha256")
            != native_build.get("prefix_plan_sha256")
            or bindings.get("prefix_order_sha256")
            != native_build.get("prefix_order_sha256")
        ):
            raise O1C78RunError("publication recovery freeze differs")
    ...
    try:
        ...
        raise O1C78RunError("recovery initial frontier plan differs") from exc
    if (
        ...
        raise O1C78RunError("recovery initial frontier plan differs")
    staging_path = _validate_artifact_row(
        capsule / "initial",
    ...
    try:
        ...
        raise O1C78RunError("recovery initial staging plan differs") from exc
    if (
        ...
        raise O1C78RunError("recovery initial staging plan differs")
    prefix_path = _validate_artifact_row(
        capsule / "initial",
        initial_rows.get("prefix_plan"),
        "recovery initial prefix plan",
    )
    prefix_document_path = _validate_artifact_row(
        capsule / "initial",
        initial_rows.get("prefix_plan_document"),
        "recovery initial prefix plan document",
    )
    try:
        prefix_plan = parse_rescue_prefix_preemption_plan(
            prefix_path.read_bytes(), active_vault=state.active_projection
        )
        validate_rescue_prefix_preemption_plan(
            prefix_plan,
            active_vault=state.active_projection,
        ...
        raise O1C78RunError("recovery initial prefix plan differs") from exc
    if (
        invocation.get("prefix_plan") != prefix_plan.describe()
        or _read_json(
            prefix_document_path, "recovery prefix plan document"
        ).get("plan")
        != prefix_plan.describe()
    ):
        raise O1C78RunError("recovery initial prefix plan differs")
    episodes = _sequence(source.get("episodes"), "recovery episodes")
    if len(episodes) > MAXIMUM_NATIVE_SOLVER_CALLS:
        raise O1C78RunError("recovery episode count differs")
    science_inputs: set[str] = set(INHERITED_SCIENCE_INPUT_SHA256)
    stopped = False
    for index, raw_episode in enumerate(episodes):
        ...
        if (
            ...
            raise O1C78RunError("recovery episode schedule differs")
        if episode.get("completed") is True:
            ...
            native_call_issued = failure.get("native_call_issued")
            if not isinstance(native_call_issued, bool):
                raise O1C78RunError("recovery failed call state differs")
            expected_native_calls = int(native_call_issued)
            expected_requested_conflicts = (
                REQUESTED_CONFLICTS_PER_EPISODE
                if native_call_issued
                else 0
            )
            if (
                ...
                != staging_plan.describe()
                or failed_intent.get("prefix_plan")
                != prefix_plan.describe()
                ...
                or _validate_artifact_row(
                    capsule / "initial",
                    failed_intent.get("prefix_plan_artifact"),
                    "recovery failed prefix plan",
                ).read_bytes()
                != prefix_plan.serialized
                ...
                or (
                    failure.get("native_result_returned") is True
                    and not native_call_issued
                )
                ...
                or episode.get("native_call_issued") is not native_call_issued
                or episode.get("native_calls_consumed")
                != expected_native_calls
                ...
                raise O1C78RunError("recovery failed episode differs")
            stopped = True
        else:
            raise O1C78RunError("recovery episode completion differs")
    ...
    derived_native_calls = 0
    derived_requested_conflicts = 0
    ...
    for raw_episode in episodes:
        ...
        episode_native_calls = _nonnegative_int(
            episode.get("native_calls_consumed"),
            "recovery episode native calls",
        )
        episode_requested_conflicts = _nonnegative_int(
            episode.get("requested_conflicts"),
            "recovery episode requested conflicts",
        )
        if (
            episode_native_calls not in (0, 1)
            or episode_requested_conflicts
            != episode_native_calls * REQUESTED_CONFLICTS_PER_EPISODE
        ):
            raise O1C78RunError("recovery episode call ledger differs")
        derived_native_calls += episode_native_calls
        derived_requested_conflicts += episode_requested_conflicts
        ...
        activation = _mapping(
            episode.get("prefix_preemption_activation"),
            "recovery conclusion prefix activation",
        )
        if not isinstance(
            ...
            raise O1C78RunError("recovery prefix conclusion differs")
    ...
    if derived_classification is None:
        if len(episodes) != MAXIMUM_NATIVE_SOLVER_CALLS:
            raise O1C78RunError("recovery unterminated stream differs")
        if derived_safe_prunes:
            ...
            derived_stop_reason = (
                "qualified-prefix-preemption-without-predeclared-science-gain"
            )
        else:
            derived_classification = NO_ACTIVATION
            derived_stop_reason = "no-qualified-prefix-preemption-or-science-gain"
    ...
    if (
        ...
        or resources.get("native_solver_calls") != derived_native_calls
        or resources.get("requested_conflicts")
        != derived_requested_conflicts
        ...
        or claim_boundary.get("qualified_prefix_preemption_activated")
        is not derived_mechanism_activated
        or claim_boundary.get("baseline_trace_changed")
        is not derived_baseline_trace_changed
    ):
        raise O1C78RunError("publication recovery conclusion differs")
...
def run(config_path: str | Path = CONFIG_RELATIVE) -> dict[str, object]:
    """Execute, recover, or return the frozen one-call prefix attempt."""
    ...
    if existing_capsules:
        if len(existing_capsules) != 1:
            raise O1C78RunError("multiple existing O1C78 capsules block replay")
        ...
        if not (capsule / RECOVERY_SOURCE_NAME).is_file():
            raise O1C78RunError(
                "partial O1C78 capsule without pre-finalization recovery "
                "source blocks replay"
            )
        if any((capsule / name).exists() for name in ("RUN.md", "result.json")):
            raise O1C78RunError(
                "partially published O1C78 capsule requires manual recovery"
            )
    ...
    if executable_binding != preflight_row.get("native_executable"):
        raise O1C78RunError("post-preflight native executable differs")
    ...
    _atomic_json(
        capsule / "native-build.json",
        {
            ...
            "prefix_plan_sha256": native["prefix_plan_sha256"],
            "prefix_order_sha256": native["prefix_order_sha256"],
    ...
    _atomic_create(
        ...
        (
            ...
            "o1_crypto_lab.o1c78_apple8_rescue_prefix_preemption_run run "
    ...
    bindings: dict[str, object] = {
        ...
        "prefix_order_sha256": prepared.prefix_plan.prefix_order_sha256,
    ...
    def invoke(
        ...
        if (
            ...
            or _sha256_file(prefix_plan) != prepared.prefix_plan.sha256
        ):
            raise O1C78RunError("native residency invocation identity differs")
        observed_executable = validate_native_executable(
            executable,
            expected_sha256=cast(str, native["expected_executable_sha256"]),
        )
        if observed_executable != executable_binding:
            raise O1C78RunError("native executable changed in call window")
        result = _native_v19.run_joint_score_sieve(
        ...
        if (
            ...
            raise O1C78RunError("native executable changed during call window")
        return result
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Preflight, run, or recover O1C78's prefix call"
    )
    ...
    try:
        if args.command == "preflight":
            ...
            if not capsule.is_relative_to(root / "runs"):
                raise O1C78RunError("recovery capsule escapes run root")
            value = recover_publication(capsule)
            finalize_capsule(capsule, root / RESULT_RELATIVE, value)
        sys.stdout.buffer.write(canonical_json_bytes(value))
        return 0
    except O1C78RunError as exc:
        print(f"O1C78: {exc}", file=sys.stderr)
        return 2
