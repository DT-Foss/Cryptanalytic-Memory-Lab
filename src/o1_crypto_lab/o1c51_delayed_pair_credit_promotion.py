"""Prospective W11 promotion screen for frozen delayed pair credit."""
...
ATTEMPT_ID = "O1C-0051"
RESULT_SCHEMA = "o1-256-delayed-pair-credit-promotion-result-v1"
...
RUNNER_SOURCE = Path("src/o1_crypto_lab/o1c51_delayed_pair_credit_promotion.py")
...
_CALL_PLAN = (
    ("qualification", "delayed", "primary", "post-reveal-w11"),
    ("matched-w11", "static", "primary", "post-reveal-w11"),
    ("matched-w11", "delayed", "key_rotated", "post-reveal-w11"),
    ("matched-w11", "delayed", "clause_rotated", "post-reveal-w11"),
    ("full256-promotion", "delayed", "primary", "full256"),
    ("full256-promotion", "delayed", "key_rotated", "full256"),
    ("full256-promotion", "delayed", "clause_rotated", "full256"),
)


class O1C51PromotionError(RuntimeError):
    """The frozen delayed-credit promotion boundary was violated."""


def _mapping(value: object, field: str) -> Mapping[str, object]:
    if not isinstance(value, Mapping):
        raise O1C51PromotionError(f"{field} differs")
    return value
...
def _json_object(payload: bytes, field: str) -> dict[str, object]:
    try:
        ...
        raise O1C51PromotionError(f"{field} JSON differs") from exc
    if not isinstance(value, dict):
        raise O1C51PromotionError(f"{field} JSON root differs")
    return value
...
def _hashed_json(path: Path, expected_sha256: str, field: str) -> dict[str, object]:
    try:
        ...
        raise O1C51PromotionError(f"{field} read failed") from exc
    if hashlib.sha256(payload).hexdigest() != expected_sha256:
        raise O1C51PromotionError(f"{field} hash differs")
...
def _commit_bound_bytes(
    ...
    try:
        ...
        raise O1C51PromotionError(f"{field} is not bound to source commit") from exc
    if completed.stdout != payload:
        raise O1C51PromotionError(f"{field} differs from source commit")
...
def _call_descriptor(ordinal: int) -> dict[str, object]:
    ...
    return {
        ...
        "authorization": (
            ...
            if ordinal == 1
            else "exact-delayed-primary-w11-only"
        ),
    }


def call_ledger(delayed_primary_w11_exact: bool) -> dict[str, object]:
    """Return the complete deterministic call ledger for either gate branch."""

    exact = _boolean(delayed_primary_w11_exact, "delayed_primary_w11_exact")
    ...
    return {
        "policy": (
            "call-1-delayed-primary-w11;calls-2-through-7-only-after-exact-call-1"
        ),
        "planned_calls": plan,
...
def evaluate_gate(
    *,
    ...
    """Authorize follow-ups only from an exact delayed-primary W11 result."""

    if not isinstance(status, str):
        raise O1C51PromotionError("delayed primary W11 status differs")
    ...
    return {
        "passed": exact,
        "selected_tier": (
            "exact-delayed-primary-w11" if exact else "no-exact-delayed-primary-w11"
        ),
        "delayed_primary_w11_exact": exact,
        "status": status,
...
def _exact_mapping(
    ...
    if set(value) != set(names):
        raise O1C51PromotionError(f"{field} arm set differs")
...
def evaluate_promotion(
    *,
    ...
    delayed_full256_exact_by_arm: Mapping[str, bool],
    ...
    delayed_full = _exact_mapping(
        delayed_full256_exact_by_arm, PAIR_ARMS, "delayed Full256 exact"
    )
    static_full = _exact_mapping(
        static_full256_exact_by_arm, PAIR_ARMS, "static Full256 exact"
    )
    if not w11_exact["delayed_primary"]:
        raise O1C51PromotionError(
            "promotion evidence exists without exact delayed-primary W11"
        )
    primary_conflicts = w11_conflicts["delayed_primary"]
    ...
    any_delayed_full = any(delayed_full.values())
    strict_primary_full = bool(
        delayed_full["primary"]
        and not any(delayed_full[name] for name in ROTATION_ARMS)
        and not any(static_full.values())
    )
    if strict_primary_full:
        selected_tier = "strict-delayed-primary-full256"
    elif delayed_full["primary"]:
        selected_tier = "delayed-primary-full256-without-strict-margin"
    elif any(delayed_full[name] for name in ROTATION_ARMS):
        selected_tier = "delayed-control-full256"
...
def _classification(
    ...
    if gate.get("passed") is not True:
        return "DELAYED_PAIR_CREDIT_NO_EXACT_W11_CLOSE"
    if promotion is None:
        raise O1C51PromotionError("promotion summary is absent after W11 pass")
    if promotion.get("strict_primary_full256_recovery") is True:
        return "PUBLIC_INPUT_CONSUMED_DELAYED_PRIMARY_STRICT_FULL256_RECOVERY"
    delayed_full = _mapping(
        promotion.get("delayed_full256_exact_by_arm"), "delayed Full256 promotion"
    )
    if delayed_full.get("primary") is True:
        return "DELAYED_PAIR_CREDIT_PRIMARY_FULL256_WITHOUT_STRICT_MARGIN"
    if any(delayed_full.get(name) is True for name in ROTATION_ARMS):
        return "DELAYED_PAIR_CREDIT_CONTROL_FULL256_RECOVERY"
    if promotion.get("primary_specific_w11") is True:
        return "DELAYED_PAIR_CREDIT_PRIMARY_SPECIFIC_W11_EXPANSION"
    return "DELAYED_PAIR_CREDIT_EXACT_W11_WITHOUT_PRIMARY_SPECIFICITY"


def _model_honors_fixed_spins(model: bytes, fixed_spins: Mapping[int, int]) -> bool:
    if len(model) != 32:
        raise O1C51PromotionError("search model width differs")
    bits = key_bits(model)
    return all(
...
def _search_row(
    *,
    ...
    mechanism: str,
    arm: str,
    result: DelayedPairCreditSearchResult | PairEnvelopeSearchResult,
    ...
    if result.conflict_limit != CONFLICT_LIMIT:
        raise O1C51PromotionError(f"conflict cap differs for {name}")
    ...
    if result.status_name == "SAT" and not publicly_verified:
        raise O1C51PromotionError(f"SAT model failed public verification for {name}")
    ...
    if isinstance(result, DelayedPairCreditSearchResult):
        row["delayed"] = dict(result.delayed)
    else:
        row["pair_envelope"] = dict(result.pair_envelope)
    if residual is not None:
        if fixed is None or prefix is None:
            raise O1C51PromotionError(f"residual annotation differs for {name}")
        honors_prefix = bool(
            model is not None and _model_honors_fixed_spins(model, fixed)
        )
        if model is not None and not honors_prefix:
            raise O1C51PromotionError(f"model violates truth prefix for {name}")
        row.update(
            {
...
def _validate_delayed_contract(
    row: Mapping[str, object],
    *,
    decision_sha256: str,
    pair_count: int,
) -> None:
    delayed = _mapping(row.get("delayed"), "delayed row")
    state = _mapping(delayed.get("state"), "delayed state")
    if (
        delayed.get("decision_rule") != DELAYED_PAIR_CREDIT_DECISION_RULE
        or delayed.get("decision_scope") != PAIR_ENVELOPE_DECISION_SCOPE
        or delayed.get("decision_variables_sha256") != decision_sha256
        or _integer(delayed.get("pair_count"), "delayed pair count") != pair_count
        or _integer(delayed.get("group_width"), "delayed group width") != 2
        or _integer(state.get("bounded_state_bytes"), "delayed bounded state bytes")
        != DELAYED_PAIR_CREDIT_STATE_BYTES
    ):
        raise O1C51PromotionError("frozen delayed native contract differs")
...
def _validate_static_contract(
    ...
    *,
    ...
    if (
        ...
        raise O1C51PromotionError("frozen static native contract differs")
...
def _frozen_static_full256_rows(
    *,
    ...
    if (
        ...
        raise O1C51PromotionError("frozen static Full256 identity differs")
    candidates = [
        row
    ...
    if len(candidates) != len(PAIR_ARMS) or [
        ...
        raise O1C51PromotionError("frozen static Full256 arm order differs")
    primary = next(row for row in candidates if row.get("name") == "primary")
    if o1c49.get("static_full256_baseline") != primary:
        raise O1C51PromotionError("O1C-0048/O1C-0049 static primary differs")
    ...
    for row in candidates:
        ...
        if (
            ...
            raise O1C51PromotionError(f"frozen static Full256 row differs for {name}")
...
def _next_action(
    ...
    if gate.get("passed") is not True:
        return (
            "close delayed credit on the frozen disjoint pair groups after one W11 call"
        )
    if promotion is None:
        raise O1C51PromotionError("promotion summary is absent after W11 pass")
    if promotion.get("strict_primary_full256_recovery") is True:
        return "report the consumed public-input strict-primary Full256 recovery"
    if promotion.get("any_delayed_full256_recovery") is True:
        return "retain the consumed public-input Full256 result with its control classification"
    return "retain the W11 expansion and close this unchanged scheduler at its measured ceiling"
...
def run(
    ...
    for name, payload in source_bytes.items():
        if hashlib.sha256(payload).hexdigest() != expected.get(name):
            raise O1C51PromotionError(f"frozen O1C-0048 source differs: {name}")
    ...
    if output != (root / RESULT_RELATIVE).resolve():
        raise O1C51PromotionError("result path differs")
    if output.exists():
        raise O1C51PromotionError("authoritative result already exists")
    ...
    if (
        ...
        raise O1C51PromotionError("consumed public target differs")
    ...
    try:
        ...
        raise O1C51PromotionError("revealed key differs") from exc
    ...
    if (
        ...
        raise O1C51PromotionError("revealed key fails target/public verification")
    ...
    if plans["primary"].describe() != _EXPECTED_PRIMARY_PLAN:
        raise O1C51PromotionError("frozen primary pair plan differs")
    ...
    if (
        ...
        or old50_architecture.get("delayed_source_sha256")
        != hashlib.sha256(implementation_bytes["delayed_source"]).hexdigest()
        or old50_architecture.get("delayed_adapter_sha256")
        != hashlib.sha256(implementation_bytes["delayed_adapter"]).hexdigest()
        or _mapping(o1c50.get("static_w10_baseline"), "O1C-0050 static W10").get(
            "model_sha256"
        )
        != truth_sha256
    ):
        raise O1C51PromotionError("frozen O1C-0050 mechanism boundary differs")
    ...
    if old50_residual != sorted(key_order[:10]):
        raise O1C51PromotionError("frozen O1C-0050 residual order differs")
    residual = set(key_order[:RESIDUAL_WIDTH])
    if (
        len(residual) != RESIDUAL_WIDTH
        or sorted(residual) != _EXPECTED_W11_RESIDUAL_VARIABLES
    ):
        raise O1C51PromotionError("W11 residual coordinates differ")
    fixed = {
        variable: spin
    ...
    w11_rotations: list[dict[str, object]] = []
    delayed_full256_rows: list[dict[str, object]] = []
    static_full256_rows: list[dict[str, object]] = []
    static_full_exact: dict[str, bool] = {}
    static_full256_provenance: dict[str, list[str]] = {}
    baseline_hashes = {"O1C-0050": O1C50_RESULT_SHA256}
    all_executed_rows: list[dict[str, object]] = []
    with tempfile.TemporaryDirectory(prefix="o1c51-") as temporary:
        workspace = Path(temporary)
        delayed_executable = workspace / "cadical-o1-delayed-pair-credit"
        static_executable = workspace / "cadical-o1-static-pair-envelope"
        delayed_build = build_native_delayed_pair_credit_search(
            source=implementation_paths["delayed_source"],
            output=delayed_executable,
        )
        static_build = None
        ...
        if (
            ...
            raise O1C51PromotionError("public Full256 instance differs")
        ...
        if prefix != _EXPECTED_W11_PREFIX:
            raise O1C51PromotionError("frozen W11 prefix differs")
        ...
        for name in PAIR_ARMS:
            ...
            decision_hashes[name] = write_delayed_pair_credit_decision_variables(
        ...
        delayed_primary_w11 = run_delayed(
            "primary",
            residual_cnf,
            residual_call=True,
            name="delayed_primary_w11",
        )
        executed_calls.append(_call_descriptor(1))
        gate = evaluate_gate(
            status=str(delayed_primary_w11["status"]),
            publicly_verified=delayed_primary_w11["model_publicly_verified"] is True,
            truth_exact=delayed_primary_w11["model_truth_exact"] is True,
            matches_truth_prefix=delayed_primary_w11["model_matches_truth_fixed_prefix"]
            is True,
        )
        ...
        if primary_w11_exact:
            ...
            for ordinal, arm in enumerate(ROTATION_ARMS, start=3):
                row = run_delayed(
                    ...
                    residual_cnf,
                    residual_call=True,
                    name=f"delayed_{arm}_w11",
                )
                w11_rotations.append(row)
            ...
            for ordinal, arm in enumerate(PAIR_ARMS, start=5):
                row = run_delayed(
                    ...
                    public_cnf,
                    residual_call=False,
                    name=f"delayed_{arm}_full256",
                )
                delayed_full256_rows.append(row)
                executed_calls.append(_call_descriptor(ordinal))

        expected_ledger = call_ledger(primary_w11_exact)
        if executed_calls != expected_ledger["executed_calls"]:
            raise O1C51PromotionError("native solver call order differs")

        promotion: dict[str, object] | None = None
        if primary_w11_exact:
            if w11_static_primary is None:
                raise O1C51PromotionError("matched static W11 row is absent")
            w11_rotation_by_arm = {str(row["arm"]): row for row in w11_rotations}
            full_by_arm = {str(row["arm"]): row for row in delayed_full256_rows}
            if set(w11_rotation_by_arm) != set(ROTATION_ARMS) or set(
                full_by_arm
            ) != set(PAIR_ARMS):
                raise O1C51PromotionError("promotion result arm set differs")
            w11_by_name = {
                "delayed_primary": delayed_primary_w11,
                "static_primary": w11_static_primary,
                "delayed_key_rotated": w11_rotation_by_arm["key_rotated"],
                "delayed_clause_rotated": w11_rotation_by_arm["clause_rotated"],
            }
        ...
        if elapsed > maximum_wall or peak_rss > MAXIMUM_PEAK_RSS_BYTES:
            raise O1C51PromotionError("promotion resource boundary exceeded")
        ...
        result: dict[str, object] = {
            ...
            "claim_level": "CONSUMED_POST_REVEAL_PROMOTION_SCREEN",
            "hypothesis": (
                "the unchanged O1C-0050 delayed owner rule expands exact "
            ...
            "boundary": {
                ...
                "first_native_call_is_delayed_primary_w11": True,
                "followups_require_exact_delayed_primary_w11": True,
                ...
                "same_delayed_mechanism_as_o1c50": True,
            ...
            "architecture": {
                ...
                "delayed_native_build": delayed_build.describe(),
                "static_native_build": (
            ...
            "w11_search": {
                "delayed_primary": delayed_primary_w11,
                "static_primary": w11_static_primary,
                "delayed_rotations": w11_rotations,
            },
            "delayed_full256": delayed_full256_rows,
    ...
    if config_file.read_bytes() != config_bytes:
        raise O1C51PromotionError("config changed during promotion")
    for name, before in source_bytes.items():
        if paths[name].read_bytes() != before:
            raise O1C51PromotionError(f"source changed during promotion: {name}")
    for name, before in implementation_bytes.items():
        if implementation_paths[name].read_bytes() != before:
            raise O1C51PromotionError(
                f"implementation changed during promotion: {name}"
            )
    baseline_paths = {
        "o1c48": o1c48_path,
        "o1c49": o1c49_path,
    ...
    for name, before in baseline_bytes.items():
        ...
        if path.read_bytes() != before:
            raise O1C51PromotionError(
                f"frozen baseline changed during promotion: {name}"
            )
    if output.exists():
        raise O1C51PromotionError("authoritative result appeared during promotion")
