"""Burn-on-intent runner for the single O1C-0086 Page-11 call.

The runner has one scientific side effect: after an immutable intent is
persisted it may invoke native v22 once, with seed zero and 128 requested
...
the published, sealed O1C-0086 causal-rollover bundle; it never regenerates
...
from . import joint_score_sieve_v25 as _native_v25
...
from .o1c86_page11_causal_rollover_prepare import (
...
NATIVE_BUILD_SCHEMA = "o1-256-o1c86-native-v22-build-v1"
...
CONFIG_RELATIVE = Path("configs/o1c86_apple8_parent_centered_continuation_v1.json")
RESULT_RELATIVE = Path(
    "research/O1C0086_APPLE8_PARENT_CENTERED_CONTINUATION_RESULT_20260720.json"
)
CAPSULE_SUFFIX = "O1C-0086_apple8-parent-centered-continuation-v1"

LOCAL_EPISODE_ORDINAL = 0
LINEAGE_CALL_ORDINAL = 24
SEED = 0
...
PUBLISHED_PREPARATION_RELATIVE = Path(
    "research/o1c86_page11_causal_rollover_seed_20260720"
)
...
NATIVE_V22_USAGE = (
    b"usage: cadical_o1_joint_score_sieve_v22 --cnf PATH "
...
PUBLISHED_ARTIFACT_SEALS: Mapping[str, tuple[int, str, str]] = {
    ...
    NEW_CHUNK_NAME: (
        268_803,
        "7689d51af4a51ea2539bf7f84ae52a97ae625229aa464f9056733b7147fd12df",
        "immutable-all-new-lineage-23-evidence-chunk",
    ),
    RECEIPT_NAME: (
        PRIORITY_RECEIPT_BYTES,
        PRIORITY_RECEIPT_SHA256,
        "canonical-o1c85-evolved-priority-state-receipt",
    ),
    OCCURRENCES_NAME: (
        296_668,
    ...
    ACTIVE_PROJECTION_NAME: (
        PAGE11_SERIALIZED_BYTES,
        PAGE11_SHA256,
        "fresh-lineage-24-page11-science-input",
    ),
    RESIDENCY_NAME: (
        39_207,
...
SOURCE_PATHS = {
    "runner": "src/o1_crypto_lab/o1c86_apple8_parent_centered_continuation_run.py",
    "runner_tests": "tests/test_o1c86_apple8_parent_centered_continuation_run.py",
    "adapter_v25": "src/o1_crypto_lab/joint_score_sieve_v25.py",
    "adapter_v25_tests": "tests/test_joint_score_sieve_v25.py",
    "page11_causal_rollover_preparation": "src/o1_crypto_lab/o1c86_page11_causal_rollover_prepare.py",
    "page11_causal_rollover_preparation_tests": "tests/test_o1c86_page11_causal_rollover_prepare.py",
    ...
    "native_v22": "native/cadical_o1_joint_score_sieve_v22.cpp",
    "native_v22_tests": "tests/test_o1c86_native_parent_centered_continuation_operator.py",
...
class O1C86RunError(RuntimeError):
...
def _mapping(value: object, field: str) -> Mapping[str, object]:
    if not isinstance(value, Mapping) or not all(isinstance(k, str) for k in value):
        raise O1C86RunError(f"{field} differs")
...
def _relative_text(value: object, field: str) -> str:
    ...
    if path.is_absolute() or not path.parts or ".." in path.parts:
        raise O1C86RunError(f"{field} escapes the lab")
...
def _lab_path(
    ...
    try:
        ...
        raise O1C86RunError(f"{field} cannot be resolved") from exc
    expected = (
    ...
    if (
        ...
        raise O1C86RunError(f"{field} is not a canonical lab path")
    return resolved


def _external_path(value: object, field: str, *, directory: bool = False) -> Path:
    if not isinstance(value, str) or not Path(value).is_absolute():
        raise O1C86RunError(f"{field} is not absolute")
    ...
    if stat.S_ISLNK(metadata.st_mode) or not expected:
        raise O1C86RunError(f"{field} is not canonical")
    return resolved
...
def _read_canonical_json(path: Path, field: str) -> Mapping[str, object]:
    ...
    try:
        ...
        raise O1C86RunError(f"{field} is not JSON") from exc
    if canonical_json_bytes(value) != payload:
        raise O1C86RunError(f"{field} is not canonical JSON")
    return value


def _atomic_create(path: Path, payload: bytes, *, mode: int = 0o444) -> None:
    if not isinstance(payload, bytes):
        raise O1C86RunError("atomic payload differs")
    ...
    try:
        ...
        if path.read_bytes() != payload:
            raise O1C86RunError(f"atomic write verification failed for {path.name}")
        path.chmod(mode)
        ...
        raise O1C86RunError(f"artifact already exists: {path.name}") from exc
...
def load_config(path: str | Path, *, root: Path | None = None) -> dict[str, object]:
    ...
    if not config_path.is_relative_to(lab) or config_path.is_symlink():
        raise O1C86RunError("config escapes the lab")
    config = dict(_read_canonical_json(config_path, "O1C86 config"))
    if _pending(config):
        raise O1C86RunError("config contains PENDING: " + ", ".join(_pending(config)))
    if (
        set(config)
        != {
            "schema",
        ...
        raise O1C86RunError("config fields differ")
    ...
    if (
        ...
        raise O1C86RunError("frozen O1C85 parent differs")
    ...
    if (
        ...
        raise O1C86RunError("frozen preparation source differs")
    ...
    if set(inputs) != {
        ...
        raise O1C86RunError("input fields differ")
    ...
    if (
        ...
        or native.get("adapter_schema") != _native_v25.JOINT_SCORE_SIEVE_ADAPTER_SCHEMA
        or native.get("result_schema") != _native_v25.JOINT_SCORE_SIEVE_RESULT_SCHEMA
        or native.get("page11_sha256") != PAGE11_SHA256
        or native.get("page11_bytes") != PAGE11_SERIALIZED_BYTES
    ...
    if dict(paths) != SOURCE_PATHS or set(digests) != set(SOURCE_PATHS):
        raise O1C86RunError("source inventory differs")
    ...
    if dict(budgets) != expected_budgets:
        raise O1C86RunError("budgets differ")
    return config
...
def _sibling_solver_pids() -> tuple[int, ...]:
    try:
        ...
        raise O1C86RunError("solver process census failed") from exc
    current = os.getpid()
    found: list[int] = []
...
def _validate_priority_receipt(payload: bytes, bank: bytes) -> Mapping[str, object]:
    try:
        ...
        raise O1C86RunError("priority state receipt is not JSON") from exc
    if canonical_json_bytes(receipt) != payload:
        raise O1C86RunError("priority state receipt is not canonical JSON")
    hexadecimal = receipt.get("bank_hex")
    try:
        receipt_bank = bytes.fromhex(cast(str, hexadecimal))
    except (TypeError, ValueError) as exc:
        raise O1C86RunError("priority state receipt bank differs") from exc
    if (
        receipt.get("schema") != "o1-256-o1c85-live-parent-centered-continuation-priority-state-v1"
        ...
        raise O1C86RunError("priority state receipt contract differs")
    return receipt
...
def _validate_occurrence_ledger(payload: bytes) -> frozenset[str]:
    ...
    if (
        ...
        raise O1C86RunError("published occurrence ledger contract differs")
    ...
    for ordinal, value in enumerate(records):
        ...
        if (
            ...
            raise O1C86RunError("published occurrence record differs")
    ...
    if set(by_index) != set(range(ATTIC_UNION_CLAUSE_COUNT)) or len(digests) != (
        ...
        raise O1C86RunError("published attic union digest inventory differs")
    return digests
...
def _validate_published_documents(artifacts: Mapping[str, bytes]) -> frozenset[str]:
    ...
    projection = _mapping(
        residency.get("current_projection"), "published Page-11 projection"
    )
    encoding = _mapping(projection.get("encoding_only"), "published Page-11 vault")
    ...
    if (
        ...
        or len(activation_entries) != 12
        or len(used) != 12
        or used[-1] != PAGE11_SHA256
        or last_activation.get("lineage_ordinal") != LINEAGE_CALL_ORDINAL
        or last_activation.get("active_sha256") != PAGE11_SHA256
        or last_activation.get("selected_clause_count") != PAGE11_CLAUSE_COUNT
        or residency.get("schema") != "o1-score-threshold-causal-residency-v1"
        or residency.get("active_limit") != PAGE11_ACTIVE_LIMIT
        or projection.get("lineage_ordinal") != LINEAGE_CALL_ORDINAL
        or projection.get("category_counts") != PAGE11_CATEGORY_COUNTS
        or encoding.get("sha256") != PAGE11_SHA256
        or encoding.get("clause_count") != PAGE11_CLAUSE_COUNT
        or encoding.get("literal_count") != PAGE11_LITERAL_COUNT
        or encoding.get("serialized_bytes") != PAGE11_SERIALIZED_BYTES
        or attic.get("chunk_count") != ATTIC_CHUNK_COUNT
        or residency_union.get("clause_count") != ATTIC_UNION_CLAUSE_COUNT
    ):
        raise O1C86RunError("published Page-11 residency contract differs")
    if (
        ...
        raise O1C86RunError("published supporting artifact contract differs")
...
def _validate_published_manifest(
    ...
    if (
        ...
        raise O1C86RunError("published preparation manifest identity differs")
    ...
    page11 = _mapping(manifest.get("page11"), "published Page-11")
    ...
    if dict(zero_call) != {
        ...
        raise O1C86RunError("published zero-call authorization differs")
    if (
        parent.get("attempt_id") != "O1C-0085"
        or parent.get("capsule_manifest_sha256") != PARENT_CAPSULE_MANIFEST_SHA256
        ...
        or parent.get("lineage23_burned") is not True
        or parent.get("page10_burned") is not True
        or parent.get("retry_or_replay_authorized") is not False
    ):
        raise O1C86RunError("published parent boundary differs")
    if (
        rollover.get("clause_count") != 23
        or rollover.get("chunk_sha256")
        != "7689d51af4a51ea2539bf7f84ae52a97ae625229aa464f9056733b7147fd12df"
        or rollover.get("all_occurrences_globally_novel_against_807_clause_attic")
        is not True
        or attic.get("chunk_count") != ATTIC_CHUNK_COUNT
        ...
        raise O1C86RunError("published rollover/attic boundary differs")
    if (
        page11.get("lineage_ordinal") != LINEAGE_CALL_ORDINAL
        or page11.get("active_limit") != PAGE11_ACTIVE_LIMIT
        or page11.get("active_sha256") != PAGE11_SHA256
        or page11.get("clause_count") != PAGE11_CLAUSE_COUNT
        or page11.get("literal_count") != PAGE11_LITERAL_COUNT
        or page11.get("serialized_bytes") != PAGE11_SERIALIZED_BYTES
        or page11.get("category_counts") != PAGE11_CATEGORY_COUNTS
        or page11.get("headroom") != PAGE11_HEADROOM
        or page11.get("fresh_identity") is not True
    ):
        raise O1C86RunError("published Page-11 seal differs")
    if (
        ...
        != "o1c85-live-continuation-bank-with-state-receipt"
        or bank.get("eligible_coordinate_count") != 255
        or bank.get("maximum_evolved_count") != 829
        ...
        raise O1C86RunError("published continuation bank seal differs")
    rows_raw = _mapping(manifest.get("artifacts"), "published artifact rows")
    if set(rows_raw) != set(PUBLISHED_ARTIFACT_SEALS):
        raise O1C86RunError("published artifact row inventory differs")
    ...
    for name, (size, digest, role) in PUBLISHED_ARTIFACT_SEALS.items():
        ...
        if (
            ...
            raise O1C86RunError("published artifact seal differs")
        rows[name] = row
...
def _load_published_preparation(
    ...
    try:
        ...
        raise O1C86RunError("published preparation inventory is unreadable") from exc
    if (
        manifest_path != directory / PREPARATION_MANIFEST_NAME
        or {path.name for path in children} != PREPARATION_ARTIFACT_NAMES
    ):
        raise O1C86RunError("published preparation inventory differs")
    ...
    for path in children:
        try:
            ...
            raise O1C86RunError("published preparation artifact is unreadable") from exc
        if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
            raise O1C86RunError("published preparation artifact type differs")
    ...
    if (
        ...
        raise O1C86RunError("published preparation manifest seal differs")
...
def _preflight_bundle(
    ...
    *,
    ...
    if (
        ...
        raise O1C86RunError("parent input SHA preflight differs")
    ...
    if (
        ...
        raise O1C86RunError("published preparation/receipt preflight differs")
    ...
    for name in ("cnf", "potential", "grouping", "o1c73_config"):
        ...
        if observed != inputs[f"{name}_sha256"]:
            raise O1C86RunError(f"input {name} SHA preflight differs")
    ...
    for name in SOURCE_PATHS:
        ...
        if observed != expected[name]:
            raise O1C86RunError(f"source {name} SHA preflight differs")
    ...
    if (
        ...
        or source_sha["native_v22"] != native["source_sha256"]
    ):
        raise O1C86RunError("native toolchain/source SHA preflight differs")
    ...
    if (
        ...
        raise O1C86RunError("Darwin/arm64/RAM/disk/process preflight differs")
    row = {
...
def _executable_identity(path: Path, *, relative_path: str) -> dict[str, object]:
    try:
        ...
        raise O1C86RunError("native executable is unreadable") from exc
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
        raise O1C86RunError("native executable type differs")
    size = path.stat().st_size
    if size <= 0:
        raise O1C86RunError("native executable is empty")
    return {
        "path": relative_path,
...
def _compile_native(
    *,
    ...
    library = _external_path(native["cadical_library"], "CaDiCaL library")
    source = _lab_path(root, native["source"], "native v22 source")
    if (
        _sha_file(source) != native["source_sha256"]
        or _sha_file(library) != native["cadical_library_sha256"]
    ):
        raise O1C86RunError("native build input changed after preflight")
    ...
    try:
        ...
        raise O1C86RunError("native v22 compilation could not run") from exc
    ...
    try:
        ...
        raise O1C86RunError("native v22 --help smoke could not run") from exc
    ...
    if (
        ...
        or smoke_stdout != NATIVE_V22_USAGE
        or smoke_stderr
        or after_smoke != before_smoke
    ):
        raise O1C86RunError("native v22 --help smoke seal differs")
...
def _stage_capsule(
    *,
    ...
    if final.exists() or stage.exists():
        raise O1C86RunError("timestamped capsule already exists")
...
def _rehash_call_inputs(
    *,
    ...
    if _sha_file(config_path) != preflight_row.get("config_sha256"):
        raise O1C86RunError("config changed before native call")
    ...
    for name in ("cnf", "potential", "grouping", "o1c73_config"):
        if (
            ...
            raise O1C86RunError(f"input {name} changed before native call")
    ...
    if prepared_rows.get(RECEIPT_NAME) != receipt_row:
        raise O1C86RunError("staged priority receipt seal differs")
    ...
    for name, value in expected_rows.items():
        ...
        if path.stat().st_size != row.get("serialized_bytes") or _sha_file(
            ...
            raise O1C86RunError("staged call artifact seal differs")
    ...
    if invocation_path.read_bytes() != canonical_json_bytes(invocation) or _sha_file(
        ...
        raise O1C86RunError("staged invocation/build seal differs")
    ...
    if (
        ...
        or smoke.get("stdout")
        != {
            "serialized_bytes": len(NATIVE_V22_USAGE),
            "sha256": sha256_bytes(NATIVE_V22_USAGE),
        }
        ...
        raise O1C86RunError("staged native build/smoke seal differs")
...
def _native_stdout(result: object) -> bytes:
    ...
    if isinstance(value, str):
        ...
        raise O1C86RunError("adapter omitted byte-exact native stdout")
    if _sha(
        getattr(result, "native_stdout_sha256", None), "native stdout digest"
    ) != sha256_bytes(payload):
        raise O1C86RunError("native stdout digest differs")
    return payload


def _validate_native_result(result: object, stdout: bytes) -> NativeEvidence:
    if not isinstance(result, _native_v25.JointScoreSieveV25Result):
        raise O1C86RunError("adapter v25 result type differs")
    raw = _mapping(result.raw, "native result")
    try:
        decoded = _mapping(json.loads(stdout), "native stdout JSON")
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise O1C86RunError("native stdout JSON differs") from exc
    if (
        decoded != raw
        or raw.get("schema") != _native_v25.JOINT_SCORE_SIEVE_RESULT_SCHEMA
    ):
        raise O1C86RunError("native stdout/result projection differs")
    ...
    if (
        ...
        or raw.get("active_vault_sha256") != PAGE11_SHA256
        ...
        raise O1C86RunError("native v22 production binding differs")
    if (
        raw.get("priority_state") != priority_state
        or raw.get("priority_actions") != actions
    ):
        raise O1C86RunError("native promoted priority projection differs")
    if raw.get("decision_ownership") != ownership:
        raise O1C86RunError("native promoted ownership projection differs")
    if result.status == 10 and not isinstance(result.key_model, bytes):
        raise O1C86RunError("SAT result omitted key model")
    if result.status != 10 and result.key_model is not None:
        raise O1C86RunError("non-SAT result returned key model")
...
def _science_and_operation(
    ...
    *,
    ...
    if action_count != len(actions):
        raise O1C86RunError("priority action count differs")
    ...
    for row in emitted_rows:
        ...
        try:
            ...
            raise O1C86RunError("emitted clause serialization differs") from exc
    ...
    if novel > active_new:
        raise O1C86RunError("global novelty exceeds active-vault novelty")
    ...
    if evidence.status == 10 and not public_model_verified:
        raise O1C86RunError("SAT key failed exact public ChaCha verification")
    if evidence.status != 10 and public_model_verified:
        raise O1C86RunError("public model verification without SAT differs")
    model_gain = evidence.status == 10 and public_model_verified
    closure_gain = evidence.status == 20 or terminal_empty
    # Native v22 emits no entropy or domain estimate.  These fields stay
    ...
    science = {
        ...
        "active_page11_new_clauses": active_new,
    ...
    *,
    ...
    try:
        ...
        result = adapter_run(
            ...
            sealed_page11_path=capsule / "initial" / ACTIVE_PROJECTION_NAME,
        )
...
def _manifest_bytes(capsule: Path) -> tuple[bytes, int]:
    ...
    for path in sorted(capsule.rglob("*")):
        ...
        if stat.S_ISLNK(metadata.st_mode):
            raise O1C86RunError("capsule contains a symlink")
        if stat.S_ISDIR(metadata.st_mode):
            continue
        if not stat.S_ISREG(metadata.st_mode):
            raise O1C86RunError("capsule contains a special file")
...
def _run_markdown(result: Mapping[str, object]) -> bytes:
    ...
    lines = [
        "# O1C-0086 Page-11 parent-centered run",
        "",
        ...
        "- Page 11 and lineage 24 burned when `episodes/00/intent.json` was persisted.",
...
def _publish_exact(path: Path, payload: bytes) -> None:
    if path.exists():
        if path.is_symlink() or not path.is_file() or path.read_bytes() != payload:
            raise O1C86RunError("authoritative result already differs")
        return
...
def _finalize_capsule(
    *, capsule: Path, authoritative: Path, result: Mapping[str, object]
    ...
    if persistent_bytes > MAXIMUM_PERSISTENT_ARTIFACT_BYTES:
        raise O1C86RunError("persistent artifact budget exceeded")
...
def _existing_result(root: Path) -> dict[str, object] | None:
    ...
    if not authoritative.exists():
        return None
    value = dict(_read_canonical_json(authoritative, "authoritative O1C86 result"))
    if value.get("schema") != RESULT_SCHEMA or value.get("attempt_id") != ATTEMPT_ID:
        raise O1C86RunError("authoritative O1C86 result differs")
    return value
...
def _validated_capsule_result(
    ...
    if not all(path.is_file() and not path.is_symlink() for path in required):
        if (capsule / "episodes/00/intent.json").is_file():
            raise O1C86RunError("burned incomplete capsule forbids retry or replay")
        raise O1C86RunError("incomplete pre-intent capsule blocks a fresh call")
    manifest, _ = _manifest_bytes(capsule)
    if manifest != (capsule / "artifacts.sha256").read_bytes():
        raise O1C86RunError("existing capsule manifest differs")
    result_path = capsule / "result.json"
    ...
    if (
        ...
        raise O1C86RunError("existing capsule result differs")
...
def _public_verifier(
    *, root: Path, config: Mapping[str, object]
    ...
    if _sha_file(config_path) != inputs["o1c73_config_sha256"]:
        raise O1C86RunError("public verifier config SHA differs")
    try:
        ...
        raise O1C86RunError("public ChaCha verifier cannot be established") from exc
...
def run(
    ...
    *,
    root: Path | None = None,
    adapter_run: AdapterRun = _native_v25.run_joint_score_sieve,
    ...
    if existing is not None:
        if len(capsules) != 1:
            raise O1C86RunError("authoritative O1C86 result lacks one sealed capsule")
        ...
        if capsule_payload != authoritative_payload or capsule_result != existing:
            raise O1C86RunError("authoritative O1C86 result differs from capsule")
        return existing
    if capsules:
        if len(capsules) != 1:
            raise O1C86RunError("multiple O1C86 capsules forbid replay")
    ...
    result = {
        ...
        "next_action": "Never retry or replay Page 11 / lineage 24, Page 10, or Page 9; preserve the final priority bank only as a continuation seed.",
    }
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Preflight or execute O1C86's one Page-11 parent-centered call"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 0
    except O1C86RunError as exc:
        print(f"O1C86: {exc}", file=sys.stderr)
        return 2
