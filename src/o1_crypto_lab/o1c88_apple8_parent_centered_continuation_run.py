"""Burn-on-intent runner for the single O1C-0088 Page-12 call.

The runner has one scientific side effect: after an immutable intent is
persisted it may invoke native v23 once, with seed zero and 128 requested
...
the published, sealed O1C-0087 causal-rollover bundle; it never regenerates
...
from . import joint_score_sieve_v26 as _native_v26
...
from .o1c87_page12_causal_rollover_prepare import (
...
NATIVE_BUILD_SCHEMA = "o1-256-o1c88-native-v23-build-v1"
...
CONFIG_RELATIVE = Path("configs/o1c88_apple8_parent_centered_continuation_v1.json")
RESULT_RELATIVE = Path(
    "research/O1C0088_APPLE8_PARENT_CENTERED_CONTINUATION_RESULT_20260720.json"
)
CAPSULE_SUFFIX = "O1C-0088_apple8-parent-centered-continuation-v1"

LOCAL_EPISODE_ORDINAL = 0
LINEAGE_CALL_ORDINAL = 25
SEED = 0
...
INPUT_BANK_TOTAL_COUNT = 182_368

NATIVE_V23_USAGE = (
    b"usage: cadical_o1_joint_score_sieve_v23 --cnf PATH "
...
PUBLISHED_ARTIFACT_SEALS: Mapping[str, tuple[int, str, str]] = {
    ...
    NEW_CHUNK_NAME: (
        2_188_455,
        "d5338ef8314f03e0bb046c51011d0759dd732615d7f51788a306531e4adbf61c",
        "immutable-all-new-lineage-24-evidence-chunk",
    ),
    RECEIPT_NAME: (
        PRIORITY_RECEIPT_BYTES,
        PRIORITY_RECEIPT_SHA256,
        "canonical-o1c86-evolved-priority-state-receipt",
    ),
    OCCURRENCES_NAME: (
        368_342,
    ...
    ACTIVE_PROJECTION_NAME: (
        PAGE12_SERIALIZED_BYTES,
        PAGE12_SHA256,
        "fresh-lineage-25-page12-science-input",
    ),
    RESIDENCY_NAME: (
        42_512,
...
SOURCE_PATHS = {
    "runner": "src/o1_crypto_lab/o1c88_apple8_parent_centered_continuation_run.py",
    "runner_tests": "tests/test_o1c88_apple8_parent_centered_continuation_run.py",
    "adapter_v26": "src/o1_crypto_lab/joint_score_sieve_v26.py",
    "adapter_v26_tests": "tests/test_joint_score_sieve_v26.py",
    "page12_causal_rollover_preparation": "src/o1_crypto_lab/o1c87_page12_causal_rollover_prepare.py",
    "page12_causal_rollover_preparation_tests": "tests/test_o1c87_page12_causal_rollover_prepare.py",
    ...
    "native_v23": "native/cadical_o1_joint_score_sieve_v23.cpp",
    "native_v23_tests": "tests/test_o1c87_native_parent_centered_continuation_operator.py",
...
class O1C88RunError(RuntimeError):
...
def _mapping(value: object, field: str) -> Mapping[str, object]:
    if not isinstance(value, Mapping) or not all(isinstance(k, str) for k in value):
        raise O1C88RunError(f"{field} differs")
...
def _relative_text(value: object, field: str) -> str:
    ...
    if path.is_absolute() or not path.parts or ".." in path.parts:
        raise O1C88RunError(f"{field} escapes the lab")
...
def _lab_path(
    ...
    try:
        ...
        raise O1C88RunError(f"{field} cannot be resolved") from exc
    expected = (
    ...
    if (
        ...
        raise O1C88RunError(f"{field} is not a canonical lab path")
    return resolved


def _external_path(value: object, field: str, *, directory: bool = False) -> Path:
    if not isinstance(value, str) or not Path(value).is_absolute():
        raise O1C88RunError(f"{field} is not absolute")
    ...
    if stat.S_ISLNK(metadata.st_mode) or not expected:
        raise O1C88RunError(f"{field} is not canonical")
    return resolved
...
def _read_canonical_json(path: Path, field: str) -> Mapping[str, object]:
    ...
    try:
        ...
        raise O1C88RunError(f"{field} is not JSON") from exc
    if canonical_json_bytes(value) != payload:
        raise O1C88RunError(f"{field} is not canonical JSON")
    return value


def _atomic_create(path: Path, payload: bytes, *, mode: int = 0o444) -> None:
    if not isinstance(payload, bytes):
        raise O1C88RunError("atomic payload differs")
    ...
    try:
        ...
        if path.read_bytes() != payload:
            raise O1C88RunError(f"atomic write verification failed for {path.name}")
        path.chmod(mode)
        ...
        raise O1C88RunError(f"artifact already exists: {path.name}") from exc
...
def load_config(path: str | Path, *, root: Path | None = None) -> dict[str, object]:
    ...
    if not config_path.is_relative_to(lab) or config_path.is_symlink():
        raise O1C88RunError("config escapes the lab")
    config = dict(_read_canonical_json(config_path, "O1C88 config"))
    if _pending(config):
        raise O1C88RunError("config contains PENDING: " + ", ".join(_pending(config)))
    if (
        set(config)
        != {
            "schema",
        ...
        raise O1C88RunError("config fields differ")
    ...
    if (
        ...
        raise O1C88RunError("frozen O1C86 parent differs")
    ...
    if (
        ...
        raise O1C88RunError("frozen preparation source differs")
    ...
    if set(inputs) != {
        ...
        raise O1C88RunError("input fields differ")
    ...
    if (
        ...
        or native.get("adapter_schema") != _native_v26.JOINT_SCORE_SIEVE_ADAPTER_SCHEMA
        or native.get("result_schema") != _native_v26.JOINT_SCORE_SIEVE_RESULT_SCHEMA
        or native.get("page12_sha256") != PAGE12_SHA256
        or native.get("page12_bytes") != PAGE12_SERIALIZED_BYTES
    ...
    if dict(paths) != SOURCE_PATHS or set(digests) != set(SOURCE_PATHS):
        raise O1C88RunError("source inventory differs")
    ...
    if dict(budgets) != expected_budgets:
        raise O1C88RunError("budgets differ")
    return config
...
def _sibling_solver_pids() -> tuple[int, ...]:
    try:
        ...
        raise O1C88RunError("solver process census failed") from exc
    current = os.getpid()
    found: list[int] = []
...
def _validate_priority_receipt(payload: bytes, bank: bytes) -> Mapping[str, object]:
    try:
        ...
        raise O1C88RunError("priority state receipt is not JSON") from exc
    if canonical_json_bytes(receipt) != payload:
        raise O1C88RunError("priority state receipt is not canonical JSON")
    hexadecimal = receipt.get("bank_hex")
    try:
        receipt_bank = bytes.fromhex(cast(str, hexadecimal))
    except (TypeError, ValueError) as exc:
        raise O1C88RunError("priority state receipt bank differs") from exc
    if (
        receipt.get("schema") != "o1-256-o1c86-live-parent-centered-continuation-priority-state-v1"
        ...
        raise O1C88RunError("priority state receipt contract differs")
    return receipt
...
def _validate_occurrence_ledger(payload: bytes) -> frozenset[str]:
    ...
    if (
        ...
        raise O1C88RunError("published occurrence ledger contract differs")
    ...
    for ordinal, value in enumerate(records):
        ...
        if (
            ...
            raise O1C88RunError("published occurrence record differs")
    ...
    if set(by_index) != set(range(ATTIC_UNION_CLAUSE_COUNT)) or len(digests) != (
        ...
        raise O1C88RunError("published attic union digest inventory differs")
    return digests
...
def _validate_published_documents(artifacts: Mapping[str, bytes]) -> frozenset[str]:
    ...
    projection = _mapping(
        residency.get("current_projection"), "published Page-12 projection"
    )
    encoding = _mapping(projection.get("encoding_only"), "published Page-12 vault")
    ...
    if (
        ...
        or len(activation_entries) != PAGE12_ACTIVATION_COUNT
        or len(used) != PAGE12_ACTIVATION_COUNT
        or used[-1] != PAGE12_SHA256
        or last_activation.get("lineage_ordinal") != LINEAGE_CALL_ORDINAL
        or last_activation.get("active_sha256") != PAGE12_SHA256
        or last_activation.get("selected_clause_count") != PAGE12_CLAUSE_COUNT
        or residency.get("schema") != "o1-score-threshold-causal-residency-v1"
        or residency.get("active_limit") != PAGE12_ACTIVE_LIMIT
        or projection.get("lineage_ordinal") != LINEAGE_CALL_ORDINAL
        or projection.get("category_counts") != PAGE12_CATEGORY_COUNTS
        or encoding.get("sha256") != PAGE12_SHA256
        or encoding.get("clause_count") != PAGE12_CLAUSE_COUNT
        or encoding.get("literal_count") != PAGE12_LITERAL_COUNT
        or encoding.get("serialized_bytes") != PAGE12_SERIALIZED_BYTES
        or attic.get("chunk_count") != ATTIC_CHUNK_COUNT
        or residency_union.get("clause_count") != ATTIC_UNION_CLAUSE_COUNT
    ):
        raise O1C88RunError("published Page-12 residency contract differs")
    if (
        ...
        raise O1C88RunError("published supporting artifact contract differs")
...
def _validate_published_manifest(
    ...
    if (
        ...
        or manifest.get("attempt_id") != "O1C-0087"
    ):
        raise O1C88RunError("published preparation manifest identity differs")
    ...
    page12 = _mapping(manifest.get("page12"), "published Page-12")
    ...
    if dict(zero_call) != {
        ...
        raise O1C88RunError("published zero-call authorization differs")
    if (
        parent.get("attempt_id") != "O1C-0086"
        or parent.get("capsule_manifest_sha256") != PARENT_CAPSULE_MANIFEST_SHA256
        ...
        raise O1C88RunError("published parent boundary differs")
    if (
        rollover.get("clause_count") != 202
        ...
        raise O1C88RunError("published rollover/attic boundary differs")
    if (
        page12.get("lineage_ordinal") != LINEAGE_CALL_ORDINAL
        or page12.get("active_limit") != PAGE12_ACTIVE_LIMIT
        or page12.get("active_sha256") != PAGE12_SHA256
        or page12.get("clause_count") != PAGE12_CLAUSE_COUNT
        or page12.get("literal_count") != PAGE12_LITERAL_COUNT
        or page12.get("serialized_bytes") != PAGE12_SERIALIZED_BYTES
        or page12.get("category_counts") != PAGE12_CATEGORY_COUNTS
        or page12.get("headroom") != PAGE12_HEADROOM
        or page12.get("fresh_identity") is not True
    ):
        raise O1C88RunError("published Page-12 seal differs")
    if (
        ...
        != "o1c86-live-continuation-bank-with-state-receipt"
        or bank.get("eligible_coordinate_count") != 255
        or bank.get("aggregate_evolved_count") != INPUT_BANK_TOTAL_COUNT
        or bank.get("maximum_evolved_count") != 1_576
        or bank.get("minimum_nonzero_evolved_count") != 139
        ...
        raise O1C88RunError("published continuation bank seal differs")
    rows_raw = _mapping(manifest.get("artifacts"), "published artifact rows")
    if set(rows_raw) != set(PUBLISHED_ARTIFACT_SEALS):
        raise O1C88RunError("published artifact row inventory differs")
    ...
    for name, (size, digest, role) in PUBLISHED_ARTIFACT_SEALS.items():
        ...
        if (
            ...
            raise O1C88RunError("published artifact seal differs")
        rows[name] = row
...
def _load_published_preparation(
    ...
    try:
        ...
        raise O1C88RunError("published preparation inventory is unreadable") from exc
    if (
        manifest_path != directory / PREPARATION_MANIFEST_NAME
        or {path.name for path in children} != PREPARATION_ARTIFACT_NAMES
    ):
        raise O1C88RunError("published preparation inventory differs")
    ...
    for path in children:
        try:
            ...
            raise O1C88RunError("published preparation artifact is unreadable") from exc
        if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
            raise O1C88RunError("published preparation artifact type differs")
    ...
    if (
        ...
        raise O1C88RunError("published preparation manifest seal differs")
...
def _preflight_bundle(
    ...
    *,
    ...
    if (
        ...
        raise O1C88RunError("parent input SHA preflight differs")
    ...
    if (
        ...
        raise O1C88RunError("published preparation/receipt preflight differs")
    ...
    for name in ("cnf", "potential", "grouping", "o1c73_config"):
        ...
        if observed != inputs[f"{name}_sha256"]:
            raise O1C88RunError(f"input {name} SHA preflight differs")
    ...
    for name in SOURCE_PATHS:
        ...
        if observed != expected[name]:
            raise O1C88RunError(f"source {name} SHA preflight differs")
    ...
    if (
        ...
        or source_sha["native_v23"] != native["source_sha256"]
    ):
        raise O1C88RunError("native toolchain/source SHA preflight differs")
    ...
    if (
        ...
        raise O1C88RunError("Darwin/arm64/RAM/disk/process preflight differs")
    row = {
...
def _executable_identity(path: Path, *, relative_path: str) -> dict[str, object]:
    try:
        ...
        raise O1C88RunError("native executable is unreadable") from exc
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
        raise O1C88RunError("native executable type differs")
    size = path.stat().st_size
    if size <= 0:
        raise O1C88RunError("native executable is empty")
    return {
        "path": relative_path,
...
def _compile_native(
    *,
    ...
    library = _external_path(native["cadical_library"], "CaDiCaL library")
    source = _lab_path(root, native["source"], "native v23 source")
    if (
        _sha_file(source) != native["source_sha256"]
        or _sha_file(library) != native["cadical_library_sha256"]
    ):
        raise O1C88RunError("native build input changed after preflight")
    ...
    try:
        ...
        raise O1C88RunError("native v23 compilation could not run") from exc
    ...
    try:
        ...
        raise O1C88RunError("native v23 --help smoke could not run") from exc
    ...
    if (
        ...
        or smoke_stdout != NATIVE_V23_USAGE
        or smoke_stderr
        or after_smoke != before_smoke
    ):
        raise O1C88RunError("native v23 --help smoke seal differs")
...
def _stage_capsule(
    *,
    ...
    if final.exists() or stage.exists():
        raise O1C88RunError("timestamped capsule already exists")
...
def _rehash_call_inputs(
    *,
    ...
    if _sha_file(config_path) != preflight_row.get("config_sha256"):
        raise O1C88RunError("config changed before native call")
    ...
    for name in ("cnf", "potential", "grouping", "o1c73_config"):
        if (
            ...
            raise O1C88RunError(f"input {name} changed before native call")
    ...
    if prepared_rows.get(RECEIPT_NAME) != receipt_row:
        raise O1C88RunError("staged priority receipt seal differs")
    ...
    for name, value in expected_rows.items():
        ...
        if path.stat().st_size != row.get("serialized_bytes") or _sha_file(
            ...
            raise O1C88RunError("staged call artifact seal differs")
    ...
    if invocation_path.read_bytes() != canonical_json_bytes(invocation) or _sha_file(
        ...
        raise O1C88RunError("staged invocation/build seal differs")
    ...
    if (
        ...
        or smoke.get("stdout")
        != {
            "serialized_bytes": len(NATIVE_V23_USAGE),
            "sha256": sha256_bytes(NATIVE_V23_USAGE),
        }
        ...
        raise O1C88RunError("staged native build/smoke seal differs")
...
def _native_stdout(result: object) -> bytes:
    ...
    if isinstance(value, str):
        ...
        raise O1C88RunError("adapter omitted byte-exact native stdout")
    if _sha(
        getattr(result, "native_stdout_sha256", None), "native stdout digest"
    ) != sha256_bytes(payload):
        raise O1C88RunError("native stdout digest differs")
    return payload
...
def _validate_result_conservation(
    ...
    if set(counters) != {*outcome_names, "child_bound_evaluations"}:
        raise O1C88RunError("native post-result conservation differs")
    outcomes = tuple(
...
def _validate_native_result(result: object, stdout: bytes) -> NativeEvidence:
    if not isinstance(result, _native_v26.JointScoreSieveV26Result):
        raise O1C88RunError("adapter v26 result type differs")
    raw = _mapping(result.raw, "native result")
    try:
        decoded = _mapping(json.loads(stdout), "native stdout JSON")
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise O1C88RunError("native stdout JSON differs") from exc
    if (
        decoded != raw
        or raw.get("schema") != _native_v26.JOINT_SCORE_SIEVE_RESULT_SCHEMA
    ):
        raise O1C88RunError("native stdout/result projection differs")
    ...
    if (
        ...
        or raw.get("active_vault_sha256") != PAGE12_SHA256
        ...
        raise O1C88RunError("native v23 production binding differs")
    if (
        raw.get("priority_state") != priority_state
        or raw.get("priority_actions") != actions
    ):
        raise O1C88RunError("native promoted priority projection differs")
    if raw.get("decision_ownership") != ownership:
        raise O1C88RunError("native promoted ownership projection differs")
    if result.status == 10 and not isinstance(result.key_model, bytes):
        raise O1C88RunError("SAT result omitted key model")
    if result.status != 10 and result.key_model is not None:
        raise O1C88RunError("non-SAT result returned key model")
...
def _science_and_operation(
    ...
    *,
    ...
    if action_count != len(actions):
        raise O1C88RunError("priority action count differs")
    ...
    for row in emitted_rows:
        ...
        try:
            ...
            raise O1C88RunError("emitted clause serialization differs") from exc
    ...
    if novel > active_new:
        raise O1C88RunError("global novelty exceeds active-vault novelty")
    ...
    if evidence.status == 10 and not public_model_verified:
        raise O1C88RunError("SAT key failed exact public ChaCha verification")
    if evidence.status != 10 and public_model_verified:
        raise O1C88RunError("public model verification without SAT differs")
    model_gain = evidence.status == 10 and public_model_verified
    closure_gain = evidence.status == 20 or terminal_empty
    # Native v23 emits no entropy or domain estimate.  These fields stay
    ...
    science = {
        ...
        "active_page12_new_clauses": active_new,
    ...
    *,
    ...
    try:
        ...
        result = adapter_run(
            ...
            sealed_page12_path=capsule / "initial" / ACTIVE_PROJECTION_NAME,
        )
...
def _manifest_bytes(capsule: Path) -> tuple[bytes, int]:
    ...
    for path in sorted(capsule.rglob("*")):
        ...
        if stat.S_ISLNK(metadata.st_mode):
            raise O1C88RunError("capsule contains a symlink")
        if stat.S_ISDIR(metadata.st_mode):
            continue
        if not stat.S_ISREG(metadata.st_mode):
            raise O1C88RunError("capsule contains a special file")
...
def _run_markdown(result: Mapping[str, object]) -> bytes:
    ...
    lines = [
        "# O1C-0088 Page-12 parent-centered run",
        "",
        ...
        "- Page 12 and lineage 25 burned when `episodes/00/intent.json` was persisted.",
...
def _publish_exact(path: Path, payload: bytes) -> None:
    if path.exists():
        if path.is_symlink() or not path.is_file() or path.read_bytes() != payload:
            raise O1C88RunError("authoritative result already differs")
        return
...
def _finalize_capsule(
    *, capsule: Path, authoritative: Path, result: Mapping[str, object]
    ...
    if persistent_bytes > MAXIMUM_PERSISTENT_ARTIFACT_BYTES:
        raise O1C88RunError("persistent artifact budget exceeded")
...
def _existing_result(root: Path) -> dict[str, object] | None:
    ...
    if not authoritative.exists():
        return None
    value = dict(_read_canonical_json(authoritative, "authoritative O1C88 result"))
    if value.get("schema") != RESULT_SCHEMA or value.get("attempt_id") != ATTEMPT_ID:
        raise O1C88RunError("authoritative O1C88 result differs")
    return value
...
def _validated_capsule_result(
    ...
    if not all(path.is_file() and not path.is_symlink() for path in required):
        if (capsule / "episodes/00/intent.json").is_file():
            raise O1C88RunError("burned incomplete capsule forbids retry or replay")
        raise O1C88RunError("incomplete pre-intent capsule blocks a fresh call")
    manifest, _ = _manifest_bytes(capsule)
    if manifest != (capsule / "artifacts.sha256").read_bytes():
        raise O1C88RunError("existing capsule manifest differs")
    result_path = capsule / "result.json"
    ...
    if (
        ...
        raise O1C88RunError("existing capsule result differs")
...
def _public_verifier(
    *, root: Path, config: Mapping[str, object]
    ...
    if _sha_file(config_path) != inputs["o1c73_config_sha256"]:
        raise O1C88RunError("public verifier config SHA differs")
    try:
        ...
        raise O1C88RunError("public ChaCha verifier cannot be established") from exc
...
def run(
    ...
    *,
    root: Path | None = None,
    adapter_run: AdapterRun = _native_v26.run_joint_score_sieve,
    ...
    if existing is not None:
        if len(capsules) != 1:
            raise O1C88RunError("authoritative O1C88 result lacks one sealed capsule")
        ...
        if capsule_payload != authoritative_payload or capsule_result != existing:
            raise O1C88RunError("authoritative O1C88 result differs from capsule")
        return existing
    if capsules:
        if len(capsules) != 1:
            raise O1C88RunError("multiple O1C88 capsules forbid replay")
    ...
    result = {
        ...
        "next_action": "Never retry or replay Page 12 / lineage 25, Page 11, Page 10, or Page 9; preserve the final priority bank only as a continuation seed.",
    }
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Preflight or execute O1C88's one Page-12 parent-centered call"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 0
    except O1C88RunError as exc:
        print(f"O1C88: {exc}", file=sys.stderr)
        return 2
