"""Fail-closed O1C-0101 adapter for Page 18 and the evolved live bank.

Version 32 preserves v31's proof-mining and result-validation semantics while
moving the native boundary to result/source v29 and fresh lineage-31 Page 18.
...
JOINT_SCORE_SIEVE_ADAPTER_SCHEMA = "o1-256-joint-score-sieve-v32-adapter-v1"
JOINT_SCORE_SIEVE_RESULT_SCHEMA = "o1-256-cadical-joint-score-sieve-result-v29"
...
PRIORITY_ACTION_SCHEMA = "o1-256-o1c101-failure-first-proof-mining-actions-v1"
...
O1C100_MANIFEST_SCHEMA = "o1-256-o1c100-page18-telemetry-recovery-preparation-v1"
O1C100_MANIFEST_BYTES = 6_865
O1C100_MANIFEST_SHA256 = (
    "c0050ae08738f424505a92278759702bee4fcab23139a31137e715087ae437d9"
)
O1C100_MANIFEST_ARTIFACTS = frozenset(
    {
        "page-18-active.bin",
        "residency.json",
...
O1C100_PUBLISHED_ARTIFACTS = O1C100_MANIFEST_ARTIFACTS | {
    "telemetry-recovery-preparation-manifest.json"
}
O1C100_MANIFEST_RELATIVE = Path(
    "research/o1c100_page18_telemetry_recovery_seed_20260721/"
    "telemetry-recovery-preparation-manifest.json"
)
O1C100_PAGE18_RELATIVE = Path(
    "research/o1c100_page18_telemetry_recovery_seed_20260721/page-18-active.bin"
)
...
O1C99_FAILURE_RECEIPT_BYTES = 22_520
O1C99_FAILURE_RECEIPT_SHA256 = (
    "fd52665283e901bb8237923efba081ee1f20d85e1070e9d5e6b2421e9ada46b4"
)
PRODUCTION_PAGE18_BYTES = 2_680_827
PRODUCTION_PAGE18_SHA256 = (
    "5d89bbe07c8b988b4f1ce5dc2a31b860ab59192d3efc02854e27b8f779de417c"
)
PRODUCTION_PAGE18_LINEAGE_ORDINAL = 31
PRODUCTION_PAGE18_ACTIVE_LIMIT = 249
PRODUCTION_PAGE18_CLAUSE_COUNT = 249
PRODUCTION_PAGE18_LITERAL_COUNT = 669_910
PRODUCTION_PAGE18_CATEGORY_COUNTS: Mapping[str, int] = {
    "structural_root": 9,
    "pinned_core": 43,
    "inherited_debt": 0,
    "new_debt": 65,
...
PRODUCTION_PAGE18_HEADROOM: Mapping[str, int] = {
    "clauses": 263,
    "literals": 930_090,
...
NATIVE_SOURCE_RELATIVE = Path("native/cadical_o1_joint_score_sieve_v29.cpp")
NATIVE_SOURCE_BYTES = 51_956
NATIVE_SOURCE_SHA256 = (
    "bbb78c53154ca691ecef3bd0c8f1499a8a73b3f075437844449318e6bbdb1125"
)
O1C100_LIVE_BANK_RELATIVE = Path(
    "research/o1c100_page18_telemetry_recovery_seed_20260721/"
    "final-parent-centered-priority-bank.bin"
)
...
O1C99_FAILURE_RECEIPT_RELATIVE = Path(
    "research/o1c100_page18_telemetry_recovery_seed_20260721/"
    "o1c99-terminal-failure-receipt.json"
)
...
_SEED_FIELDS = {
    ...
    "source_terminal_failure_receipt_sha256",
    "source_terminal_failure_receipt_bytes",
...
class JointScoreSieveV32Error(O1RelationalSearchError):
    """Native-v29 execution or continuation validation failed."""
...
@dataclass(frozen=True)
class JointScoreSieveV32Result:
    """Typed native-v29 result and its validated evolved bank."""
...
def _error(field: str) -> JointScoreSieveV32Error:
    return JointScoreSieveV32Error(f"joint-score-sieve-v32 {field} differs")
...
def _read_page18(path: str | Path) -> tuple[Path, bytes]:
    io_v1 = _v9._v8._v7._v1
    resolved, payload, digest = io_v1._read_input(path, "Page18")
    ...
    if len(payload) != PRODUCTION_PAGE18_BYTES or digest != PRODUCTION_PAGE18_SHA256:
        raise _error("Page18 active projection seal")
    return resolved, payload
...
def _manifest_contract() -> tuple[str, int, str, frozenset[str]]:
    """Return the sealed O1C-0100 Page-18 telemetry-recovery contract."""

    return (
        O1C100_MANIFEST_SCHEMA,
        _positive(O1C100_MANIFEST_BYTES, "O1C100 manifest byte count"),
        _sha(O1C100_MANIFEST_SHA256, "O1C100 manifest digest"),
        O1C100_MANIFEST_ARTIFACTS,
    )
...
def _validate_manifest(payload: bytes) -> None:
    """Validate the exact zero-call O1C-0100 Page-18 recovery manifest."""
    ...
    try:
        ...
        raise _error("O1C100 manifest JSON") from exc
    if (
        canonical != payload
        or len(payload) != expected_bytes
        or hashlib.sha256(payload).hexdigest() != expected_sha256
    ):
        raise _error("O1C100 canonical manifest seal")
    ...
    recovery = _mapping(root.get("telemetry_recovery"), "manifest telemetry recovery")
    attic = _mapping(root.get("attic"), "manifest attic")
    page18 = _mapping(root.get("page18"), "manifest Page18")
    debt = _mapping(page18.get("debt_completion"), "manifest Page18 debt")
    final_bank = _mapping(root.get("final_priority_bank"), "manifest final bank")

    if (
        set(root) != expected_top
        or root.get("schema") != schema
        or root.get("attempt_id") != "O1C-0100"
        or set(artifacts) != artifact_names
        or O1C100_PUBLISHED_ARTIFACTS
        != artifact_names | {O1C100_MANIFEST_RELATIVE.name}
        or zero_call
        != {
        ...
        or parent.get("attempt_id") != "O1C-0099"
        ...
        != "native-decision-ownership-event-cap-exceeded"
        or parent.get("failure_phase") != "POST_CALL"
        or parent.get("terminal_failure_sha256") != O1C99_FAILURE_RECEIPT_SHA256
        or parent.get("terminal_failure_serialized_bytes")
        != O1C99_FAILURE_RECEIPT_BYTES
        or parent.get("page17_burned") is not True
        or parent.get("lineage30_burned") is not True
        ...
        or parent.get("priority_receipt_sha256") != O1C97_PRIORITY_RECEIPT_SHA256
        ...
        or page18.get("lineage_ordinal") != PRODUCTION_PAGE18_LINEAGE_ORDINAL
        or page18.get("active_limit") != PRODUCTION_PAGE18_ACTIVE_LIMIT
        or page18.get("active_sha256") != PRODUCTION_PAGE18_SHA256
        or page18.get("clause_count") != PRODUCTION_PAGE18_CLAUSE_COUNT
        or page18.get("literal_count") != PRODUCTION_PAGE18_LITERAL_COUNT
        or page18.get("serialized_bytes") != PRODUCTION_PAGE18_BYTES
        or page18.get("category_counts") != PRODUCTION_PAGE18_CATEGORY_COUNTS
        or page18.get("headroom") != PRODUCTION_PAGE18_HEADROOM
        ...
        or final_bank.get("semantic_role") != "unchanged-sealed-live-continuation-bytes"
    ):
        raise _error("O1C100 Page18 lineage-31 manifest contract")

    _artifact_contract(
        artifacts,
        "page-18-active.bin",
        serialized_bytes=PRODUCTION_PAGE18_BYTES,
        sha256=PRODUCTION_PAGE18_SHA256,
        role="fresh-lineage-31-page18-science-input",
    )
    _artifact_contract(
        artifacts,
        "final-parent-centered-priority-bank.bin",
    ...
    _artifact_contract(
        artifacts,
        "o1c97-priority-state-receipt.json",
        serialized_bytes=O1C97_PRIORITY_RECEIPT_BYTES,
        sha256=O1C97_PRIORITY_RECEIPT_SHA256,
        role="unchanged-canonical-o1c97-priority-state-receipt",
    )
    _artifact_contract(
        artifacts,
        "o1c99-terminal-failure-receipt.json",
        serialized_bytes=O1C99_FAILURE_RECEIPT_BYTES,
        sha256=O1C99_FAILURE_RECEIPT_SHA256,
        role="canonical-o1c99-post-call-terminal-failure-receipt",
    )
...
def _validate_failure_receipt(payload: bytes) -> None:
    try:
        failure = _mapping(
            _v21.load_native_json(payload.decode("utf-8")),
            "O1C99 terminal failure receipt",
        )
        canonical = (
            json.dumps(
                failure,
        ...
        raise _error("O1C99 terminal failure receipt JSON") from exc
    if (
        canonical != payload
        or len(payload) != O1C99_FAILURE_RECEIPT_BYTES
        or hashlib.sha256(payload).hexdigest() != O1C99_FAILURE_RECEIPT_SHA256
        ...
        raise _error("O1C99 terminal failure receipt contract")
...
def _validate_seed_report(
    ...
    if (
        ...
        != O1C97_PRIORITY_RECEIPT_BYTES
        or report.get("source_preparation_manifest_sha256") != O1C100_MANIFEST_SHA256
        or report.get("source_preparation_manifest_bytes") != O1C100_MANIFEST_BYTES
        or report.get("source_terminal_failure_receipt_sha256")
        != O1C99_FAILURE_RECEIPT_SHA256
        or report.get("source_terminal_failure_receipt_bytes")
        != O1C99_FAILURE_RECEIPT_BYTES
...
def _parse_native_payload(
    ...
    *,
...
) -> JointScoreSieveV32Result:
    """Validate one native-v29 document and its unchanged v6 lifecycle."""
    ...
    return JointScoreSieveV32Result(
        status=base.status,
...
def _validate_prelaunch(
    *,
    ...
    if _sha(expected_source_sha256, "expected native source digest") != (
        ...
        raise _error("native-v29 source identity")
    ...
    failure_receipt, failure_receipt_bytes = _read_exact(
        failure_receipt_path,
        label="O1C99 terminal failure receipt",
        expected_bytes=O1C99_FAILURE_RECEIPT_BYTES,
        expected_sha256=O1C99_FAILURE_RECEIPT_SHA256,
    )
    bank, bank_bytes = _read_live_bank(bank_path)
    page18, page18_bytes = _read_page18(page18_path)
    ...
    _validate_failure_receipt(failure_receipt_bytes)
...
def run_joint_score_sieve(
    *,
    ...
    terminal_failure_receipt_path: str | Path | None = None,
    sealed_page18_path: str | Path | None = None,
) -> JointScoreSieveV32Result:
    """Validate a prebuilt native-v29 executable, then launch it exactly once."""
    ...
    try:
        ...
        manifest = (
            ...
            else root / O1C100_MANIFEST_RELATIVE
        )
        receipt = (
        ...
        failure_receipt = (
            Path(terminal_failure_receipt_path)
            if terminal_failure_receipt_path is not None
            else root / O1C99_FAILURE_RECEIPT_RELATIVE
        )
        page18 = (
            Path(sealed_page18_path)
            if sealed_page18_path is not None
            else root / O1C100_PAGE18_RELATIVE
        )
        ...
        if (
            len(vault_bytes) != PRODUCTION_PAGE18_BYTES
            or vault_sha != PRODUCTION_PAGE18_SHA256
            or vault_bytes != sealed.page18_bytes
        ):
            raise _error("Page18 production seal")
        ...
        if (
            input_vault.sha256 != PRODUCTION_PAGE18_SHA256
            or _candidate_order(field) != PRODUCTION_CANDIDATES
        ):
            raise _error("Page18 parsed production seal")
        ...
        for original, resolved, before, name in (
            ...
            (manifest, sealed.manifest_path, sealed.manifest_bytes, "O1C100 manifest"),
            (receipt, sealed.receipt_path, sealed.receipt_bytes, "priority receipt"),
            (
                failure_receipt,
                sealed.failure_receipt_path,
            ...
            (page18, sealed.page18_path, sealed.page18_bytes, "sealed Page18"),
        ...
        if completed.returncode:
            ...
            raise JointScoreSieveV32Error(
                "joint-score-sieve-v32 execution failed: "
        ...
        message = str(exc)
        if not message.startswith("joint-score-sieve-v32"):
            message = f"joint-score-sieve-v32 adapter failed: {message}"
