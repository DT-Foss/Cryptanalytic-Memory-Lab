"""Strict explicit phase-one reader adapter for native sieve v8.

Native v8 compiles the frozen native-v6 solver and makes its default phase-one
choice explicit and immutable.  This adapter independently reconstructs that
reader specification, validates the exact top-level reader object, projects the
remaining payload to native-v6 for v9 validation, and preserves v9's observed-
work conflict ledger, bounded vault contract, and successful-process evidence.
"""
...
from . import joint_score_sieve_v10 as _v10
...
JOINT_SCORE_SIEVE_ADAPTER_SCHEMA = "o1-256-joint-score-sieve-v11-adapter-v1"
JOINT_SCORE_SIEVE_RESULT_SCHEMA = "o1-256-cadical-joint-score-sieve-result-v8"
...
# The complete public v10/v9 mechanism surface is inherited without changing
# its interpretation.  Only the native result and adapter reader identities
# above are new.
...
JointScoreSieveV10Result = _v10.JointScoreSieveV10Result
...
class JointScoreSieveV11Result(_v9.JointScoreSieveV9Result):
    """A v9-validated result plus the normalized explicit phase-one reader."""

    reader: Mapping[str, object]


def forced_initial_phase_reader_spec_bytes() -> bytes:
    """Return the independently reconstructed phase-one reader preimage."""
...
def _expected_reader() -> dict[str, object]:
    ...
    if digest != FORCED_INITIAL_PHASE_READER_SPEC_SHA256:
        raise O1RelationalSearchError(
            "joint-score-sieve-v11 frozen reader preimage differs"
        )
    return {
...
def validate_forced_initial_phase_reader(raw: object) -> dict[str, object]:
    """Validate every phase-one reader field with exact JSON scalar types."""

    reader = _v9._v8._v1._mapping(raw, "reader")
    if set(reader) != _READER_FIELDS:
        raise O1RelationalSearchError("joint-score-sieve-v11 reader fields differ")
    ...
    if (
        ...
        raise O1RelationalSearchError("joint-score-sieve-v11 reader contract differs")
...
def validate_native_lifecycle(payload: Mapping[str, object]) -> dict[str, int | str]:
    """Validate native-v8 lifecycle provenance through the v9 parent rules."""
    ...
    if (
        ...
        raise O1RelationalSearchError(
            "joint-score-sieve-v11 lifecycle contract differs"
        )
...
def _promote_result(
    ...
    *,
    ...
    return JointScoreSieveV11Result(
...
def _parse_native_payload(
    ...
    *,
    ...
    """Validate native-v8 reader identity before inherited payload parsing."""

    if not isinstance(payload, Mapping) or set(payload) != _TOP_LEVEL_FIELDS:
        raise O1RelationalSearchError("joint-score-sieve-v11 result fields differ")
    if (
        ...
        raise O1RelationalSearchError("joint-score-sieve-v11 result contract differs")
    ...
    try:
        ...
        raise O1RelationalSearchError(
            "joint-score-sieve-v11 native payload validation failed"
        ) from exc
...
def _run_joint_score_sieve_native_contract(
    *,
    ...
    """Run hard-coded phase-one native v8 through the strict v11 boundary."""

    requested = _v9._requested_conflicts(conflict_limit)
    if not isinstance(vault_caps, VaultCaps) or vault_caps != O1C66_VAULT_CAPS:
        raise O1RelationalSearchError("joint-score-sieve-v11 native vault caps differ")
    if (
        ...
        raise O1RelationalSearchError(
            "joint-score-sieve-v11 reader, threshold, timeout, or memory limit differs"
        )
    ...
    if grouping.potential_sha256 != potential_sha:
        raise O1RelationalSearchError(
            "joint-score-sieve-v11 grouping potential identity differs"
        )
    try:
        ...
        raise O1RelationalSearchError(
            "joint-score-sieve-v11 input vault differs"
        ) from exc
    try:
        ...
        raise O1RelationalSearchError(
            "joint-score-sieve-v11 input vault certification differs"
        ) from exc
    ...
    if execution_error is not None:
        raise O1RelationalSearchError(
            "joint-score-sieve-v11 execution failed"
        ) from execution_error
    if execution is None:
        raise O1RelationalSearchError("joint-score-sieve-v11 execution failed")
    ...
    if completed.returncode:
        ...
        raise O1RelationalSearchError(
            f"joint-score-sieve-v11 execution failed: {detail}"
        ) from failure
    ...
    try:
        ...
        raise O1RelationalSearchError(
            "joint-score-sieve-v11 result JSON is invalid"
        ) from exc
    except Exception as exc:
        _v9._attach_native_process_evidence(
            exc,
...
def run_joint_score_sieve(
    *,
    ...
    """Run native v8 and retain v9's bounded failure/process evidence."""
    ...
    try:
        ...
        message = str(exc)
        if not message.startswith("joint-score-sieve-v11"):
            message = f"joint-score-sieve-v11 adapter failed: {message}"
