"""Bounded pattern action-credit decisions for the frozen O1C-0048 pairs."""
...
PATTERN_CREDIT_RESULT_SCHEMA = "o1-256-cadical-pattern-credit-search-result-v1"
PATTERN_CREDIT_DECISION_RULE = "delayed_trail_owner_pair_pattern_credit"
PATTERN_CREDIT_GROUPS = 63
PATTERN_CREDIT_VARIABLES = 126
...
PATTERN_CREDIT_ACTION_BYTES_PER_CELL = 8
PATTERN_CREDIT_ACTION_CELLS_PER_GROUP = 4
PATTERN_CREDIT_ACTION_BYTES_PER_GROUP = 32
PATTERN_CREDIT_OWNER_BYTES_PER_MEMBER = 5
PATTERN_CREDIT_OWNER_MEMBERS_PER_GROUP = 2
PATTERN_CREDIT_OWNER_BYTES_PER_GROUP = 10
PATTERN_CREDIT_BYTES_PER_GROUP = 42
PATTERN_CREDIT_ACTION_STATE_BYTES = 2016
PATTERN_CREDIT_OWNER_STATE_BYTES = 630
PATTERN_CREDIT_STATE_BYTES = 2646
PATTERN_CREDIT_MIN = -(1 << 15)
PATTERN_CREDIT_MAX = (1 << 15) - 1
PATTERN_CREDIT_COUNTER_MAX = (1 << 16) - 1
...
class PatternCreditSearchResult:
    status: int
    conflict_limit: int
    ...
    def state_sha256(self) -> str:
        state = _mapping(self.pattern["state"], "pattern.state")
...
def build_native_pattern_credit_search(
    *, source: str | Path, output: str | Path
    ...
    if build_error is not None:
        raise build_error
    if result is None:
        raise O1RelationalSearchError("pattern-credit native build failed")
    return result


def write_pattern_credit_potential(
    path: str | Path, field: CriticalityPotentialField
) -> str:
    return write_pair_envelope_potential(path, field)


def write_pattern_credit_decision_variables(
...
def _mapping(value: object, field: str) -> Mapping[str, object]:
    if not isinstance(value, Mapping):
        raise O1RelationalSearchError(f"pattern-credit {field} differs")
    return value
...
def _nonnegative_integer_group(
    ...
    if set(payload) != names:
        raise O1RelationalSearchError(f"pattern-credit {field} fields differ")
    result: dict[str, int] = {}
    for name, raw in payload.items():
        if isinstance(raw, bool) or not isinstance(raw, int) or raw < 0:
            raise O1RelationalSearchError(f"pattern-credit {field}.{name} differs")
        result[str(name)] = raw
    return result
...
def _read_input(path: str | Path, field: str) -> tuple[Path, bytes, str]:
    try:
        ...
        raise O1RelationalSearchError(f"pattern-credit {field} input differs") from exc
    if not payload:
        raise O1RelationalSearchError(f"pattern-credit {field} input differs")
...
def _decision_values(payload: bytes) -> tuple[int, ...]:
    try:
        ...
        raise O1RelationalSearchError(
            "pattern-credit decision-variable input differs"
        ) from exc
    if (
        len(values) != PATTERN_CREDIT_VARIABLES
        or len(set(values)) != len(values)
        or any(not 1 <= value <= 256 for value in values)
    ):
        raise O1RelationalSearchError(
            "pattern-credit requires exactly 63 unique ordered key pairs"
        )
    return values
...
def _potential_field(payload: bytes) -> CriticalityPotentialField:
    try:
        ...
        raise O1RelationalSearchError("pattern-credit potential input differs") from exc
...
def _verify_stable_input(
    ...
    try:
        ...
        raise O1RelationalSearchError(
            f"pattern-credit {field} changed during execution"
        ) from exc
...
def _sat_signed(value: int) -> int:
    return min(PATTERN_CREDIT_MAX, max(PATTERN_CREDIT_MIN, value))


def pattern_owner_weight(*, current_level: int, owner_level: int) -> int:
    ...
    if (
        ...
        raise O1RelationalSearchError("pattern-credit owner weight differs")
    return 32 >> min(current_level - owner_level, 4)


def pattern_action_credit_update(
    credits: tuple[int, int, int, int],
    owners: tuple[tuple[int, int], tuple[int, int]],
    *,
    current_level: int,
    new_level: int,
    ...
    """Reference exact-mask owner-undo update; not used in the native loop."""
    ...
    if (
        ...
        or any(
            ...
            or not PATTERN_CREDIT_MIN <= credit <= PATTERN_CREDIT_MAX
        ...
        or any(owner[0] > current_level for owner in owners)
        or not isinstance(conflict_since_previous_backtrack, bool)
    ):
        raise O1RelationalSearchError("pattern-credit bounded update differs")
    penalties = [0, 0, 0, 0]
    remaining: list[tuple[int, int]] = []
    undone = 0
    for level, mask in owners:
        if level > new_level:
            penalties[mask] += (1 + int(conflict_since_previous_backtrack)) * (
                pattern_owner_weight(current_level=current_level, owner_level=level)
            )
    ...
    return (
        cast(
            tuple[int, int, int, int],
            tuple(
                _sat_signed(credit - penalties[mask])
                for mask, credit in enumerate(credits)
            ),
        ),
        cast(tuple[tuple[int, int], tuple[int, int]], tuple(remaining)),
        undone,
        cast(tuple[int, int, int, int], tuple(penalties)),
    )
...
def _state_blocks(
    ...
    if len(groups) != PATTERN_CREDIT_GROUPS:
        raise O1RelationalSearchError("pattern-credit group-state count differs")
    action_output = bytearray()
    owner_output = bytearray()
    for index, group in enumerate(groups):
        if set(group) != _GROUP_FIELDS:
            raise O1RelationalSearchError(
                f"pattern-credit state group {index} fields differ"
            )
        ...
        if any(
            ...
            raise O1RelationalSearchError(
                f"pattern-credit state group {index} value differs"
            )
        ...
        if (
            ...
            raise O1RelationalSearchError(
                f"pattern-credit state group {index} contract differs"
            )
        actions_raw = group["actions"]
        if not isinstance(actions_raw, list) or len(actions_raw) != 4:
            raise O1RelationalSearchError(
                f"pattern-credit state group {index} actions differ"
            )
        for mask, raw_action in enumerate(actions_raw):
            ...
            if set(action) != _ACTION_FIELDS:
                raise O1RelationalSearchError(
                    f"pattern-credit state group {index} action fields differ"
                )
            action_values = tuple(
                action[name]
                for name in (
            ...
            if any(
                ...
                raise O1RelationalSearchError(
                    f"pattern-credit state group {index} action value differs"
                )
            ...
            if (
                ...
                or not PATTERN_CREDIT_MIN <= credit <= PATTERN_CREDIT_MAX
                or any(
                    not 0 <= counter <= PATTERN_CREDIT_COUNTER_MAX
                    for counter in (visits, conflicts, backtracks)
                )
            ):
                raise O1RelationalSearchError(
                    f"pattern-credit state group {index} action contract differs"
                )
...
def run_pattern_credit_search(
    *,
    ...
    if (
        ...
        raise O1RelationalSearchError(
            "pattern-credit search limit, seed, or timeout differs"
        )
    ...
    if not set(decisions).issubset(field.observed_variables):
        raise O1RelationalSearchError(
            "pattern-credit decision variables are absent from potential"
        )
    command = [
    ...
    if execution_error is not None:
        raise O1RelationalSearchError(
            "pattern-credit search execution failed"
        ) from execution_error
    if completed is None:
        raise O1RelationalSearchError("pattern-credit search execution failed")
    if completed.returncode:
        detail = completed.stderr.strip() or completed.stdout.strip()
        raise O1RelationalSearchError(
            f"pattern-credit search execution failed: {detail}"
        )
    try:
        payload = json.loads(completed.stdout)
    except json.JSONDecodeError as exc:
        raise O1RelationalSearchError("pattern-credit search JSON is invalid") from exc
    if not isinstance(payload, Mapping) or set(payload) != _TOP_LEVEL_FIELDS:
        raise O1RelationalSearchError("pattern-credit result fields differ")
    variables = payload["variables"]
    reported_limit = payload["conflict_limit"]
    reported_seed = payload["seed"]
    status = payload["status"]
    if (
        payload["schema"] != PATTERN_CREDIT_RESULT_SCHEMA
        ...
        raise O1RelationalSearchError("pattern-credit result contract differs")
    model_hex = payload["key_model_hex"]
    if status == 10:
        if not isinstance(model_hex, str) or len(model_hex) != 64:
            raise O1RelationalSearchError("SAT pattern-credit result lacks key")
        try:
            key_model = bytes.fromhex(model_hex)
        except ValueError as exc:
            raise O1RelationalSearchError(
                "pattern-credit key encoding differs"
            ) from exc
    elif model_hex is not None:
        raise O1RelationalSearchError("non-SAT pattern-credit result contains key")
    else:
        key_model = None
    ...
    if set(pattern) != _PATTERN_FIELDS:
        raise O1RelationalSearchError("pattern-credit pattern fields differ")
    ...
    identity_integers = (
        ...
        (pattern["pair_count"], PATTERN_CREDIT_GROUPS),
    ...
    if any(
        ...
        raise O1RelationalSearchError("pattern-credit identity differs")
    reported_offset = pattern["offset"]
    if (
        pattern["decision_rule"] != PATTERN_CREDIT_DECISION_RULE
        ...
        or pattern["update_formula"] != PATTERN_CREDIT_UPDATE_FORMULA
        or pattern["selection_formula"] != PATTERN_CREDIT_SELECTION_FORMULA
    ):
        raise O1RelationalSearchError("pattern-credit identity differs")
    ...
    backtrack_credit = _nonnegative_integer_group(
        pattern["backtrack_credit"],
        field="pattern.backtrack_credit",
    ...
    if set(selection) != _SELECTION_FIELDS:
        raise O1RelationalSearchError("pattern-credit selection fields differ")
    for name in (
        "cold_group_selections",
        "credit_modulated_group_selections",
        "zero_gap_fallbacks",
        ...
        if isinstance(value, bool) or not isinstance(value, int) or value < 0:
            raise O1RelationalSearchError(f"pattern-credit selection.{name} differs")
    ...
    if (
        ...
        or distinct_selected > 4 * PATTERN_CREDIT_GROUPS
        or differentiated > PATTERN_CREDIT_GROUPS
        or penalized > 4 * PATTERN_CREDIT_GROUPS
    ):
        raise O1RelationalSearchError("pattern-credit selection metric ledger differs")
    if not _is_sha256(selection["trace_sha256"]):
        raise O1RelationalSearchError("pattern-credit selection trace differs")
    ...
    if tickets["opened"]:
        if (
            ...
            or not 0 <= first_group < PATTERN_CREDIT_GROUPS
            or isinstance(first_mask, bool)
            or not isinstance(first_mask, int)
            or not 0 <= first_mask < 4
        ):
            raise O1RelationalSearchError("pattern-credit first cold selection differs")
    elif first_group is not None or first_mask is not None:
        raise O1RelationalSearchError(
            "pattern-credit empty selection contains first action"
        )
    if (
        ...
        raise O1RelationalSearchError("pattern-credit telemetry ledger differs")
    if (
        ...
        or backtrack_credit["eligible_undo_action_cells"]
        ...
        > backtrack_credit["eligible_undo_action_cells"]
        or backtrack_credit["eligible_undo_action_cells"]
        ...
        raise O1RelationalSearchError("pattern-credit backtrack-credit ledger differs")

    state = _mapping(pattern["state"], "pattern.state")
    if set(state) != _STATE_FIELDS:
        raise O1RelationalSearchError("pattern-credit state fields differ")
    groups_raw = state["groups"]
    if not isinstance(groups_raw, list) or any(
        not isinstance(group, Mapping) for group in groups_raw
    ):
        raise O1RelationalSearchError("pattern-credit group states differ")
    ...
    state_integers = (
        (state["bytes_per_group"], PATTERN_CREDIT_BYTES_PER_GROUP),
        (
        ...
        (state["bounded_state_bytes"], PATTERN_CREDIT_STATE_BYTES),
        (state["credit_min"], PATTERN_CREDIT_MIN),
        (state["credit_max"], PATTERN_CREDIT_MAX),
        (state["counter_max"], PATTERN_CREDIT_COUNTER_MAX),
        (state["owner_level_max"], (1 << 32) - 1),
        (state["owner_mask_max"], 3),
    )
    if (
        state["encoding"] != PATTERN_CREDIT_STATE_ENCODING
        or state["action_encoding"] != PATTERN_CREDIT_ACTION_STATE_ENCODING
        or state["owner_encoding"] != PATTERN_CREDIT_OWNER_STATE_ENCODING
        or state["counter_semantics"] != PATTERN_CREDIT_COUNTER_SEMANTICS
        or any(
            isinstance(value, bool) or not isinstance(value, int) or value != expected
        ...
        raise O1RelationalSearchError("pattern-credit bounded state differs")
    for name in ("saturated_credit_updates", "saturated_counter_updates"):
        value = state[name]
        if isinstance(value, bool) or not isinstance(value, int) or value < 0:
            raise O1RelationalSearchError(f"pattern-credit state.{name} differs")
    ...
    if (
        ...
        raise O1RelationalSearchError("pattern-credit owner state differs")
    ...
    if (
        ...
        raise O1RelationalSearchError("pattern-credit owner ledger differs")
    actions = [
    ...
    if sum(cast(int, action["visits"]) for action in actions) > tickets["closed"]:
        raise O1RelationalSearchError("pattern-credit state visit ledger differs")
    if any(
        cast(int, action["credit"]) > 0
        or cast(int, action["conflict_hits"]) > cast(int, action["backtrack_hits"])
        for action in actions
    ):
        raise O1RelationalSearchError("pattern-credit action state differs")
    ...
    if (
        ...
        raise O1RelationalSearchError("pattern-credit differentiation ledger differs")
    ...
    observed_credit_penalty = -sum(cast(int, action["credit"]) for action in actions)
    if (
        ...
        raise O1RelationalSearchError("pattern-credit penalty state differs")
    if any(delta > stats[name] for name, delta in deltas.items()):
        raise O1RelationalSearchError(
            "pattern-credit solver counter delta ledger differs"
        )
    ...
    normalized_pattern["backtrack_credit"] = backtrack_credit
    ...
    return PatternCreditSearchResult(
        status=status,
...
write_criticality_potential = write_pattern_credit_potential
write_decision_variables = write_pattern_credit_decision_variables
