"""O1C-0075 bounded, nonrepeating causal-residency science stream.
...
This module deliberately owns its execution and recovery state machine.  It
does not call O1C-0074's two-chunk loader or its static-projection executor.
"""
...
from .o1c75_apple8_causal_residency_prepare import (
...
ATTEMPT_ID = "O1C-0075"
CONFIG_SCHEMA = "o1-256-apple8-causal-residency-stream-config-v1"
PREFLIGHT_SCHEMA = "o1-256-apple8-causal-residency-stream-preflight-v1"
TARGET_FREE_GATE_SCHEMA = (
    "o1-256-o1c75-target-free-causal-residency-stream-preflight-v1"
)
TARGET_FREE_GATE_CLASSIFICATION = (
    "O1C75_TARGET_FREE_CAUSAL_RESIDENCY_STREAM_PREFLIGHT_PASS"
)
INVOCATION_SCHEMA = "o1-256-apple8-causal-residency-stream-invocation-v1"
INTENT_SCHEMA = "o1-256-apple8-causal-residency-stream-intent-v1"
EPISODE_SCHEMA = "o1-256-apple8-causal-residency-stream-episode-v1"
RESULT_SCHEMA = "o1-256-apple8-causal-residency-stream-result-v1"
RECOVERY_SOURCE_SCHEMA = (
    "o1-256-apple8-causal-residency-stream-pre-finalization-source-v1"
)
PUBLICATION_RECOVERY_SCHEMA = (
    "o1-256-apple8-causal-residency-stream-publication-recovery-v1"
)
...
CONFIG_RELATIVE = Path("configs/o1c75_apple8_causal_residency_stream_v1.json")
TARGET_FREE_PREFLIGHT_RELATIVE = Path(
    "research/O1C0075_TARGET_FREE_CAUSAL_RESIDENCY_STREAM_PREFLIGHT_20260720.json"
)
RESULT_RELATIVE = Path(
    "research/O1C0075_APPLE8_CAUSAL_RESIDENCY_STREAM_RESULT_20260720.json"
)
CAPSULE_SUFFIX = "O1C-0075_apple8-causal-residency-stream-v1"
...
PARENT_SOURCE_COMMIT = "a5f2ad130e2e13c39a5e888f927d86d5fdd68d78"
PARENT_LAST_LINEAGE_ORDINAL = 13
RANK_SOURCE_SHA256 = (
...
INITIAL_CHUNK_CLAUSES = (202, 311, 0, 37, 0, 0)
INITIAL_NEVER_RESIDENT_DEBT = 79

ACTIVE_CLAUSE_LIMIT = 256
LOCAL_EPISODES = (0, 1)
LINEAGE_ORDINALS = (14, 15)
REQUESTED_CONFLICTS_PER_EPISODE = 128
MAXIMUM_NATIVE_SOLVER_CALLS = 2
MAXIMUM_TOTAL_REQUESTED_CONFLICTS = 256
SEED = 0
...
EXPECTED_NATIVE_EXECUTABLE_SHA256 = (
    "58be5089730798ad83a0a3e1cb705ced208c7dc10e710e69b0e3c2d92771dbcb"
)
...
NOVEL_CLAUSE_GAIN = "CAUSAL_RESIDENCY_STREAM_NOVEL_CLAUSE_GAIN"
NO_GAIN = "CAUSAL_RESIDENCY_STREAM_NO_NOVEL_GAIN"
OPERATIONAL_TERMINAL = "CAUSAL_RESIDENCY_STREAM_OPERATIONAL_TERMINAL"
...
SOURCE_PATHS = {
    ...
    "preparation": (
        "src/o1_crypto_lab/o1c75_apple8_causal_residency_prepare.py"
    ),
...
class O1C75RunError(RuntimeError):
...
def _mapping(value: object, field: str) -> Mapping[str, object]:
    if not isinstance(value, Mapping) or not all(
        ...
        raise O1C75RunError(f"{field} differs")
...
def _relative_contract(value: object, field: str) -> str:
    ...
    if path.is_absolute() or not path.parts or ".." in path.parts:
        raise O1C75RunError(f"{field} escapes the lab")
...
def _relative(root: Path, value: object, field: str) -> Path:
    ...
    try:
        ...
        raise O1C75RunError(f"{field} cannot be resolved") from exc
...
def _regular(path: Path, field: str) -> Path:
    try:
        ...
        raise O1C75RunError(f"{field} cannot be read") from exc
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
        raise O1C75RunError(f"{field} is not a regular file")
    return path
...
def _sha256_file(path: Path) -> str:
    ...
    try:
        ...
        raise O1C75RunError(f"cannot hash {path}") from exc
...
def _read_json(path: Path, field: str) -> Mapping[str, object]:
    ...
    try:
        ...
        raise O1C75RunError(f"{field} JSON differs") from exc
...
def _atomic_create(path: Path, payload: bytes, *, immutable: bool = False) -> None:
    if not isinstance(payload, bytes) or path.exists() or path.is_symlink():
        raise O1C75RunError(f"owned artifact already exists: {path.name}")
    try:
        ...
        if immutable:
            path.chmod(0o444)
    except O1C75RunError:
        raise
    except OSError as exc:
        raise O1C75RunError(f"cannot publish owned artifact: {path.name}") from exc
    if path.read_bytes() != payload:
        raise O1C75RunError(f"owned artifact reread differs: {path.name}")
...
def _canonical_gzip(payload: bytes) -> bytes:
    if not isinstance(payload, bytes):
        raise O1C75RunError("native evidence payload differs")
    output = io.BytesIO()
    with gzip.GzipFile(
    ...
    try:
        ...
        raise O1C75RunError("native evidence compression differs") from exc
    if restored != payload:
        raise O1C75RunError("native evidence compression differs")
    return compressed
...
def _read_compressed_json(
    ...
    if metadata.get("schema") != NATIVE_EVIDENCE_SCHEMA:
        raise O1C75RunError(f"{field} schema differs")
    name = metadata.get("path")
    if not isinstance(name, str) or Path(name).name != name:
        raise O1C75RunError(f"{field} path differs")
    ...
    if (
        ...
        raise O1C75RunError(f"{field} compressed artifact differs")
    try:
        payload = gzip.decompress(compressed)
        value = json.loads(payload)
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise O1C75RunError(f"{field} payload differs") from exc
    if (
        len(payload)
        ...
        raise O1C75RunError(f"{field} canonical payload differs")
    return _mapping(value, field)
...
def _validate_artifact_row(root: Path, value: object, field: str) -> Path:
    ...
    if path_value.is_absolute() or ".." in path_value.parts:
        raise O1C75RunError(f"{field} path escapes")
    ...
    if (
        ...
        raise O1C75RunError(f"{field} artifact differs")
    return path
...
def _validate_regular_capsule_tree(capsule: Path) -> None:
    try:
        ...
        if stat.S_ISLNK(root_metadata.st_mode) or not stat.S_ISDIR(
            ...
            raise O1C75RunError("capsule tree root differs")
        for path in capsule.rglob("*"):
            mode = path.lstat().st_mode
            if stat.S_ISLNK(mode) or not (
                stat.S_ISREG(mode) or stat.S_ISDIR(mode)
            ):
                raise O1C75RunError("capsule tree contains a non-regular entry")
    except O1C75RunError:
        raise
    except OSError as exc:
        raise O1C75RunError("capsule tree cannot be inspected") from exc
    ...
    *,
    ...
    try:
        ...
        if vault.identity != identity:
            raise O1C75RunError("causal-residency vault identity differs")
        return vault
    except O1C75RunError:
        raise
    except ThresholdNoGoodVaultError as exc:
        raise O1C75RunError("causal-residency vault artifact differs") from exc
...
def _parse_occurrence_document(
    ...
    if document.get("schema") != CAUSAL_ATTIC_OCCURRENCE_SCHEMA:
        raise O1C75RunError("occurrence schema differs")
    ...
    for ordinal, raw in enumerate(records):
        ...
        if row.get("ordinal") != ordinal or union_index >= len(clauses):
            raise O1C75RunError("occurrence ordinal differs")
...
def _coerce_prepared(value: PreparedResidency) -> PreparedStream:
    ...
    try:
        ...
        raise O1C75RunError("prepared causal-residency object differs") from exc
    if (
        directory is not None and not isinstance(directory, Path)
    ) or not isinstance(state, CausalResidencyState):
        raise O1C75RunError("prepared causal-residency object differs")
...
def load_prepared_stream(
    ...
    """Load the O1C75 preparation through its attempt-specific validator."""

    try:
        prepared = load_prepared_residency(
            directory, expected_manifest_sha256=expected_manifest_sha256
        )
    except Exception as exc:
        if isinstance(exc, O1C75RunError):
            raise
        raise O1C75RunError("prepared causal-residency validation differs") from exc
...
def prepared_stream_from_state(
    ...
    *,
    ...
    if not isinstance(state, CausalResidencyState):
        raise O1C75RunError("prepared residency state differs")
...
def _validate_archived_v16_provenance(
    *,
    ...
    try:
        ...
        raise O1C75RunError("archived native conflict provenance differs") from exc
    ...
    if (
        ...
        raise O1C75RunError("archived native v16 provenance differs")
...
def _validated_episode_result(
    ...
    *,
    ...
    """Bind one adapter-certified v16 return to its exact residency page."""

    try:
        if require_concrete_result and not isinstance(
            result, _native_v16.JointScoreSieveV16Result
        ):
            raise O1C75RunError("native result type differs")
        ...
        if (
            ...
            raise O1C75RunError("native split-vault result binding differs")
        ...
        if peak > MEMORY_LIMIT_BYTES or wall > int(TIMEOUT_SECONDS * 1_000_000):
            raise O1C75RunError("native resource boundary differs")
        ...
        if pending != 0:
            raise O1C75RunError("native result retained an incomplete clause")
        ...
        if (
            ...
            raise O1C75RunError("native emitted ledger input binding differs")
        ...
        if not isinstance(eligible, Sequence) or isinstance(
            ...
            raise O1C75RunError("native typed emission ledger differs")
        for item, occurrence in zip(eligible, parsed.occurrences, strict=True):
            if (
                ...
                raise O1C75RunError("native typed emission occurrence differs")
        ...
        if not isinstance(available, bool):
            raise O1C75RunError("native next-vault availability differs")
        if available:
            if terminal_reason is not None or not isinstance(
                next_vault, ThresholdNoGoodVault
            ):
                raise O1C75RunError("available native next vault differs")
        elif terminal_reason not in CAPACITY_REASONS or next_vault is not None:
            raise O1C75RunError("native rollover reason differs")
        ...
        if status == 10:
            ...
            raise O1C75RunError("non-SAT stream result returned a candidate")
        ...
        raise O1C75RunError("native O1C-0075 result differs") from exc
...
def validate_native_executable(
    ...
    try:
        ...
        if (
            ...
            raise O1C75RunError("native executable mode differs")
        ...
        raise O1C75RunError("native executable cannot be read") from exc
    ...
    if (
        ...
        raise O1C75RunError("native executable identity differs")
...
def _initial_artifacts(
    ...
    if initial.exists():
        raise O1C75RunError("initial causal-residency directory already exists")
...
def _activation_ledger_document(
    ...
    raise O1C75RunError("residency activation ledger differs")
...
def _validate_call_window_executable(
    ...
    if observed != binding:
        raise O1C75RunError(f"native executable changed {when} call")
...
def execute_stream(
    *,
    ...
    """Consume exactly local 0/1 and lineage 14/15, without replay or retry."""
    ...
    if (
        ...
        raise O1C75RunError("stream execution input differs")
    try:
        validate_activation_replay(prepared.state)
    except CausalResidencyError as exc:
        raise O1C75RunError("initial residency replay differs") from exc
    ...
    if rank_path.read_bytes() != prepared.rank_source.serialized:
        raise O1C75RunError("rank-source capsule bytes differ")
    ...
    for local_ordinal, lineage_ordinal in zip(
        ...
        if reread_active.serialized != active_before.serialized:
            raise O1C75RunError("episode active input reread differs")
        intent = {
        ...
        try:
            ...
            stream_id = f"o1c75-episode-{local_ordinal:02d}"
            ...
            try:
                ...
                raise O1C75RunError("episode residency reprojection differs") from exc
            if (
            ...
            if reread_rollover.serialized != rollover.serialized:
                raise O1C75RunError("rollover chunk reread differs")
            ...
            if recovered_active.serialized != next_state.active_projection.serialized:
                raise O1C75RunError("episode active output reread differs")
            occurrence_delta = {
    ...
    if native_calls != MAXIMUM_NATIVE_SOLVER_CALLS or requested_total != (
        ...
        raise O1C75RunError("stream bounded call ledger differs")
    classification = NOVEL_CLAUSE_GAIN if global_novel_total else NO_GAIN
    reason = (
        "globally-novel-exact-clauses-retained-in-causal-residency"
...
def build_result(
    *,
    ...
    if (
        ...
        raise O1C75RunError("result input differs")
    return {
        ...
        "claim_boundary": {
            ...
            "stopped_after_second_fresh_call_even_on_gain": (
        ...
        "next_action": (
            "Do not retry O1C-0074 or replay lineage 10..15; preserve the "
            "complete attic and evaluate only the frozen O1C-0075 result gate."
        ),
    }
...
def write_recovery_source(capsule: Path, result: Mapping[str, object]) -> Path:
    ...
    if (
        ...
        raise O1C75RunError("pre-finalization recovery result differs")
...
def _markdown(result: Mapping[str, object]) -> bytes:
    ...
    return (
        ...
        "- Active residency limit: `256` clauses\n"
...
def _manifest_payload(
    ...
    if capsule.is_symlink() or not capsule.is_dir():
        raise O1C75RunError("capsule manifest root differs")
    payloads: dict[str, bytes] = {}
    for path in capsule.rglob("*"):
        metadata = path.lstat()
        if stat.S_ISLNK(metadata.st_mode):
            raise O1C75RunError("capsule manifest contains a symlink")
        if stat.S_ISREG(metadata.st_mode):
            relative = path.relative_to(capsule).as_posix()
            if relative != "artifacts.sha256" and relative not in virtual:
                payloads[relative] = path.read_bytes()
        elif not stat.S_ISDIR(metadata.st_mode):
            raise O1C75RunError("capsule manifest contains a special file")
...
def finalize_capsule(
    ...
    if (
        ...
        raise O1C75RunError("terminal O1C75 publication already exists")
    ...
    for _ in range(16):
        ...
        raise O1C75RunError("persistent artifact ledger did not converge")
    result_payload = canonical_json_bytes(result)
    run_payload = _markdown(result)
    manifest, persistent = _manifest_payload(
        capsule, {"RUN.md": run_payload, "result.json": result_payload}
    ...
    if (
        ...
        raise O1C75RunError("persistent artifact byte budget differs")
    ...
    if authoritative.read_bytes() != result_payload:
        raise O1C75RunError("authoritative result reread differs")


def _existing_authoritative(path: Path) -> dict[str, object] | None:
    if not path.exists():
        return None
    value = dict(_read_json(path, "authoritative O1C75 result"))
    if value.get("schema") != RESULT_SCHEMA or value.get("attempt_id") != ATTEMPT_ID:
        raise O1C75RunError("authoritative O1C75 result differs")
    if canonical_json_bytes(value) != path.read_bytes():
        raise O1C75RunError("authoritative O1C75 result is not canonical")
    return value
...
def _republish_sealed_capsule(
    ...
    try:
        ...
        raise O1C75RunError("sealed capsule escapes lab") from exc
    if (
        ...
        raise O1C75RunError("sealed O1C75 capsule differs")
    payload = result_path.read_bytes()
    if canonical_json_bytes(result) != payload:
        raise O1C75RunError("sealed O1C75 result canonical bytes differ")
...
def load_config(
    ...
    """Load the complete frozen O1C75 contract without writing or calling."""

    lab = (root or lab_root()).resolve(strict=True)
    config_path = Path(path).resolve(strict=True)
    if not config_path.is_relative_to(lab):
        raise O1C75RunError("O1C75 config escapes the lab")
    config = dict(_read_json(config_path, "O1C75 config"))
    required_root = {
        "schema",
        "attempt_id",
        "parent",
    ...
    if (
        ...
        raise O1C75RunError("frozen O1C75 config fields differ")
    ...
    if (
        ...
        raise O1C75RunError("frozen O1C74 parent differs")
    preparation = _mapping(config["preparation"], "config preparation")
    if set(preparation) != {"directory", "manifest_sha256"}:
        raise O1C75RunError("prepared causal-residency config differs")
    ...
    if set(inputs) != required_inputs:
        raise O1C75RunError("frozen stream inputs differ")
    ...
    if (
        ...
        != "build/o1c75/native-joint-score-sieve"
        ...
        raise O1C75RunError("native split-vault config differs")
    ...
    if executable_digest not in ("PENDING", EXPECTED_NATIVE_EXECUTABLE_SHA256):
        raise O1C75RunError("frozen native executable digest differs")
    ...
    if (
        ...
        raise O1C75RunError("source freeze config differs")
    ...
    if native.get("expected_source_sha256") != expected_sources.get("native_v13"):
        raise O1C75RunError("native/source digest binding differs")
    gate = _mapping(
    ...
    if (
        ...
        raise O1C75RunError("target-free preflight config differs")
    ...
    if dict(budgets) != expected_budgets:
        raise O1C75RunError("frozen O1C75 budgets differ")
    return config
...
def _validate_initial_contract(prepared: PreparedStream) -> None:
    ...
    if facts != expected:
        raise O1C75RunError("frozen prepared causal residency differs")
    if any(
        digest in INHERITED_SCIENCE_INPUT_SHA256
        for digest in state.used_active_sha256[1:]
    ):
        raise O1C75RunError("prepared residency reuses inherited science input")
    validate_activation_replay(state)
...
def _validate_target_free_gate(
    ...
    if sha256_bytes(payload) != expected_sha256:
        raise O1C75RunError("target-free preflight digest differs")
    try:
        value = json.loads(payload)
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise O1C75RunError("target-free preflight JSON differs") from exc
    row = _mapping(value, "target-free preflight")
    required_true = (
        "two_page_coverage_fixture_passed",
        "no_repeat_fixture_passed",
        "inherited_debt_first_fixture_passed",
        "dynamic_hot_event_fixture_passed",
        "duplicate_occurrence_fixture_passed",
    ...
    if (
        ...
        raise O1C75RunError("target-free preflight contract differs")
    return dict(row)
...
def preflight(
    ...
    *,
    ...
    if (
        ...
        raise O1C75RunError("O1C74 parent artifact identity differs")
    ...
    for name in ("cnf", "potential", "grouping", "o1c73_config"):
        ...
        if observed != expected:
            raise O1C75RunError(f"frozen input {name} differs")
    ...
    for name, relative in paths.items():
        ...
        if observed != expected:
            raise O1C75RunError(f"source {name} differs")
    ...
    try:
        ...
        raise O1C75RunError("source commit binding cannot be established") from exc
    if require_commit_binding:
        try:
            ...
            if ancestor.returncode != 0:
                raise O1C75RunError("source freeze is not an execution ancestor")
            for name, relative in paths.items():
            ...
            raise O1C75RunError("source commit blob binding differs") from exc
        if dirty:
            raise O1C75RunError("execution worktree is not clean")
    try:
        disk_free = shutil.disk_usage(lab).free
    except OSError as exc:
        raise O1C75RunError("disk resource preflight failed") from exc
    if disk_free < MINIMUM_DISK_FREE_BYTES:
        raise O1C75RunError("disk resource preflight differs")
    return {
...
def _rebuild_initial_from_capsule(
    ...
    if not chunk_rows:
        raise O1C75RunError("recovery initial chunks differ")
    ...
    for index, raw in enumerate(chunk_rows):
        ...
        if row.get("chunk_index") != index:
            raise O1C75RunError("recovery initial chunk ordering differs")
        ...
        try:
            ...
            raise O1C75RunError("recovery initial chunk differs") from exc
        chunks.append(chunk)
    ...
    try:
        ...
        raise O1C75RunError("recovery initial attic differs") from exc
    ...
    if _read_json(relation_path, "recovery initial relations") != (
        ...
        raise O1C75RunError("recovery initial relations differ")
    ...
    try:
        ...
        raise O1C75RunError("recovery initial residency differs") from exc
    if (
        ...
        raise O1C75RunError("recovery initial residency artifacts differ")
    return state
...
def _recover_completed_episode(
    *,
    ...
    if journal != expected or journal.get("schema") != EPISODE_SCHEMA:
        raise O1C75RunError("recovery episode journal differs")
    ...
    if (
        ...
        raise O1C75RunError("recovery intent binding differs")
    active_path = _validate_artifact_row(
    ...
    if (
        ...
        raise O1C75RunError("recovery active input differs")
    ...
    try:
        parsed = parse_vault_telemetry(
            telemetry_payload,
            stream_id=f"o1c75-episode-{local:02d}",
            expected_sha256=sha256_bytes(telemetry_payload),
        )
    except CausalAtticError as exc:
        raise O1C75RunError("recovery occurrence ledger differs") from exc
    if (
        parsed.input_identity != active.identity
        or parsed.input_vault_sha256 != active.sha256
        or parsed.input_clause_count != active.clause_count
        ...
        raise O1C75RunError("recovery telemetry input differs")
    ...
    if status not in (0, 10, 20):
        raise O1C75RunError("recovery native status differs")
    work = _mapping(expected.get("work"), "recovery work")
    try:
        stats = validate_vault_soft_conflict_ledger(work)
    except Exception as exc:
        raise O1C75RunError("recovery conflict ledger differs") from exc
    ...
    if (
        ...
        raise O1C75RunError("recovery native episode ledger differs")
    raw_model = native_raw.get("key_model_hex")
    if status == 10:
        if not isinstance(raw_model, str) or len(raw_model) != 64:
            raise O1C75RunError("recovery public model encoding differs")
        try:
            key_model: bytes | None = bytes.fromhex(raw_model)
        except ValueError as exc:
            raise O1C75RunError("recovery public model encoding differs") from exc
    else:
        key_model = None
    _validate_archived_v16_provenance(
        raw=native_raw,
    ...
    if (
        ...
        raise O1C75RunError("recovery native/public provenance differs")
    ...
    for occurrence in parsed.occurrences:
        ...
        if key not in globally_known:
            if occurrence.classification != "new":
                raise O1C75RunError("recovery global novelty differs")
            globally_known.add(key)
            expected_novel.append(occurrence.clause)
    if chunk.clauses != tuple(expected_novel):
        raise O1C75RunError("recovery rollover population differs")
    try:
        next_state = advance_causal_residency(
            state,
            chunk=chunk,
        ...
        raise O1C75RunError("recovery residency reprojection differs") from exc
    consumed_science_inputs = science_inputs | {active.sha256}
    if next_state.active_projection.sha256 in consumed_science_inputs:
        raise O1C75RunError("recovery residency repeats a science input")
    ...
    if (
        ...
        or occurrence_delta.get("stream_id") != f"o1c75-episode-{local:02d}"
        or occurrence_delta.get("occurrence_count") != len(parsed.occurrences)
        or occurrence_delta.get("records") != expected_records
    ):
        raise O1C75RunError("recovery occurrence delta differs")
    ...
    if (
        ...
        raise O1C75RunError("recovery residency boundary differs")
...
def _read_recovery_source(
    ...
    if recovery_source is None:
        ...
        if canonical_json_bytes(source) != source_path.read_bytes():
            raise O1C75RunError("pre-finalization recovery source bytes differ")
    else:
        source = _read_json(Path(recovery_source), "pre-finalization recovery source")
        if canonical_json_bytes(source) != source_path.read_bytes():
            raise O1C75RunError("pre-finalization recovery source path differs")
    ...
    if (
        ...
        raise O1C75RunError("pre-finalization recovery source contract differs")
...
def recover_publication(
    ...
    """Replay sealed O1C75 sidecars without issuing native/verification calls."""
    ...
    if (
        ...
        raise O1C75RunError("recovery invocation differs")
    invocation_sha = _sha256_file(invocation_path)
    ...
    if bindings.get("test_fixture") != "synthetic-target-free":
        ...
        if (
            ...
            raise O1C75RunError("publication recovery freeze differs")
    ...
    if len(episodes) > MAXIMUM_NATIVE_SOLVER_CALLS:
        raise O1C75RunError("recovery episode count differs")
    science_inputs: set[str] = set(INHERITED_SCIENCE_INPUT_SHA256)
    stopped = False
    for index, raw_episode in enumerate(episodes):
        ...
        if (
            ...
            raise O1C75RunError("recovery episode schedule differs")
        if episode.get("completed") is True:
            ...
            if (
                ...
                raise O1C75RunError("recovery failed episode differs")
            stopped = True
        else:
            raise O1C75RunError("recovery episode completion differs")
    ...
    if derived_classification is None:
        if len(episodes) != MAXIMUM_NATIVE_SOLVER_CALLS:
            raise O1C75RunError("recovery unterminated stream differs")
    ...
    if (
        ...
        raise O1C75RunError("publication recovery conclusion differs")
...
def run(config_path: str | Path = CONFIG_RELATIVE) -> dict[str, object]:
    """Execute, recover, or idempotently return the frozen two-page stream."""
    ...
    if existing_capsules:
        if len(existing_capsules) != 1:
            raise O1C75RunError("multiple existing O1C75 capsules block replay")
        ...
        if not (capsule / RECOVERY_SOURCE_NAME).is_file():
            raise O1C75RunError(
                "partial O1C75 capsule without pre-finalization recovery "
                "source blocks replay"
            )
        if any((capsule / name).exists() for name in ("RUN.md", "result.json")):
            raise O1C75RunError(
                "partially published O1C75 capsule requires manual recovery"
            )
    ...
    if executable_binding != preflight_row.get("native_executable"):
        raise O1C75RunError("post-preflight native executable differs")
    ...
    _atomic_create(
        ...
        (
            ...
            "o1_crypto_lab.o1c75_apple8_causal_residency_stream_run run "
    ...
    def invoke(
        ...
        if (
            ...
            or _sha256_file(active_vault) in INHERITED_SCIENCE_INPUT_SHA256
        ):
            raise O1C75RunError("native residency invocation identity differs")
        observed_executable = validate_native_executable(
            executable,
            expected_sha256=cast(str, native["expected_executable_sha256"]),
        )
        if observed_executable != executable_binding:
            raise O1C75RunError("native executable changed in call window")
        ...
        if (
            ...
            raise O1C75RunError("native executable changed during call window")
        return result
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Preflight, run, or recover O1C75's causal-residency stream"
    )
    ...
    try:
        if args.command == "preflight":
            ...
            if not capsule.is_relative_to(root / "runs"):
                raise O1C75RunError("recovery capsule escapes run root")
            value = recover_publication(capsule)
            finalize_capsule(capsule, root / RESULT_RELATIVE, value)
        sys.stdout.buffer.write(canonical_json_bytes(value))
        return 0
    except O1C75RunError as exc:
        print(f"O1C75: {exc}", file=sys.stderr)
        return 2
