"""O1C-0068: one complementary phase-reader call from O1C-0067's vault.

O1C-0067 consumed lineage ordinal 3 and sealed a 12-clause fixed point for the
default initial phase.  O1C-0068 imports that exact vault, persists a new local
intent for lineage ordinal 4, and authorizes exactly one native-v7 process whose
only scientific change is the forced complementary initial phase.
"""
...
from . import joint_score_sieve_v10 as _native_v10
from . import o1c67_apple8_episodic_vault_continuation_run as _o1c67
...
_o1c65 = _o1c67._o1c65
_o1c66 = _o1c67._o1c66
_vault_v1 = _o1c67._vault_v1

ATTEMPT_ID = "O1C-0068"
CONFIG_SCHEMA = "o1-256-apple8-complementary-phase-config-v1"
PREFLIGHT_SCHEMA = "o1-256-apple8-complementary-phase-preflight-v1"
INVOCATION_SCHEMA = "o1-256-apple8-complementary-phase-invocation-v1"
INTENT_SCHEMA = "o1-256-apple8-complementary-phase-intent-v1"
EPISODE_SCHEMA = "o1-256-apple8-complementary-phase-episode-v1"
RESULT_SCHEMA = "o1-256-apple8-complementary-phase-result-v1"
FAILURE_EVIDENCE_SCHEMA = "o1-256-o1c68-native-failure-evidence-v1"

INVOCATION_ID = "O1C-0068-apple8-complementary-phase-v1-call-0004"
CAPSULE_SUFFIX = "O1C-0068_apple8-complementary-phase-v1"
PUBLICATION_SOURCE_NAME = "publication_source.json"
RESULT_RELATIVE = Path(
    "research/O1C0068_APPLE8_COMPLEMENTARY_PHASE_RESULT_20260719.json"
)
DESIGN_RELATIVE = Path("research/O1C0068_APPLE8_COMPLEMENTARY_PHASE_DESIGN_20260719.md")
CONFIG_RELATIVE = Path("configs/o1c68_apple8_complementary_phase_v1.json")
...
PARENT_SOURCE_COMMIT = "865634458ef3f5b01a5881208eb028404b96f135"
...
PARENT_LINEAGE_CALLS = 4
PARENT_COMPLETED_EPISODES = 1
PARENT_LAST_CONSUMED_ORDINAL = 3
PARENT_LAST_COMPLETED_ORDINAL = 3
PARENT_FAILED_ORDINAL = 2
PARENT_KNOWN_COMPLETED_BILLED_CONFLICTS = 1_539
...
READER_SPEC_SHA256 = "a68b3c3b1721b756314dac11ce725adf0709e9f358125cb1f8d388737d1ddddc"
...
NATIVE_RESULT_SCHEMA = "o1-256-cadical-joint-score-sieve-result-v7"
NATIVE_IMPLEMENTATION_PARENT_SCHEMA = "o1-256-cadical-joint-score-sieve-result-v6"
NATIVE_ADAPTER_SCHEMA = "o1-256-joint-score-sieve-v10-adapter-v1"
NATIVE_LEDGER_SCHEMA = _native_v10.O1C67_VAULT_SOFT_CONFLICT_LEDGER_SCHEMA
NATIVE_VAULT_TELEMETRY_SCHEMA = _o1c67.NATIVE_VAULT_TELEMETRY_SCHEMA

PUBLIC_EXACT_RECOVERY = "PUBLIC_EXACT_RECOVERY"
COMPLEMENTARY_PHASE_GAIN = "EPISODIC_VAULT_COMPLEMENTARY_PHASE_GAIN"
COMPLEMENTARY_PHASE_NO_GAIN = "EPISODIC_VAULT_COMPLEMENTARY_PHASE_NO_GAIN"
THRESHOLD_REGION_EXHAUSTED = "EPISODIC_VAULT_THRESHOLD_REGION_EXHAUSTED"
CAPACITY_TERMINAL = "EPISODIC_VAULT_CAPACITY_TERMINAL"
OPERATIONAL_TERMINAL = "EPISODIC_VAULT_COMPLEMENTARY_PHASE_OPERATIONAL_TERMINAL"
INVALID_RESULT_TERMINAL = "EPISODIC_VAULT_COMPLEMENTARY_PHASE_INVALID_RESULT"

SOURCE_RELATIVES = {
    "runner": "src/o1_crypto_lab/o1c68_apple8_complementary_phase_run.py",
    ...
    "native_source": "native/cadical_o1_joint_score_sieve_v7.cpp",
...
class O1C68RunError(RuntimeError):
    """O1C-0068 provenance, protocol, reader, or result differs."""


class O1C68ParentError(O1C68RunError):
    """The sealed O1C-0067 parent or retained vault differs."""


class EpisodeInvoker(Protocol):
    def __call__(self, local_ordinal: int, lineage_ordinal: int, vault: Path) -> object:
        """Make the one authorized fresh native-v7 process call."""
...
def _mapping(value: object, field: str) -> Mapping[str, object]:
    if not isinstance(value, dict) or not all(isinstance(key, str) for key in value):
        raise O1C68RunError(f"{field} must be an object")
    return cast(Mapping[str, object], value)


def _sequence(value: object, field: str) -> Sequence[object]:
    if not isinstance(value, list):
        raise O1C68RunError(f"{field} must be an array")
    return cast(Sequence[object], value)


def _nonnegative_int(value: object, field: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or value < 0:
        raise O1C68RunError(f"{field} differs")
    return value
...
def _relative(root: Path, value: object, field: str) -> Path:
    ...
    if candidate.is_absolute() or ".." in candidate.parts:
        raise O1C68RunError(f"{field} escapes the lab")
    try:
        path = (root / candidate).resolve(strict=True)
    except (OSError, RuntimeError) as exc:
        raise O1C68RunError(f"{field} cannot be resolved") from exc
...
def _read_json(path: Path, field: str) -> Mapping[str, object]:
    try:
        ...
        raise O1C68RunError(f"{field} cannot be read") from exc
...
def _validate_reader(value: object, field: str = "reader") -> dict[str, object]:
    ...
    if set(reader) != set(READER_BINDING):
        raise O1C68RunError(f"{field} fields differ")
    integer_fields = {
        "phase_before_override": 1,
        "seed": 0,
        "quiet": 1,
...
def load_config(path: str | Path) -> dict[str, object]:
    """Load the exact O1C-0068 protocol without a call or filesystem write."""

    root = lab_root().resolve(strict=True)
    config_path = Path(path).resolve(strict=True)
    if not config_path.is_relative_to(root):
        raise O1C68RunError("config escapes the lab")
    config = dict(_read_json(config_path, "O1C68 config"))
    ...
    if (
        ...
        or config.get("slug") != "apple8-complementary-phase-v1"
        ...
        or parent.get("attempt_id") != "O1C-0067"
        ...
        or parent.get("classification") != _o1c67.SATURATED_NO_GAIN
        ...
        raise O1C68RunError("frozen O1C-0068 config differs")
    ...
    if source_native != expected["native_source"] or executable == "":
        raise O1C68RunError("native source/executable binding differs")
    return config
...
def _manifest_inventory(capsule: Path) -> dict[str, str]:
    ...
    if sha256_file(manifest) != PARENT_MANIFEST_SHA256:
        raise O1C68ParentError("O1C-0067 manifest identity differs")
    inventory: dict[str, str] = {}
    try:
        for line in manifest.read_text(encoding="ascii").splitlines():
            digest, separator, relative = line.partition("  ")
            if separator != "  " or relative in inventory:
                raise O1C68ParentError("O1C-0067 manifest syntax differs")
            _sha256(digest, "O1C-0067 manifest digest")
            path = Path(relative)
            if path.is_absolute() or ".." in path.parts:
                raise O1C68ParentError("O1C-0067 manifest path escapes")
            inventory[relative] = digest
    except (OSError, UnicodeError, ValueError) as exc:
        raise O1C68ParentError("O1C-0067 manifest cannot be read") from exc
    return inventory
...
def validate_parent_and_import_vault(
    ...
    """Verify and dual-parse O1C-0067's exact sealed output vault."""
    ...
    if (
        ...
        raise O1C68ParentError("O1C-0067 parent identity differs")
    result = _read_json(result_path, "O1C-0067 result")
    episode = _mapping(result.get("episode"), "O1C-0067 episode")
    final_vault = _mapping(result.get("final_vault"), "O1C-0067 final vault")
    resources = _mapping(result.get("resources"), "O1C-0067 resources")
    parent = _mapping(result.get("parent"), "O1C-0067 lineage parent")
    claim = _mapping(result.get("claim_boundary"), "O1C-0067 claim boundary")
    if (
        result.get("attempt_id") != "O1C-0067"
        or result.get("classification") != _o1c67.SATURATED_NO_GAIN
        ...
        or result.get("lineage_call_ordinal") != 3
        ...
        or parent.get("known_completed_billed_conflicts") != 1_025
        or parent.get("failed_call_billed_conflicts") is not None
        ...
        raise O1C68ParentError("O1C-0067 terminal evidence differs")
    ...
    if any(inventory.get(name) != digest for name, digest in required.items()):
        raise O1C68ParentError("O1C-0067 manifest inventory differs")
    for relative, digest in required.items():
        if sha256_file(capsule / relative) != digest:
            raise O1C68ParentError("O1C-0067 capsule artifact differs")
    ...
    try:
        ...
        if (
            ...
            raise O1C68ParentError("retained vault file identity differs")
        payload = source.read_bytes()
    except O1C68ParentError:
        raise
    except OSError as exc:
        raise O1C68ParentError("retained vault cannot be read") from exc
    if hashlib.sha256(payload).hexdigest() != PARENT_RETAINED_VAULT_SHA256:
        raise O1C68ParentError("retained vault hash differs")
    ...
    try:
        ...
        raise O1C68ParentError("retained vault dual parse differs") from exc
    if (
        ...
        raise O1C68ParentError("retained vault parsers disagree")
...
def preflight(
    ...
    for name in SOURCE_NAMES:
        if expected[name] != "PENDING" and observed[name] != expected[name]:
            raise O1C68RunError(f"source hash differs for {name}")
    if require_commit_binding and unresolved:
        raise O1C68RunError("science source hashes remain PENDING")
    if (
        getattr(_native_v10, "JOINT_SCORE_SIEVE_ADAPTER_SCHEMA", None)
        != NATIVE_ADAPTER_SCHEMA
        or getattr(_native_v10, "JOINT_SCORE_SIEVE_RESULT_SCHEMA", None)
        != NATIVE_RESULT_SCHEMA
        or getattr(_native_v10, "JOINT_SCORE_SIEVE_IMPLEMENTATION_PARENT_SCHEMA", None)
        != NATIVE_IMPLEMENTATION_PARENT_SCHEMA
    ):
        raise O1C68RunError("adapter-v10 schema binding differs")
    ...
    if require_commit_binding:
        if not clean:
            raise O1C68RunError("science sources/config are not clean")
        for name in SOURCE_NAMES:
            _commit_bound_bytes(
    ...
    if memory_free is not None and memory_free < MINIMUM_MEMORY_FREE_PERCENT:
        raise O1C68RunError("memory-pressure preflight is below gate")
    if disk_free < MINIMUM_DISK_FREE_BYTES:
        raise O1C68RunError("disk-free preflight is below gate")
    if normalized_load > MAXIMUM_NORMALIZED_LOAD_1M:
        raise O1C68RunError("normalized system load is above gate")
    if active:
        raise O1C68RunError("conflicting science process is active")
    return {
        ...
        "adapter_source_sha256": observed["joint_score_sieve_v10"],
...
def _validated_native(
    ...
    try:
        ...
        stats = _native_v10.validate_vault_soft_conflict_ledger(
        ...
        if (
            ...
            raise O1C68RunError("native O1C-0068 contract differs")
    ...
    except O1C68RunError:
        raise
    except Exception as exc:
        raise O1C68RunError("native O1C-0068 result differs") from exc
...
def execute_single_continuation(
    *,
    ...
    """Persist ordinal 4, consume exactly one call, and never retry it."""

    if not capsule.is_dir() or imported_vault.sha256 != PARENT_RETAINED_VAULT_SHA256:
        raise O1C68RunError("single-continuation capsule or vault differs")
    ...
    if imported_path.read_bytes() != adapter_vault.serialized:
        raise O1C68RunError("dual-parsed imported vault bytes differ")
    ...
    if reread != imported_vault:
        raise O1C68RunError("capsule imported vault reread differs")
    ...
    if not invocation_path.is_file() or not intent_path.is_file():
        raise O1C68RunError("invocation/intent not durable before call")
    ...
    try:
        ...
        if not isinstance(eligible_raw, Sequence):
            raise O1C68RunError("eligible emitted clause sequence differs")
        ...
        if (
            ...
            raise O1C68RunError("native input vault differs")
        ...
        if status == 10:
            if not isinstance(key_model, bytes) or not verify_public_model(key_model):
                raise O1C68RunError("SAT model failed public eight-block verification")
        elif key_model is not None:
            raise O1C68RunError("non-SAT continuation returned a key")
        ...
        if not isinstance(next_available, bool):
            raise O1C68RunError("next-vault availability differs")
        if next_available:
            if terminal_reason is not None or adapter_next is None:
                raise O1C68RunError("available next vault differs")
            ...
            if parsed_next != expected_next:
                raise O1C68RunError("cumulative continuation vault differs")
        else:
            if (
                terminal_reason
                not in {
                ...
                raise O1C68RunError("terminal next vault differs")
        ...
        if archive_output:
            ...
            if (
                ...
                raise O1C68RunError("continuation output vault reread differs")
        ...
        if status == 10:
            ...
            classification, stop_reason = (
                COMPLEMENTARY_PHASE_GAIN,
                "novel-exact-clauses",
            )
        else:
            classification, stop_reason = (
                COMPLEMENTARY_PHASE_NO_GAIN,
...
def validate_native_build_identity(
    ...
    *,
    ...
    try:
        ...
        raise O1C68RunError("native-v7 build identity differs") from exc
    if (
        ...
        raise O1C68RunError("native-v7 build identity differs")
...
def invoke_native_episode(
    *, executable: Path, cnf: Path, potential: Path, grouping: Path, vault: Path
    ...
    if vault.is_symlink() or sha256_file(vault) != PARENT_RETAINED_VAULT_SHA256:
        raise O1C68RunError("complementary-phase input vault differs before call")
    return _native_v10.run_joint_score_sieve(
    ...
    *,
    ...
    return {
        ...
        "claim_boundary": {
            ...
            "only_science_change": "forced-complementary-initial-phase-reader",
            "reader_spec_sha256": READER_SPEC_SHA256,
        ...
        "next_action": (
            "Do not replay lineage ordinal 4 or sweep phase/horizon; choose the "
...
def _markdown(result: Mapping[str, object]) -> str:
    return (
        ...
        "- New native calls: `1`\n"
        "- Local / lineage ordinal: `0 / 4`\n"
        "- Reader: `forcephase=true`, `phase=0` (false polarity), seed `0`\n"
...
def finalize_capsule(
    ...
    *,
    ...
    if authoritative.exists() or (capsule / "artifacts.sha256").exists():
        raise O1C68RunError("O1C-0068 terminal publication already exists")
    ...
    for _ in range(12):
        ...
        raise O1C68RunError("persistent artifact ledger did not converge")
    if persistent > MAXIMUM_PERSISTENT_ARTIFACT_BYTES:
        raise O1C68RunError("persistent artifact byte budget exceeded")
    ...
    try:
        ...
        if authoritative.read_bytes() != payload or result_path.read_bytes() != payload:
            raise O1C68RunError("terminal publication bytes differ")
        _o1c65._assert_immutable_tree(capsule)
    except Exception:
        _o1c65._restore_owned_capsule_for_recovery(capsule)
        if manifest_published:
            _o1c65._unlink_owned_exact(
                manifest_path, manifest, "O1C68 capsule manifest"
            )
        if authoritative_published:
            _o1c65._unlink_owned_exact(
                authoritative, payload, "O1C68 authoritative result"
            )
        raise
...
def _validate_recovery_file(
    ...
    *,
    ...
    try:
        ...
        raise O1C68RunError(f"{field} cannot be read during recovery") from exc
    if path.is_symlink() or not stat.S_ISREG(status.st_mode):
        raise O1C68RunError(f"{field} differs during recovery")
    if expected_bytes is not None:
        size = _nonnegative_int(expected_bytes, f"{field} bytes")
        if status.st_size != size:
            raise O1C68RunError(f"{field} size differs during recovery")
    if sha256_file(path) != expected_sha:
        raise O1C68RunError(f"{field} hash differs during recovery")
...
def _validate_recovery_sidecars(
    ...
    if (
        ...
        raise O1C68RunError("recovery invocation/intent journal differs")
    ...
    if terminal is None:
        ...
        if native_failure.exists() or terminal_failure.exists():
            raise O1C68RunError("completed recovery has failure sidecars")
        ...
        if (
            ...
            raise O1C68RunError("completed recovery vault/result differs")
        if archived:
            if episode.get("output_vault_sidecar") != "episodes/00/vault-output.bin":
                raise O1C68RunError("recovery output vault path differs")
            _validate_recovery_file(
                vault_output,
            ...
            raise O1C68RunError("unarchived recovery output vault differs")
        return
    ...
    if (
        ...
        raise O1C68RunError("failed recovery journal differs")
    failure_sha = terminal.get("native_execution_failure_sha256")
    if failure_sha is None:
        if native_failure.exists():
            raise O1C68RunError("invalid-result recovery failure sidecar differs")
    else:
        _validate_recovery_file(
            native_failure,
...
def _validate_publication_source(root: Path, capsule: Path) -> dict[str, object]:
    if (
        ...
        raise O1C68RunError("O1C-0068 recovery capsule differs")
    source = dict(
        _read_json(capsule / PUBLICATION_SOURCE_NAME, "O1C-0068 publication source")
    )
    ...
    persisted = _read_json(
        capsule / "episodes" / "00" / "episode.json", "persisted O1C-0068 episode"
    )
    if (
        ...
        raise O1C68RunError("O1C-0068 publication source differs")
...
def recover_publication(
    *,
    ...
    if authoritative.exists():
        raise O1C68RunError("O1C-0068 authoritative publication already exists")
    ...
    failure = {
        ...
        "retry_authorized": False,
        "original_science_classification": source.get("classification"),
        "original_science_stop_reason": source.get("stop_reason"),
        "reader": dict(READER_BINDING),
...
def run(config_path: str | Path) -> dict[str, object]:
    ...
    if authoritative.exists():
        raise O1C68RunError("O1C-0068 already exists")
    if partial_capsules:
        if len(partial_capsules) != 1:
            raise O1C68RunError("multiple O1C-0068 recovery capsules exist")
    ...
    with tempfile.TemporaryDirectory(prefix="o1c68-apple8-complementary-phase-") as raw:
        workspace = Path(raw)
        native_build = _native_v10.build_native_joint_score_sieve(
        ...
        if observed_sources != {
            ...
            raise O1C68RunError("source identity changed after preflight")
        ...
        _atomic_bytes(
            ...
            (
                ...
                "o1_crypto_lab.o1c68_apple8_complementary_phase_run run "
        ...
        bindings = {
            ...
            "adapter_source_sha256": observed_sources["joint_score_sieve_v10"],
        ...
        def invoke(local_ordinal: int, lineage_ordinal: int, vault: Path) -> object:
            if (
                ...
                raise O1C68RunError("complementary-phase ordinal differs")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Preflight or run O1C-0068's one complementary phase call"
    )
