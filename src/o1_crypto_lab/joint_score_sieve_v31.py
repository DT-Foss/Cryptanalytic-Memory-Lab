"""Fail-closed O1C-0099 adapter for Page 17 and the evolved live bank.

Version 31 preserves v30's proof-mining and result-validation semantics while
moving the native boundary to result/source v28 and fresh lineage-30 Page 17.
...
JOINT_SCORE_SIEVE_ADAPTER_SCHEMA = "o1-256-joint-score-sieve-v31-adapter-v1"
JOINT_SCORE_SIEVE_RESULT_SCHEMA = "o1-256-cadical-joint-score-sieve-result-v28"
JOINT_SCORE_SIEVE_IMPLEMENTATION_PARENT_SCHEMA = (
    "o1-256-cadical-joint-score-sieve-result-v6"
)
PRIORITY_STATE_SCHEMA = (
    "o1-256-o1c99-live-parent-centered-continuation-priority-state-v1"
)
PRIORITY_ACTION_SCHEMA = "o1-256-o1c99-failure-first-proof-mining-actions-v1"
...
LIVE_BANK_TOTAL_COUNT = 316_312
LIVE_BANK_MAXIMUM_COUNT = 2_921
LIVE_BANK_MINIMUM_NONZERO_COUNT = 230
LIVE_BANK_ELIGIBLE_COORDINATES = 255

O1C98_MANIFEST_SCHEMA = "o1-256-o1c98-page17-causal-rollover-preparation-v1"
O1C98_MANIFEST_BYTES = 18_785
O1C98_MANIFEST_SHA256 = (
    "ba7ad5d9417542d62725ab588dea4a85bc7eff8847f5276bf79a847f44c5470d"
)
O1C98_MANIFEST_ARTIFACTS = frozenset(
    {
        "lineage-30-new-chunk.vault",
        "page-17-active.bin",
        "residency.json",
...
O1C98_PUBLISHED_ARTIFACTS = O1C98_MANIFEST_ARTIFACTS | {
    "causal-rollover-preparation-manifest.json"
}
O1C98_MANIFEST_RELATIVE = Path(
    "research/o1c98_page17_causal_rollover_seed_20260720/"
    "causal-rollover-preparation-manifest.json"
)
O1C98_PAGE17_RELATIVE = Path(
    "research/o1c98_page17_causal_rollover_seed_20260720/page-17-active.bin"
)

O1C97_PRIORITY_RECEIPT_SCHEMA = (
    "o1-256-o1c97-live-parent-centered-continuation-priority-state-v1"
)
O1C97_PRIORITY_RECEIPT_BYTES = 52_011
O1C97_PRIORITY_RECEIPT_SHA256 = (
    "050551fc658de62b54b7856996fba0418194c3c2f2608e04a8e9ccc2f51fedad"
)
PRODUCTION_PAGE16_BYTES = 2_773_919
PRODUCTION_PAGE16_SHA256 = (
    "0c25ce470df0945fb05914bab107ecea05531166575ec88ebf7d15bb9a22fbfd"
)
PRODUCTION_PAGE16_LINEAGE_ORDINAL = 30
PRODUCTION_PAGE16_ACTIVE_LIMIT = 249
PRODUCTION_PAGE16_CLAUSE_COUNT = 249
PRODUCTION_PAGE16_LITERAL_COUNT = 693_183
...
NATIVE_SOURCE_RELATIVE = Path("native/cadical_o1_joint_score_sieve_v28.cpp")
NATIVE_SOURCE_BYTES = 50_566
NATIVE_SOURCE_SHA256 = (
    "4a7449229c75d672c091ded2e98b65f53c73cae25277af839429ed7216147e6d"
)
O1C98_LIVE_BANK_RELATIVE = Path(
    "research/o1c98_page17_causal_rollover_seed_20260720/"
    "final-parent-centered-priority-bank.bin"
)
O1C97_PRIORITY_RECEIPT_RELATIVE = Path(
    "research/o1c98_page17_causal_rollover_seed_20260720/"
    "o1c97-priority-state-receipt.json"
)
...
class JointScoreSieveV31Error(O1RelationalSearchError):
    """Native-v28 execution or continuation validation failed."""
...
@dataclass(frozen=True)
class JointScoreSieveV31Result:
    """Typed native-v28 result and its validated evolved bank."""
...
def _error(field: str) -> JointScoreSieveV31Error:
    return JointScoreSieveV31Error(f"joint-score-sieve-v31 {field} differs")
...
def _read_page17(path: str | Path) -> tuple[Path, bytes]:
    io_v1 = _v9._v8._v7._v1
    resolved, payload, digest = io_v1._read_input(path, "Page17")
    ...
    if len(payload) != PRODUCTION_PAGE16_BYTES or digest != PRODUCTION_PAGE16_SHA256:
        raise _error("Page17 active projection seal")
    return resolved, payload
...
def _manifest_contract() -> tuple[str, int, str, frozenset[str]]:
    """Return the sealed O1C-0098 Page-17 causal-rollover contract."""

    return (
        O1C98_MANIFEST_SCHEMA,
        _positive(O1C98_MANIFEST_BYTES, "O1C98 manifest byte count"),
        _sha(O1C98_MANIFEST_SHA256, "O1C98 manifest digest"),
        O1C98_MANIFEST_ARTIFACTS,
    )
...
def _disabled_o1c96_manifest_validator(payload: bytes) -> None:
    """Historical v30 contract retained as non-executable provenance only.
    ...
    try:
        ...
        raise _error("O1C98 manifest JSON") from exc
    if canonical != payload:
        raise _error("O1C98 canonical manifest encoding")
    if (
        len(payload) != expected_bytes
        or hashlib.sha256(payload).hexdigest() != expected_sha256
    ):
        raise _error("O1C98 manifest seal")
    ...
    page17 = _mapping(root.get("page17"), "manifest Page17")
    debt = _mapping(page17.get("debt_completion"), "manifest Page17 debt")
    ...
    if (
        ...
        or root.get("attempt_id") != "O1C-0098"
        or set(artifacts) != artifact_names
        or O1C98_PUBLISHED_ARTIFACTS
        != artifact_names | {O1C98_MANIFEST_RELATIVE.name}
        or zero_call
        != {
        ...
        or page17.get("lineage_ordinal") != PRODUCTION_PAGE16_LINEAGE_ORDINAL
        or page17.get("active_limit") != PRODUCTION_PAGE16_ACTIVE_LIMIT
        or page17.get("active_sha256") != PRODUCTION_PAGE16_SHA256
        or page17.get("clause_count") != PRODUCTION_PAGE16_CLAUSE_COUNT
        or page17.get("literal_count") != PRODUCTION_PAGE16_LITERAL_COUNT
        or page17.get("serialized_bytes") != PRODUCTION_PAGE16_BYTES
        or page17.get("category_counts") != PRODUCTION_PAGE16_CATEGORY_COUNTS
        or page17.get("headroom") != PRODUCTION_PAGE16_HEADROOM
        or page17.get("selected_union_indices_sha256")
        != "27f64155956dea34b73a26808aca324b60550908a43a2ebd9cc23ef889c77166"
        or page17.get("selection_order_sha256")
        != "3337fc6f229c2bbbd503d5c1ab880eaf4c7e20dccaf5bcfa36c4a9808e557b42"
        or page17.get("fresh_identity") is not True
        or page17.get("activation_ledger_prior_entry_count") != 16
        or page17.get("activation_ledger_next_entry_count") != 17
        or page17.get("activation_entries_added") != 1
        or debt
        != {
        ...
        raise _error("O1C98 Page17 lineage-29 manifest contract")
    ...
    _artifact_contract(
        ...
        role="fresh-lineage-29-page17-science-input",
    )
    _artifact_contract(
        artifacts,
        "final-parent-centered-priority-bank.bin",
...
def _validate_manifest(payload: bytes) -> None:
    """Validate the sealed zero-call O1C98 Page-17 rollover contract."""
    ...
    current_duplicate = _mapping(
        rollover.get("current_duplicate"), "manifest current duplicate"
    )
    attic = _mapping(root.get("attic"), "manifest attic")
    page17 = _mapping(root.get("page17"), "manifest Page17")
    capacity = _mapping(page17.get("native_capacity_proof"), "manifest native capacity")
    clause_headroom = _mapping(
        capacity.get("clause_headroom_guarantee"), "manifest clause headroom"
    )
    carried = _mapping(root.get("carried_context"), "manifest carried context")
    final_bank = _mapping(root.get("final_priority_bank"), "manifest final bank")

    if (
        set(root) != expected_top
        or root.get("schema") != schema
        or root.get("attempt_id") != "O1C-0098"
        or set(artifacts) != artifact_names
        or O1C98_PUBLISHED_ARTIFACTS != artifact_names | {O1C98_MANIFEST_RELATIVE.name}
        or zero_call
        != {
            "native_solver_calls": 0,
            "native_preflight_calls": 0,
        ...
        or parent.get("attempt_id") != "O1C-0097"
        or parent.get("classification")
        ...
        or parent.get("source_active_sha256") != BURNED_PAGE16_SHA256
        ...
        or parent.get("global_novelty_baseline_clause_count") != 1_812
        or science.get("imported_science_attempt_id") != "O1C-0097"
        or science.get("imported_fully_emitted_clause_count") != 263
        or science.get("imported_fully_emitted_literal_count") != 748_011
        or science.get("imported_globally_novel_clause_count") != 262
        or science.get("imported_globally_novel_literal_count") != 745_152
        or science.get("imported_current_duplicate_clause_count") != 1
        or science.get("imported_current_duplicate_literal_count") != 2_859
        or science.get("historical_failed_or_retry_evidence_imported") is not False
        or science.get("priority_state_magnitude_alone_imported_as_science")
        is not False
        ...
        or rollover.get("literal_count") != 745_152
        or rollover.get("serialized_bytes") != 2_981_847
        or rollover.get("fully_emitted_occurrence_count") != 263
        or rollover.get("classification_counts") != {"current_duplicate": 1, "new": 262}
        or current_duplicate.get("emission_index") != 7
        or current_duplicate.get("duplicate_source_emission_index") != 6
        or current_duplicate.get("clause_sha256")
        != "d479f1335c455aa61873154205c94b1a98cb050a0851fc8df65a5ed536baee2f"
        or current_duplicate.get("literal_count") != 2_859
        or attic.get("chunk_count") != 19
        or attic.get("union_clause_count") != 2_074
        or attic.get("union_literal_count") != 5_835_680
        or attic.get("union_serialized_bytes") != 23_351_207
        or attic.get("occurrence_count") != 2_083
        or attic.get("duplicate_occurrence_count") != 9
        or attic.get("strict_subsumption_pair_count") != 14
        or attic.get("undominated_clause_count") != 2_063
        or page17.get("lineage_ordinal") != PRODUCTION_PAGE16_LINEAGE_ORDINAL
        or page17.get("active_limit") != PRODUCTION_PAGE16_ACTIVE_LIMIT
        or page17.get("active_sha256") != PRODUCTION_PAGE16_SHA256
        or page17.get("clause_count") != PRODUCTION_PAGE16_CLAUSE_COUNT
        ...
        or page17.get("headroom") != PRODUCTION_PAGE16_HEADROOM
        or page17.get("fresh_identity") is not True
        or clause_headroom.get("native_vault_maximum_clauses") != 512
        or clause_headroom.get("page17_input_clauses") != 249
        or clause_headroom.get("maximum_additional_clauses_before_capacity_terminal")
        != 263
        or clause_headroom.get("parent_centered_action_capacity") != 256
        or clause_headroom.get("spare_clause_slots_beyond_action_capacity") != 7
        or clause_headroom.get("equal_parent_burst_clause_count") != 263
        or clause_headroom.get("equal_parent_burst_fits_exactly") is not True
        or clause_headroom.get("proved_sufficient") is not True
        ...
        or final_bank.get("serialized_bytes") != BANK_BYTES
        or final_bank.get("sha256") != LIVE_BANK_SHA256
        or final_bank.get("receipt_serialized_bytes") != O1C97_PRIORITY_RECEIPT_BYTES
        or final_bank.get("receipt_sha256") != O1C97_PRIORITY_RECEIPT_SHA256
        or final_bank.get("maximum_evolved_count") != LIVE_BANK_MAXIMUM_COUNT
        or final_bank.get("minimum_nonzero_evolved_count")
        != LIVE_BANK_MINIMUM_NONZERO_COUNT
        or final_bank.get("aggregate_evolved_count") != LIVE_BANK_TOTAL_COUNT
        ...
        or final_bank.get("receipt_bank_hex_byte_equal") is not True
        or final_bank.get("priority_is_key_bit_belief") is not False
        or final_bank.get("semantic_role") != "sealed-evolved-live-continuation-bytes"
    ):
        raise _error("O1C98 Page17 lineage-30 manifest contract")
    ...
    expected_artifacts = {
        ...
        "o1c97-priority-state-receipt.json": (
            O1C97_PRIORITY_RECEIPT_BYTES,
            O1C97_PRIORITY_RECEIPT_SHA256,
            "canonical-o1c97-evolved-priority-state-receipt",
        ),
    }
    for name, (serialized_bytes, sha256, role) in expected_artifacts.items():
        _artifact_contract(
            artifacts,
            name,
            serialized_bytes=serialized_bytes,
...
def _validate_receipt(payload: bytes, bank: bytes) -> None:
    try:
        document = _mapping(
            ...
            "O1C97 priority receipt",
        )
        if document.get("schema") != O1C97_PRIORITY_RECEIPT_SCHEMA:
            raise _error("O1C97 priority receipt schema")
        ...
        raise _error("O1C97 priority receipt contract") from exc
    if receipt_bank != bank:
        raise _error("O1C97 priority receipt bank linkage")
...
def _parse_native_payload(
    ...
    *,
...
) -> JointScoreSieveV31Result:
    """Validate one native-v28 document and its unchanged v6 lifecycle."""
    ...
    return JointScoreSieveV31Result(
        status=base.status,
...
def _validate_prelaunch(
    *,
    ...
    if _sha(expected_source_sha256, "expected native source digest") != (
        ...
        raise _error("native-v28 source identity")
    ...
    manifest, manifest_bytes = _read_exact(
        manifest_path,
        label="O1C98 manifest",
        expected_bytes=manifest_expected_bytes,
        expected_sha256=manifest_expected_sha256,
    )
    receipt, receipt_bytes = _read_exact(
        receipt_path,
        label="O1C97 priority receipt",
        expected_bytes=O1C97_PRIORITY_RECEIPT_BYTES,
        expected_sha256=O1C97_PRIORITY_RECEIPT_SHA256,
    )
    bank, bank_bytes = _read_live_bank(bank_path)
    page17, page17_bytes = _read_page17(page17_path)
...
def run_joint_score_sieve(
    *,
    ...
    sealed_page17_path: str | Path | None = None,
) -> JointScoreSieveV31Result:
    """Validate a prebuilt native-v28 executable, then launch it exactly once."""
    ...
    try:
        ...
        manifest = (
            ...
            else root / O1C98_MANIFEST_RELATIVE
        )
        receipt = (
            Path(priority_state_receipt_path)
            if priority_state_receipt_path is not None
            else root / O1C97_PRIORITY_RECEIPT_RELATIVE
        )
        page17 = (
            Path(sealed_page17_path)
            if sealed_page17_path is not None
            else root / O1C98_PAGE17_RELATIVE
        )
        ...
        if (
            ...
            or vault_bytes != sealed.page17_bytes
        ):
            raise _error("Page17 production seal")
        ...
        if (
            ...
            raise _error("Page17 parsed production seal")
        ...
        for original, resolved, before, name in (
            ...
            (manifest, sealed.manifest_path, sealed.manifest_bytes, "O1C98 manifest"),
            ...
            (page17, sealed.page17_path, sealed.page17_bytes, "sealed Page17"),
        ...
        message = str(exc)
        if not message.startswith("joint-score-sieve-v31"):
            message = f"joint-score-sieve-v31 adapter failed: {message}"
