"""Burn-on-intent runner for the single O1C-0107 Page-21 call.

The runner has one scientific side effect: after an immutable intent is
persisted it may invoke native v32 once, with seed zero and 128 requested
...
the published, sealed O1C-0106 type-safe causal-rollover bundle; it never regenerates
...
from . import joint_score_sieve_v35 as _native_v35
...
ACTIVE_PROJECTION_NAME = _o1c106.ACTIVE_PROJECTION_NAME
...
DEFAULT_PARENT_CAPSULE_RELATIVE = _o1c106.DEFAULT_PARENT_CAPSULE_RELATIVE
DEFAULT_PARENT_RESULT_RELATIVE = _o1c106.DEFAULT_PARENT_RESULT_RELATIVE
...
CONTINUATION_BANK_BYTES = _o1c106.FINAL_BANK_BYTES
CONTINUATION_BANK_SHA256 = _o1c106.FINAL_BANK_SHA256
PREPARED_CONTINUATION_BANK_NAME = _o1c106.FINAL_BANK_NAME
LOGICAL_KNOWN_CLAUSE_COUNT = _o1c104.LOGICAL_KNOWN_CLAUSE_COUNT
LOGICAL_KNOWN_SHA256 = _o1c104.LOGICAL_KNOWN_SHA256
NEW_CHUNK_NAME = _o1c106.NEW_CHUNK_NAME
...
PAGE21_ACTIVE_LIMIT = _o1c106.PAGE21_ACTIVE_LIMIT
PAGE21_ACTIVATION_DOCUMENT_BYTES = 67_070
PAGE21_ACTIVATION_DOCUMENT_SHA256 = (
    "cd74577f064a70f8725a0e11c1ced134814e2735e594945d8bdfb01827f5230a"
)
PAGE21_CATEGORY_COUNTS: Mapping[str, int] = {
    "emitted_hot_event": 0,
...
PAGE21_CLAUSE_COUNT = _o1c106.PAGE21_ACTIVE_LIMIT
PAGE21_HEADROOM: Mapping[str, int] = {
    "clauses": 265,
    "literals": 909_670,
    "serialized_bytes": 5_626_109,
}
PAGE21_LITERAL_COUNT = _o1c106.PAGE21_LITERAL_COUNT
PAGE21_RESIDENCY_DOCUMENT_BYTES = 474_289
PAGE21_RESIDENCY_DOCUMENT_SHA256 = (
    "b55e8cb25a84c64883bd5a90ff620f5c4e3bfb62960ae97ca2dfc4eef9987f75"
)
PAGE21_SERIALIZED_BYTES = _o1c106.PAGE21_SERIALIZED_BYTES
PAGE21_SHA256 = _o1c106.PAGE21_SHA256
PURE_EMITTED_CANDIDATE_SHA256 = _o1c106.PAGE20_PURE_EMITTED_SHA256
EMITTED_ONLY_ACTIVE_PROJECTION_SHA256 = (
    "99bb42bc553102d2b1c2ae37e80634490b4f63aba5c82ff307667c768a4fd138"
)
PARENT_CAPSULE_MANIFEST_SHA256 = _o1c106.PARENT_CAPSULE_MANIFEST_SHA256
PARENT_RESULT_SHA256 = _o1c106.PARENT_RESULT_SHA256
PARENT_INTENT_SHA256 = _o1c106.PARENT_INTENT_SHA256
PREPARATION_MANIFEST_NAME = _o1c106.PREPARATION_MANIFEST_NAME
PREPARATION_MANIFEST_BYTES = 9_376
PREPARATION_MANIFEST_SHA256 = (
    "91044c235473c1a24fdeeb283454babc5ebc800ea19236840dd7193d6f3c96c2"
)
PREPARATION_SCHEMA = _o1c106.PREPARATION_SCHEMA
...
CERTIFICATION_AUDIT_NAME = _o1c106.CERTIFICATION_AUDIT_NAME
CERTIFICATION_AUDIT_BYTES = _o1c106.CERTIFICATION_AUDIT_BYTES
CERTIFICATION_AUDIT_SHA256 = _o1c106.CERTIFICATION_AUDIT_SHA256
...
NATIVE_BUILD_SCHEMA = "o1-256-o1c107-native-v32-build-v2"
...
CONFIG_RELATIVE = Path("configs/o1c107_apple8_parent_centered_continuation_v1.json")
RESULT_RELATIVE = Path(
    "research/O1C0107_APPLE8_PARENT_CENTERED_CONTINUATION_RESULT_20260721.json"
)
CAPSULE_SUFFIX = "O1C-0107_apple8-parent-centered-continuation-v1"

LOCAL_EPISODE_ORDINAL = 0
LINEAGE_CALL_ORDINAL = 34
SEED = 0
...
NATIVE_V32_USAGE = (
    b"usage: cadical_o1_joint_score_sieve_v32 --cnf PATH "
...
PUBLISHED_ARTIFACT_SEALS: Mapping[str, tuple[int, str, str]] = {
    ACTIVATION_LEDGER_NAME: (
        PAGE21_ACTIVATION_DOCUMENT_BYTES,
        PAGE21_ACTIVATION_DOCUMENT_SHA256,
    ...
    ACTIVE_PROJECTION_NAME: (
        PAGE21_SERIALIZED_BYTES,
        PAGE21_SHA256,
        "fresh-lineage-34-v8-certified-composed-page21-science-input",
    ),
    CERTIFICATION_AUDIT_NAME: (
    ...
    RESIDENCY_NAME: (
        PAGE21_RESIDENCY_DOCUMENT_BYTES,
        PAGE21_RESIDENCY_DOCUMENT_SHA256,
...
SOURCE_PATHS = {
    "runner": "src/o1_crypto_lab/o1c107_apple8_parent_centered_continuation_run.py",
    "runner_tests": "tests/test_o1c107_apple8_parent_centered_continuation_run.py",
    "adapter_v35": "src/o1_crypto_lab/joint_score_sieve_v35.py",
    "adapter_v35_tests": "tests/test_joint_score_sieve_v35.py",
    ...
    "page21_type_safe_rollover_preparation": "src/o1_crypto_lab/o1c106_page21_type_safe_rollover_prepare.py",
    "page21_type_safe_rollover_preparation_tests": "tests/test_o1c106_page21_type_safe_rollover_prepare.py",
    ...
    "native_v32": "native/cadical_o1_joint_score_sieve_v32.cpp",
    "native_v32_tests": "tests/test_cadical_o1_joint_score_sieve_v32.py",
...
class O1C107RunError(RuntimeError):
...
def _mapping(value: object, field: str) -> Mapping[str, object]:
    if not isinstance(value, Mapping) or not all(isinstance(k, str) for k in value):
        raise O1C107RunError(f"{field} differs")
...
def _relative_text(value: object, field: str) -> str:
    ...
    if path.is_absolute() or not path.parts or ".." in path.parts:
        raise O1C107RunError(f"{field} escapes the lab")
...
def _lab_path(
    ...
    try:
        ...
        raise O1C107RunError(f"{field} cannot be resolved") from exc
    expected = (
    ...
    if (
        ...
        raise O1C107RunError(f"{field} is not a canonical lab path")
    return resolved


def _external_path(value: object, field: str, *, directory: bool = False) -> Path:
    if not isinstance(value, str) or not Path(value).is_absolute():
        raise O1C107RunError(f"{field} is not absolute")
    ...
    if stat.S_ISLNK(metadata.st_mode) or not expected:
        raise O1C107RunError(f"{field} is not canonical")
    return resolved
...
def _read_canonical_json(path: Path, field: str) -> Mapping[str, object]:
    ...
    try:
        ...
        raise O1C107RunError(f"{field} is not JSON") from exc
    if canonical_json_bytes(value) != payload:
        raise O1C107RunError(f"{field} is not canonical JSON")
    return value


def _atomic_create(path: Path, payload: bytes, *, mode: int = 0o444) -> None:
    if not isinstance(payload, bytes):
        raise O1C107RunError("atomic payload differs")
    ...
    try:
        ...
        if path.read_bytes() != payload:
            raise O1C107RunError(f"atomic write verification failed for {path.name}")
        path.chmod(mode)
        ...
        raise O1C107RunError(f"artifact already exists: {path.name}") from exc
...
def load_config(path: str | Path, *, root: Path | None = None) -> dict[str, object]:
    ...
    if not config_path.is_relative_to(lab) or config_path.is_symlink():
        raise O1C107RunError("config escapes the lab")
    config = dict(_read_canonical_json(config_path, "O1C107 config"))
    if _pending(config):
        raise O1C107RunError("config contains PENDING: " + ", ".join(_pending(config)))
    if (
        set(config)
        != {
            "schema",
        ...
        raise O1C107RunError("config fields differ")
    ...
    if (
        ...
        raise O1C107RunError("frozen O1C105 parent differs")
    ...
    if (
        ...
        raise O1C107RunError("frozen preparation source differs")
    ...
    if set(inputs) != {
        ...
        raise O1C107RunError("input fields differ")
    ...
    if (
        ...
        or native.get("adapter_schema") != _native_v35.JOINT_SCORE_SIEVE_ADAPTER_SCHEMA
        or native.get("result_schema") != _native_v35.JOINT_SCORE_SIEVE_RESULT_SCHEMA
        or native.get("page21_sha256") != PAGE21_SHA256
        or native.get("page21_bytes") != PAGE21_SERIALIZED_BYTES
    ...
    if dict(paths) != SOURCE_PATHS or set(digests) != set(SOURCE_PATHS):
        raise O1C107RunError("source inventory differs")
    ...
    if dict(budgets) != expected_budgets:
        raise O1C107RunError("budgets differ")
    return config
...
def _sibling_solver_pids() -> tuple[int, ...]:
    try:
        ...
        raise O1C107RunError("solver process census failed") from exc
    current = os.getpid()
    found: list[int] = []
...
def _validate_priority_receipt(payload: bytes, bank: bytes) -> Mapping[str, object]:
    try:
        ...
        raise O1C107RunError("priority state receipt is not JSON") from exc
    if canonical_json_bytes(receipt) != payload:
        raise O1C107RunError("priority state receipt is not canonical JSON")
    hexadecimal = receipt.get("bank_hex")
    try:
        receipt_bank = bytes.fromhex(cast(str, hexadecimal))
    except (TypeError, ValueError) as exc:
        raise O1C107RunError("priority state receipt bank differs") from exc
    if (
        receipt.get("schema")
        ...
        raise O1C107RunError("priority state receipt contract differs")
    return receipt
...
def _validate_occurrence_ledger(payload: bytes) -> frozenset[str]:
    ...
    if (
        ...
        raise O1C107RunError("published occurrence ledger contract differs")
    ...
    for ordinal, value in enumerate(records):
        ...
        if (
            ...
            raise O1C107RunError("published occurrence record differs")
    ...
    if (
        ...
        raise O1C107RunError("published attic union digest inventory differs")
    return digests
...
def _validate_logical_known_inventory(
    ...
    if (
        ...
        raise O1C107RunError("published logical known inventory differs")
    return logical
...
def _validate_published_documents(artifacts: Mapping[str, bytes]) -> frozenset[str]:
    ...
    certification = _canonical_payload(
        artifacts[CERTIFICATION_AUDIT_NAME],
        "published Page-21 v8 certification audit",
    )
    ...
    projection = _mapping(
        residency.get("current_projection"), "published Page21 projection"
    )
    projection_encoding = _mapping(
        projection.get("encoding_only"), "published Page21 encoding"
    )
    logical = _mapping(
        residency.get("logical_known_registry"), "published logical registry"
    )
    emitted = _mapping(residency.get("emitted_causal_attic"), "published emitted attic")
    ...
    emitted_selector_projection = _mapping(
        emitted_selector.get("encoding_only"),
        "published pure-emitted selector projection",
    )
    emitted_selector_encoding = _mapping(
        emitted_selector_projection.get("encoding_only"),
    ...
    if (
        activation.get("schema") != _o1c106.COMPOSED_ACTIVATION_SCHEMA
        or activation.get("forbidden_nonactivated_candidate_sha256")
        != PURE_EMITTED_CANDIDATE_SHA256
        or len(composed_entries) != 1
        or len(used) != ATTIC_CHUNK_COUNT + 1
        or used[-1] != PAGE21_SHA256
        or final_activation.get("lineage_ordinal") != LINEAGE_CALL_ORDINAL
        or final_activation.get("active_sha256") != PAGE21_SHA256
        or final_activation.get("certification_audit_sha256")
        != CERTIFICATION_AUDIT_SHA256
        or len(
            _sequence(
        ...
        != _o1c106.PAGE21_NEW_DERIVED_COUNT
        or len(
            _sequence(
        ...
        != _o1c106.PAGE21_EMITTED_COUNT
        or residency.get("schema") != _o1c106.COMPOSED_RESIDENCY_SCHEMA
        or residency.get("lineage_ordinal") != LINEAGE_CALL_ORDINAL
        or residency.get("active_limit") != PAGE21_ACTIVE_LIMIT
        or projection.get("category_counts") != PAGE21_CATEGORY_COUNTS
        or projection_encoding.get("sha256") != PAGE21_SHA256
        or projection_encoding.get("clause_count") != PAGE21_CLAUSE_COUNT
        or projection_encoding.get("literal_count") != PAGE21_LITERAL_COUNT
        or projection_encoding.get("serialized_bytes") != PAGE21_SERIALIZED_BYTES
        ...
        != EMITTED_ONLY_ACTIVE_PROJECTION_SHA256
        or emitted_active_encoding.get("sha256") == PAGE21_SHA256
        or emitted_selector_encoding.get("sha256") != PURE_EMITTED_CANDIDATE_SHA256
        ...
        or logical.get("combined_inventory_sha256") != COMBINED_KNOWN_INVENTORY_SHA256
        or logical.get("combined_encoding_sha256") != LOGICAL_KNOWN_SHA256
        or certification.get("schema") != _o1c106.CERTIFICATION_AUDIT_SCHEMA
        or certification.get("attempt_id") != "O1C-0106"
        or certification.get("passed") is not True
        or certification.get("publication_gate")
        != "all-247-active-v8-certifications-finished-before-publication"
        or len(
            _sequence(
                certification.get("active_rows_in_serialization_order"),
                "published Page-21 certification rows",
            )
        )
        != PAGE21_CLAUSE_COUNT
        or _mapping(certification.get("page21"), "published Page-21 audit row").get(
            "sha256"
        )
        != PAGE21_SHA256
        or _mapping(certification.get("page21"), "published Page-21 audit row").get(
        ...
        raise O1C107RunError("published composed Page21 residency contract differs")
    _validate_priority_receipt(
        artifacts[RECEIPT_NAME], artifacts[CONTINUATION_BANK_NAME]
    )
    _native_v35._validate_inherited_derived_receipt(
        artifacts[INHERITED_DERIVED_RECEIPT_NAME]
    )
    _native_v35._validate_new_derived_receipt(artifacts[DERIVED_RECEIPT_NAME])
...
def _validate_native_capacity_proof(value: object) -> Mapping[str, object]:
    """Derive and verify the exact Page-21 native-capacity proof before intent."""

    page = _mapping(value, "published Page-21 capacity source")
    expected_caps = {
        "maximum_clauses": 512,
        "maximum_literals": 1_600_000,
        "maximum_serialized_bytes": 8_388_608,
    }
    headroom = _mapping(page.get("headroom"), "published Page-21 headroom")
    input_clauses = _nonnegative(page.get("clause_count"), "Page-21 input clauses")
    additional = _nonnegative(headroom.get("clauses"), "Page-21 clause headroom")
    if (
        O1C66_VAULT_CAPS.describe() != expected_caps
        or input_clauses != PAGE21_CLAUSE_COUNT
        or additional != PAGE21_HEADROOM["clauses"]
        or dict(headroom) != PAGE21_HEADROOM
        or input_clauses + additional != expected_caps["maximum_clauses"]
        or additional < PARENT_CENTERED_ACTION_CAPACITY
    ):
        raise O1C107RunError("published Page-21 native capacity proof differs")
    proof: dict[str, object] = {
        "caps": expected_caps,
        "clause_headroom_guarantee": {
            "native_vault_maximum_clauses": expected_caps["maximum_clauses"],
            "page21_input_clauses": input_clauses,
        ...
        "recorded_residual_headroom": {
            "literals": PAGE21_HEADROOM["literals"],
            "serialized_bytes": PAGE21_HEADROOM["serialized_bytes"],
        },
...
def _disabled_o1c96_published_manifest_validator(
    ...
    if (
        ...
        raise O1C107RunError("published preparation manifest identity differs")
    ...
    page21 = _mapping(manifest.get("page21"), "published Page-16")
    _validate_native_capacity_proof(page21)
    ...
    if dict(zero_call) != {
        ...
        raise O1C107RunError("published zero-call authorization differs")
    if (
        ...
        raise O1C107RunError("published parent boundary differs")
    if (
        ...
        raise O1C107RunError("published rollover/attic boundary differs")
    if (
        page21.get("lineage_ordinal") != LINEAGE_CALL_ORDINAL
        or page21.get("active_limit") != PAGE21_ACTIVE_LIMIT
        or page21.get("active_sha256") != PAGE21_SHA256
        or page21.get("clause_count") != PAGE21_CLAUSE_COUNT
        or page21.get("literal_count") != PAGE21_LITERAL_COUNT
        or page21.get("serialized_bytes") != PAGE21_SERIALIZED_BYTES
        or page21.get("category_counts") != PAGE21_CATEGORY_COUNTS
        or page21.get("headroom") != PAGE21_HEADROOM
        or page21.get("fresh_identity") is not True
    ):
        raise O1C107RunError("published Page-16 seal differs")
    if (
        ...
        raise O1C107RunError("published continuation bank seal differs")
    rows_raw = _mapping(manifest.get("artifacts"), "published artifact rows")
    if set(rows_raw) != set(PUBLISHED_ARTIFACT_SEALS):
        raise O1C107RunError("published artifact row inventory differs")
    ...
    for name, (size, digest, role) in PUBLISHED_ARTIFACT_SEALS.items():
        ...
        if (
            ...
            raise O1C107RunError("published artifact seal differs")
        rows[name] = row
    ...
    raise O1C107RunError("historical manifest validator is disabled")


def _validate_published_manifest(
    manifest: Mapping[str, object], artifacts: Mapping[str, bytes]
) -> tuple[dict[str, Mapping[str, object]], frozenset[str]]:
    """Validate exact O1C106 bundle and all 2,692 logical identities."""

    payload = canonical_json_bytes(manifest)
    _native_v35._validate_manifest(payload)
    page21 = _mapping(manifest.get("page21"), "published Page-21")
    logical = _mapping(
    ...
    parent = _mapping(manifest.get("parent_terminal"), "published terminal parent")
    certification = _mapping(
        manifest.get("certification"), "published Page-21 certification"
    )
    authorization = _mapping(manifest.get("authorization"), "published authorization")
    capacity = _validate_native_capacity_proof(page21)
    if (
        manifest.get("schema") != PREPARATION_SCHEMA
        or manifest.get("attempt_id") != "O1C-0106"
        ...
        or authorization.get("page20_retry_or_replay_authorized") is not False
        or authorization.get("lineage33_retry_or_replay_authorized") is not False
        or authorization.get("page21_burned") is not False
        or authorization.get("lineage34_burned") is not False
        or authorization.get("intent_created") is not False
        or authorization.get("science_call_authorized") is not False
        or page21.get("lineage_ordinal") != LINEAGE_CALL_ORDINAL
        or page21.get("active_limit") != PAGE21_ACTIVE_LIMIT
        or page21.get("active_sha256") != PAGE21_SHA256
        or page21.get("clause_count") != PAGE21_CLAUSE_COUNT
        or page21.get("literal_count") != PAGE21_LITERAL_COUNT
        ...
        or page21.get("selected_emitted_clause_count") != _o1c106.PAGE21_EMITTED_COUNT
        or page21.get("selected_inherited_derived_clause_count")
        != _o1c106.PAGE21_INHERITED_DERIVED_COUNT
        or page21.get("selected_new_derived_clause_count")
        != _o1c106.PAGE21_NEW_DERIVED_COUNT
        or capacity["clause_headroom_guarantee"]
        != {
            "native_vault_maximum_clauses": 512,
            "page21_input_clauses": 247,
        ...
        or new.get("resident_clause_count") != _o1c106.PAGE21_NEW_DERIVED_COUNT
        or certification.get("sha256") != CERTIFICATION_AUDIT_SHA256
        or certification.get("serialized_bytes") != CERTIFICATION_AUDIT_BYTES
        or certification.get("active_pass_count") != PAGE21_CLAUSE_COUNT
        ...
        raise O1C107RunError("published O1C106 Page-21 boundary differs")
...
def _load_published_preparation(
    ...
    try:
        ...
        raise O1C107RunError("published preparation inventory is unreadable") from exc
    if (
        manifest_path != directory / PREPARATION_MANIFEST_NAME
        or {path.name for path in children} != PREPARATION_ARTIFACT_NAMES
    ):
        raise O1C107RunError("published preparation inventory differs")
    ...
    for path in children:
        try:
            ...
            raise O1C107RunError(
                "published preparation artifact is unreadable"
            ) from exc
        if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
            raise O1C107RunError("published preparation artifact type differs")
    ...
    if (
        ...
        raise O1C107RunError("published preparation manifest seal differs")
...
def _preflight_bundle(
    ...
    *,
    ...
    if (
        ...
        raise O1C107RunError("parent input SHA preflight differs")
    ...
    if (
        ...
        raise O1C107RunError("published preparation/receipt preflight differs")
    ...
    for name in ("cnf", "potential", "grouping", "o1c73_config"):
        ...
        if observed != inputs[f"{name}_sha256"]:
            raise O1C107RunError(f"input {name} SHA preflight differs")
    ...
    for name in SOURCE_PATHS:
        ...
        if observed != expected[name]:
            raise O1C107RunError(f"source {name} SHA preflight differs")
    ...
    if (
        ...
        or source_sha["native_v32"] != native["source_sha256"]
    ):
        raise O1C107RunError("native toolchain/source SHA preflight differs")

    prepared = _load_published_preparation(published_directory, published_manifest)
    page21_manifest = _mapping(
        prepared.manifest.get("page21"), "published Page-21 preflight"
    )
    capacity_proof = _validate_native_capacity_proof(page21_manifest)
    prepared_rows = {
        name: {
    ...
    _native_v35._validate_inherited_derived_receipt(inherited_receipt_payload)
    _native_v35._validate_new_derived_receipt(new_receipt_payload)
    ...
    if any(
        ...
        raise O1C107RunError("derived namespace publication linkage differs")
    try:
        _native_v35.validate_o1c107_page21_inputs(
        ...
        raise O1C107RunError(
            "real Page-21 v8 input certification failed before intent"
        ) from exc
    ...
    if (
        ...
        raise O1C107RunError("Darwin/arm64/RAM/disk/process preflight differs")
    row = {
...
def _executable_identity(path: Path, *, relative_path: str) -> dict[str, object]:
    try:
        ...
        raise O1C107RunError("native executable is unreadable") from exc
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
        raise O1C107RunError("native executable type differs")
    size = path.stat().st_size
    if size <= 0:
        raise O1C107RunError("native executable is empty")
    return {
        "path": relative_path,
...
def _verify_staged_native_closure(
    *,
    ...
    if len(expected_files) != len(NATIVE_CLOSURE_NAMES):
        raise O1C107RunError("staged native closure basenames collide")
    ...
    try:
        ...
        for candidate in closure_root.rglob("*"):
            ...
            if stat.S_ISLNK(metadata.st_mode):
                raise O1C107RunError("staged native closure contains a symlink")
            if stat.S_ISDIR(metadata.st_mode):
                observed_directories.add(relative)
                if stat.S_IMODE(metadata.st_mode) != 0o555:
                    raise O1C107RunError("staged native closure directory is writable")
            elif stat.S_ISREG(metadata.st_mode):
                observed_files.add(relative)
                if stat.S_IMODE(metadata.st_mode) != 0o444:
                    raise O1C107RunError("staged native closure file is writable")
            else:
                raise O1C107RunError("staged native closure contains a special file")
    except OSError as exc:
        raise O1C107RunError("staged native closure is unreadable") from exc
    if (
        ...
        raise O1C107RunError("staged native closure inventory differs")
    ...
    for name in NATIVE_CLOSURE_NAMES:
        ...
        if row["sha256"] != digests.get(name):
            raise O1C107RunError(f"staged native closure {name} digest differs")
        rows.append(row)
    normalized_expected = [
        dict(_mapping(value, "expected staged closure row")) for value in expected_rows
    ]
    if rows != normalized_expected:
        raise O1C107RunError("staged native closure rows differ")
    return rows
    ...
    *,
    ...
    for name in NATIVE_CLOSURE_NAMES:
        ...
        if approved_source_sha256.get(name) != expected_sha:
            raise O1C107RunError("native closure preflight/config seal differs")
        source_path = _lab_path(root, configured, f"native closure {name}")
        payload = source_path.read_bytes()
        if sha256_bytes(payload) != expected_sha:
            raise O1C107RunError(f"native closure {name} changed after preflight")
...
def _compile_native(
    *,
    ...
    staged_source = _relative_text(source_paths["native_v32"], "staged v32 source")
    if (
        native["source"] != staged_source
        or _mapping(verified_closure[0], "staged v32 row").get("sha256")
        != native["source_sha256"]
        or _sha_file(library) != native["cadical_library_sha256"]
    ):
        raise O1C107RunError("native build input changed after preflight")
    ...
    try:
        ...
        raise O1C107RunError("native v32 compilation could not run") from exc
    ...
    try:
        ...
        raise O1C107RunError("native v32 --help smoke could not run") from exc
    ...
    if (
        ...
        or smoke_stdout != NATIVE_V32_USAGE
        or smoke_stderr
        or after_smoke != before_smoke
    ):
        raise O1C107RunError("native v32 --help smoke seal differs")
    verified_closure = _verify_staged_native_closure(
        stage=stage,
        config=config,
        expected_rows=closure_rows,
    ...
    if _sha_file(library) != native["cadical_library_sha256"]:
        raise O1C107RunError("native library changed during build/smoke")
    normalized_command = [
        *command[:-1],
    ...
    build = {
        ...
        "staged_source": dict(_mapping(verified_closure[0], "staged v32 row")),
...
def _validate_staged_native_build(
    *,
    ...
    if (
        ...
        or smoke.get("stdout")
        != {
            "serialized_bytes": len(NATIVE_V32_USAGE),
            "sha256": sha256_bytes(NATIVE_V32_USAGE),
        }
        or smoke.get("stderr") != {"serialized_bytes": 0, "sha256": sha256_bytes(b"")}
        or smoke.get("executable") != executable
    ):
        raise O1C107RunError("staged native build provenance differs")
...
def _stage_capsule(
    *,
    ...
    if final.exists() or stage.exists():
        raise O1C107RunError("timestamped capsule already exists")
    ...
    try:
        ...
        if sha256_bytes(config_payload) != bundle.row.get(
            ...
            raise O1C107RunError("config changed before closure snapshot")
...
def _rehash_call_inputs(
    *,
    ...
    if _sha_file(config_path) != preflight_row.get("config_sha256"):
        raise O1C107RunError("config changed before native call")
    ...
    for name in ("cnf", "potential", "grouping", "o1c73_config"):
        if (
            ...
            raise O1C107RunError(f"input {name} changed before native call")
    ...
    if prepared_rows.get(RECEIPT_NAME) != receipt_row or invocation.get(
        ...
        raise O1C107RunError("staged receipt seal differs")
    triplet_contract = (
        (
            inherited_triplet,
            "receipt",
    ...
    for triplet, component, artifact_name, invocation_field in triplet_contract:
        ...
        if prepared_rows.get(artifact_name) != row or invocation.get(
            ...
            raise O1C107RunError("staged derived namespace seal differs")
    ...
    for name, value in expected_rows.items():
        ...
        if path.stat().st_size != row.get("serialized_bytes") or _sha_file(
            ...
            raise O1C107RunError("staged call artifact seal differs")
    ...
    if (
        ...
        raise O1C107RunError("staged invocation/build seal differs")
    ...
    if (
        ...
        raise O1C107RunError("staged native build/smoke seal differs")
    if require_executable_identity:
        _require_executable_identity(
...
def _native_stdout(result: object) -> bytes:
    ...
    if isinstance(value, str):
        ...
        raise O1C107RunError("adapter omitted byte-exact native stdout")
    if _sha(
        getattr(result, "native_stdout_sha256", None), "native stdout digest"
    ) != sha256_bytes(payload):
        raise O1C107RunError("native stdout digest differs")
    return payload
...
def _validate_result_conservation(
    ...
    if set(counters) != {*outcome_names, "child_bound_evaluations"}:
        raise O1C107RunError("native post-result conservation differs")
    outcomes = tuple(
...
def _adapter_memory_samples(
    ...
    for raw in rows_raw:
        ...
        if (
            ...
            raise O1C107RunError("adapter memory sample differs")
    ...
    if (
        ...
        != _native_v35._v9.JOINT_SCORE_SIEVE_MEMORY_SERIES_SCHEMA
        or memory.get("memory_sample_limit")
        != _native_v35._v9.JOINT_SCORE_SIEVE_MAXIMUM_MEMORY_SAMPLES
        ...
        raise O1C107RunError("adapter memory ledger differs")
    return tuple(rows)


def _validate_native_result(result: object, stdout: bytes) -> NativeEvidence:
    if not isinstance(result, _native_v35.JointScoreSieveV35Result):
        raise O1C107RunError("adapter v35 result type differs")
    ...
    try:
        ...
        raise O1C107RunError("native stdout JSON differs") from exc
    if (
        decoded != raw
        or raw.get("schema") != _native_v35.JOINT_SCORE_SIEVE_RESULT_SCHEMA
    ):
        raise O1C107RunError("native stdout/result projection differs")
    ...
    if (
        ...
        or raw.get("active_vault_sha256") != PAGE21_SHA256
        ...
        raise O1C107RunError("native v32 production binding differs")
    if (
        raw.get("priority_state") != priority_state
        or raw.get("priority_actions") != actions
    ):
        raise O1C107RunError("native promoted priority projection differs")
    if raw.get("decision_ownership") != ownership:
        raise O1C107RunError("native promoted ownership projection differs")
    if result.status == 10 and not isinstance(result.key_model, bytes):
        raise O1C107RunError("SAT result omitted key model")
    if result.status != 10 and result.key_model is not None:
        raise O1C107RunError("non-SAT result returned key model")
...
def _science_and_operation(
    ...
    *,
    ...
    if action_count != len(actions):
        raise O1C107RunError("priority action count differs")
    ...
    for row in emitted_rows:
        ...
        try:
            ...
            raise O1C107RunError("emitted clause serialization differs") from exc
    ...
    if novel > active_new:
        raise O1C107RunError("global novelty exceeds active-vault novelty")
    ...
    if evidence.status == 10 and not public_model_verified:
        raise O1C107RunError("SAT key failed exact public ChaCha verification")
    if evidence.status != 10 and public_model_verified:
        raise O1C107RunError("public model verification without SAT differs")
    model_gain = evidence.status == 10 and public_model_verified
    closure_gain = evidence.status == 20 or terminal_empty
    # Native v32 emits no entropy or domain estimate.  These fields stay
    ...
    science = {
        ...
        "active_page21_new_clauses": active_new,
...
def _execute_once(
    *,
    ...
    try:
        ...
        # Adapter v35 attaches the completed process record even when semantic
...
def _manifest_bytes(capsule: Path) -> tuple[bytes, int]:
    ...
    for path in sorted(capsule.rglob("*")):
        metadata = path.lstat()
        if stat.S_ISLNK(metadata.st_mode):
            raise O1C107RunError("capsule contains a symlink")
        if stat.S_ISDIR(metadata.st_mode):
            continue
        if not stat.S_ISREG(metadata.st_mode):
            raise O1C107RunError("capsule contains a special file")
...
def _run_markdown(result: Mapping[str, object]) -> bytes:
    ...
    lines = [
        "# O1C-0107 Page-21 parent-centered run",
        "",
        ...
        "- Page 21 and lineage 34 burned when `episodes/00/intent.json` was persisted.",
...
def _publish_exact(path: Path, payload: bytes) -> None:
    if path.exists():
        if path.is_symlink() or not path.is_file() or path.read_bytes() != payload:
            raise O1C107RunError("authoritative result already differs")
        return
...
def _finalize_capsule(
    *, capsule: Path, authoritative: Path, result: Mapping[str, object]
    ...
    if persistent_bytes > MAXIMUM_PERSISTENT_ARTIFACT_BYTES:
        raise O1C107RunError("persistent artifact budget exceeded")
...
def _existing_result(root: Path) -> dict[str, object] | None:
    ...
    if not authoritative.exists():
        return None
    value = dict(_read_canonical_json(authoritative, "authoritative O1C107 result"))
    if value.get("schema") != RESULT_SCHEMA or value.get("attempt_id") != ATTEMPT_ID:
        raise O1C107RunError("authoritative O1C107 result differs")
    return value
...
def _validate_capsule_artifact_row(
    ...
    *,
    ...
    try:
        ...
        raise O1C107RunError(f"capsule artifact {name} is unreadable") from exc
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
        raise O1C107RunError(f"capsule artifact {name} type differs")
    expected = _artifact_row(path, relative_to=directory)
    if set(row) != {"path", "serialized_bytes", "sha256"} or row != expected:
        raise O1C107RunError(f"capsule artifact {name} seal differs")
    if canonical_json:
...
def _validated_timestamp(value: object, field: str) -> str:
    ...
    try:
        ...
        raise O1C107RunError(f"{field} differs") from exc
...
def _semantic_replay_completed_native(
    *,
    ...
    """Replay archived stdout through v35 using the original sealed inputs."""
    ...
    for name in ("cnf", "potential", "grouping"):
        ...
        if sha256_bytes(payload) != input_sha[name]:
            raise O1C107RunError(f"semantic replay {name} seal differs")
        payloads[name] = payload
    page21 = prepared.artifacts[ACTIVE_PROJECTION_NAME]
    bank = prepared.artifacts[CONTINUATION_BANK_NAME]
    if (
        len(page21) != PAGE21_SERIALIZED_BYTES
        or sha256_bytes(page21) != PAGE21_SHA256
        or len(bank) != CONTINUATION_BANK_BYTES
        or sha256_bytes(bank) != CONTINUATION_BANK_SHA256
    ):
        raise O1C107RunError("semantic replay Page21/bank seal differs")
    io_v1 = _native_v35._v9._v8._v7._v1
    try:
        field = io_v1._potential(payloads["potential"])
        grouping = _native_v35._v9.validate_joint_score_sieve_grouping(
            field, payloads["grouping"]
        )
        if grouping.potential_sha256 != input_sha["potential"]:
            raise O1C107RunError("semantic replay grouping identity differs")
        input_vault = _native_v35.parse_threshold_no_good_vault(
            page21,
            observed_variables=field.observed_variables,
            caps=O1C66_VAULT_CAPS,
        )
        identity = _native_v35.vault_identity_from_sources(
            cnf_sha256=input_sha["cnf"],
            potential_sha256=input_sha["potential"],
            grouping_sha256=input_sha["grouping"],
            observed_variables=field.observed_variables,
            bound_rule=_native_v35._v9.JOINT_SCORE_SIEVE_BOUND_RULE,
            threshold=THRESHOLD,
        )
        _native_v35.validate_threshold_no_good_vault_identity(
            input_vault, expected=identity
        )
        _native_v35._v9._v8._certify_input_vault(
            input_vault,
            field=field,
            grouping=grouping,
            threshold=THRESHOLD,
        )
        priority_records = _native_v35._decode_live_bank(
            bank,
            expected_sha256=CONTINUATION_BANK_SHA256,
            sealed_input=True,
        )
        memory_samples = _adapter_memory_samples(adapter_memory)
        raw = _native_v35._v21.load_native_json(stdout)
        replayed = _native_v35._parse_native_payload(
            raw,
        ...
        raise O1C107RunError("archived native semantic replay differs") from exc
    ...
    if replay_memory != adapter_memory:
        raise O1C107RunError("semantic replay adapter memory differs")
    ...
    *,
    ...
    if not all(path.is_file() and not path.is_symlink() for path in required):
        if (capsule / "episodes/00/intent.json").is_file():
            raise O1C107RunError("burned incomplete capsule forbids retry or replay")
        raise O1C107RunError("incomplete pre-intent capsule blocks a fresh call")
    manifest, _ = _manifest_bytes(capsule)
    if manifest != (capsule / "artifacts.sha256").read_bytes():
        raise O1C107RunError("existing capsule manifest differs")
    episode_dir = capsule / "episodes" / "00"
    ...
    try:
        ...
        raise O1C107RunError("existing capsule inventory is unreadable") from exc
    if (
        ...
        raise O1C107RunError("existing capsule inventory differs")
    ...
    if (
        ...
        raise O1C107RunError("existing capsule config differs")
    ...
    if (
        ...
        != _validate_native_capacity_proof(prepared.manifest["page21"])
        ...
        is not True
        or preflight.get("page21_sha256") != PAGE21_SHA256
        ...
        raise O1C107RunError("existing capsule preflight provenance differs")
    ...
    if (
        ...
        or invocation.get("page21_sha256") != PAGE21_SHA256
        ...
        raise O1C107RunError("existing capsule invocation provenance differs")
    _rehash_call_inputs(
        root=root,
        config_path=config_path,
        config=config,
    ...
    if (
        ...
        or intent.get("page21_sha256") != PAGE21_SHA256
        ...
        raise O1C107RunError("existing capsule burn intent differs")

    episode = dict(_read_canonical_json(episode_path, "capsule episode"))
    if episode != episode_preview:
        raise O1C107RunError("capsule episode changed during validation")
    ...
    if (
        ...
        raise O1C107RunError("existing capsule episode lineage differs")
    ...
    if episode.get("completed") is True:
        ...
        if (
            ...
            raise O1C107RunError("existing completed episode call contract differs")
        ...
        if set(archived) != set(NATIVE_ARCHIVE_NAMES):
            raise O1C107RunError("archived native component inventory differs")
        component_paths = {
        ...
        if episode.get("final_priority_bank") != archived[FINAL_BANK_NAME]:
            raise O1C107RunError("final priority bank row differs")
        ...
        try:
            raw = dict(_native_v35._v21.load_native_json(stdout))
        except Exception as exc:
            raise O1C107RunError("archived native stdout differs") from exc
        archived_raw = dict(
        ...
        if status == 10:
            if (
                ...
                raise O1C107RunError("archived SAT key model differs")
            if (
                len(key_model) != 32
                ...
                raise O1C107RunError("archived SAT key model is not publicly verified")
        elif status in (0, 20):
            if key_hex is not None or episode.get("key_model_sha256") is not None:
                raise O1C107RunError("archived non-SAT key model differs")
        else:
            raise O1C107RunError("archived native status differs")
        try:
            bank_hex = bytes.fromhex(cast(str, priority_state.get("bank_hex")))
        except (TypeError, ValueError) as exc:
            raise O1C107RunError("archived final bank encoding differs") from exc
        if (
            raw != archived_raw
            or evidence.raw != raw
            ...
            raise O1C107RunError("archived native result cross-binding differs")
        operation, science, classification, stop = _science_and_operation(
            evidence,
        ...
        if (
            ...
            raise O1C107RunError("completed episode science projection differs")
        ...
        if (
            ...
            raise O1C107RunError("terminal episode call count differs")
        if (
            ...
            raise O1C107RunError("terminal episode contract differs")
        ...
        if (
            ...
            or failure.get("page21_burned") is not True
            or failure.get("lineage34_burned") is not True
            ...
            raise O1C107RunError("terminal failure cross-binding differs")
        stdout_row = episode.get("native_stdout")
        if failure.get("native_stdout") != stdout_row:
            raise O1C107RunError("terminal native stdout row differs")
        if not native_inventory:
            if not (
                failure.get("phase") == "PRE_CALL"
                and failure.get("exception_type") == "O1C107RunError"
                ...
                raise O1C107RunError("missing terminal executable provenance differs")
        else:
            ...
            if observed_executable != executable and not (
                ...
                raise O1C107RunError("terminal executable provenance differs")
        expected_episode_inventory = {
            "intent.json",
            "episode.json",
        ...
        raise O1C107RunError("existing capsule episode completion differs")

    try:
        observed_episode_inventory = {path.name for path in episode_dir.iterdir()}
    except OSError as exc:
        raise O1C107RunError("episode inventory is unreadable") from exc
    if observed_episode_inventory != expected_episode_inventory:
        raise O1C107RunError("episode artifact inventory differs")
    ...
    if (
        ...
        != "Never retry or replay Page 21 / lineage 34 or Pages 20 through 9; preserve the final priority bank only as a continuation seed."
        or (capsule / "RUN.md").read_bytes() != _run_markdown(result)
    ):
        raise O1C107RunError("existing capsule result differs")
...
def _public_verifier(
    *, root: Path, config: Mapping[str, object]
    ...
    if _sha_file(config_path) != inputs["o1c73_config_sha256"]:
        raise O1C107RunError("public verifier config SHA differs")
    try:
        ...
        raise O1C107RunError("public ChaCha verifier cannot be established") from exc
...
def run(
    ...
    *,
    root: Path | None = None,
    adapter_run: AdapterRun = _native_v35.run_joint_score_sieve,
    ...
    if existing is not None:
        if len(capsules) != 1:
            raise O1C107RunError("authoritative O1C107 result lacks one sealed capsule")
        ...
        if capsule_payload != authoritative_payload or capsule_result != existing:
            raise O1C107RunError("authoritative O1C107 result differs from capsule")
        return existing
    if capsules:
        if len(capsules) != 1:
            raise O1C107RunError("multiple O1C107 capsules forbid replay")
    ...
    result = {
        ...
        "next_action": "Never retry or replay Page 21 / lineage 34 or Pages 20 through 9; preserve the final priority bank only as a continuation seed.",
    }
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Preflight or execute O1C107's one Page-21 parent-centered call"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 0
    except O1C107RunError as exc:
        print(f"O1C107: {exc}", file=sys.stderr)
        return 2
