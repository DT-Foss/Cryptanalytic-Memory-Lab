...
JOINT_SCORE_SIEVE_ADAPTER_SCHEMA = "o1-256-joint-score-sieve-v23-adapter-v1"
JOINT_SCORE_SIEVE_RESULT_SCHEMA = "o1-256-cadical-joint-score-sieve-result-v20"
...
PRIORITY_ACTION_SCHEMA = "o1-256-o1c84-failure-first-proof-mining-actions-v1"
...
O1C83_MANIFEST_BYTES = 4_255
O1C83_MANIFEST_SHA256 = (
...
PRODUCTION_PAGE9_BYTES = 2_885_959
PRODUCTION_PAGE9_SHA256 = (
    "8c3b8cc33badd4aa23920caabc5ea3fc5006675d93805578b74b2b20788c8204"
)

NATIVE_SOURCE_RELATIVE = Path("native/cadical_o1_joint_score_sieve_v20.cpp")
O1C83_MANIFEST_RELATIVE = Path(
    "research/o1c83_causal_rollover_seed_20260720/"
    "causal-rollover-preparation-manifest.json"
)
O1C83_LIVE_BANK_RELATIVE = Path(
    "research/o1c83_causal_rollover_seed_20260720/"
    "final-parent-centered-priority-bank.bin"
)
O1C83_PAGE9_RELATIVE = Path(
...
class JointScoreSieveV23Error(O1RelationalSearchError):
    """Native-v20 build, execution, or continuation validation failed."""
...
@dataclass(frozen=True)
class JointScoreSieveV23Result:
    """Typed native-v20 result and its validated evolved bank."""
...
def _error(field: str) -> JointScoreSieveV23Error:
    return JointScoreSieveV23Error(f"joint-score-sieve-v23 {field} differs")
...
def _validate_manifest(payload: bytes) -> None:
    try:
        ...
        raise _error("O1C83 manifest JSON") from exc
    artifacts = _mapping(root.get("artifacts"), "manifest artifacts")
    bank = _mapping(
        artifacts.get("final-parent-centered-priority-bank.bin"),
        "manifest live bank",
    )
    page9 = _mapping(artifacts.get("page-09-active.bin"), "manifest Page9")
    final_bank = _mapping(root.get("final_priority_bank"), "manifest final bank")
    page9_report = _mapping(root.get("page9"), "manifest Page9 report")
    if (
        root.get("schema") != "o1-256-o1c83-page9-causal-rollover-preparation-v1"
        or bank.get("serialized_bytes") != BANK_BYTES
        or bank.get("sha256") != LIVE_BANK_SHA256
        or page9.get("serialized_bytes") != PRODUCTION_PAGE9_BYTES
        or page9.get("sha256") != PRODUCTION_PAGE9_SHA256
        ...
        != LIVE_BANK_ELIGIBLE_COORDINATES
        or page9_report.get("active_sha256") != PRODUCTION_PAGE9_SHA256
        or page9_report.get("serialized_bytes") != PRODUCTION_PAGE9_BYTES
        or page9_report.get("lineage_ordinal") != 22
    ):
        raise _error("O1C83 manifest contract")
...
def _parse_native_payload(
    ...
    *,
...
) -> JointScoreSieveV23Result:
    """Validate one native-v20 document and its unchanged v6 lifecycle."""
    ...
    return JointScoreSieveV23Result(
        status=base.status,
...
def _validate_prelaunch(
    *,
    ...
    executable, executable_bytes = _read_exact(
        ...
        expected_bytes=Path(executable_path).resolve(strict=True).stat().st_size,
        expected_sha256=expected_executable_sha256,
    )
    manifest, manifest_bytes = _read_exact(
        manifest_path,
        label="O1C83 manifest",
        expected_bytes=O1C83_MANIFEST_BYTES,
        expected_sha256=O1C83_MANIFEST_SHA256,
    )
    ...
    page9, page9_bytes = _read_exact(
        page9_path,
        label="Page9",
        expected_bytes=PRODUCTION_PAGE9_BYTES,
        expected_sha256=PRODUCTION_PAGE9_SHA256,
    )
...
def run_joint_score_sieve(
    *,
    ...
    sealed_page9_path: str | Path | None = None,
    public_fixture: bool = False,
) -> JointScoreSieveV23Result:
    """Build native-v20 if absent, then validate all seals before one launch."""
    ...
    try:
        ...
        manifest = (
            ...
            else root / O1C83_MANIFEST_RELATIVE
        )
        receipt = (
        ...
        page9 = (
            Path(sealed_page9_path)
            if sealed_page9_path is not None
            else root / O1C83_PAGE9_RELATIVE
        )
        ...
        if not public_fixture and (
            input_vault.sha256 != PRODUCTION_PAGE9_SHA256
            or len(vault_bytes) != PRODUCTION_PAGE9_BYTES
            or vault_bytes != sealed.page9_bytes
            or _candidate_order(field) != PRODUCTION_CANDIDATES
        ):
            raise _error("Page9 production seal")
        ...
        for original, resolved, before, name in (
            ...
            (manifest, sealed.manifest_path, sealed.manifest_bytes, "O1C83 manifest"),
            ...
            (page9, sealed.page9_path, sealed.page9_bytes, "sealed Page9"),
        ...
        if completed.returncode:
            ...
            raise JointScoreSieveV23Error(
                "joint-score-sieve-v23 execution failed: "
                + (completed.stderr.strip() or completed.stdout.strip())
            ) from failure
        try:
        ...
        message = str(exc)
        if not message.startswith("joint-score-sieve-v23"):
            message = f"joint-score-sieve-v23 adapter failed: {message}"
...
def build_native_joint_score_sieve(
    *,
    ...
    """Build native-v20 with the O1C-0084 fixture gate when requested."""
