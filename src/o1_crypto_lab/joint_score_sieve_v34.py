"""Fail-closed O1C-0105 adapter for composed Page 20 and its live bank.

Version 34 preserves v33's proof-mining and result-validation semantics while
moving the native boundary to result/source v31 and fresh lineage-33 Page 20.
Both immutable derived namespaces are sealed independently as receipt,
closure, and overlay triplets before the executable can be launched.
...
JOINT_SCORE_SIEVE_ADAPTER_SCHEMA = "o1-256-joint-score-sieve-v34-adapter-v1"
JOINT_SCORE_SIEVE_RESULT_SCHEMA = "o1-256-cadical-joint-score-sieve-result-v31"
JOINT_SCORE_SIEVE_IMPLEMENTATION_PARENT_SCHEMA = (
    "o1-256-cadical-joint-score-sieve-result-v6"
)
PRIORITY_STATE_SCHEMA = (
    "o1-256-o1c105-live-parent-centered-continuation-priority-state-v1"
)
PRIORITY_ACTION_SCHEMA = "o1-256-o1c105-failure-first-proof-mining-actions-v1"
...
O1C104_MANIFEST_SCHEMA = _o1c104.PREPARATION_SCHEMA
O1C104_MANIFEST_BYTES = _o1c104.PREPARATION_MANIFEST_BYTES
O1C104_MANIFEST_SHA256 = _o1c104.PREPARATION_MANIFEST_SHA256
O1C104_MANIFEST_ARTIFACTS = frozenset(
    {
        _o1c104.NEW_CHUNK_NAME,
        _o1c104.ACTIVE_PROJECTION_NAME,
        _o1c104.RESIDENCY_NAME,
        _o1c104.ACTIVATION_LEDGER_NAME,
        _o1c104.OCCURRENCES_NAME,
        _o1c104.RELATIONS_NAME,
        _o1c104.COMMON_CORE_AUDIT_NAME,
        _o1c104.FINAL_BANK_NAME,
        _o1c104.PRIORITY_RECEIPT_NAME,
        _o1c104.INHERITED_DERIVED_RECEIPT_NAME,
        _o1c104.INHERITED_DERIVED_CLOSURE_NAME,
        _o1c104.INHERITED_DERIVED_OVERLAY_NAME,
        _o1c104.DERIVED_RECEIPT_NAME,
        _o1c104.DERIVED_CLOSURE_NAME,
        _o1c104.DERIVED_OVERLAY_NAME,
    }
)
O1C104_PUBLISHED_ARTIFACTS = O1C104_MANIFEST_ARTIFACTS | {
    _o1c104.PREPARATION_MANIFEST_NAME
}
O1C104_BUNDLE_RELATIVE = Path(
    "research/o1c104_page20_causal_rollover_seed_20260721"
)
O1C104_MANIFEST_RELATIVE = O1C104_BUNDLE_RELATIVE / _o1c104.PREPARATION_MANIFEST_NAME
O1C104_PAGE20_RELATIVE = O1C104_BUNDLE_RELATIVE / _o1c104.ACTIVE_PROJECTION_NAME

O1C103_PRIORITY_RECEIPT_SCHEMA = PRIORITY_STATE_SCHEMA.replace("o1c105", "o1c103")
...
PRODUCTION_PAGE20_BYTES = _o1c104.PAGE20_SERIALIZED_BYTES
PRODUCTION_PAGE20_SHA256 = _o1c104.PAGE20_SHA256
PRODUCTION_PAGE20_LINEAGE_ORDINAL = _o1c104.PAGE20_LINEAGE_ORDINAL
PRODUCTION_PAGE20_ACTIVE_LIMIT = _o1c104.PAGE20_ACTIVE_LIMIT
PRODUCTION_PAGE20_CLAUSE_COUNT = _o1c104.PAGE20_ACTIVE_LIMIT
PRODUCTION_PAGE20_LITERAL_COUNT = _o1c104.PAGE20_LITERAL_COUNT
PRODUCTION_PAGE20_CATEGORY_COUNTS: Mapping[str, int] = {
    "emitted_hot_event": 0,
    "emitted_inherited_debt": 0,
    "emitted_new_debt": 140,
    "emitted_pinned_core": 43,
    "emitted_recycled": 0,
    "emitted_structural_root": 9,
    "inherited_derived_structural_root": 3,
    "new_derived_structural_root": 52,
}
PRODUCTION_PAGE20_HEADROOM: Mapping[str, int] = {
    "clauses": 265,
    "literals": 909_681,
...
NATIVE_SOURCE_RELATIVE = Path("native/cadical_o1_joint_score_sieve_v31.cpp")
NATIVE_SOURCE_BYTES = 52_803
NATIVE_SOURCE_SHA256 = (
    "4b39d577d9728a5fcb09452c747f574c1900cd8cba06eaabdeda7fd2244a3939"
)
O1C104_LIVE_BANK_RELATIVE = O1C104_BUNDLE_RELATIVE / _o1c104.FINAL_BANK_NAME
O1C103_PRIORITY_RECEIPT_RELATIVE = (
    O1C104_BUNDLE_RELATIVE / _o1c104.PRIORITY_RECEIPT_NAME
)
INHERITED_DERIVED_RECEIPT_RELATIVE = (
    O1C104_BUNDLE_RELATIVE / _o1c104.INHERITED_DERIVED_RECEIPT_NAME
)
INHERITED_DERIVED_CLOSURE_RELATIVE = (
    O1C104_BUNDLE_RELATIVE / _o1c104.INHERITED_DERIVED_CLOSURE_NAME
)
INHERITED_DERIVED_OVERLAY_RELATIVE = (
    O1C104_BUNDLE_RELATIVE / _o1c104.INHERITED_DERIVED_OVERLAY_NAME
)
NEW_DERIVED_RECEIPT_RELATIVE = O1C104_BUNDLE_RELATIVE / _o1c104.DERIVED_RECEIPT_NAME
NEW_DERIVED_CLOSURE_RELATIVE = O1C104_BUNDLE_RELATIVE / _o1c104.DERIVED_CLOSURE_NAME
NEW_DERIVED_OVERLAY_RELATIVE = O1C104_BUNDLE_RELATIVE / _o1c104.DERIVED_OVERLAY_NAME
...
class JointScoreSieveV34Error(O1RelationalSearchError):
    """Native-v31 execution or continuation validation failed."""
...
@dataclass(frozen=True)
class JointScoreSieveV34Result:
    """Typed native-v31 result and its validated evolved bank."""
...
def _error(field: str) -> JointScoreSieveV34Error:
    return JointScoreSieveV34Error(f"joint-score-sieve-v34 {field} differs")
...
def _read_page20(path: str | Path) -> tuple[Path, bytes]:
    io_v1 = _v9._v8._v7._v1
    resolved, payload, digest = io_v1._read_input(path, "Page20")
    ...
    if len(payload) != PRODUCTION_PAGE20_BYTES or digest != PRODUCTION_PAGE20_SHA256:
        raise _error("Page20 active projection seal")
    return resolved, payload
...
def _manifest_contract() -> tuple[str, int, str, frozenset[str]]:
    """Return the sealed O1C-0104 composed Page-20 contract."""

    return (
        O1C104_MANIFEST_SCHEMA,
        _positive(O1C104_MANIFEST_BYTES, "O1C104 manifest byte count"),
        _sha(O1C104_MANIFEST_SHA256, "O1C104 manifest digest"),
        O1C104_MANIFEST_ARTIFACTS,
    )
...
def _validate_manifest(payload: bytes) -> None:
    """Validate the exact canonical O1C-0104 Page-20 preparation manifest."""
    ...
    try:
        ...
        raise _error("O1C104 manifest JSON") from exc
    if (
        canonical != payload
        or len(payload) != expected_bytes
        or hashlib.sha256(payload).hexdigest() != expected_sha256
    ):
        raise _error("O1C104 canonical manifest seal")
    ...
    namespaces = _mapping(
        root.get("derived_resolution_namespaces"), "manifest derived namespaces"
    )
    ...
    page20 = _mapping(root.get("page20"), "manifest Page20")
    capacity = _mapping(page20.get("native_capacity_proof"), "manifest capacity")
    final_bank = _mapping(root.get("final_priority_bank"), "manifest final bank")
    if (
        root.get("schema") != schema
        or root.get("attempt_id") != "O1C-0104"
        or set(artifacts) != artifact_names
        or O1C104_PUBLISHED_ARTIFACTS
        != artifact_names | {_o1c104.PREPARATION_MANIFEST_NAME}
        or zero_call
        != {
        ...
        or authorization.get("page20_burned") is not False
        or authorization.get("lineage33_burned") is not False
        or authorization.get("science_call_authorized") is not False
        or authorization.get("page19_retry_or_replay_authorized") is not False
        or emitted.get("union_clause_count") != 2_603
        or emitted.get("occurrence_count") != 2_613
        or emitted.get("duplicate_occurrence_count") != 10
        or emitted.get("strict_subsumption_pair_count") != 14
        or emitted.get("undominated_clause_count") != 2_592
        ...
        or logical.get("combined_encoding_sha256") != _o1c104.LOGICAL_KNOWN_SHA256
        or logical.get("emitted_inventory_sha256")
        != _o1c104.EMITTED_KNOWN_INVENTORY_SHA256
        or logical.get("inherited_inventory_sha256")
        != _o1c104.INHERITED_DERIVED_INVENTORY_SHA256
        or logical.get("new_inventory_sha256")
        ...
        or logical.get("strict_subsumption_pair_count") != 119
        or logical.get("undominated_clause_count") != 2_579
        ...
        != {
            ...
            "closure_sha256": INHERITED_DERIVED_CLOSURE_SHA256,
            "overlay_clause_count": 3,
            "overlay_sha256": INHERITED_DERIVED_OVERLAY_SHA256,
            "receipt_sha256": INHERITED_DERIVED_RECEIPT_SHA256,
        }
        or new
        != {
            "closure_clause_count": 84,
            "closure_sha256": NEW_DERIVED_CLOSURE_SHA256,
            "overlay_clause_count": 52,
            "overlay_sha256": NEW_DERIVED_OVERLAY_SHA256,
            "receipt_sha256": NEW_DERIVED_RECEIPT_SHA256,
        }
        or science.get("inherited_certified_derived_resolution_clause_count") != 5
        or science.get("new_certified_derived_resolution_clause_count") != 84
        ...
        or page20.get("lineage_ordinal") != PRODUCTION_PAGE20_LINEAGE_ORDINAL
        or page20.get("active_limit") != PRODUCTION_PAGE20_ACTIVE_LIMIT
        or page20.get("active_sha256") != PRODUCTION_PAGE20_SHA256
        or page20.get("clause_count") != PRODUCTION_PAGE20_CLAUSE_COUNT
        or page20.get("literal_count") != PRODUCTION_PAGE20_LITERAL_COUNT
        or page20.get("serialized_bytes") != PRODUCTION_PAGE20_BYTES
        or page20.get("category_counts") != PRODUCTION_PAGE20_CATEGORY_COUNTS
        or page20.get("headroom") != PRODUCTION_PAGE20_HEADROOM
        or page20.get("selected_inherited_derived_clause_count") != 3
        or page20.get("selected_new_derived_clause_count") != 52
        or page20.get("selected_emitted_clause_count") != 192
        or page20.get("pure_emitted_candidate_sha256")
        != PURE_EMITTED_CANDIDATE_SHA256
        or page20.get("pure_emitted_candidate_activated") is not False
        or page20.get("never_resident_undominated_clause_count") != 132
        ...
        != {
            ...
            "observed_parent_unique_burst_clause_count": 265,
        ...
        or final_bank.get("receipt_sha256") != O1C103_PRIORITY_RECEIPT_SHA256
        ...
        raise _error("O1C104 Page20 lineage-33 manifest contract")

    exact_artifacts = {
        _o1c104.ACTIVE_PROJECTION_NAME: (
            PRODUCTION_PAGE20_BYTES,
            PRODUCTION_PAGE20_SHA256,
...
def _validate_new_derived_receipt(payload: bytes) -> None:
    ...
    if (
        ...
        or binding.get("active_sha256") != PRODUCTION_PAGE20_SHA256
        or binding.get("active_clause_count") != PRODUCTION_PAGE20_CLAUSE_COUNT
...
def _validate_seed_report(
    ...
    if (
        ...
        != O1C103_PRIORITY_RECEIPT_BYTES
        or report.get("source_preparation_manifest_sha256") != O1C104_MANIFEST_SHA256
        or report.get("source_preparation_manifest_bytes") != O1C104_MANIFEST_BYTES
...
def _parse_native_payload(
    ...
    *,
...
) -> JointScoreSieveV34Result:
    """Validate one native-v31 document and its unchanged v6 lifecycle."""
    ...
    return JointScoreSieveV34Result(
        status=base.status,
...
def _validate_prelaunch(
    *,
    ...
    priority_receipt_path: str | Path,
    inherited_derived_receipt_path: str | Path,
    inherited_derived_closure_path: str | Path,
    inherited_derived_overlay_path: str | Path,
    new_derived_receipt_path: str | Path,
    new_derived_closure_path: str | Path,
    new_derived_overlay_path: str | Path,
    bank_path: str | Path,
    page20_path: str | Path,
    ...
    if _sha(expected_source_sha256, "expected native source digest") != (
        ...
        raise _error("native-v31 source identity")
    ...
    manifest, manifest_bytes = _read_exact(
        manifest_path,
        label="O1C104 manifest",
        expected_bytes=manifest_expected_bytes,
        expected_sha256=manifest_expected_sha256,
    )
    priority_receipt, priority_receipt_bytes = _read_exact(
        priority_receipt_path,
        label="O1C103 priority receipt",
        expected_bytes=O1C103_PRIORITY_RECEIPT_BYTES,
        expected_sha256=O1C103_PRIORITY_RECEIPT_SHA256,
    )
    inherited_receipt, inherited_receipt_bytes = _read_exact(
        inherited_derived_receipt_path,
        label="inherited O1C102 derived receipt",
        expected_bytes=INHERITED_DERIVED_RECEIPT_BYTES,
        expected_sha256=INHERITED_DERIVED_RECEIPT_SHA256,
    )
    inherited_closure, inherited_closure_bytes = _read_exact(
        inherited_derived_closure_path,
        label="inherited O1C102 derived closure",
        expected_bytes=INHERITED_DERIVED_CLOSURE_BYTES,
        expected_sha256=INHERITED_DERIVED_CLOSURE_SHA256,
    )
    inherited_overlay, inherited_overlay_bytes = _read_exact(
        inherited_derived_overlay_path,
        label="inherited O1C102 derived overlay",
        expected_bytes=INHERITED_DERIVED_OVERLAY_BYTES,
        expected_sha256=INHERITED_DERIVED_OVERLAY_SHA256,
    )
    new_receipt, new_receipt_bytes = _read_exact(
        new_derived_receipt_path,
        label="new O1C104 derived receipt",
        expected_bytes=NEW_DERIVED_RECEIPT_BYTES,
        expected_sha256=NEW_DERIVED_RECEIPT_SHA256,
    )
    new_closure, new_closure_bytes = _read_exact(
        new_derived_closure_path,
        label="new O1C104 derived closure",
        expected_bytes=NEW_DERIVED_CLOSURE_BYTES,
        expected_sha256=NEW_DERIVED_CLOSURE_SHA256,
    )
    new_overlay, new_overlay_bytes = _read_exact(
        new_derived_overlay_path,
        label="new O1C104 derived overlay",
        expected_bytes=NEW_DERIVED_OVERLAY_BYTES,
        expected_sha256=NEW_DERIVED_OVERLAY_SHA256,
    )
    bank, bank_bytes = _read_live_bank(bank_path)
    page20, page20_bytes = _read_page20(page20_path)
    ...
    _validate_priority_receipt(priority_receipt_bytes, bank_bytes)
    _validate_inherited_derived_receipt(inherited_receipt_bytes)
    _validate_new_derived_receipt(new_receipt_bytes)
    return _SealedPrelaunch(
        ...
        inherited_receipt,
        inherited_receipt_bytes,
        inherited_closure,
        inherited_closure_bytes,
        inherited_overlay,
        inherited_overlay_bytes,
        new_receipt,
        new_receipt_bytes,
        new_closure,
        new_closure_bytes,
...
def run_joint_score_sieve(
    *,
    ...
    inherited_derived_resolution_receipt_path: str | Path | None = None,
    inherited_derived_resolution_closure_path: str | Path | None = None,
    inherited_derived_resolution_overlay_path: str | Path | None = None,
    new_derived_resolution_receipt_path: str | Path | None = None,
    new_derived_resolution_closure_path: str | Path | None = None,
    new_derived_resolution_overlay_path: str | Path | None = None,
    sealed_page20_path: str | Path | None = None,
) -> JointScoreSieveV34Result:
    """Validate a prebuilt native-v31 executable, then launch it exactly once."""
    ...
    try:
        ...
        manifest = (
            ...
            else root / O1C104_MANIFEST_RELATIVE
        )
        priority_receipt = (
            Path(priority_state_receipt_path)
            if priority_state_receipt_path is not None
            else root / O1C103_PRIORITY_RECEIPT_RELATIVE
        )
        inherited_receipt = (
            Path(inherited_derived_resolution_receipt_path)
            if inherited_derived_resolution_receipt_path is not None
            else root / INHERITED_DERIVED_RECEIPT_RELATIVE
        )
        inherited_closure = (
            Path(inherited_derived_resolution_closure_path)
            if inherited_derived_resolution_closure_path is not None
            else root / INHERITED_DERIVED_CLOSURE_RELATIVE
        )
        inherited_overlay = (
            Path(inherited_derived_resolution_overlay_path)
            if inherited_derived_resolution_overlay_path is not None
            else root / INHERITED_DERIVED_OVERLAY_RELATIVE
        )
        new_receipt = (
            Path(new_derived_resolution_receipt_path)
            if new_derived_resolution_receipt_path is not None
            else root / NEW_DERIVED_RECEIPT_RELATIVE
        )
        new_closure = (
            Path(new_derived_resolution_closure_path)
            if new_derived_resolution_closure_path is not None
            else root / NEW_DERIVED_CLOSURE_RELATIVE
        )
        new_overlay = (
            Path(new_derived_resolution_overlay_path)
            if new_derived_resolution_overlay_path is not None
            else root / NEW_DERIVED_OVERLAY_RELATIVE
        )
        page20 = (
            Path(sealed_page20_path)
            if sealed_page20_path is not None
            else root / O1C104_PAGE20_RELATIVE
        )
        ...
        sealed = _validate_prelaunch(
            ...
            priority_receipt_path=priority_receipt,
        ...
        if (
            len(vault_bytes) == BURNED_PAGE19_BYTES
            and vault_sha == BURNED_PAGE19_SHA256
        ):
            raise _error("burned Page19 vault rejection")
        if (
        ...
        if (
            len(vault_bytes) != PRODUCTION_PAGE20_BYTES
            or vault_sha != PRODUCTION_PAGE20_SHA256
            or vault_bytes != sealed.page20_bytes
        ):
            raise _error("Page20 production seal")
        ...
        if (
            input_vault.sha256 != PRODUCTION_PAGE20_SHA256
            or _candidate_order(field) != PRODUCTION_CANDIDATES
        ):
            raise _error("Page20 parsed production seal")
        ...
        for original, resolved, before, name in (
            ...
            (manifest, sealed.manifest_path, sealed.manifest_bytes, "O1C104 manifest"),
            (
                priority_receipt,
                sealed.priority_receipt_path,
                sealed.priority_receipt_bytes,
                "priority receipt",
            ),
            (
                inherited_receipt,
                sealed.inherited_derived_receipt_path,
                sealed.inherited_derived_receipt_bytes,
                "inherited derived receipt",
            ),
            (
                inherited_closure,
                sealed.inherited_derived_closure_path,
                sealed.inherited_derived_closure_bytes,
                "inherited derived closure",
            ),
            (
                inherited_overlay,
                sealed.inherited_derived_overlay_path,
                sealed.inherited_derived_overlay_bytes,
                "inherited derived overlay",
            ),
            (
                new_receipt,
                sealed.new_derived_receipt_path,
                sealed.new_derived_receipt_bytes,
                "new derived receipt",
            ),
            (
                new_closure,
                sealed.new_derived_closure_path,
                sealed.new_derived_closure_bytes,
                "new derived closure",
            ),
            (
                new_overlay,
                sealed.new_derived_overlay_path,
            ...
            (page20, sealed.page20_path, sealed.page20_bytes, "sealed Page20"),
        ...
        if completed.returncode:
            ...
            raise JointScoreSieveV34Error(
                "joint-score-sieve-v34 execution failed: "
        ...
        message = str(exc)
        if not message.startswith("joint-score-sieve-v34"):
            message = f"joint-score-sieve-v34 adapter failed: {message}"
