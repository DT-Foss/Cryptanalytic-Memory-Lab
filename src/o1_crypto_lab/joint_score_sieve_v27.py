"""Fail-closed O1C-0089 adapter for Page 13 and the sealed live bank.

Version 27 preserves v26's proof-mining and result-validation semantics while
moving the native boundary to result/source v24 and fresh lineage 26 Page 13.
...
JOINT_SCORE_SIEVE_ADAPTER_SCHEMA = "o1-256-joint-score-sieve-v27-adapter-v1"
JOINT_SCORE_SIEVE_RESULT_SCHEMA = "o1-256-cadical-joint-score-sieve-result-v24"
...
PRIORITY_ACTION_SCHEMA = "o1-256-o1c90-failure-first-proof-mining-actions-v1"
...
LIVE_BANK_SHA256 = "0203de9f1732b095bf30062cb8a07b018ded829ee99f18ffbca715c653c0cc6a"
LIVE_BANK_TOTAL_COUNT = 215_781
LIVE_BANK_MAXIMUM_COUNT = 1_752
LIVE_BANK_ELIGIBLE_COORDINATES = 255

O1C89_MANIFEST_SCHEMA = "o1-256-o1c89-page13-causal-rollover-preparation-v1"
O1C89_MANIFEST_BYTES = 15_590
O1C89_MANIFEST_SHA256 = (
    "467e519df281db4fc10de9223195dfedba9fd51edc93b40883f59fd3821e29ec"
)
O1C89_MANIFEST_ARTIFACTS = frozenset(
    {
...
O1C89_PUBLISHED_ARTIFACTS = O1C89_MANIFEST_ARTIFACTS | {
    "causal-rollover-preparation-manifest.json"
}
O1C89_MANIFEST_RELATIVE = Path(
    "research/o1c89_page13_causal_rollover_seed_20260720/"
    "causal-rollover-preparation-manifest.json"
)
O1C89_PAGE13_RELATIVE = Path(
    "research/o1c89_page13_causal_rollover_seed_20260720/page-13-active.bin"
)

O1C88_PRIORITY_RECEIPT_SCHEMA = (
    "o1-256-o1c87-live-parent-centered-continuation-priority-state-v1"
)
O1C88_PRIORITY_RECEIPT_BYTES = 52_009
O1C88_PRIORITY_RECEIPT_SHA256 = (
    "9ecec7df26d93de464bc779b19f5ccab22588b8f809c443987e62ce6265a8eb8"
)
PRODUCTION_PAGE13_BYTES = 2_846_623
PRODUCTION_PAGE13_SHA256 = (
    "4c1b7d5a6d40fad9439d95433bcc7a60ff3e7ddc0e4542b0cf003cdf4581e546"
)
PRODUCTION_PAGE13_LINEAGE_ORDINAL = 26
PRODUCTION_PAGE13_ACTIVE_LIMIT = 253
PRODUCTION_PAGE13_CLAUSE_COUNT = 253
PRODUCTION_PAGE13_LITERAL_COUNT = 711_355
PRODUCTION_PAGE13_CATEGORY_COUNTS: Mapping[str, int] = {
    "structural_root": 5,
    "pinned_core": 43,
    "inherited_debt": 0,
    "new_debt": 205,
...
PRODUCTION_PAGE13_HEADROOM: Mapping[str, int] = {
    "clauses": 259,
    "literals": 888_645,
    "serialized_bytes": 5_541_985,
}
PRODUCTION_PAGE13_MISSING_NEW_UNION_INDICES = (
    1032,
    1036,
    1051,
    1055,
...
NATIVE_SOURCE_RELATIVE = Path("native/cadical_o1_joint_score_sieve_v24.cpp")
O1C89_LIVE_BANK_RELATIVE = Path(
    "research/o1c89_page13_causal_rollover_seed_20260720/"
    "final-parent-centered-priority-bank.bin"
)
O1C88_PRIORITY_RECEIPT_RELATIVE = Path(
    "research/o1c89_page13_causal_rollover_seed_20260720/"
    "o1c88-priority-state-receipt.json"
)
...
class JointScoreSieveV27Error(O1RelationalSearchError):
    """Native-v24 execution or continuation validation failed."""
...
@dataclass(frozen=True)
class JointScoreSieveV27Result:
    """Typed native-v24 result and its validated evolved bank."""
...
def _error(field: str) -> JointScoreSieveV27Error:
    return JointScoreSieveV27Error(f"joint-score-sieve-v27 {field} differs")
...
def _read_page13(path: str | Path) -> tuple[Path, bytes]:
    io_v1 = _v9._v8._v7._v1
    resolved, payload, digest = io_v1._read_input(path, "Page13")
    ...
    if len(payload) != PRODUCTION_PAGE13_BYTES or digest != PRODUCTION_PAGE13_SHA256:
        raise _error("Page13 active projection seal")
    return resolved, payload


def _manifest_contract() -> tuple[str, int, str, frozenset[str]]:
    """Return the sealed O1C-0089 preparation contract."""

    return (
        O1C89_MANIFEST_SCHEMA,
        _positive(O1C89_MANIFEST_BYTES, "O1C89 manifest byte count"),
        _sha(O1C89_MANIFEST_SHA256, "O1C89 manifest digest"),
        O1C89_MANIFEST_ARTIFACTS,
    )


def _validate_new_clause_residency(value: object) -> None:
    report = _mapping(value, "manifest Page13 new-clause residency")
    missing_indices = list(PRODUCTION_PAGE13_MISSING_NEW_UNION_INDICES)
    resident_indices = sorted(
        set(range(1_032, 1_291)) - set(PRODUCTION_PAGE13_MISSING_NEW_UNION_INDICES)
    )
    rows_value = report.get("missing_clauses")
    if not isinstance(rows_value, list):
        raise _error("manifest Page13 new-clause residency")
    rows: list[dict[str, object]] = []
    for position, row_value in enumerate(rows_value):
        row = _mapping(row_value, "manifest Page13 missing clause")
        union_index = (
            missing_indices[position] if position < len(missing_indices) else -1
        )
        if (
            set(row) != {"clause_sha256", "source_index", "union_index"}
            or row.get("union_index") != union_index
            or row.get("source_index") != union_index - 1_032
        ):
            raise _error("manifest Page13 missing clause")
        rows.append(
            {
                "clause_sha256": _sha(
                    row.get("clause_sha256"), "manifest Page13 missing clause digest"
                ),
                "source_index": union_index - 1_032,
                "union_index": union_index,
            }
        )
    if report != {
        "new_union_index_start": 1_032,
        "new_union_index_stop_exclusive": 1_291,
        "attic_retained_clause_count": 259,
        "resident_clause_count": 205,
        "resident_union_indices": resident_indices,
        "missing_clause_count": 54,
...
def _validate_manifest(payload: bytes) -> None:
    ...
    try:
        ...
        raise _error("O1C89 manifest JSON") from exc
    if canonical != payload:
        raise _error("O1C89 canonical manifest encoding")
    ...
    page13 = _mapping(artifacts.get("page-13-active.bin"), "manifest Page13")
    receipt = _mapping(
        artifacts.get("o1c88-priority-state-receipt.json"),
        "manifest priority receipt",
    )
    final_bank = _mapping(root.get("final_priority_bank"), "manifest final bank")
    page13_report = _mapping(root.get("page13"), "manifest Page13 report")
    capacity = _mapping(
        page13_report.get("native_capacity_proof"), "manifest Page13 capacity"
    )
    clause_capacity = _mapping(
        capacity.get("clause_headroom_guarantee"),
        "manifest Page13 clause capacity",
    )
    residual_headroom = _mapping(
        capacity.get("recorded_residual_headroom"),
        "manifest Page13 residual headroom",
    )
    _validate_new_clause_residency(page13_report.get("new_clause_residency"))
    if (
        set(root)
        != {
            "schema",
        ...
        or root.get("attempt_id") != "O1C-0089"
        or set(artifacts) != artifact_names
        or O1C89_PUBLISHED_ARTIFACTS != artifact_names | {O1C89_MANIFEST_RELATIVE.name}
        or zero_call
        != {
        ...
        or parent.get("attempt_id") != "O1C-0088"
        ...
        or parent.get("source_lineage_ordinal") != 25
        or parent.get("source_active_sha256") != BURNED_PAGE12_SHA256
        ...
        or parent.get("global_novelty_baseline_clause_count") != 1_032
        or science.get("imported_science_attempt_id") != "O1C-0088"
        or science.get("imported_science_kind")
        != "fully-emitted-globally-novel-clauses"
        or science.get("imported_fully_emitted_clause_count") != 259
        or science.get("imported_globally_novel_clause_count") != 259
        or science.get("imported_literal_count") != 744_973
        ...
        or rollover.get("source_active_sha256") != BURNED_PAGE12_SHA256
        or rollover.get("clause_count") != 259
        or rollover.get("literal_count") != 744_973
        ...
        is not True
        or rollover.get("api")
        != "advance_causal_residency(next_lineage_ordinal=26,next_active_limit=253)"
        or attic.get("chunk_count") != 16
        or attic.get("union_sha256")
        != "d4ea6d6d1bf1dfa3560aa81737914624c40c2dd9dacfb7fee38badc2aabab726"
        or attic.get("union_clause_count") != 1_291
        or attic.get("union_literal_count") != 3_590_320
        or attic.get("union_serialized_bytes") != 14_366_635
        or attic.get("occurrence_count") != 1_299
        or attic.get("duplicate_occurrence_count") != 8
        or attic.get("strict_subsumption_pair_count") != 10
        or attic.get("new_strict_subsumption_pair_count") != 0
        or attic.get("new_strict_subsumption_relations") != []
        or attic.get("prior_relation_set_preserved_exactly") is not True
        or attic.get("undominated_clause_count") != 1_284
        or attic.get("prior_1032_clause_union_is_exact_prefix") is not True
        ...
        or page13.get("serialized_bytes") != PRODUCTION_PAGE13_BYTES
        or page13.get("sha256") != PRODUCTION_PAGE13_SHA256
        or page13.get("role") != "fresh-lineage-26-page13-science-input"
        or receipt.get("serialized_bytes") != O1C88_PRIORITY_RECEIPT_BYTES
        or receipt.get("sha256") != O1C88_PRIORITY_RECEIPT_SHA256
        or receipt.get("role") != "canonical-o1c88-evolved-priority-state-receipt"
        or final_bank.get("serialized_bytes") != BANK_BYTES
        or final_bank.get("sha256") != LIVE_BANK_SHA256
        or final_bank.get("receipt_serialized_bytes") != O1C88_PRIORITY_RECEIPT_BYTES
        or final_bank.get("receipt_sha256") != O1C88_PRIORITY_RECEIPT_SHA256
        ...
        or final_bank.get("maximum_evolved_count_variables") != [21]
        or final_bank.get("minimum_nonzero_evolved_count") != 200
        ...
        or page13_report.get("active_sha256") != PRODUCTION_PAGE13_SHA256
        or page13_report.get("serialized_bytes") != PRODUCTION_PAGE13_BYTES
        or page13_report.get("lineage_ordinal") != PRODUCTION_PAGE13_LINEAGE_ORDINAL
        or page13_report.get("active_limit") != PRODUCTION_PAGE13_ACTIVE_LIMIT
        or page13_report.get("clause_count") != PRODUCTION_PAGE13_CLAUSE_COUNT
        or page13_report.get("literal_count") != PRODUCTION_PAGE13_LITERAL_COUNT
        or page13_report.get("category_counts") != PRODUCTION_PAGE13_CATEGORY_COUNTS
        or page13_report.get("headroom") != PRODUCTION_PAGE13_HEADROOM
        or page13_report.get("fresh_identity") is not True
        ...
        != {
            ...
            "page13_input_clauses": 253,
            "maximum_additional_clauses_before_capacity_terminal": 259,
            "parent_centered_action_capacity": 256,
            "spare_clause_slots_beyond_action_capacity": 3,
            "proved_sufficient": True,
        }
        or residual_headroom != {"literals": 888_645, "serialized_bytes": 5_541_985}
        ...
        raise _error("O1C89 Page13 lineage-26 manifest contract")
...
def _validate_receipt(payload: bytes, bank: bytes) -> None:
    try:
        document = _mapping(
            ...
            "O1C88 priority receipt",
        )
        if document.get("schema") != O1C88_PRIORITY_RECEIPT_SCHEMA:
            raise _error("O1C88 priority receipt schema")
        ...
        raise _error("O1C88 priority receipt contract") from exc
    if receipt_bank != bank:
        raise _error("O1C88 priority receipt bank linkage")
...
def _parse_native_payload(
    ...
    *,
...
) -> JointScoreSieveV27Result:
    """Validate one native-v24 document and its unchanged v6 lifecycle."""
    ...
    return JointScoreSieveV27Result(
        status=base.status,
...
def _validate_prelaunch(
    *,
    ...
    manifest, manifest_bytes = _read_exact(
        manifest_path,
        label="O1C89 manifest",
        expected_bytes=manifest_expected_bytes,
        expected_sha256=manifest_expected_sha256,
    )
    receipt, receipt_bytes = _read_exact(
        receipt_path,
        label="O1C88 priority receipt",
        expected_bytes=O1C88_PRIORITY_RECEIPT_BYTES,
        expected_sha256=O1C88_PRIORITY_RECEIPT_SHA256,
    )
    ...
    page13, page13_bytes = _read_page13(page13_path)
...
def run_joint_score_sieve(
    *,
    ...
    sealed_page13_path: str | Path | None = None,
) -> JointScoreSieveV27Result:
    """Validate a prebuilt native-v24 executable, then launch it exactly once."""
    ...
    try:
        ...
        manifest = (
            ...
            else root / O1C89_MANIFEST_RELATIVE
        )
        receipt = (
            Path(priority_state_receipt_path)
            if priority_state_receipt_path is not None
            else root / O1C88_PRIORITY_RECEIPT_RELATIVE
        )
        page13 = (
            Path(sealed_page13_path)
            if sealed_page13_path is not None
            else root / O1C89_PAGE13_RELATIVE
        )
        ...
        if (
            len(vault_bytes) != PRODUCTION_PAGE13_BYTES
            or vault_sha != PRODUCTION_PAGE13_SHA256
            or vault_bytes != sealed.page13_bytes
        ):
            raise _error("Page13 production seal")
        ...
        if (
            input_vault.sha256 != PRODUCTION_PAGE13_SHA256
            or _candidate_order(field) != PRODUCTION_CANDIDATES
        ):
            raise _error("Page13 parsed production seal")
        ...
        for original, resolved, before, name in (
            ...
            (manifest, sealed.manifest_path, sealed.manifest_bytes, "O1C89 manifest"),
            ...
            (page13, sealed.page13_path, sealed.page13_bytes, "sealed Page13"),
        ...
        if completed.returncode:
            ...
            raise JointScoreSieveV27Error(
                "joint-score-sieve-v27 execution failed: "
        ...
        message = str(exc)
        if not message.startswith("joint-score-sieve-v27"):
            message = f"joint-score-sieve-v27 adapter failed: {message}"
