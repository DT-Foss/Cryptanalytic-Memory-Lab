"""O1C-0071: one vault-conditioned ranked-decision call from O1C-0070.
...
O1C-0071 imports O1C-0070's sealed 202-clause vault, journals local ordinal 0 /
lineage ordinal 7, and authorizes one fresh native-v10 process with no retry,
...
from . import joint_score_sieve_v13 as _native_v13
...
ATTEMPT_ID = "O1C-0071"
CONFIG_SCHEMA = "o1-256-apple8-vault-ranked-decision-config-v1"
PREFLIGHT_SCHEMA = "o1-256-apple8-vault-ranked-decision-preflight-v1"
INVOCATION_SCHEMA = "o1-256-apple8-vault-ranked-decision-invocation-v1"
INTENT_SCHEMA = "o1-256-apple8-vault-ranked-decision-intent-v1"
EPISODE_SCHEMA = "o1-256-apple8-vault-ranked-decision-episode-v1"
RESULT_SCHEMA = "o1-256-apple8-vault-ranked-decision-result-v1"
FAILURE_EVIDENCE_SCHEMA = "o1-256-o1c71-native-failure-evidence-v1"
PUBLICATION_RECOVERY_SCHEMA = "o1-256-o1c71-publication-recovery-v1"

INVOCATION_ID = "O1C-0071-apple8-vault-ranked-decision-v1-call-0007"
CAPSULE_SUFFIX = "O1C-0071_apple8-vault-ranked-decision-v1"
...
DESIGN_RELATIVE = Path(
    "research/O1C0071_APPLE8_VAULT_RANKED_DECISION_DESIGN_20260719.md"
)
CONFIG_RELATIVE = Path("configs/o1c71_apple8_vault_ranked_decision_v1.json")
...
PARENT_SOURCE_COMMIT = "c5ad5c40f0ac84f65d281cf2366d2ca6b6c49a52"
...
PARENT_LINEAGE_CALLS = 7
PARENT_COMPLETED_EPISODES = 1
PARENT_LAST_CONSUMED_ORDINAL = 6
PARENT_LAST_COMPLETED_ORDINAL = 6
PARENT_FAILED_ORDINAL = 2
PARENT_KNOWN_COMPLETED_BILLED_CONFLICTS = 3_079
PARENT_FAILED_CALL_BILLED_CONFLICTS: None = None
PARENT_LINEAGE_ACTUAL_BILLED_CONFLICTS: None = None
PARENT_LAST_DECISIONS = 2_297
PARENT_LAST_PROPAGATIONS = 1_169_826
PARENT_LAST_MINIMUM_UPPER_BOUND = 18.846601115977638

LOCAL_EPISODE_ORDINAL = 0
LINEAGE_CALL_ORDINAL = 7
...
TARGET_FREE_PREFLIGHT_RELATIVE = Path(
    "research/O1C0071_TARGET_FREE_VAULT_RANKED_DECISION_PREFLIGHT_20260719.json"
)
TARGET_FREE_PREFLIGHT_SCHEMA = (
    "o1-256-o1c71-target-free-vault-ranked-decision-preflight-v1"
)
TARGET_FREE_PREFLIGHT_CLASSIFICATION = (
    "O1C71_TARGET_FREE_VAULT_RANKED_DECISION_PREFLIGHT_PASS"
)

CAPACITY_RESERVATION: dict[str, object] = {
    "schema": "o1-256-o1c71-observed-envelope-capacity-reservation-v1",
...
RANK_SOURCE_BINDING: dict[str, object] = {
    "schema": "o1-256-o1c71-vault-ranked-decision-source-v1",
...
READER_SCHEMA = _rank_v1.VAULT_RANKED_DECISION_READER_SCHEMA
READER_OPERATOR = _rank_v1.VAULT_RANKED_DECISION_OPERATOR
READER_SPEC_SHA256 = _rank_v1.VAULT_RANKED_DECISION_SPEC_SHA256

NATIVE_RESULT_SCHEMA = _native_v13.JOINT_SCORE_SIEVE_RESULT_SCHEMA
NATIVE_IMPLEMENTATION_PARENT_SCHEMA = (
    _native_v13.JOINT_SCORE_SIEVE_IMPLEMENTATION_PARENT_SCHEMA
)
NATIVE_ADAPTER_SCHEMA = _native_v13.JOINT_SCORE_SIEVE_ADAPTER_SCHEMA
NATIVE_LEDGER_SCHEMA = _native_v13.O1C67_VAULT_SOFT_CONFLICT_LEDGER_SCHEMA
NATIVE_VAULT_TELEMETRY_SCHEMA = _o1c67.NATIVE_VAULT_TELEMETRY_SCHEMA
NATIVE_DECISION_TELEMETRY_SCHEMA = "o1-256-cadical-vault-ranked-decision-telemetry-v1"

PUBLIC_EXACT_RECOVERY = "PUBLIC_EXACT_RECOVERY"
VAULT_RANKED_DECISION_GAIN = "EPISODIC_VAULT_ACTIVE_RANKED_DECISION_GAIN"
VAULT_RANKED_DECISION_NO_GAIN = "EPISODIC_VAULT_ACTIVE_RANKED_DECISION_NO_GAIN"
THRESHOLD_REGION_EXHAUSTED = "EPISODIC_VAULT_THRESHOLD_REGION_EXHAUSTED"
CAPACITY_TERMINAL = "EPISODIC_VAULT_CAPACITY_TERMINAL"
OPERATIONAL_TERMINAL = "EPISODIC_VAULT_ACTIVE_RANKED_DECISION_OPERATIONAL_TERMINAL"
INVALID_RESULT_TERMINAL = "EPISODIC_VAULT_ACTIVE_RANKED_DECISION_INVALID_RESULT"

SOURCE_RELATIVES = {
    "runner": "src/o1_crypto_lab/o1c71_apple8_vault_ranked_decision_run.py",
    ...
    "native_source": "native/cadical_o1_joint_score_sieve_v10.cpp",
...
class O1C71RunError(RuntimeError):
    """O1C-0071 provenance, protocol, reader, or result differs."""


class O1C71ParentError(O1C71RunError):
    """The sealed O1C-0070 parent or retained vault differs."""
...
class EpisodeInvoker(Protocol):
    def __call__(
        ...
        """Make the one authorized fresh native-v10 process call."""
...
def _mapping(value: object, field: str) -> Mapping[str, object]:
    if not isinstance(value, dict) or not all(isinstance(key, str) for key in value):
        raise O1C71RunError(f"{field} must be an object")
    return cast(Mapping[str, object], value)


def _sequence(value: object, field: str) -> Sequence[object]:
    if not isinstance(value, list):
        raise O1C71RunError(f"{field} must be an array")
    return cast(Sequence[object], value)


def _nonnegative_int(value: object, field: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or value < 0:
        raise O1C71RunError(f"{field} differs")
    return value
...
def _relative(root: Path, value: object, field: str) -> Path:
    ...
    if candidate.is_absolute() or ".." in candidate.parts:
        raise O1C71RunError(f"{field} escapes the lab")
    try:
        path = (root / candidate).resolve(strict=True)
    except (OSError, RuntimeError) as exc:
        raise O1C71RunError(f"{field} cannot be resolved") from exc
...
def _read_json(path: Path, field: str) -> Mapping[str, object]:
    try:
        ...
        raise O1C71RunError(f"{field} cannot be read") from exc
    return _mapping(value, field)


def _validate_reader(value: object, field: str = "reader") -> dict[str, object]:
    """Validate the exact production ranked-decision reader binding."""
    ...
    if (
        ...
        raise O1C71RunError(f"{field} ranked-decision binding differs")
    return dict(reader)
...
def _baseline_compat_config(config: Mapping[str, object]) -> dict[str, object]:
    ...
    if "frozen_sha256" in config:
        raise O1C71RunError("legacy frozen_sha256 is forbidden in O1C-0071")
    ...
    if config.get("apple8_baseline_attempt_id") != "APPLE-VIEW-0008-MATCHED":
        raise O1C71RunError("APPLE8 baseline attempt differs")
...
def validate_apple8_baseline(root: Path, config: Mapping[str, object]) -> object:
    ...
    try:
        ...
        raise O1C71RunError("frozen positive APPLE8 input differs") from exc
...
def validate_grouping_provenance(
    ...
    try:
        ...
        raise O1C71RunError("APPLE-VIEW-0009 grouping provenance differs") from exc


def _validate_rank_config(value: object) -> dict[str, object]:
    row = _mapping(value, "ranked_decision")
    expected_fields = {
        "schema",
        "operator",
        "reader_spec_sha256",
    ...
    spec = _digest_or_pending(
        row.get("reader_spec_sha256"), "ranked_decision.reader_spec_sha256"
    )
    if (
        ...
        raise O1C71RunError("ranked-decision config differs")
    return dict(row)


def load_config(path: str | Path) -> dict[str, object]:
    """Load the exact O1C-0071 protocol without a call or filesystem write."""

    root = lab_root().resolve(strict=True)
    config_path = Path(path).resolve(strict=True)
    if not config_path.is_relative_to(root):
        raise O1C71RunError("config escapes the lab")
    config = dict(_read_json(config_path, "O1C71 config"))
    ...
    _validate_rank_config(config.get("ranked_decision"))
    if (
        set(config) != CONFIG_FIELDS
        or config.get("schema") != CONFIG_SCHEMA
        or config.get("attempt_id") != ATTEMPT_ID
        or config.get("slug") != "apple8-vault-ranked-decision-v1"
        ...
        or parent.get("attempt_id") != "O1C-0070"
        ...
        or parent.get("classification") != _o1c70.VAULT_PHASE_READER_NO_GAIN
        ...
        raise O1C71RunError("frozen O1C-0071 config differs")
    ...
    if configured_spec != "PENDING" and configured_spec != READER_SPEC_SHA256:
        raise O1C71RunError("native reader specification differs")
    ...
    if source_native != expected["native_source"] or executable == "":
        raise O1C71RunError("native source/executable binding differs")
    return config
...
def validate_capacity_reservation(value: object) -> dict[str, object]:
    ...
    if dict(capacity) != CAPACITY_RESERVATION:
        raise O1C71RunError("capacity reservation differs")
    ...
    if (
        ...
        raise O1C71RunError("capacity reservation exceeds a hard vault cap")
...
def _manifest_inventory(capsule: Path) -> dict[str, str]:
    ...
    if sha256_file(manifest) != PARENT_MANIFEST_SHA256:
        raise O1C71ParentError("O1C-0070 manifest identity differs")
    inventory: dict[str, str] = {}
    try:
        for line in manifest.read_text(encoding="ascii").splitlines():
            digest, separator, relative = line.partition("  ")
            if separator != "  " or relative in inventory:
                raise O1C71ParentError("O1C-0070 manifest syntax differs")
            _sha256(digest, "O1C-0070 manifest digest")
            path = Path(relative)
            if path.is_absolute() or ".." in path.parts:
                raise O1C71ParentError("O1C-0070 manifest path escapes")
            inventory[relative] = digest
    except (OSError, UnicodeError, ValueError) as exc:
        raise O1C71ParentError("O1C-0070 manifest cannot be read") from exc
    return inventory
...
def _parse_bound_vault(
    ...
    *,
    ...
    try:
        ...
        raise O1C71ParentError(f"{field} dual parse differs") from exc
    if (
        ...
        raise O1C71ParentError(f"{field} parsers disagree")
...
def _derive_ranked_decision(
    *, root: Path, vault_payload: bytes, frozen: object
    ...
    try:
        ...
        raise O1C71ParentError("production vault-ranked decision differs") from exc
    rows = decision.rows
    ...
    if (
        ...
        or _validate_reader(reader, "derived reader binding") != dict(reader)
        ...
        raise O1C71ParentError("production vault-ranked decision seal differs")
    return decision
...
def validate_parent_and_import_vault(
    ...
    """Verify O1C-0070 and import its exact retained 202-clause vault."""
    ...
    if (
        ...
        raise O1C71ParentError("O1C-0070 parent identity differs")
    result = _read_json(result_path, "O1C-0070 result")
    episode = _mapping(result.get("episode"), "O1C-0070 episode")
    final_vault = _mapping(result.get("final_vault"), "O1C-0070 final vault")
    resources = _mapping(result.get("resources"), "O1C-0070 resources")
    parent = _mapping(result.get("parent"), "O1C-0070 lineage parent")
    claim = _mapping(result.get("claim_boundary"), "O1C-0070 claim boundary")
    eligible = _mapping(episode.get("eligible_emitted"), "O1C-0070 eligible emitted")
    if (
        result.get("attempt_id") != "O1C-0070"
        or result.get("classification") != _o1c70.VAULT_PHASE_READER_NO_GAIN
        ...
        or result.get("lineage_call_ordinal") != 6
        ...
        or parent.get("known_completed_billed_conflicts") != 2_565
        or parent.get("failed_call_billed_conflicts") is not None
        ...
        raise O1C71ParentError("O1C-0070 terminal evidence differs")
    ...
    if any(inventory.get(name) != digest for name, digest in required.items()):
        raise O1C71ParentError("O1C-0070 manifest inventory differs")
    for relative, digest in required.items():
        if sha256_file(capsule / relative) != digest:
            raise O1C71ParentError("O1C-0070 capsule artifact differs")
    ...
    try:
        ...
        if (
            ...
            raise O1C71ParentError("retained vault file identity differs")
        payload = source.read_bytes()
    except O1C71ParentError:
        raise
    except OSError as exc:
        raise O1C71ParentError("retained vault cannot be read") from exc
    if hashlib.sha256(payload).hexdigest() != PARENT_RETAINED_VAULT_SHA256:
        raise O1C71ParentError("retained vault hash differs")
...
def _validate_final_target_free_preflight(
    ...
    if (
        ...
        or _nonnegative_int(native_repeat.get("repeat_count"), "native repeat count")
        < 2
        or native_repeat.get("byte_deterministic") is not True
        or adapter_repeat.get("passed") is not True
        or _nonnegative_int(adapter_repeat.get("repeat_count"), "adapter repeat count")
        < 2
        ...
        or value.get("public_fixture_sha256") != gates.get("public_fixture_sha256")
        ...
        raise O1C71RunError("final target-free ranked-decision preflight differs")
    return dict(value)
...
def validate_final_target_free_preflight(
    ...
    *,
    ...
    if unresolved:
        if required:
            raise O1C71RunError("target-free final preflight fields remain PENDING")
        return None, None, False
    if gates.get("reader_spec_sha256") != READER_SPEC_SHA256:
        raise O1C71RunError("target-free reader specification differs")
    path = root / TARGET_FREE_PREFLIGHT_RELATIVE
    try:
        status = path.stat(follow_symlinks=False)
    except OSError as exc:
        raise O1C71RunError("final target-free preflight cannot be read") from exc
    if path.is_symlink() or not stat.S_ISREG(status.st_mode):
        raise O1C71RunError("final target-free preflight identity differs")
    observed = sha256_file(path)
    if observed != gates.get("final_preflight_sha256"):
        raise O1C71RunError("final target-free preflight hash differs")
...
def preflight(
    ...
    for name in SOURCE_NAMES:
        if expected[name] != "PENDING" and observed[name] != expected[name]:
            raise O1C71RunError(f"source hash differs for {name}")
    if require_commit_binding and unresolved:
        raise O1C71RunError("science source hashes remain PENDING")
    if (
        getattr(_native_v13, "JOINT_SCORE_SIEVE_ADAPTER_SCHEMA", None)
        != NATIVE_ADAPTER_SCHEMA
        or getattr(_native_v13, "JOINT_SCORE_SIEVE_RESULT_SCHEMA", None)
        != NATIVE_RESULT_SCHEMA
        or getattr(_native_v13, "JOINT_SCORE_SIEVE_IMPLEMENTATION_PARENT_SCHEMA", None)
        != NATIVE_IMPLEMENTATION_PARENT_SCHEMA
        ...
        raise O1C71RunError("adapter-v13/ranked-decision schema binding differs")
    ...
    reader = _validate_reader(
        imported.ranked_decision.reader_binding(),
        "derived reader binding",
    )
    ...
    if require_commit_binding:
        if not clean:
            raise O1C71RunError("science sources/config are not clean")
        for name in SOURCE_NAMES:
            _commit_bound_bytes(
    ...
    if memory_free is not None and memory_free < MINIMUM_MEMORY_FREE_PERCENT:
        raise O1C71RunError("memory-pressure preflight is below gate")
    if disk_free < MINIMUM_DISK_FREE_BYTES:
        raise O1C71RunError("disk-free preflight is below gate")
    if normalized_load > MAXIMUM_NORMALIZED_LOAD_1M:
        raise O1C71RunError("normalized system load is above gate")
    if active:
        raise O1C71RunError("conflicting science process is active")
    return {
        ...
        "adapter_source_sha256": observed["joint_score_sieve_v13"],
...
def _capture_preflight_config(
    ...
    try:
        ...
        raise O1C71RunError("preflight-bound config cannot be captured") from exc
    if (
        hashlib.sha256(payload).hexdigest() != expected_sha256
        or not isinstance(decoded, dict)
        or decoded != config
    ):
        raise O1C71RunError("config identity changed after preflight")
    return payload
...
def _validate_native_reader(
    ...
    """Split and validate v13's static reader binding plus runtime telemetry."""
    ...
    calls = _nonnegative_int(runtime["cb_decide_calls"], "cb_decide calls")
    nonzero = _nonnegative_int(runtime["cb_decide_nonzero"], "cb_decide nonzero")
    zero = _nonnegative_int(runtime["cb_decide_zero"], "cb_decide zero")
    count = _nonnegative_int(
        runtime["returned_sequence_count"], "returned sequence count"
    )
    byte_count = _nonnegative_int(
        runtime["returned_sequence_bytes"], "returned sequence bytes"
    )
    unique = _nonnegative_int(
        runtime["unique_returned_variables"], "unique returned variables"
    )
    redecisions = _nonnegative_int(runtime["redecisions"], "redecisions")
    phase_calls = _nonnegative_int(runtime["solver_phase_calls"], "solver phase calls")
    sequence_hex = runtime["returned_sequence_hex"]
    try:
        sequence = bytes.fromhex(sequence_hex) if isinstance(sequence_hex, str) else b""
    except ValueError as exc:
        raise O1C71RunError("native returned decision sequence differs") from exc
    returns = (
        struct.unpack(f"<{len(sequence) // 4}i", sequence)
        if sequence and len(sequence) % 4 == 0
        else ()
    )
    allowed = set(cast(Sequence[int], reader["ranked_literals"]))
    actual_first_fallback = next(
        (index for index, literal in enumerate(returns, start=1) if literal == 0),
        None,
    )
    actual_unique = len({abs(literal) for literal in returns if literal})
    first_fallback = runtime["first_fallback_call"]
    if (
        set(combined) != set(reader) | _NATIVE_READER_RUNTIME_FIELDS
        or runtime["decision_rule"] != _native_v13.VAULT_RANKED_DECISION_DECISION_RULE
        or runtime["callback_rule"] != _native_v13.VAULT_RANKED_DECISION_CALLBACK_RULE
        or runtime["returned_sequence_encoding"]
        != _native_v13.VAULT_RANKED_DECISION_RETURNED_SEQUENCE_ENCODING
        ...
        or len(sequence) != byte_count
        or runtime["returned_sequence_sha256"] != hashlib.sha256(sequence).hexdigest()
        or any(literal not in allowed for literal in returns if literal)
        or unique != actual_unique
        or redecisions != nonzero - unique
        or phase_calls != 0
        or first_fallback != actual_first_fallback
    ):
        raise O1C71RunError("native ranked-decision telemetry differs")
    telemetry = {
...
def _validated_native(
    ...
    *,
    ...
    try:
        ...
        stats = _native_v13.validate_vault_soft_conflict_ledger(
        ...
        if (
            ...
            raise O1C71RunError("native O1C-0071 contract differs")
    ...
    except O1C71RunError:
        raise
    except Exception as exc:
        raise O1C71RunError("native O1C-0071 result differs") from exc
...
def execute_single_continuation(
    *,
    ...
    """Persist lineage ordinal 7, consume exactly one call, and never retry."""
    ...
    if not capsule.is_dir() or imported_vault.sha256 != PARENT_RETAINED_VAULT_SHA256:
        raise O1C71RunError("single-continuation capsule or vault differs")
    if (
        ...
        raise O1C71RunError("capacity reservation is not call-safe")
    ...
    if imported_path.read_bytes() != adapter_vault.serialized:
        raise O1C71RunError("dual-parsed imported vault bytes differ")
    ...
    if reread != imported_vault:
        raise O1C71RunError("capsule imported vault reread differs")
    ...
    if not invocation_path.is_file() or not intent_path.is_file():
        raise O1C71RunError("invocation/intent not durable before call")
    ...
    try:
        (
            ...
            returned_reader,
            decision_active,
        ) = _validated_native(result, expected_reader=reader)
        eligible_raw = getattr(result, "eligible_emitted_clauses")
        if not isinstance(eligible_raw, Sequence):
            raise O1C71RunError("eligible emitted clause sequence differs")
        ...
        if (
            ...
            raise O1C71RunError("native input vault differs")
        status = cast(int, ledger["status"])
        key_model = getattr(result, "key_model", None)
        if status in (0, 10) and not decision_active:
            raise O1C71RunError("ranked decision telemetry was inactive")
        if status == 10:
            if not isinstance(key_model, bytes) or not verify_public_model(key_model):
                raise O1C71RunError("SAT model failed public eight-block verification")
        elif key_model is not None:
            raise O1C71RunError("non-SAT continuation returned a key")
        ...
        if not isinstance(next_available, bool):
            raise O1C71RunError("next-vault availability differs")
        if next_available:
            if terminal_reason is not None or adapter_next is None:
                raise O1C71RunError("available next vault differs")
            ...
            if parsed_next != expected_next:
                raise O1C71RunError("cumulative continuation vault differs")
        else:
            if (
                terminal_reason
                not in {
                ...
                raise O1C71RunError("terminal next vault differs")
        ...
        if archive_output:
            ...
            if (
                ...
                raise O1C71RunError("continuation output vault reread differs")

        novel_count = len(novel)
        if status == 10:
            classification, stop_reason = (
                PUBLIC_EXACT_RECOVERY,
                "public-verified-candidate-with-active-ranked-decision",
            )
        elif status == 20:
            classification, stop_reason = (
            ...
            classification, stop_reason = (
                VAULT_RANKED_DECISION_GAIN,
                "novel-exact-clauses-with-active-ranked-decision",
            )
        else:
            classification, stop_reason = (
                VAULT_RANKED_DECISION_NO_GAIN,
                "zero-novel-eligible-clauses-with-active-ranked-decision",
            )
        episode = {
            "schema": EPISODE_SCHEMA,
            "completed": True,
            ...
            "ranked_decision_active": decision_active,
...
def validate_native_build_identity(
    ...
    *,
    ...
    try:
        ...
        raise O1C71RunError("native-v10 build identity differs") from exc
    if (
        ...
        raise O1C71RunError("native-v10 build identity differs")
...
def invoke_native_episode(
    *,
    ...
    if vault.is_symlink() or sha256_file(vault) != PARENT_RETAINED_VAULT_SHA256:
        raise O1C71RunError("vault-ranked-decision input vault differs before call")
    try:
        ...
        observed_reader = _validate_reader(
            decision.reader_binding(), "pre-invocation ranked reader"
        )
    except (OSError, _rank_v1.VaultRankedDecisionError) as exc:
        raise O1C71RunError("vault-ranked decision differs before call") from exc
    if observed_reader != dict(expected_reader):
        raise O1C71RunError("vault-ranked reader differs before call")
    return _native_v13.run_joint_score_sieve(
    ...
    *,
    ...
    return {
        ...
        "claim_boundary": {
            ...
            "only_science_change": (
                "sealed-vault-vote-strength-and-singleton-grouped-gap-cb_decide-order"
            ),
            "reader_spec_sha256": READER_SPEC_SHA256,
            "ranked_order_sha256": RANKED_ORDER_SHA256,
            "ranked_table_sha256": RANKED_TABLE_SHA256,
            ...
            "ranked_decision_telemetry_required_for_gain_or_recovery": True,
            ...
            "ranked_reader_closed_on_null": (
                outcome.classification == VAULT_RANKED_DECISION_NO_GAIN
            ),
            "second_ranked_call_or_sweep_authorized": False,
            "phase_calls": 0,
        ...
        "next_action": (
            "Close this exact ranked reader. Do not retry, replay lineage ordinal 7, "
            "call phase(), or sweep rank variants."
        ),
    }
...
def _markdown(result: Mapping[str, object]) -> str:
    return (
        ...
        "- New native calls: `1`\n"
        "- Local / lineage ordinal: `0 / 7`\n"
        "- Reader: abs(vault delta), singleton grouped-gap, variable order\n"
...
def finalize_capsule(
    ...
    *,
    ...
    if authoritative.exists() or (capsule / "artifacts.sha256").exists():
        raise O1C71RunError("O1C-0071 terminal publication already exists")
    ...
    for _ in range(12):
        ...
        raise O1C71RunError("persistent artifact ledger did not converge")
    if persistent > MAXIMUM_PERSISTENT_ARTIFACT_BYTES:
        raise O1C71RunError("persistent artifact byte budget exceeded")
    ...
    try:
        ...
        if authoritative.read_bytes() != payload or result_path.read_bytes() != payload:
            raise O1C71RunError("terminal publication bytes differ")
        _o1c65._assert_immutable_tree(capsule)
    except Exception:
        _o1c65._restore_owned_capsule_for_recovery(capsule)
        if manifest_published:
            _o1c65._unlink_owned_exact(
                manifest_path, manifest, "O1C71 capsule manifest"
            )
        if authoritative_published:
            _o1c65._unlink_owned_exact(
                authoritative, payload, "O1C71 authoritative result"
            )
        raise
...
def _validate_recovery_file(
    ...
    *,
    ...
    try:
        ...
        raise O1C71RunError(f"{field} cannot be read during recovery") from exc
    if path.is_symlink() or not stat.S_ISREG(status.st_mode):
        raise O1C71RunError(f"{field} differs during recovery")
    if expected_bytes is not None:
        size = _nonnegative_int(expected_bytes, f"{field} bytes")
        if status.st_size != size:
            raise O1C71RunError(f"{field} size differs during recovery")
    if sha256_file(path) != expected_sha:
        raise O1C71RunError(f"{field} hash differs during recovery")
...
def _validate_recovery_sidecars(
    ...
    if (
        ...
        raise O1C71RunError("recovery invocation/intent journal differs")
    ...
    if terminal is None:
        ...
        if (
            ...
            or episode.get("ranked_decision_active") is not decision_active
            or native_failure.exists()
            or terminal_failure.exists()
        ):
            raise O1C71RunError("completed recovery decision evidence differs")
        ...
        if work.get("status") == 20:
            ...
            if (
                ...
                raise O1C71RunError("status-20 recovery vault/result differs")
            return
        ...
        if status == 10:
            ...
            if (
                ...
                raise O1C71RunError("recovery public model conclusion differs")
            ...
            if not decision_active:
                raise O1C71RunError("recovery ranked-decision activity differs")
            if not available:
                if terminal_reason not in {
                    ...
                    raise O1C71RunError("recovery capacity conclusion differs")
                expected_classification = CAPACITY_TERMINAL
                expected_stop_reason = terminal_reason
            elif novel_count:
                expected_classification = VAULT_RANKED_DECISION_GAIN
                expected_stop_reason = "novel-exact-clauses-with-active-ranked-decision"
            else:
                expected_classification = VAULT_RANKED_DECISION_NO_GAIN
                expected_stop_reason = (
                    "zero-novel-eligible-clauses-with-active-ranked-decision"
                )
        else:
            raise O1C71RunError("completed recovery status conclusion differs")
        if (
            ...
            raise O1C71RunError("completed recovery vault/result differs")
        if archived:
            if episode.get("output_vault_sidecar") != "episodes/00/vault-output.bin":
                raise O1C71RunError("recovery output vault path differs")
            _validate_recovery_file(
                vault_output,
            ...
            raise O1C71RunError("unarchived recovery output vault differs")
        return
    ...
    if (
        ...
        raise O1C71RunError("failed recovery journal differs")
    ...
    if failure_sha is None:
        if (
            ...
            raise O1C71RunError("invalid-result recovery failure sidecar differs")
    else:
        if (
            ...
            raise O1C71RunError("operational recovery conclusion differs")
        _validate_recovery_file(
            native_failure,
...
def _validate_publication_source(root: Path, capsule: Path) -> dict[str, object]:
    if (
        ...
        raise O1C71RunError("O1C-0071 recovery capsule differs")
    source = dict(
        _read_json(capsule / PUBLICATION_SOURCE_NAME, "O1C-0071 publication source")
    )
    ...
    persisted = _read_json(
        capsule / "episodes" / "00" / "episode.json", "persisted O1C-0071 episode"
    )
    ...
    if (
        ...
        raise O1C71RunError("O1C-0071 publication source differs")
...
def recover_publication(
    *,
    ...
    if authoritative.exists():
        raise O1C71RunError("O1C-0071 authoritative publication already exists")
...
def run(config_path: str | Path) -> dict[str, object]:
    ...
    if authoritative.exists():
        raise O1C71RunError("O1C-0071 already exists")
    if partial_capsules:
        if len(partial_capsules) != 1:
            raise O1C71RunError("multiple O1C-0071 recovery capsules exist")
    ...
    with tempfile.TemporaryDirectory(
        prefix="o1c71-apple8-vault-ranked-decision-"
    ) as raw:
        workspace = Path(raw)
        native_build = _native_v13.build_native_joint_score_sieve(
        ...
        if observed_sources != {
            ...
            raise O1C71RunError("source identity changed after preflight")
        validate_final_target_free_preflight(root, config, required=True)
        imported = validate_parent_and_import_vault(root, config, frozen)
        reader = _validate_reader(
            imported.ranked_decision.reader_binding(),
            "post-build science reader binding",
        )
        ...
        _atomic_bytes(
            ...
            (
                ...
                "o1_crypto_lab.o1c71_apple8_vault_ranked_decision_run run "
        ...
        bindings = {
            ...
            "adapter_source_sha256": observed_sources["joint_score_sieve_v13"],
        ...
        def invoke(local_ordinal: int, lineage_ordinal: int, vault: Path) -> object:
            if (
                ...
                raise O1C71RunError("vault-ranked-decision ordinal differs")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Preflight or run O1C-0071's one vault-ranked decision call"
    )
