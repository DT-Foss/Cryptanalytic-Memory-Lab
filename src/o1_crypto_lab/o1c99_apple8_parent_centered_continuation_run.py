"""Burn-on-intent runner for the single O1C-0099 Page-17 call.

The runner has one scientific side effect: after an immutable intent is
persisted it may invoke native v28 once, with seed zero and 128 requested
...
the published, sealed O1C-0098 causal-rollover bundle; it never regenerates
...
from . import joint_score_sieve_v31 as _native_v31
...
from .o1c98_page17_causal_rollover_prepare import (
    ...
    PAGE17_ACTIVE_LIMIT as PAGE16_ACTIVE_LIMIT,
    PAGE17_ACTIVATION_COUNT as PAGE16_ACTIVATION_COUNT,
    PAGE17_ACTIVATION_DOCUMENT_BYTES as PAGE16_ACTIVATION_DOCUMENT_BYTES,
    PAGE17_ACTIVATION_DOCUMENT_SHA256 as PAGE16_ACTIVATION_DOCUMENT_SHA256,
    PAGE17_CATEGORY_COUNTS as PAGE16_CATEGORY_COUNTS,
    PAGE17_CLAUSE_COUNT as PAGE16_CLAUSE_COUNT,
    PAGE17_HEADROOM as PAGE16_HEADROOM,
    PAGE17_LITERAL_COUNT as PAGE16_LITERAL_COUNT,
    PAGE17_RESIDENCY_DOCUMENT_BYTES as PAGE16_RESIDENCY_DOCUMENT_BYTES,
    PAGE17_RESIDENCY_DOCUMENT_SHA256 as PAGE16_RESIDENCY_DOCUMENT_SHA256,
    PAGE17_SERIALIZED_BYTES as PAGE16_SERIALIZED_BYTES,
    PAGE17_SHA256 as PAGE16_SHA256,
    PARENT_CAPSULE_MANIFEST_SHA256,
    PARENT_CAPSULE_ENTRY_COUNT,
...
NATIVE_BUILD_SCHEMA = "o1-256-o1c99-native-v28-build-v2"
...
CONFIG_RELATIVE = Path("configs/o1c99_apple8_parent_centered_continuation_v1.json")
RESULT_RELATIVE = Path(
    "research/O1C0099_APPLE8_PARENT_CENTERED_CONTINUATION_RESULT_20260720.json"
)
CAPSULE_SUFFIX = "O1C-0099_apple8-parent-centered-continuation-v1"

LOCAL_EPISODE_ORDINAL = 0
LINEAGE_CALL_ORDINAL = 30
SEED = 0
...
PUBLISHED_PREPARATION_RELATIVE = Path(
    "research/o1c98_page17_causal_rollover_seed_20260720"
)
...
NATIVE_V28_USAGE = (
    b"usage: cadical_o1_joint_score_sieve_v28 --cnf PATH "
...
PUBLISHED_ARTIFACT_SEALS: Mapping[str, tuple[int, str, str]] = {
    ...
    RECEIPT_NAME: (
        PRIORITY_RECEIPT_BYTES,
        PRIORITY_RECEIPT_SHA256,
        "canonical-o1c97-evolved-priority-state-receipt",
    ),
    OCCURRENCES_NAME: (
        OCCURRENCE_DOCUMENT_BYTES,
        OCCURRENCE_DOCUMENT_SHA256,
    ...
    NEW_CHUNK_NAME: (
        NEW_CHUNK_SERIALIZED_BYTES,
        NEW_CHUNK_SHA256,
        "immutable-unique-lineage-29-evidence-chunk",
    ),
    ACTIVE_PROJECTION_NAME: (
        PAGE16_SERIALIZED_BYTES,
        PAGE16_SHA256,
        "fresh-lineage-30-page17-science-input",
    ),
    RESIDENCY_NAME: (
...
SOURCE_PATHS = {
    "runner": "src/o1_crypto_lab/o1c99_apple8_parent_centered_continuation_run.py",
    "runner_tests": "tests/test_o1c99_apple8_parent_centered_continuation_run.py",
    "adapter_v31": "src/o1_crypto_lab/joint_score_sieve_v31.py",
    "adapter_v31_tests": "tests/test_joint_score_sieve_v31.py",
    "page17_causal_rollover_preparation": "src/o1_crypto_lab/o1c98_page17_causal_rollover_prepare.py",
    "page17_causal_rollover_preparation_tests": "tests/test_o1c98_page17_causal_rollover_prepare.py",
    ...
    "native_v28": "native/cadical_o1_joint_score_sieve_v28.cpp",
    "native_v28_tests": "tests/test_cadical_o1_joint_score_sieve_v28.py",
...
class O1C99RunError(RuntimeError):
...
def _mapping(value: object, field: str) -> Mapping[str, object]:
    if not isinstance(value, Mapping) or not all(isinstance(k, str) for k in value):
        raise O1C99RunError(f"{field} differs")
...
def _relative_text(value: object, field: str) -> str:
    ...
    if path.is_absolute() or not path.parts or ".." in path.parts:
        raise O1C99RunError(f"{field} escapes the lab")
...
def _lab_path(
    ...
    try:
        ...
        raise O1C99RunError(f"{field} cannot be resolved") from exc
    expected = (
    ...
    if (
        ...
        raise O1C99RunError(f"{field} is not a canonical lab path")
    return resolved


def _external_path(value: object, field: str, *, directory: bool = False) -> Path:
    if not isinstance(value, str) or not Path(value).is_absolute():
        raise O1C99RunError(f"{field} is not absolute")
    ...
    if stat.S_ISLNK(metadata.st_mode) or not expected:
        raise O1C99RunError(f"{field} is not canonical")
    return resolved
...
def _read_canonical_json(path: Path, field: str) -> Mapping[str, object]:
    ...
    try:
        ...
        raise O1C99RunError(f"{field} is not JSON") from exc
    if canonical_json_bytes(value) != payload:
        raise O1C99RunError(f"{field} is not canonical JSON")
    return value


def _atomic_create(path: Path, payload: bytes, *, mode: int = 0o444) -> None:
    if not isinstance(payload, bytes):
        raise O1C99RunError("atomic payload differs")
    ...
    try:
        ...
        if path.read_bytes() != payload:
            raise O1C99RunError(f"atomic write verification failed for {path.name}")
        path.chmod(mode)
        ...
        raise O1C99RunError(f"artifact already exists: {path.name}") from exc
...
def load_config(path: str | Path, *, root: Path | None = None) -> dict[str, object]:
    ...
    if not config_path.is_relative_to(lab) or config_path.is_symlink():
        raise O1C99RunError("config escapes the lab")
    config = dict(_read_canonical_json(config_path, "O1C99 config"))
    if _pending(config):
        raise O1C99RunError("config contains PENDING: " + ", ".join(_pending(config)))
    if (
        set(config)
        != {
            "schema",
        ...
        raise O1C99RunError("config fields differ")
    ...
    if (
        ...
        raise O1C99RunError("frozen O1C97 parent differs")
    ...
    if (
        ...
        raise O1C99RunError("frozen preparation source differs")
    ...
    if set(inputs) != {
        ...
        raise O1C99RunError("input fields differ")
    ...
    if (
        ...
        or native.get("adapter_schema") != _native_v31.JOINT_SCORE_SIEVE_ADAPTER_SCHEMA
        or native.get("result_schema") != _native_v31.JOINT_SCORE_SIEVE_RESULT_SCHEMA
        or native.get("page17_sha256") != PAGE16_SHA256
        or native.get("page17_bytes") != PAGE16_SERIALIZED_BYTES
    ...
    if dict(paths) != SOURCE_PATHS or set(digests) != set(SOURCE_PATHS):
        raise O1C99RunError("source inventory differs")
    ...
    if dict(budgets) != expected_budgets:
        raise O1C99RunError("budgets differ")
    return config
...
def _sibling_solver_pids() -> tuple[int, ...]:
    try:
        ...
        raise O1C99RunError("solver process census failed") from exc
    current = os.getpid()
    found: list[int] = []
...
def _validate_priority_receipt(payload: bytes, bank: bytes) -> Mapping[str, object]:
    try:
        ...
        raise O1C99RunError("priority state receipt is not JSON") from exc
    if canonical_json_bytes(receipt) != payload:
        raise O1C99RunError("priority state receipt is not canonical JSON")
    hexadecimal = receipt.get("bank_hex")
    try:
        receipt_bank = bytes.fromhex(cast(str, hexadecimal))
    except (TypeError, ValueError) as exc:
        raise O1C99RunError("priority state receipt bank differs") from exc
    if (
        receipt.get("schema")
        ...
        raise O1C99RunError("priority state receipt contract differs")
    return receipt
...
def _validate_occurrence_ledger(payload: bytes) -> frozenset[str]:
    ...
    if (
        ...
        raise O1C99RunError("published occurrence ledger contract differs")
    ...
    for ordinal, value in enumerate(records):
        ...
        if (
            ...
            raise O1C99RunError("published occurrence record differs")
    ...
    if set(by_index) != set(range(ATTIC_UNION_CLAUSE_COUNT)) or len(digests) != (
        ...
        raise O1C99RunError("published attic union digest inventory differs")
    return digests
...
def _validate_published_documents(artifacts: Mapping[str, bytes]) -> frozenset[str]:
    ...
    projection = _mapping(
        residency.get("current_projection"), "published Page-17 projection"
    )
    encoding = _mapping(projection.get("encoding_only"), "published Page-17 vault")
    ...
    if (
        ...
        or len(activation_entries) != PAGE16_ACTIVATION_COUNT
        or len(used) != PAGE16_ACTIVATION_COUNT
        or used[-1] != PAGE16_SHA256
        or last_activation.get("lineage_ordinal") != LINEAGE_CALL_ORDINAL
        or last_activation.get("active_sha256") != PAGE16_SHA256
        or last_activation.get("selected_clause_count") != PAGE16_CLAUSE_COUNT
        or residency.get("schema") != "o1-score-threshold-causal-residency-v1"
        or residency.get("active_limit") != PAGE16_ACTIVE_LIMIT
        or projection.get("lineage_ordinal") != LINEAGE_CALL_ORDINAL
        or projection.get("category_counts") != PAGE16_CATEGORY_COUNTS
        or encoding.get("sha256") != PAGE16_SHA256
        or encoding.get("clause_count") != PAGE16_CLAUSE_COUNT
        or encoding.get("literal_count") != PAGE16_LITERAL_COUNT
        or encoding.get("serialized_bytes") != PAGE16_SERIALIZED_BYTES
        or attic.get("chunk_count") != ATTIC_CHUNK_COUNT
        or residency_union.get("clause_count") != ATTIC_UNION_CLAUSE_COUNT
    ):
        raise O1C99RunError("published Page-17 residency contract differs")
    if (
        ...
        raise O1C99RunError("published supporting artifact contract differs")
...
def _validate_native_capacity_proof(value: object) -> Mapping[str, object]:
    """Derive and verify the exact Page-17 native-capacity proof before intent."""

    page = _mapping(value, "published Page-17 capacity source")
    expected_caps = {
        "maximum_clauses": 512,
        "maximum_literals": 1_600_000,
        "maximum_serialized_bytes": 8_388_608,
    }
    headroom = _mapping(page.get("headroom"), "published Page-17 headroom")
    input_clauses = _nonnegative(page.get("clause_count"), "Page-17 input clauses")
    additional = _nonnegative(headroom.get("clauses"), "Page-17 clause headroom")
    if (
        O1C66_VAULT_CAPS.describe() != expected_caps
        or input_clauses != PAGE16_CLAUSE_COUNT
        or additional != PAGE16_HEADROOM["clauses"]
        or dict(headroom) != PAGE16_HEADROOM
        or input_clauses + additional != expected_caps["maximum_clauses"]
        or additional < PARENT_CENTERED_ACTION_CAPACITY
    ):
        raise O1C99RunError("published Page-17 native capacity proof differs")
    proof: dict[str, object] = {
        "caps": expected_caps,
        "clause_headroom_guarantee": {
            "native_vault_maximum_clauses": expected_caps["maximum_clauses"],
            "page17_input_clauses": input_clauses,
        ...
        "recorded_residual_headroom": {
            "literals": PAGE16_HEADROOM["literals"],
            "serialized_bytes": PAGE16_HEADROOM["serialized_bytes"],
        },
...
def _disabled_o1c96_published_manifest_validator(
    ...
    if (
        ...
        raise O1C99RunError("published preparation manifest identity differs")
    ...
    page17 = _mapping(manifest.get("page17"), "published Page-16")
    _validate_native_capacity_proof(page17)
    ...
    if dict(zero_call) != {
        ...
        raise O1C99RunError("published zero-call authorization differs")
    if (
        ...
        raise O1C99RunError("published parent boundary differs")
    if (
        ...
        raise O1C99RunError("published rollover/attic boundary differs")
    if (
        page17.get("lineage_ordinal") != LINEAGE_CALL_ORDINAL
        or page17.get("active_limit") != PAGE16_ACTIVE_LIMIT
        or page17.get("active_sha256") != PAGE16_SHA256
        or page17.get("clause_count") != PAGE16_CLAUSE_COUNT
        or page17.get("literal_count") != PAGE16_LITERAL_COUNT
        or page17.get("serialized_bytes") != PAGE16_SERIALIZED_BYTES
        or page17.get("category_counts") != PAGE16_CATEGORY_COUNTS
        or page17.get("headroom") != PAGE16_HEADROOM
        or page17.get("fresh_identity") is not True
    ):
        raise O1C99RunError("published Page-16 seal differs")
    if (
        ...
        raise O1C99RunError("published continuation bank seal differs")
    rows_raw = _mapping(manifest.get("artifacts"), "published artifact rows")
    if set(rows_raw) != set(PUBLISHED_ARTIFACT_SEALS):
        raise O1C99RunError("published artifact row inventory differs")
    ...
    for name, (size, digest, role) in PUBLISHED_ARTIFACT_SEALS.items():
        ...
        if (
            ...
            raise O1C99RunError("published artifact seal differs")
        rows[name] = row
    ...
    raise O1C99RunError("historical manifest validator is disabled")


def _validate_published_manifest(
    manifest: Mapping[str, object], artifacts: Mapping[str, bytes]
) -> tuple[dict[str, Mapping[str, object]], frozenset[str]]:
    """Validate O1C97 science imported by O1C98, then stage Page 17."""

    payload = canonical_json_bytes(manifest)
    _native_v31._validate_manifest(payload)
    if (
        set(manifest)
        != {
            "schema",
    ...
    parent = _mapping(manifest.get("parent"), "published preparation parent")
    science = _mapping(manifest.get("science_boundary"), "published science")
    rollover = _mapping(manifest.get("rollover"), "published rollover")
    attic = _mapping(manifest.get("attic"), "published preparation attic")
    page17 = _mapping(manifest.get("page17"), "published Page-17")
    capacity = _validate_native_capacity_proof(page17)
    bank = _mapping(manifest.get("final_priority_bank"), "published continuation bank")
    carried = _mapping(manifest.get("carried_context"), "published carried context")
    if (
        parent.get("attempt_id") != "O1C-0097"
        or parent.get("capsule_manifest_sha256") != PARENT_CAPSULE_MANIFEST_SHA256
        or parent.get("capsule_entry_count") != PARENT_CAPSULE_ENTRY_COUNT
        or parent.get("result_sha256") != PARENT_RESULT_SHA256
        or parent.get("intent_sha256") != PARENT_INTENT_SHA256
        ...
        or parent.get("source_lineage_ordinal") != 29
        or parent.get("page16_burned") is not True
        or parent.get("lineage29_burned") is not True
        or parent.get("retry_or_replay_authorized") is not False
        or science.get("imported_science_attempt_id") != "O1C-0097"
        or science.get("imported_fully_emitted_clause_count") != 263
        or science.get("imported_globally_novel_clause_count") != 262
        or science.get("imported_current_duplicate_clause_count") != 1
        or rollover.get("chunk_sha256") != NEW_CHUNK_SHA256
        or rollover.get("clause_count") != 262
        or rollover.get("fully_emitted_occurrence_count") != 263
        or attic.get("chunk_count") != ATTIC_CHUNK_COUNT
        or attic.get("union_clause_count") != ATTIC_UNION_CLAUSE_COUNT
        or attic.get("occurrence_count") != ATTIC_OCCURRENCE_COUNT
        or attic.get("duplicate_occurrence_count") != 9
        or attic.get("strict_subsumption_pair_count")
        != ATTIC_SUBSUMPTION_RELATION_COUNT
        or attic.get("undominated_clause_count") != ATTIC_UNDOMINATED_CLAUSE_COUNT
        or page17.get("lineage_ordinal") != LINEAGE_CALL_ORDINAL
        or page17.get("active_limit") != PAGE16_ACTIVE_LIMIT
        or page17.get("active_sha256") != PAGE16_SHA256
        or page17.get("clause_count") != PAGE16_CLAUSE_COUNT
        or page17.get("literal_count") != PAGE16_LITERAL_COUNT
        ...
        or carried.get("historical_failure_receipt_carried_forward") is not False
        ...
        or bank.get("eligible_coordinate_count") != 255
        or bank.get("aggregate_evolved_count") != INPUT_BANK_TOTAL_COUNT
        or bank.get("maximum_evolved_count") != 2_921
        or bank.get("minimum_nonzero_evolved_count") != 230
        or bank.get("zero_coordinate_variables") != [MISSING_VARIABLE]
        or bank.get("receipt_sha256") != PRIORITY_RECEIPT_SHA256
        or bank.get("receipt_serialized_bytes") != PRIORITY_RECEIPT_BYTES
        or bank.get("receipt_bank_hex_byte_equal") is not True
    ):
        raise O1C99RunError("published O1C98 science/rollover boundary differs")
...
def _load_published_preparation(
    ...
    try:
        ...
        raise O1C99RunError("published preparation inventory is unreadable") from exc
    if (
        manifest_path != directory / PREPARATION_MANIFEST_NAME
        or {path.name for path in children} != PREPARATION_ARTIFACT_NAMES
    ):
        raise O1C99RunError("published preparation inventory differs")
    ...
    for path in children:
        try:
            ...
            raise O1C99RunError("published preparation artifact is unreadable") from exc
        if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
            raise O1C99RunError("published preparation artifact type differs")
    ...
    if (
        ...
        raise O1C99RunError("published preparation manifest seal differs")
...
def _preflight_bundle(
    ...
    *,
    ...
    if (
        ...
        raise O1C99RunError("parent input SHA preflight differs")
    ...
    if (
        ...
        raise O1C99RunError("published preparation/receipt preflight differs")
    ...
    for name in ("cnf", "potential", "grouping", "o1c73_config"):
        ...
        if observed != inputs[f"{name}_sha256"]:
            raise O1C99RunError(f"input {name} SHA preflight differs")
    ...
    for name in SOURCE_PATHS:
        ...
        if observed != expected[name]:
            raise O1C99RunError(f"source {name} SHA preflight differs")
    ...
    if (
        ...
        or source_sha["native_v28"] != native["source_sha256"]
    ):
        raise O1C99RunError("native toolchain/source SHA preflight differs")

    prepared = _load_published_preparation(published_directory, published_manifest)
    page17_manifest = _mapping(
        prepared.manifest.get("page17"), "published Page-17 preflight"
    )
    capacity_proof = _validate_native_capacity_proof(page17_manifest)
    prepared_rows = {
        name: {
    ...
    if (
        ...
        raise O1C99RunError("Darwin/arm64/RAM/disk/process preflight differs")
    row = {
...
def _executable_identity(path: Path, *, relative_path: str) -> dict[str, object]:
    try:
        ...
        raise O1C99RunError("native executable is unreadable") from exc
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
        raise O1C99RunError("native executable type differs")
    size = path.stat().st_size
    if size <= 0:
        raise O1C99RunError("native executable is empty")
    return {
        "path": relative_path,
...
def _verify_staged_native_closure(
    *,
    ...
    if len(expected_files) != len(NATIVE_CLOSURE_NAMES):
        raise O1C99RunError("staged native closure basenames collide")
    ...
    try:
        ...
        for candidate in closure_root.rglob("*"):
            ...
            if stat.S_ISLNK(metadata.st_mode):
                raise O1C99RunError("staged native closure contains a symlink")
            if stat.S_ISDIR(metadata.st_mode):
                observed_directories.add(relative)
                if stat.S_IMODE(metadata.st_mode) != 0o555:
                    raise O1C99RunError("staged native closure directory is writable")
            elif stat.S_ISREG(metadata.st_mode):
                observed_files.add(relative)
                if stat.S_IMODE(metadata.st_mode) != 0o444:
                    raise O1C99RunError("staged native closure file is writable")
            else:
                raise O1C99RunError("staged native closure contains a special file")
    except OSError as exc:
        raise O1C99RunError("staged native closure is unreadable") from exc
    if (
        ...
        raise O1C99RunError("staged native closure inventory differs")
    ...
    for name in NATIVE_CLOSURE_NAMES:
        ...
        if row["sha256"] != digests.get(name):
            raise O1C99RunError(f"staged native closure {name} digest differs")
        rows.append(row)
    normalized_expected = [
        dict(_mapping(value, "expected staged closure row")) for value in expected_rows
    ]
    if rows != normalized_expected:
        raise O1C99RunError("staged native closure rows differ")
    return rows
    ...
    *,
    ...
    for name in NATIVE_CLOSURE_NAMES:
        ...
        if approved_source_sha256.get(name) != expected_sha:
            raise O1C99RunError("native closure preflight/config seal differs")
        source_path = _lab_path(root, configured, f"native closure {name}")
        payload = source_path.read_bytes()
        if sha256_bytes(payload) != expected_sha:
            raise O1C99RunError(f"native closure {name} changed after preflight")
...
def _compile_native(
    *,
    ...
    staged_source = _relative_text(source_paths["native_v28"], "staged v28 source")
    if (
        native["source"] != staged_source
        or _mapping(verified_closure[0], "staged v28 row").get("sha256")
        != native["source_sha256"]
        or _sha_file(library) != native["cadical_library_sha256"]
    ):
        raise O1C99RunError("native build input changed after preflight")
    ...
    try:
        ...
        raise O1C99RunError("native v28 compilation could not run") from exc
    ...
    try:
        ...
        raise O1C99RunError("native v28 --help smoke could not run") from exc
    ...
    if (
        ...
        or smoke_stdout != NATIVE_V28_USAGE
        or smoke_stderr
        or after_smoke != before_smoke
    ):
        raise O1C99RunError("native v28 --help smoke seal differs")
    verified_closure = _verify_staged_native_closure(
        stage=stage,
        config=config,
        expected_rows=closure_rows,
    ...
    if _sha_file(library) != native["cadical_library_sha256"]:
        raise O1C99RunError("native library changed during build/smoke")
    normalized_command = [
        *command[:-1],
    ...
    build = {
        ...
        "staged_source": dict(_mapping(verified_closure[0], "staged v28 row")),
...
def _validate_staged_native_build(
    *,
    ...
    if (
        ...
        or smoke.get("stdout")
        != {
            "serialized_bytes": len(NATIVE_V28_USAGE),
            "sha256": sha256_bytes(NATIVE_V28_USAGE),
        }
        or smoke.get("stderr") != {"serialized_bytes": 0, "sha256": sha256_bytes(b"")}
        or smoke.get("executable") != executable
    ):
        raise O1C99RunError("staged native build provenance differs")
...
def _stage_capsule(
    *,
    ...
    if final.exists() or stage.exists():
        raise O1C99RunError("timestamped capsule already exists")
    ...
    try:
        ...
        if sha256_bytes(config_payload) != bundle.row.get(
            ...
            raise O1C99RunError("config changed before closure snapshot")
...
def _rehash_call_inputs(
    *,
    ...
    if _sha_file(config_path) != preflight_row.get("config_sha256"):
        raise O1C99RunError("config changed before native call")
    ...
    for name in ("cnf", "potential", "grouping", "o1c73_config"):
        if (
            ...
            raise O1C99RunError(f"input {name} changed before native call")
    ...
    if prepared_rows.get(RECEIPT_NAME) != receipt_row:
        raise O1C99RunError("staged priority receipt seal differs")
    ...
    for name, value in expected_rows.items():
        ...
        if path.stat().st_size != row.get("serialized_bytes") or _sha_file(
            ...
            raise O1C99RunError("staged call artifact seal differs")
    ...
    if (
        ...
        raise O1C99RunError("staged invocation/build seal differs")
    ...
    if (
        ...
        raise O1C99RunError("staged native build/smoke seal differs")
    if require_executable_identity:
        _require_executable_identity(
...
def _native_stdout(result: object) -> bytes:
    ...
    if isinstance(value, str):
        ...
        raise O1C99RunError("adapter omitted byte-exact native stdout")
    if _sha(
        getattr(result, "native_stdout_sha256", None), "native stdout digest"
    ) != sha256_bytes(payload):
        raise O1C99RunError("native stdout digest differs")
    return payload
...
def _validate_result_conservation(
    ...
    if set(counters) != {*outcome_names, "child_bound_evaluations"}:
        raise O1C99RunError("native post-result conservation differs")
    outcomes = tuple(
...
def _adapter_memory_samples(
    ...
    for raw in rows_raw:
        ...
        if (
            ...
            raise O1C99RunError("adapter memory sample differs")
    ...
    if (
        ...
        != _native_v31._v9.JOINT_SCORE_SIEVE_MEMORY_SERIES_SCHEMA
        or memory.get("memory_sample_limit")
        != _native_v31._v9.JOINT_SCORE_SIEVE_MAXIMUM_MEMORY_SAMPLES
        ...
        raise O1C99RunError("adapter memory ledger differs")
    return tuple(rows)


def _validate_native_result(result: object, stdout: bytes) -> NativeEvidence:
    if not isinstance(result, _native_v31.JointScoreSieveV31Result):
        raise O1C99RunError("adapter v31 result type differs")
    ...
    try:
        ...
        raise O1C99RunError("native stdout JSON differs") from exc
    if (
        decoded != raw
        or raw.get("schema") != _native_v31.JOINT_SCORE_SIEVE_RESULT_SCHEMA
    ):
        raise O1C99RunError("native stdout/result projection differs")
    ...
    if (
        ...
        or raw.get("active_vault_sha256") != PAGE16_SHA256
        ...
        raise O1C99RunError("native v28 production binding differs")
    if (
        raw.get("priority_state") != priority_state
        or raw.get("priority_actions") != actions
    ):
        raise O1C99RunError("native promoted priority projection differs")
    if raw.get("decision_ownership") != ownership:
        raise O1C99RunError("native promoted ownership projection differs")
    if result.status == 10 and not isinstance(result.key_model, bytes):
        raise O1C99RunError("SAT result omitted key model")
    if result.status != 10 and result.key_model is not None:
        raise O1C99RunError("non-SAT result returned key model")
...
def _science_and_operation(
    ...
    *,
    ...
    if action_count != len(actions):
        raise O1C99RunError("priority action count differs")
    ...
    for row in emitted_rows:
        ...
        try:
            ...
            raise O1C99RunError("emitted clause serialization differs") from exc
    ...
    if novel > active_new:
        raise O1C99RunError("global novelty exceeds active-vault novelty")
    ...
    if evidence.status == 10 and not public_model_verified:
        raise O1C99RunError("SAT key failed exact public ChaCha verification")
    if evidence.status != 10 and public_model_verified:
        raise O1C99RunError("public model verification without SAT differs")
    model_gain = evidence.status == 10 and public_model_verified
    closure_gain = evidence.status == 20 or terminal_empty
    # Native v28 emits no entropy or domain estimate.  These fields stay
    ...
    science = {
        ...
        "active_page17_new_clauses": active_new,
    ...
    *,
    ...
    try:
        ...
        result = adapter_run(
            ...
            sealed_page17_path=capsule / "initial" / ACTIVE_PROJECTION_NAME,
        )
        ...
        # Adapter v31 attaches the completed process record even when semantic
...
def _manifest_bytes(capsule: Path) -> tuple[bytes, int]:
    ...
    for path in sorted(capsule.rglob("*")):
        metadata = path.lstat()
        if stat.S_ISLNK(metadata.st_mode):
            raise O1C99RunError("capsule contains a symlink")
        if stat.S_ISDIR(metadata.st_mode):
            continue
        if not stat.S_ISREG(metadata.st_mode):
            raise O1C99RunError("capsule contains a special file")
...
def _run_markdown(result: Mapping[str, object]) -> bytes:
    ...
    lines = [
        "# O1C-0099 Page-17 parent-centered run",
        "",
        ...
        "- Page 17 and lineage 30 burned when `episodes/00/intent.json` was persisted.",
...
def _publish_exact(path: Path, payload: bytes) -> None:
    if path.exists():
        if path.is_symlink() or not path.is_file() or path.read_bytes() != payload:
            raise O1C99RunError("authoritative result already differs")
        return
...
def _finalize_capsule(
    *, capsule: Path, authoritative: Path, result: Mapping[str, object]
    ...
    if persistent_bytes > MAXIMUM_PERSISTENT_ARTIFACT_BYTES:
        raise O1C99RunError("persistent artifact budget exceeded")
...
def _existing_result(root: Path) -> dict[str, object] | None:
    ...
    if not authoritative.exists():
        return None
    value = dict(_read_canonical_json(authoritative, "authoritative O1C99 result"))
    if value.get("schema") != RESULT_SCHEMA or value.get("attempt_id") != ATTEMPT_ID:
        raise O1C99RunError("authoritative O1C99 result differs")
    return value
...
def _validate_capsule_artifact_row(
    ...
    *,
    ...
    try:
        ...
        raise O1C99RunError(f"capsule artifact {name} is unreadable") from exc
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
        raise O1C99RunError(f"capsule artifact {name} type differs")
    expected = _artifact_row(path, relative_to=directory)
    if set(row) != {"path", "serialized_bytes", "sha256"} or row != expected:
        raise O1C99RunError(f"capsule artifact {name} seal differs")
    if canonical_json:
...
def _validated_timestamp(value: object, field: str) -> str:
    ...
    try:
        ...
        raise O1C99RunError(f"{field} differs") from exc
...
def _semantic_replay_completed_native(
    *,
    ...
    """Replay archived stdout through v31 using the original sealed inputs."""
    ...
    for name in ("cnf", "potential", "grouping"):
        ...
        if sha256_bytes(payload) != input_sha[name]:
            raise O1C99RunError(f"semantic replay {name} seal differs")
        payloads[name] = payload
    page17 = prepared.artifacts[ACTIVE_PROJECTION_NAME]
    bank = prepared.artifacts[CONTINUATION_BANK_NAME]
    if (
        len(page17) != PAGE16_SERIALIZED_BYTES
        or sha256_bytes(page17) != PAGE16_SHA256
        or len(bank) != CONTINUATION_BANK_BYTES
        or sha256_bytes(bank) != CONTINUATION_BANK_SHA256
    ):
        raise O1C99RunError("semantic replay Page17/bank seal differs")
    io_v1 = _native_v31._v9._v8._v7._v1
    try:
        field = io_v1._potential(payloads["potential"])
        grouping = _native_v31._v9.validate_joint_score_sieve_grouping(
            field, payloads["grouping"]
        )
        if grouping.potential_sha256 != input_sha["potential"]:
            raise O1C99RunError("semantic replay grouping identity differs")
        input_vault = _native_v31.parse_threshold_no_good_vault(
            page17,
            observed_variables=field.observed_variables,
            caps=O1C66_VAULT_CAPS,
        )
        identity = _native_v31.vault_identity_from_sources(
            cnf_sha256=input_sha["cnf"],
            potential_sha256=input_sha["potential"],
            grouping_sha256=input_sha["grouping"],
            observed_variables=field.observed_variables,
            bound_rule=_native_v31._v9.JOINT_SCORE_SIEVE_BOUND_RULE,
            threshold=THRESHOLD,
        )
        _native_v31.validate_threshold_no_good_vault_identity(
            input_vault, expected=identity
        )
        _native_v31._v9._v8._certify_input_vault(
            input_vault,
            field=field,
            grouping=grouping,
            threshold=THRESHOLD,
        )
        priority_records = _native_v31._decode_live_bank(
            bank,
            expected_sha256=CONTINUATION_BANK_SHA256,
            sealed_input=True,
        )
        memory_samples = _adapter_memory_samples(adapter_memory)
        raw = _native_v31._v21.load_native_json(stdout)
        replayed = _native_v31._parse_native_payload(
            raw,
        ...
        raise O1C99RunError("archived native semantic replay differs") from exc
    ...
    if replay_memory != adapter_memory:
        raise O1C99RunError("semantic replay adapter memory differs")
    ...
    *,
    ...
    if not all(path.is_file() and not path.is_symlink() for path in required):
        if (capsule / "episodes/00/intent.json").is_file():
            raise O1C99RunError("burned incomplete capsule forbids retry or replay")
        raise O1C99RunError("incomplete pre-intent capsule blocks a fresh call")
    manifest, _ = _manifest_bytes(capsule)
    if manifest != (capsule / "artifacts.sha256").read_bytes():
        raise O1C99RunError("existing capsule manifest differs")
    episode_dir = capsule / "episodes" / "00"
    ...
    try:
        ...
        raise O1C99RunError("existing capsule inventory is unreadable") from exc
    if (
        ...
        raise O1C99RunError("existing capsule inventory differs")
    ...
    if (
        ...
        raise O1C99RunError("existing capsule config differs")
    ...
    if (
        ...
        != _validate_native_capacity_proof(prepared.manifest["page17"])
        or published.get("native_capacity_proof_verified_before_intent") is not True
        or preflight.get("page17_sha256") != PAGE16_SHA256
        ...
        raise O1C99RunError("existing capsule preflight provenance differs")
    ...
    if (
        ...
        or invocation.get("page17_sha256") != PAGE16_SHA256
        ...
        raise O1C99RunError("existing capsule invocation provenance differs")
    _rehash_call_inputs(
        root=root,
        config_path=config_path,
        config=config,
    ...
    if (
        ...
        or intent.get("page17_sha256") != PAGE16_SHA256
        ...
        raise O1C99RunError("existing capsule burn intent differs")

    episode = dict(_read_canonical_json(episode_path, "capsule episode"))
    if episode != episode_preview:
        raise O1C99RunError("capsule episode changed during validation")
    ...
    if (
        ...
        raise O1C99RunError("existing capsule episode lineage differs")
    ...
    if episode.get("completed") is True:
        ...
        if (
            ...
            raise O1C99RunError("existing completed episode call contract differs")
        ...
        if set(archived) != set(NATIVE_ARCHIVE_NAMES):
            raise O1C99RunError("archived native component inventory differs")
        component_paths = {
        ...
        if episode.get("final_priority_bank") != archived[FINAL_BANK_NAME]:
            raise O1C99RunError("final priority bank row differs")
        ...
        try:
            raw = dict(_native_v31._v21.load_native_json(stdout))
        except Exception as exc:
            raise O1C99RunError("archived native stdout differs") from exc
        archived_raw = dict(
        ...
        if status == 10:
            if (
                ...
                raise O1C99RunError("archived SAT key model differs")
            if (
                len(key_model) != 32
                ...
                raise O1C99RunError("archived SAT key model is not publicly verified")
        elif status in (0, 20):
            if key_hex is not None or episode.get("key_model_sha256") is not None:
                raise O1C99RunError("archived non-SAT key model differs")
        else:
            raise O1C99RunError("archived native status differs")
        try:
            bank_hex = bytes.fromhex(cast(str, priority_state.get("bank_hex")))
        except (TypeError, ValueError) as exc:
            raise O1C99RunError("archived final bank encoding differs") from exc
        if (
            raw != archived_raw
            or evidence.raw != raw
            ...
            raise O1C99RunError("archived native result cross-binding differs")
        operation, science, classification, stop = _science_and_operation(
            evidence,
        ...
        if (
            ...
            raise O1C99RunError("completed episode science projection differs")
        ...
        if (
            ...
            raise O1C99RunError("terminal episode call count differs")
        if (
            ...
            raise O1C99RunError("terminal episode contract differs")
        ...
        if (
            ...
            raise O1C99RunError("terminal failure cross-binding differs")
        stdout_row = episode.get("native_stdout")
        if failure.get("native_stdout") != stdout_row:
            raise O1C99RunError("terminal native stdout row differs")
        if not native_inventory:
            if not (
                failure.get("phase") == "PRE_CALL"
                and failure.get("exception_type") == "O1C99RunError"
                ...
                raise O1C99RunError("missing terminal executable provenance differs")
        else:
            ...
            if observed_executable != executable and not (
                ...
                raise O1C99RunError("terminal executable provenance differs")
        expected_episode_inventory = {
            "intent.json",
            "episode.json",
        ...
        raise O1C99RunError("existing capsule episode completion differs")

    try:
        observed_episode_inventory = {path.name for path in episode_dir.iterdir()}
    except OSError as exc:
        raise O1C99RunError("episode inventory is unreadable") from exc
    if observed_episode_inventory != expected_episode_inventory:
        raise O1C99RunError("episode artifact inventory differs")
    ...
    if (
        ...
        != "Never retry or replay Page 17 / lineage 30 or Pages 16 through 9; preserve the final priority bank only as a continuation seed."
        or (capsule / "RUN.md").read_bytes() != _run_markdown(result)
    ):
        raise O1C99RunError("existing capsule result differs")
...
def _public_verifier(
    *, root: Path, config: Mapping[str, object]
    ...
    if _sha_file(config_path) != inputs["o1c73_config_sha256"]:
        raise O1C99RunError("public verifier config SHA differs")
    try:
        ...
        raise O1C99RunError("public ChaCha verifier cannot be established") from exc
...
def run(
    ...
    *,
    root: Path | None = None,
    adapter_run: AdapterRun = _native_v31.run_joint_score_sieve,
    ...
    if existing is not None:
        if len(capsules) != 1:
            raise O1C99RunError("authoritative O1C99 result lacks one sealed capsule")
        ...
        if capsule_payload != authoritative_payload or capsule_result != existing:
            raise O1C99RunError("authoritative O1C99 result differs from capsule")
        return existing
    if capsules:
        if len(capsules) != 1:
            raise O1C99RunError("multiple O1C99 capsules forbid replay")
    ...
    result = {
        ...
        "next_action": "Never retry or replay Page 17 / lineage 30 or Pages 16 through 9; preserve the final priority bank only as a continuation seed.",
    }
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Preflight or execute O1C99's one Page-17 parent-centered call"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 0
    except O1C99RunError as exc:
        print(f"O1C99: {exc}", file=sys.stderr)
        return 2
